...
// (both widening variants) so the test domain itself is known sound before it
// is used to validate the solver.
func TestLawSuite_CapLattice(t *testing.T) {
	cl := capLattice{top: 8}
	sample := []int{0, 1, 3, 5, 8}
	lattice.LawSuite[int]{Name: "cap-widen-top", Domain: cl.lattice(), Sample: sample}.Run(t)
	lattice.LawSuite[int]{Name: "cap-join", Domain: cl.joinOnly(), Sample: sample}.Run(t)
}
