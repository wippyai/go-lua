...
// components do; the law harness in the parent package verifies this against
// any component instantiation.
//
...
import "github.com/wippyai/go-lua/types/lattice"
...
// forward-only component is itself forward-only. LawSuite then skips the
// meet-side laws for the product, exactly as it does for the component.
func Product2[A, B any](la lattice.Lattice[A], lb lattice.Lattice[B]) lattice.Lattice[Pair[A, B]] {
	...
	// forward-only and Meet stays nil so LawSuite skips the meet-side laws.
