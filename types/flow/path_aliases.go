...
func (f PathAliasFacts) withAliasesProvedByAbsentValues(facts PathAliasFacts, absent addressAbsentProof) PathAliasFacts {
	if facts.bottom {
		return f
	}
	out := f
...
func pathAliasAbsent(absent addressAbsentProof, key constraint.PathKey) bool {
	if absent == nil {
		return false
	}
