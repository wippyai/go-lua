...
// declarations can form cycles or shared DAGs in memory while the wire form
// remains a tree. Binder-local references preserve those graph edges without
// leaking process-local type IDs into deterministic boundary bytes.
	...
	if e.semanticFunctions {
		if alias, ok := t.(*typ.Alias); ok {
			...
			if target == nil || target == t {
				return nil, fmt.Errorf("signature/wire: semantic alias %q has no acyclic target", alias.Name)
			}
