	...
	return []effectLabelWireCase{
		...
		},
		{
			...
			label: lifecycle.Acquire{
				...
				Obligation: canonicalLifecycleObligation(typestate.State("finished")),
			},
			wire: `{"kind":"lifecycle.acquire","target":{"index":0},"protocol":"transaction","to":"active","finals":["finished"]}`,
		},
		{
			...
			label: lifecycle.Acquire{
				...
				Protocol:   typestate.Protocol("transaction"),
				State:      typestate.State("active"),
				Obligation: canonicalLifecycleObligation(typestate.State("committed"), typestate.State("rolledback")),
			},
