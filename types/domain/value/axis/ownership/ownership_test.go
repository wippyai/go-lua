...
func sample() []Value {
	return []Value{Bottom(), Unique(), Shared()}
}
...
// TestChainOrder pins the intended order: Bottom < Unique < Shared, with Shared as
// Top (the conservative assumption) and Unique strictly more precise.
func TestChainOrder(t *testing.T) {
	if !Unique().Covers(Bottom()) || !Shared().Covers(Unique()) {
		t.Fatal("Bottom < Unique < Shared must hold")
	}
	if Unique().Covers(Shared()) {
		t.Fatal("Unique must not cover Shared")
	}
	if !Shared().IsTop() || !Bottom().IsBottom() {
		t.Fatal("Shared is Top and Bottom is Bottom")
	}
	if !Equal(Join(Unique(), Shared()), Shared()) {
		t.Fatalf("Unique join Shared must weaken to Shared, got %s", Join(Unique(), Shared()))
	}
	if !Equal(Top(), Shared()) {
		t.Fatal("Top must be Shared")
	}
}
