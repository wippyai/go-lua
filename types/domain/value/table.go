...
// JoinMapShape joins two compatible map observations in the table domain.
func JoinMapShape(a, b typ.Type, join func(typ.Type, typ.Type) typ.Type) (typ.Type, bool) {
	if join == nil {
	...
	am, okA := unwrap.Alias(a).(*typ.Map)
	bm, okB := unwrap.Alias(b).(*typ.Map)
	if !okA || !okB || am == nil || bm == nil {
		return nil, false
	}
	...
	if upper, ok := SelfEmbeddingUpperBound(a, b); ok {
		return wrap(upper), true
	}
	out := typ.NewMap(joinMapKey(am.Key, bm.Key, join), join(am.Value, bm.Value))
...
func joinMapRecordShapeDirected(mapType, recordType typ.Type, join func(typ.Type, typ.Type) typ.Type) (typ.Type, bool) {
	...
	if upper, ok := SelfEmbeddingUpperBound(mapType, recordType); ok {
		return upper, true
	}
...
func joinMapKey(a, b typ.Type, join func(typ.Type, typ.Type) typ.Type) typ.Type {
	if a == nil {
		return b
	}
	if b == nil {
