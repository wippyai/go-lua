...
func (read IndexRead) ContextID() keyspace.ContentID {
	if !read.Available() {
...
func (write IndexWrite) ContextID() keyspace.ContentID {
	if !write.Available() {
		return keyspace.ContentID{}
	}
...
func (base IndexBase) ContextID() keyspace.ContentID {
	if !base.Available() {
		return keyspace.ContentID{}
	}
...
func (lens IndexLens) ContextID() keyspace.ContentID {
	if !lens.Available() {
		return keyspace.ContentID{}
	}
...
func (result IndexResult) ContextID() keyspace.ContentID {
	if !result.Available() {
		return keyspace.ContentID{}
	}
...
func (values IndexValues) ContextID() keyspace.ContentID {
	if !values.Available() {
		return keyspace.ContentID{}
	}
...
func accessRoleID(domain string, input TransformerInput, term keyspace.Term, values ...uint64) keyspace.ContentID {
	if !input.Available() || !validAccessTerm(term) {
		return keyspace.ContentID{}
	}
	return transformerRoleID(domain, input.programID, func(writer *canonical.Writer) bool {
...
func accessSubroleID(domain string, input TransformerInput, access, term keyspace.Term, values ...uint64) keyspace.ContentID {
...
func writeAccessSemantic(writer *canonical.Writer, input TransformerInput, term keyspace.Term) bool {
