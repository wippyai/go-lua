...
// Flow row/ordinal.
func transformerRoleID(domain string, programID keyspace.ContentID, write func(*canonical.Writer) bool) keyspace.ContentID {
...
func transformerSemanticID(domain string, write func(*canonical.Writer) bool) keyspace.ContentID {
	hash := sha256.New()
	var writer canonical.Writer
...
func (span Span) ContextID() keyspace.ContentID {
	if !span.Available() {
		return keyspace.ContentID{}
	}
	return span.context
...
func spanContextID(span Span) keyspace.ContentID {
	if !span.availableGeometry() {
		return keyspace.ContentID{}
	}
	entryID, finishID := span.entry.ContextID(), span.finish.ContextID()
	return transformerRoleID("program/transformer/span", span.input.programID, func(writer *canonical.Writer) bool {
...
func (guard Guard) ContextID() keyspace.ContentID {
	if !guard.Available() {
		return keyspace.ContentID{}
	}
	return transformerRoleID("program/transformer/guard", guard.input.programID, func(writer *canonical.Writer) bool {
...
func (guard Guard) PathID() keyspace.ContentID {
	if !guard.Available() {
		return keyspace.ContentID{}
	}
	return guard.path
...
func (cell Cell) ContextID() keyspace.ContentID {
	if !cell.Available() {
		return keyspace.ContentID{}
	}
	if cell.role == cellRoleStorage {
		return transformerRoleID("program/transformer/storage-cell", cell.input.programID, func(writer *canonical.Writer) bool {
	...
	if cell.role == cellRoleFormal || cell.role == cellRoleVararg || cell.role == cellRoleCaptureInner || cell.role == cellRoleCaptureOuter {
		...
		return transformerSemanticID("program/transformer/cell-semantic", func(writer *canonical.Writer) bool {
			return writer.Bytes(pathID[:]) == nil && writer.Uint(uint64(cell.role)) == nil && writer.Uint(uint64(cell.index)) == nil
		})
	}
	return transformerRoleID("program/transformer/cell", cell.input.programID, func(writer *canonical.Writer) bool {
...
func (function Function) ContextID() keyspace.ContentID {
	if !function.Available() {
		return keyspace.ContentID{}
	}
	context := function.boundary.ContextID()
	return transformerRoleID("program/transformer/function", function.body.input.programID, func(writer *canonical.Writer) bool {
...
func (formal Formal) DeclaredStaticTypeReferenceID() (keyspace.ContentID, bool) {
	if !formal.Available() {
...
func (formal Formal) ContextID() keyspace.ContentID {
	if !formal.Available() {
		return keyspace.ContentID{}
	}
	bodyPath, cellID := formal.function.body.PathID(), formal.cell.ContextID()
	return transformerSemanticID("program/transformer/formal", func(writer *canonical.Writer) bool {
...
func (vararg Vararg) ContextID() keyspace.ContentID {
	if !vararg.Available() {
		return keyspace.ContentID{}
	}
	bodyPath, cellID := vararg.function.body.PathID(), vararg.cell.ContextID()
	return transformerSemanticID("program/transformer/vararg", func(writer *canonical.Writer) bool {
...
func (capture Capture) ContextID() keyspace.ContentID {
	if !capture.Available() {
		return keyspace.ContentID{}
	}
	...
	return transformerSemanticID("program/transformer/capture", func(writer *canonical.Writer) bool {
...
func (capture Capture) InnerBodyPathID() keyspace.ContentID {
	if !capture.Available() {
		return keyspace.ContentID{}
	}
...
func (capture Capture) OuterBodyPathID() keyspace.ContentID {
	if !capture.Available() {
		return keyspace.ContentID{}
	}
...
func (input TransformerInput) CallBoundary(span Span) (CallBoundary, bool) {
	...
	result.contextID = transformerRoleID("program/transformer/call-boundary", result.span.input.programID, func(writer *canonical.Writer) bool {
	...
	for index := 0; index < result.armCount; index++ {
		proof := &result.arms[index]
		proof.contextID = transformerRoleID("program/transformer/call-arm", result.span.input.programID, func(writer *canonical.Writer) bool {
...
func (boundary CallBoundary) ContextID() keyspace.ContentID {
	if !boundary.Available() {
		return keyspace.ContentID{}
	}
...
func (arm CallArm) ContextID() keyspace.ContentID {
	if !arm.Available() {
		return keyspace.ContentID{}
	}
...
func (arm CallArm) RouteDigest() (keyspace.ContentID, bool) {
	if !arm.Available() {
		return keyspace.ContentID{}, false
	}
