...
func TestTransformerCallOccurrenceExposesNoRawTermSurface(t *testing.T) {
	termType := reflect.TypeOf(keyspace.Term(0))
	types := []reflect.Type{
		reflect.TypeOf(CallOccurrence{}), reflect.TypeOf(CallFormal{}), reflect.TypeOf(CallOperand{}),
		reflect.TypeOf(CallTypeArguments{}), reflect.TypeOf(CallTypeArgument{}),
		reflect.TypeOf(CallValues{}), reflect.TypeOf(CallArgument{}),
	}
	for _, typ := range types {
		for index := 0; index < typ.NumMethod(); index++ {
			method := typ.Method(index)
			for output := 0; output < method.Type.NumOut(); output++ {
				if method.Type.Out(output) == termType {
					t.Fatalf("%s.%s returns raw keyspace.Term", typ.Name(), method.Name)
				}
			}
			for input := 1; input < method.Type.NumIn(); input++ {
				if method.Type.In(input) == termType {
					t.Fatalf("%s.%s accepts raw keyspace.Term", typ.Name(), method.Name)
				}
			}
		}
	}
