...
func TestTransformerInputForwardsOnlyExistingProgramViews(t *testing.T) {
	...
	originalReceipt := left.semanticReceipt
	left.semanticReceipt = SemanticSourceReceipt{}
	if left.TransformerInput().Available() {
		t.Fatal("TransformerInput issuance accepted a stale semantic receipt")
	}
	left.semanticReceipt = originalReceipt
