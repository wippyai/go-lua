...
return table, make
`
	left, err := programlower.Lower(programlower.Source{Name: "allocation-roles.lua", Text: []byte(text)})
	if err != nil {
		t.Fatalf("Lower(left): %v", err)
	}
	right, err := programlower.Lower(programlower.Source{Name: "allocation-roles.lua", Text: []byte(text)})
	if err != nil {
	...
	foreign, err := programlower.Lower(programlower.Source{Name: "allocation-roles-foreign.lua", Text: []byte("return {}")})
	if err != nil {
...
func TestTransformerAllocationSemanticOccurrencesAreLocalAndGeometrySensitive(t *testing.T) {
	left, err := programlower.Lower(programlower.Source{Name: "allocation-semantic.lua", Text: []byte(`
local first = {value = 1}
...
`)})
	...
	replay, err := programlower.Lower(programlower.Source{Name: "allocation-semantic.lua", Text: []byte(`
local first = {value = 1}
local second = {value = 1}
return first, second
`)})
	...
	changed, err := programlower.Lower(programlower.Source{Name: "allocation-semantic-changed.lua", Text: []byte(`
...
func lowerAllocationPathFixture(t *testing.T, text string) *program.Program {
	t.Helper()
	p, err := programlower.Lower(programlower.Source{Name: "allocation-path.lua", Text: []byte(text)})
	if err != nil {
