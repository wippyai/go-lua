...
type ProviderCapabilitySeedRef struct {
	hostID  keyspace.ContentID
	ordinal uint32
}

func (r ProviderCapabilitySeedRef) HostID() keyspace.ContentID { return r.hostID }
...
type ReplayCapabilitySeed struct {
	...
	Source                            ProviderCapabilitySource
	InitialRoot                       string
	InputFormal, OutcomeResult, Value keyspace.ContentID
}
type ReplaySelector struct {
	Value, Endpoint keyspace.ContentID
	Dispatch        HostDispatch
}
...
func (c *Component) Cold() Cold {
	...
	return Cold{content: c.authority.content, replay: cloneReplaySpec(c.authority.replay), semanticReceipt: c.authority.semanticReceipt, fence: c.authority.fence}
}
