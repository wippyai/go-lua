...
func TestProjectContentFailsClosedForUnavailableOrNoncanonicalConstituents(t *testing.T) {
	...
	id := p.ContentID()
	if got := contentID(keyspace.ContentID{}, []mountRow{{name: "main", program: p, id: id}}); got.Available() {
	...
	if got := contentID(contract.ContentID(), []mountRow{{name: "main", program: p, id: keyspace.ContentID{1}}}); got.Available() {
