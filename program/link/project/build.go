...
func buildKeys(mounts []mountRow, contract *target.Contract) ([]keyRow, []uint32, map[target.InitialValue]uint32, [][]uint32, error) {
	...
	addExact := func(value keyspace.LiteralValue) error {
		normalized, ok := programsource.NormalizeExactKey(value)
		if !ok {
	...
	addLiteral := func(value keyspace.LiteralValue) error {
		if normalized, ok := programsource.NormalizeExactKey(value); ok {
			unique[normalized] = struct{}{}
		}
		return nil
	}
	...
	sort.Slice(identities, func(left, right int) bool {
		order, ok := programsource.CompareExactKey(identities[left], identities[right])
		return ok && order < 0
	})
	...
	for index := range targetKeys {
		...
		normalized, normalizedOK := programsource.NormalizeExactKey(value)
...
func addInitialLiterals(contract *target.Contract, add func(keyspace.LiteralValue) error) (map[target.InitialValue]keyspace.LiteralValue, error) {
	...
	addValue := func(value target.InitialValue) error {
		...
		if _, ok := programsource.NormalizeExactKey(literal); ok {
