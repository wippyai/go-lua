...
func DecodeArtifact(data []byte, contract *target.Contract, programs map[keyspace.ContentID]*program.Program) (*Link, error) {
	...
	if err != nil {
		if errors.Is(err, canonical.ErrLimit) {
			return nil, ErrArtifactLimit
		}
...
func compareArtifactModule(leftID keyspace.ContentID, leftName string, rightID keyspace.ContentID, rightName string) int {
