...
import (
	...
	sourcepkg "github.com/wippyai/go-lua/program/source"
...
func findProjectKey(keys linkproject.Keys, literal keyspace.LiteralValue) (linkproject.Key, bool) {
	normalized, ok := sourcepkg.NormalizeExactKey(literal)
	if !ok {
	...
	for index := 0; index < keys.Count(); index++ {
		...
		order, comparable := sourcepkg.CompareExactKey(normalized, value)
...
func TestLinkKeyNormalizesLuaNumericEquality(t *testing.T) {
	integer, ok := sourcepkg.NormalizeExactKey(keyspace.LiteralValue{Kind: keyspace.LiteralInteger, Integer: 1})
	if !ok {
		t.Fatal("integer key rejected")
	}
	floating, ok := sourcepkg.NormalizeExactKey(keyspace.LiteralValue{Kind: keyspace.LiteralFloat, FloatBits: math.Float64bits(1.0)})
	order, ordered := sourcepkg.CompareExactKey(integer, floating)
	if !ok || !ordered || order != 0 {
		t.Fatal("1 and 1.0 did not normalize together")
	}
	zero, ok := sourcepkg.NormalizeExactKey(keyspace.LiteralValue{Kind: keyspace.LiteralInteger, Integer: 0})
	if !ok {
		t.Fatal("zero key rejected")
	}
	negativeZero, ok := sourcepkg.NormalizeExactKey(keyspace.LiteralValue{Kind: keyspace.LiteralFloat, FloatBits: math.Float64bits(math.Copysign(0, -1))})
	order, ordered = sourcepkg.CompareExactKey(zero, negativeZero)
	if !ok || !ordered || order != 0 {
		t.Fatal("-0 and 0 did not normalize together")
	}
	infinite, ok := sourcepkg.NormalizeExactKey(keyspace.LiteralValue{Kind: keyspace.LiteralFloat, FloatBits: math.Float64bits(math.Inf(1))})
	if !ok || infinite.Kind != keyspace.LiteralFloat {
		t.Fatal("infinite float key lost its exact identity")
	}
	if _, ok := sourcepkg.NormalizeExactKey(keyspace.LiteralValue{Kind: keyspace.LiteralFloat, FloatBits: math.Float64bits(math.NaN())}); ok {
