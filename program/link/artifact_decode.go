...
func decodeLinkArtifact(data []byte, contract *target.Contract, programs map[keyspace.ContentID]*program.Program) (*Link, error) {
	measure, err := canonical.Scan(data, linkArtifactMaxBytes)
	if err != nil {
		return nil, err
	}
	...
	reader, err := canonical.NewReader(data, linkArtifactMaxBytes)
	if err != nil {
		return nil, err
	}
...
func (decoder *linkArtifactDecoder) root(contract *target.Contract) ([]linkproject.Module, linkhost.ReplaySpec, linkmodule.Spec, keyspace.ContentID, error) {
	if err := decoder.record(linkArtifactRecordRoot); err != nil {
		return nil, linkhost.ReplaySpec{}, linkmodule.Spec{}, keyspace.ContentID{}, err
	}
	...
	if storedTarget != contract.ContentID() {
		return nil, linkhost.ReplaySpec{}, linkmodule.Spec{}, keyspace.ContentID{}, ErrArtifactTarget
	}
	claimed, err := decoder.id()
	if err != nil || !claimed.Available() {
		return nil, linkhost.ReplaySpec{}, linkmodule.Spec{}, keyspace.ContentID{}, ErrArtifactCanonical
	}
	...
	if !ok {
		return nil, linkhost.ReplaySpec{}, linkmodule.Spec{}, keyspace.ContentID{}, ErrArtifactLimit
	}
	...
	var priorID keyspace.ContentID
	priorName := ""
	for index := 0; index < count; index++ {
		...
		if sealed == nil || sealed.ContentID() != id {
			return nil, linkhost.ReplaySpec{}, linkmodule.Spec{}, keyspace.ContentID{}, fmt.Errorf("%w: %x", ErrArtifactProgram, id)
		}
...
func (decoder *linkArtifactDecoder) id() (keyspace.ContentID, error) {
	payload, err := decoder.r.Bytes(len(keyspace.ContentID{}))
