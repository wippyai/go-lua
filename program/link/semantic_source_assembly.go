...
// owner contributes one detached receipt; Program mounts contribute the same
// 57-token receipt once per Project Shard, including duplicate mounts.
func buildSemanticSourcePublications(l *Link) (semanticsource.Publications, error) {
	...
	for index := 0; index < mounts.Count(); index++ {
		...
		if !ok || receipt.OwnerID() != p.ContentID() || receipt.ProgramID() != p.ContentID() {
			return semanticsource.Publications{}, errSemanticSourceAssemblyFragment
		}
		if err := assembly.acceptProgram(receipt.Publications()); err != nil {
			return semanticsource.Publications{}, err
		}
	}
	...
	if err := assembly.acceptOne(semanticSourceTarget, targetReceipt.Publications()); err != nil {
		return semanticsource.Publications{}, err
	}
	linkRows, ok := l.sourcePublications()
	if !ok {
		return semanticsource.Publications{}, errSemanticSourceAssemblyFragment
	}
...
func newSemanticSourceAssembly(schema *relations.Schema) (*semanticSourceAssembly, error) {
	if schema == nil || schema.Count() != linkSemanticSourcePublicationCount || schema.Count() != semanticsource.CatalogSchema().Count() {
		return nil, errSemanticSourceAssemblySchema
	}
	rows := schema.Rows()
	assembly := &semanticSourceAssembly{rows: rows, schemaExpected: make([]semanticSourceExpectation, len(rows)), totals: make([]int, len(rows)), counted: make([]bool, len(rows))}
	for index, row := range rows {
		...
		definition, defined := semanticsource.Definition(token.Origin(), token.Facet())
...
func (assembly *semanticSourceAssembly) seal() (semanticsource.Publications, error) {
	if assembly == nil || len(assembly.schemaExpected) != linkSemanticSourcePublicationCount {
		return semanticsource.Publications{}, errSemanticSourceAssemblySchema
	}
	...
	publisher, err := semanticsource.NewPublisher(semanticsource.CatalogSchema())
	if err != nil {
		return semanticsource.Publications{}, errSemanticSourceAssemblySchema
	}
	for index, definition := range semanticsource.CatalogSchema().Definitions() {
