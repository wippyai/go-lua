...
func TestBoundaryForeignAuthoritiesAndCardinalityFormula(t *testing.T) {
	...
	fields := reflect.TypeOf(*component.authority)
	for index := 0; index < fields.NumField(); index++ {
		if kind := fields.Field(index).Type.Kind(); kind == reflect.Map || kind == reflect.Slice {
			t.Fatalf("Boundary retained product/storage field %q of kind %v", fields.Field(index).Name, kind)
		}
	}
}
...
func TestBoundaryPortableValueAndEndpointFindID(t *testing.T) {
	...
	build := func(source *program.Program, endpoints []EndpointRequest) *Component {
		projectDraft,err:=linkproject.Build(linkproject.Input{Modules:[]linkproject.Module{{Name:"main",Program:source}},Target:contract});if err!=nil{t.Fatal(err)};project,err:=projectDraft.Finalize();if err!=nil{t.Fatal(err)};draft,err:=Build(Input{Project:project,Target:contract,EndpointRequests:endpoints});if err!=nil{t.Fatal(err)};component,err:=draft.Finalize();if err!=nil{t.Fatal(err)};return component
	}
	base := build(mainProgram, []EndpointRequest{{Identity:"first",Binding:provider}})
	value,ok:=base.Values().At(0);if !ok{t.Fatal("base Value unavailable")};valueID,ok:=base.Values().ID(value);if !ok{t.Fatal("base Value ID unavailable")};foundValue,ok:=base.Values().FindID(valueID);if !ok{t.Fatal("base Value FindID unavailable")};if order,ok:=base.Values().Compare(value,foundValue);!ok||order!=0{t.Fatal("Value FindID did not rebind exact row")}
	endpoint,ok:=base.Endpoints().At(0);if !ok{t.Fatal("base Endpoint unavailable")};endpointID,ok:=base.Endpoints().ID(endpoint);if !ok{t.Fatal("base Endpoint ID unavailable")};foundEndpoint,ok:=base.Endpoints().FindID(endpointID);if !ok||foundEndpoint!=endpoint{t.Fatal("Endpoint FindID did not rebind exact row")};endpointSeed,_:=base.Endpoints().Seed(endpoint);endpointSeedID,_:=base.Seeds().ID(endpointSeed);if endpointSeedID!=endpointID{t.Fatal("Endpoint and endpoint Seed ID diverged")}
	equivalent := build(mainProgram, []EndpointRequest{{Identity:"first",Binding:provider}})
	equivalentValue,ok:=equivalent.Values().FindID(valueID);if !ok{t.Fatal("equivalent Value replay unavailable")};if _,ok:=base.Values().Compare(equivalentValue,value);ok{t.Fatal("foreign equivalent Value handle crossed owner fence")}
	equivalentEndpoint,ok:=equivalent.Endpoints().FindID(endpointID);if !ok{t.Fatal("equivalent Endpoint replay unavailable")};if _,ok:=base.Endpoints().Operation(equivalentEndpoint);ok{t.Fatal("foreign equivalent Endpoint handle crossed owner fence")}
	endpointInsertion := build(mainProgram, []EndpointRequest{{Identity:"first",Binding:provider},{Identity:"second",Binding:provider}})
	insertedValue,ok:=endpointInsertion.Values().FindID(valueID);if !ok{t.Fatal("endpoint insertion churned Value FindID")};insertedValueID,_:=endpointInsertion.Values().ID(insertedValue);if insertedValueID!=valueID{t.Fatal("endpoint insertion churned Value ID")};insertedEndpoint,ok:=endpointInsertion.Endpoints().FindID(endpointID);if !ok{t.Fatal("unrelated endpoint insertion churned existing Endpoint FindID")};insertedEndpointID,_:=endpointInsertion.Endpoints().ID(insertedEndpoint);if insertedEndpointID!=endpointID{t.Fatal("unrelated endpoint insertion churned existing Endpoint ID")}
	changedProgram,err:=lower.Lower(lower.Source{Name:"boundary-find-value",Text:[]byte(`local x = 1; return x`) });if err!=nil{t.Fatal(err)}
	valueInsertion := build(changedProgram, []EndpointRequest{{Identity:"first",Binding:provider}})
	insertedByValue,ok:=valueInsertion.Endpoints().FindID(endpointID);if !ok{t.Fatal("unrelated Value insertion churned Endpoint FindID")};insertedByValueID,_:=valueInsertion.Endpoints().ID(insertedByValue);if insertedByValueID!=endpointID{t.Fatal("unrelated Value insertion churned Endpoint ID")}
	if _,ok:=base.Values().FindID(keyspace.ContentID{});ok{t.Fatal("unavailable Value ID resolved")};if _,ok:=base.Endpoints().FindID(keyspace.ContentID{});ok{t.Fatal("unavailable Endpoint ID resolved")}
}
...
func boundaryEndpointTarget(t testing.TB) *target.Contract {
	return boundaryEndpointTargetVariant(t, true, false, false, false)
}

func boundaryEndpointTargetWithSecondDenied(t testing.TB, secondDenied bool) *target.Contract {
