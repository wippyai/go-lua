...
// nine records are the same semantic records used by contentID; keeping the
...
const (
	...
	staticArtifactRecordEffectRows   uint64 = 9
	...
	staticArtifactEffectRowWireMin        = staticArtifactUintWireMin * 5
)
...
func writeArtifactViewContent(writer *canonical.Writer, view View) error {
	if view.state != nil {
		state := view.state
		state.mu.Lock()
...
func writeArtifactContent(
	...
	if err := writer.Record(staticArtifactRecordEffectRows); err != nil {
		return err
	}
	return writeEffectRowsContent(writer, effectRows)
}
...
func ReadArtifactSection(reader *canonical.Reader) (Input, error) {
	...
	if err := decoder.record(staticArtifactRecordEffectRows); err != nil {
		return Input{}, err
	}
	if err := decoder.effectRows(&input.EffectRows); err != nil {
		return Input{}, err
	}
	return input, nil
}
...
func (decoder *staticArtifactDecoder) preflightSection() error {
	...
	if err := probe.record(staticArtifactRecordEffectRows); err != nil {
		return err
	}
	return probe.effectRows(nil)
}

// probeReader makes a value copy of canonical.Reader. Reader's semantic
...
func (decoder *staticArtifactDecoder) probeReader() (staticArtifactDecoder, error) {
	if decoder == nil || decoder.reader == nil {
		return staticArtifactDecoder{}, canonical.ErrMalformed
	}
...
func (decoder *staticArtifactDecoder) terms() ([]keyspace.Term, error) {
	return decoder.termSequenceConstraint(0, staticArtifactAnyTerm)
}

func (decoder *staticArtifactDecoder) termSequence(minimum int) ([]keyspace.Term, error) {
	return decoder.termSequenceConstraint(minimum, staticArtifactAnyTerm)
}
...
func (decoder *staticArtifactDecoder) effectRows(output *EffectRowsInput) error {
	if !decoder.probing && !decoder.preflighted {
		probe, err := decoder.probeReader()
		if err != nil {
			return err
		}
		if err := probe.effectRows(nil); err != nil {
			return err
		}
	}
	count, err := decoder.count(staticArtifactEffectRowWireMin)
	if err != nil {
		return err
	}
	if !decoder.probing && output != nil {
		output.Rows = make([]EffectRow, count)
	}
	var previous keyspace.Term
	for index := 0; index < count; index++ {
		function, err := decoder.term()
		if err != nil {
			return err
		}
		if keyspace.TermFamily(function) != keyspace.FamilyFunction ||
			(index != 0 && function <= previous) {
			return errInvalidArtifactSection
		}
		occurrences, err := decoder.count(staticArtifactUintWireMin)
		if err != nil {
			return err
		}
		// No effect-operation schema exists at this source cut.  Rejecting a
		// nonzero count keeps the artifact closed instead of dropping labels.
		if occurrences != 0 {
			return errInvalidArtifactSection
		}
		rowFormals, err := decoder.uint32()
		if err != nil {
			return err
		}
		tail, err := decoder.enum(uint64(RowVariable))
		if err != nil {
			return err
		}
		variable, err := decoder.uint32()
		if err != nil {
			return err
		}
		spec := RowSpec{RowFormals: rowFormals, Tail: RowTail(tail), Var: RowVar(variable)}
		if err := validateRowSpec(spec); err != nil {
			return errInvalidArtifactSection
		}
		previous = function
		if !decoder.probing && output != nil {
			output.Rows[index] = EffectRow{Function: function, Row: spec}
		}
	}
	return nil
}
