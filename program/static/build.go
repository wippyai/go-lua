...
func Build(input Input) (*Draft, error) {
	...
	if err := compactEffectRows(component, input.Counts, input.EffectRows); err != nil {
		return nil, err
	}
...
func writeReferencesContent(writer *canonical.Writer, store referenceStore) error {
...
func writeReferenceKeysContent(writer *canonical.Writer, keys []keyspace.Key) error {
