...
type allocationPath struct{ id keyspace.ContentID }

func (view View) BodyPath(term keyspace.Term) (keyspace.ContentID, bool) {
...
func (view View) CallPath(term keyspace.Term) (keyspace.ContentID, bool) {
...
func (view View) ValueSourcePath(term keyspace.Term) (keyspace.ContentID, bool) {
...
func (view View) SemanticTermPath(term keyspace.Term) (keyspace.ContentID, bool) {
...
func (view View) StorageAssignmentPath(term keyspace.Term) (keyspace.ContentID, bool) {
...
func (o AllocationOccurrence) ID() keyspace.ContentID {
	if !o.Available() {
...
func certificateTerm(paths *semanticpath.Certificate, sourceID, flowID, staticID, moduleID keyspace.ContentID, term keyspace.Term) (keyspace.ContentID, bool) {
	return paths.TermPathAt(sourceID, flowID, staticID, moduleID, keyspace.TermFamily(term), keyspace.TermOrdinal(term))
}

func sealCertificateValueSourcePaths(sourceView source.View, view authored.View, paths *semanticpath.Certificate, staticID, moduleID keyspace.ContentID) ([keyspace.FamilyCount][]keyspace.ContentID, error) {
	sourceID, flowID := sourceView.Identity().ContentID(), view.Cold().ContentID()
	if !paths.Matches(sourceID, flowID, staticID, moduleID) {
		return [keyspace.FamilyCount][]keyspace.ContentID{}, errors.New("certificate provenance disagrees")
	}
	var out [keyspace.FamilyCount][]keyspace.ContentID
	for f := keyspace.Family(1); f < keyspace.FamilyCount; f++ {
		if n := sourceView.Identity().FamilyCount(f); n > 0 {
			out[f] = make([]keyspace.ContentID, n)
		}
	}
...
func sealCertificateStoragePaths(sourceView source.View, view authored.View, paths *semanticpath.Certificate, staticID, moduleID keyspace.ContentID) ([keyspace.FamilyCount][]keyspace.ContentID, error) {
	sourceID, flowID := sourceView.Identity().ContentID(), view.Cold().ContentID()
	if !paths.Matches(sourceID, flowID, staticID, moduleID) {
		return [keyspace.FamilyCount][]keyspace.ContentID{}, errors.New("certificate provenance disagrees")
	}
	var out [keyspace.FamilyCount][]keyspace.ContentID
	out[keyspace.FamilyAssign] = make([]keyspace.ContentID, sourceView.Identity().FamilyCount(keyspace.FamilyAssign))
...
func sealCertificateAllocationPaths(sourceView source.View, exec *executable.Result, view authored.View, paths *semanticpath.Certificate, staticID, moduleID keyspace.ContentID) ([keyspace.FamilyCount][]allocationPath, error) {
	sourceID, flowID := sourceView.Identity().ContentID(), view.Cold().ContentID()
	if !paths.Matches(sourceID, flowID, staticID, moduleID) {
...
func sealCertificateCallPaths(sourceView source.View, view authored.View, paths *semanticpath.Certificate, staticID, moduleID keyspace.ContentID) ([]keyspace.ContentID, error) {
	sourceID, flowID := sourceView.Identity().ContentID(), view.Cold().ContentID()
	if !paths.Matches(sourceID, flowID, staticID, moduleID) {
		return nil, errors.New("certificate provenance disagrees")
	}
	c := view.Calls()
	out := make([]keyspace.ContentID, c.Count())
	for i := 0; i < c.Count(); i++ {
		term, ok := c.At(i)
		if !ok {
