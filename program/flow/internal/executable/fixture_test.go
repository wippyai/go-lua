...
func openSealFixtureWithSource(
	...
	if err != nil {
		closeSealFinalizers(sourceFinalize, staticFinalize, authored.Finalizer{}, module.Finalizer{})
	...
	if err != nil {
		closeSealFinalizers(sourceFinalize, staticFinalize, flowFinalize, module.Finalizer{})
...
func TestSealFinalSourceExcludesOutcomeAndClosesRuntimeOperands(t *testing.T) {
	...
	if result.FamilyCount(keyspace.FamilyOutcome) != 0 || result.Executable(keyspace.MakeTerm(keyspace.FamilyOutcome, 1)) {
...
func TestSealRejectsForeignEqualCardinalityProvenance(t *testing.T) {
	...
	if _, err := Seal(first.sourceView, first.flow, foreignSource.forest, first.control,
		first.staticFinalize.View().ContentID(), first.moduleFinalize.View().ContentID()); err == nil || !strings.Contains(err.Error(), "containment provenance") {
	...
	if _, err := Seal(first.sourceView, first.flow, first.forest, foreignSource.control,
		first.staticFinalize.View().ContentID(), first.moduleFinalize.View().ContentID()); err == nil || !strings.Contains(err.Error(), "source-control provenance") {
...
func TestValidateBodyRootsRequiresExactParentAndSourceAgreement(t *testing.T) {
	...
	fixture := openSealFixture(t, "direct-parent.lua", counts, [][]keyspace.Term{{bind, returned}}, matrixFlow(),
		[]source.BindCells{{Bind: bind, Cells: []keyspace.Term{keyspace.MakeTerm(keyspace.FamilyCell, 1)}}}, nil, nil)
	if _, err := validateBodyRoots(fixture.sourceView, fixture.forest, fixture.control, counts); err != nil {
...
func TestSealRootlessRuntimeStillRequiresProvenance(t *testing.T) {
	...
	if result.Count() != 1 || !result.Executable(body) || result.Executable(label) {
		t.Fatalf("rootless membership = count %d body %v label %v", result.Count(), result.Executable(body), result.Executable(label))
	}
}
...
func TestSealClosesCompleteRuntimeOperandMatrix(t *testing.T) {
	...
	for family := keyspace.Family(1); family < keyspace.FamilyCount; family++ {
		for ordinal := uint32(1); ordinal <= counts[family]; ordinal++ {
			...
			got := result.Executable(term)
			if got != want {
				t.Fatalf("Executable(%d,%d) = %v, want %v", family, ordinal, got, want)
			}
		}
	}
	if result.Executable(term(keyspace.FamilyCell, 5)) {
