...
func testSourceID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 1
	return id
}

func testFlowID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 2
	return id
}

func testStaticID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 3
	return id
}

func testModuleID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 4
	return id
}
...
func TestExecutableQueryIsAllocationFree(t *testing.T) {
	...
	if unsafe.Sizeof(r.sourceID) != unsafe.Sizeof(keyspace.ContentID{}) {
		t.Fatal("provenance is not a fixed-width Source identity")
	}
	if unsafe.Sizeof(r.flowID) != unsafe.Sizeof(keyspace.ContentID{}) || !Matches(r, testSourceID(), testFlowID(), testStaticID(), testModuleID()) {
	...
	foreignFlow := newResult(counts, testSourceID(), keyspace.ContentID{}, testStaticID(), testModuleID())
	...
	for name, malformed := range map[string]*Result{
		"source": newResult(counts, keyspace.ContentID{}, testFlowID(), testStaticID(), testModuleID()),
		"flow":   foreignFlow,
		"static": newResult(counts, testSourceID(), testFlowID(), keyspace.ContentID{}, testModuleID()),
		"module": newResult(counts, testSourceID(), testFlowID(), testStaticID(), keyspace.ContentID{}),
	} {
