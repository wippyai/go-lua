...
func TestOutcomeProvenanceFailsClosedForNilAndZero(t *testing.T) {
	...
	if Matches(nilResult, keyspace.ContentID{0: 1}, keyspace.ContentID{1: 1}, keyspace.ContentID{2: 1}, keyspace.ContentID{3: 1}) {
	...
	staticID := current.staticFinalize.View().ContentID()
	moduleID := current.moduleFinalize.View().ContentID()
	if Matches(result, keyspace.ContentID{}, flowID, staticID, moduleID) || Matches(result, sourceID, keyspace.ContentID{}, staticID, moduleID) ||
		Matches(result, sourceID, flowID, keyspace.ContentID{}, moduleID) || Matches(result, sourceID, flowID, staticID, keyspace.ContentID{}) {
		t.Fatal("Outcome Matches did not fail closed for zero identity")
	}
	zeroValue := *result
	zeroValue.sourceID = keyspace.ContentID{}
	zeroValue.flowID = keyspace.ContentID{}
	zero := &zeroValue
