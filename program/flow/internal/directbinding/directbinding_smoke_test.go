...
func TestSealSmoke(t *testing.T) {
	...
	if allocations != 0 {
		t.Fatalf("typed directbinding queries allocated %v objects", allocations)
	}
	...
	for _, test := range foreign {
		bad := *test.id
		bad[0] ^= 1
		ids := [4]keyspace.ContentID{sourceID, flowID, staticID, moduleID}
		switch test.name {
		case "Source":
			ids[0] = bad
		case "Flow":
