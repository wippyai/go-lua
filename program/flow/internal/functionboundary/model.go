...
type Result struct {
	...
	contexts     map[keyspace.ContentID]uint32 // Function context -> Function row + 1.
	bodyContexts map[keyspace.ContentID]uint32 // Body context -> Body ordinal.
}
...
func Matches(result *Result, sourceID, flowID, staticID, moduleID keyspace.ContentID) bool {
...
func (result *Result) outcomeTermAt(poolOrdinal uint32) keyspace.Term {
	if poolOrdinal == 0 || uint64(poolOrdinal) > uint64(len(result.outcomes)) {
		return 0
	}
	return result.outcomes[poolOrdinal-1].term
}
