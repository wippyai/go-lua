...
func testBoundaryID(seed byte) keyspace.ContentID {
...
func validBoundaryResultForLaw(t *testing.T) *Result {
	...
	result := &Result{
		...
		contexts:     make(map[keyspace.ContentID]uint32),
		bodyContexts: make(map[keyspace.ContentID]uint32),
	}
...
func recomputeBoundaryContextsForLaw(result *Result) {
	result.bodyContexts = make(map[keyspace.ContentID]uint32, len(result.bodies)-1)
	for index := 1; index < len(result.bodies); index++ {
		row := result.bodies[index]
		row.context = hashBodyContext(result, row)
		result.bodies[index] = row
	...
	result.contexts = make(map[keyspace.ContentID]uint32, len(result.functions))
