...
func hashContext(result *Result, row functionRow) keyspace.ContentID {
	hash := sha256.New()
	...
	hash.Write(scalar[:])
	for _, owner := range [...]keyspace.ContentID{result.sourceID, result.flowID, result.staticID, result.moduleID} {
		hash.Write(owner[:])
	}
	...
	return keyspace.ContentID(hash.Sum(nil))
}
...
func hashBodyContext(result *Result, row bodyRow) keyspace.ContentID {
	hash := sha256.New()
