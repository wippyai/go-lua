...
// lcas answers all region-tree LCA queries with iterative Tarjan union-find.
// It is linear in the baseline WTO region tree plus query count; no event
// stores a copied ancestor stack and no deep loop can turn placement into a
// quadratic scan.
func (regions hierarchyRegions) lcas(queries []regionLCAQuery) []int {
	answers := make([]int, len(queries))
	if len(regions.parent) == 0 {
		return answers
	}
	type linked struct{ other, query int }
	links := make([][]linked, len(regions.parent))
	for index, query := range queries {
		if query.left <= 0 || query.right <= 0 || query.left >= len(links) || query.right >= len(links) {
			answers[index] = 0
			continue
		}
		links[query.left] = append(links[query.left], linked{other: query.right, query: index})
		links[query.right] = append(links[query.right], linked{other: query.left, query: index})
	}
	set := make([]int, len(regions.parent))
	ancestor := make([]int, len(regions.parent))
	black := make([]bool, len(regions.parent))
	for index := range set {
		set[index] = index
	}
	...
	for len(stack) != 0 {
		...
		stack = stack[:last]
		if !current.exit {
			ancestor[find(current.node)] = current.node
			stack = append(stack, frame{node: current.node, exit: true})
			for index := len(regions.children[current.node]) - 1; index >= 0; index-- {
				stack = append(stack, frame{node: regions.children[current.node][index]})
			}
			continue
		}
		black[current.node] = true
		for _, link := range links[current.node] {
			if black[link.other] {
				answers[link.query] = ancestor[find(link.other)]
			}
		}
		parent := regions.parent[current.node]
		if parent >= 0 {
			childRoot, parentRoot := find(current.node), find(parent)
			set[childRoot] = parentRoot
...
func (draft *hierarchyDraft) placeOutcomePhasePoints(
	...
	paths := make([]keyspace.ContentID, proof.Count())
	pathSet := make(map[keyspace.ContentID]struct{}, len(paths))
	parentByPath := make(map[keyspace.ContentID]keyspace.ContentID, len(paths))
	pathOrder := make(map[keyspace.ContentID]int, len(paths))
	...
	anchors := make(map[keyspace.ContentID]anchorRange)
	children := make(map[keyspace.ContentID]map[keyspace.ContentID]struct{})
	anchorPhase := func(path keyspace.ContentID, region int) error {
	...
	for child, parent := range parentByPath {
		...
		if children[child] == nil {
			children[child] = make(map[keyspace.ContentID]struct{})
		}
	...
	queryPath := make([]keyspace.ContentID, 0, len(anchors))
	for _, path := range paths {
		current, anchored := anchors[path]
	...
	answers := regions.lcas(queries)
	finalAnchors := make(map[keyspace.ContentID]int, len(anchors))
	...
	anchorExits := make(map[int][]keyspace.ContentID, len(anchors))
	for _, path := range paths {
...
func (event HierarchyEvent) VertexPath() (keyspace.ContentID, bool) {
	if !event.path.Available() {
...
func radixOutcomePaths(paths []keyspace.ContentID) {
	if len(paths) < 2 {
		return
	}
	work := make([]keyspace.ContentID, len(paths))
	for offset := len(keyspace.ContentID{}) - 1; offset >= 0; offset-- {
		var counts [256]int
		for _, path := range paths {
			counts[path[offset]]++
		}
