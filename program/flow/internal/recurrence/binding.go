...
func (component Component) HeadPath() (keyspace.ContentID, bool) {
...
func (route BoundRoute) FromPath() (keyspace.ContentID, bool) {
	if !route.issued || !route.row.fromPath.Available() {
		return keyspace.ContentID{}, false
	}
	return route.row.fromPath, true
}
func (route BoundRoute) ToPath() (keyspace.ContentID, bool) {
...
func bindPlan(result *Result, plan *routeplan.Plan, graph *sourcecontrol.Result, parts components, heads []keyspace.Term, hierarchy HierarchyProof, sourceView source.View, flow authored.View) (*Binding, error) {
	...
	hierarchyPaths := make(map[keyspace.ContentID]struct{}, len(hierarchy.events))
...
func radixComponents(entries []componentEntry) {
	if len(entries) < 2 {
		return
	}
	work := make([]componentEntry, len(entries))
