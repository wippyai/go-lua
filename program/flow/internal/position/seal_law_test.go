...
func openPositionFixture(t *testing.T, spec positionSpec) *positionFixture {
	...
	if err != nil {
		closePositionFixture(sourceFinalize, staticFinalize, flowFinalize, module.Finalizer{})
...
func closePositionFixture(sourceFinalize source.Finalizer, staticFinalize static.Finalizer, flowFinalize authored.Finalizer, moduleFinalize module.Finalizer) {
...
func TestSealPositionsContainModuleImportThroughItsCall(t *testing.T) {
	...
	fixture := openPositionFixture(t, positionSpec{
		...
		module: module.Input{Imports: []module.Import{{Term: importTerm, Call: call, Request: keyspace.MakeTerm(keyspace.FamilyString, 1)}}},
	})
	index, err := sealPositionFixture(fixture)
	if err != nil {
		t.Fatalf("Seal: %v", err)
