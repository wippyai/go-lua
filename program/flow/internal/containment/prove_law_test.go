...
func emptyModule(t *testing.T) module.Input {
	t.Helper()
	return module.Input{}
}
...
func TestProveRejectsWrongModuleImportCallForeignKey(t *testing.T) {
	input := module.Input{Imports: []module.Import{{
		Term: keyspace.MakeTerm(keyspace.FamilyImport, 1),
		Call: keyspace.MakeTerm(keyspace.FamilyFunction, 1),
	}}}
	if _, err := module.Build(input); err == nil {
		t.Fatal("module.Build accepted an Import whose Call foreign key is not a Call")
	}
}
