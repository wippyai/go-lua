...
func directTestSourceID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 1
	return id
}

func directTestFlowID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 2
	return id
}

func directTestStaticID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 3
	return id
}

func directTestModuleID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 4
	return id
}
...
func TestResultProvenanceFailsClosedForNilAndMalformed(t *testing.T) {
	...
	call := keyspace.MakeTerm(keyspace.FamilyCall, 1)
	loop := keyspace.MakeTerm(keyspace.FamilyLoop, 1)
	var sourceID, flowID keyspace.ContentID
	var staticID, moduleID keyspace.ContentID
	sourceID[0] = 0x11
	flowID[0] = 0x22
	staticID[0] = 0x33
	moduleID[0] = 0x44
	...
	if Matches(nil, sourceID, flowID, staticID, moduleID) || Matches(result, sourceID, flowID, staticID, moduleID) ||
		Matches(result, keyspace.ContentID{}, flowID, staticID, moduleID) ||
		Matches(result, sourceID, keyspace.ContentID{}, staticID, moduleID) ||
		Matches(result, sourceID, flowID, keyspace.ContentID{}, moduleID) ||
		Matches(result, sourceID, flowID, staticID, keyspace.ContentID{}) {
