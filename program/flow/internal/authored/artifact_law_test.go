...
func TestArtifactSectionRoundTripInjectsCountsAndPreservesContentID(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			reader, err := canonical.NewReader(first, len(first))
			if err != nil {
				t.Fatal(err)
			}
...
func TestArtifactSectionPayloadOnlyLeavesSentinelsAndUsesNineRecords(t *testing.T) {
	...
	reader, err := canonical.NewReader(buffer.Bytes(), buffer.Len())
	if err != nil {
		t.Fatal(err)
	}
	if err := reader.Header("program/flow-test", 1); err != nil {
...
func TestArtifactSectionRejectsMalformedTermCountRangeEnumOrderAndTruncation(t *testing.T) {
	cases := []struct {
		...
		{
			name:  "record-order",
			write: func(writer *canonical.Writer) { _ = writer.Record(2) },
		},
	}
...
func TestArtifactSectionRejectsNoncanonicalAndRemainingHostilePayloads(t *testing.T) {
	if err := readMalformedArtifact(func(writer *canonical.Writer) {
		_ = writer.Record(1)
	...
	// Record(1) with an overlong uvarint payload is not canonical.
	noncanonical := append(append([]byte(nil), header.Bytes()...), 3, 2, 0x81, 0x00)
	reader, err := canonical.NewReader(noncanonical, len(noncanonical))
	if err != nil {
		t.Fatal(err)
	}
	if err := reader.Header("program/flow-test", 1); err != nil {
...
func readMalformedArtifact(write func(*canonical.Writer)) error {
	var buffer bytes.Buffer
	var writer canonical.Writer
	if err := writer.Reset(&buffer, "program/flow-test", 1); err != nil {
		return err
...
func writeZeroTerm(writer *canonical.Writer) {
	_ = writer.Uint(0)
	_ = writer.Uint(0)
}
