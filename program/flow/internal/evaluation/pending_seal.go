...
func validatePendingParentForest(builder *pendingBuilder, counts [keyspace.FamilyCount]int) error {
	scratch, err := newPendingProofScratch(counts)
	if err != nil {
		return err
	}
	return validatePendingParentForestWithScratch(builder, counts, scratch)
}
...
func markPendingDemand(builder *pendingBuilder, counts [keyspace.FamilyCount]int) error {
	scratch, err := newPendingProofScratch(counts)
	if err != nil {
		return err
	}
	return markPendingDemandWithScratch(builder, counts, scratch)
}
	...
	for _, family := range pendingSubjectFamilies {
		for ordinal := 1; ordinal <= counts[family]; ordinal++ {
			...
			for current != 0 {
				...
				if builder.executable == nil || !builder.executable.Executable(current) {
