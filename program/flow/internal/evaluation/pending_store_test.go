...
func TestPendingCountDistinguishesEmptySubject(t *testing.T) {
	...
	pending := &Pending{
		sourceID: keyspace.ContentID{0: 1},
		flowID:   keyspace.ContentID{0: 2},
		staticID: keyspace.ContentID{0: 3},
		moduleID: keyspace.ContentID{0: 4},
		nodes:    []pendingNode{{}},
		sealed:   true,
	}
	pending.roots[keyspace.FamilyCall] = []uint32{0, 1}
	if !MatchesPending(pending, keyspace.ContentID{0: 1}, keyspace.ContentID{0: 2}, keyspace.ContentID{0: 3}, keyspace.ContentID{0: 4}) {
		t.Fatal("matching Pending provenance was rejected")
	}
	if MatchesPending(pending, keyspace.ContentID{0: 1}, keyspace.ContentID{0: 2}, keyspace.ContentID{0: 9}, keyspace.ContentID{0: 4}) ||
		MatchesPending(pending, keyspace.ContentID{}, keyspace.ContentID{0: 2}, keyspace.ContentID{0: 3}, keyspace.ContentID{0: 4}) {
