...
func validateInputs(
	...
	for family := keyspace.Family(1); family < keyspace.FamilyCount; family++ {
		...
		actual := authoredCount(flow, family)
...
func continuationCounts(identity source.Identity) ([keyspace.FamilyCount]uint32, error) {
	var counts [keyspace.FamilyCount]uint32
	if identity.ContentID() == (keyspace.ContentID{}) || identity.Name() == "" || identity.TermCount() == 0 {
	...
	if total != uint64(identity.TermCount()) || counts[keyspace.FamilyBody] == 0 {
...
func subjectFrom(exec *executable.Result, cand *candidates.Result, term keyspace.Term) bool {
	if exec == nil || cand == nil || !exec.Executable(term) {
		return false
	}
