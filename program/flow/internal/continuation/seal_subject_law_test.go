...
func TestContinuationSealDirectCallAndExactSubjectPlane(t *testing.T) {
	...
	for _, foreign := range []keyspace.ContentID{sourceID, flowID, fixture.staticID, fixture.moduleID} {
		candidate := [4]keyspace.ContentID{sourceID, flowID, fixture.staticID, fixture.moduleID}
		index := 0
...
func TestContinuationSealUnavailableAndForeignInputsFailClosed(t *testing.T) {
	...
	if _, err := Seal(left.sourceView, left.flow, left.bodies, left.binding, left.executable, left.candidates, left.causal, keyspace.ContentID{}, left.moduleID); err == nil {
		t.Fatal("Seal accepted an unavailable Static identity")
	}
	if _, err := Seal(left.sourceView, left.flow, left.bodies, left.binding, left.executable, left.candidates, left.causal, left.staticID, keyspace.ContentID{}); err == nil {
