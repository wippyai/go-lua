...
func continuationIDs() (keyspace.ContentID, keyspace.ContentID, keyspace.ContentID, keyspace.ContentID) {
	return keyspace.ContentID{0: 1}, keyspace.ContentID{0: 2}, keyspace.ContentID{0: 3}, keyspace.ContentID{0: 4}
}
...
func TestContinuationQueryLaws(t *testing.T) {
	...
	ids := []keyspace.ContentID{sourceID, flowID, staticID, moduleID}
	for index := range ids {
		foreign := ids[index]
		foreign[0]++
		candidate := []keyspace.ContentID{sourceID, flowID, staticID, moduleID}
	...
	if Matches(result, keyspace.ContentID{}, flowID, staticID, moduleID) ||
