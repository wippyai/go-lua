...
func TestCandidateQueriesFailClosedForUnavailableAndMalformedStorage(t *testing.T) {
	newProjection := func(sourceID, flowID keyspace.ContentID) *Result {
		return &Result{
			sourceID: sourceID,
			flowID:   flowID,
			staticID: candidateValidID(3),
	...
	for _, owner := range []struct {
		...
		{name: "nil", result: nil},
		{name: "zero", result: newProjection(keyspace.ContentID{}, keyspace.ContentID{})},
		{name: "zero-source", result: newProjection(keyspace.ContentID{}, candidateValidID(2))},
		{name: "zero-flow", result: newProjection(candidateValidID(1), keyspace.ContentID{})},
	} {
...
func TestCandidateSealRejectsZeroForeignAndExpiredAuthority(t *testing.T) {
	if _, err := Seal(source.Identity{}, authored.View{}, nil, keyspace.ContentID{}, keyspace.ContentID{}); err == nil {
		t.Fatal("zero identity/proof accepted")
	}

	identity, view, finish := minimalOwners(t)
	defer finish()
	if _, err := Seal(identity, view, nil, keyspace.ContentID{}, keyspace.ContentID{}); err == nil {
		t.Fatal("nil proof accepted for a valid identity/view")
	}
	if _, err := Seal(identity, view, &executable.Result{}, keyspace.ContentID{}, keyspace.ContentID{}); err == nil {
	...
	expiredFinish()
	if _, err := Seal(expired, expiredView, nil, keyspace.ContentID{}, keyspace.ContentID{}); err == nil {
...
func candidateValidID(seed byte) keyspace.ContentID {
