...
func (r *Result) componentPath(head keyspace.Term) (keyspace.ContentID, bool) {
...
func regionID(path keyspace.ContentID) keyspace.ContentID {
	if !path.Available() {
...
func (r *Result) buildLocal() bool {
	...
	store := localStore{
		...
		byID:        make(map[keyspace.ContentID]uint32, len(r.components)),
...
func (v Local) Resolve(id keyspace.ContentID) (Region, bool) {
...
func (region Region) ID() keyspace.ContentID {
	if !region.Available() {
		return keyspace.ContentID{}
	}
	return region.id
