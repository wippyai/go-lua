...
type wtoStore struct {
	...
	points      []wtoPoint
	pointByPath map[keyspace.ContentID]uint32
}
...
func wtoID(domain string, semantic keyspace.ContentID) keyspace.ContentID {
...
func (region WTORegion) ID() keyspace.ContentID {
	row, ok := region.row()
	if !ok {
		return keyspace.ContentID{}
	}
...
func (region WTORegion) ParentID() keyspace.ContentID {
	row, ok := region.row()
	if !ok {
		return keyspace.ContentID{}
	}
...
func (view LocalWTO) Resolve(id keyspace.ContentID) (WTORegion, bool) {
...
func (point WTOPoint) PathID() keyspace.ContentID {
	row, ok := point.row()
	if !ok {
		return keyspace.ContentID{}
	}
...
func (s Successor) WTORegionID() keyspace.ContentID {
...
func (r *Result) prepareWTOReceipts() error {
	...
	pathNode := make(map[keyspace.ContentID]uint32)
	paths := make([]keyspace.ContentID, 0)
	...
	addSite := func(path keyspace.ContentID, site Site) bool {
...
func (r *Result) finalizeLocalWTO() error {
	...
	store := wtoStore{byID: make(map[keyspace.ContentID]uint32), pointByPath: make(map[keyspace.ContentID]uint32)}
	vertexPaths, pathsOK := r.issuedVertexPaths()
	if !pathsOK {
		return failWTO(wtoFailurePhaseVertexPaths, wtoFailureReasonVertexPaths, -1, -1, -1)
	}
	nodeForPath := make(map[keyspace.ContentID]uint32, len(vertexPaths))
...
func (r *Result) issuedVertexPaths() ([]keyspace.ContentID, bool) {
	if r == nil || len(r.pendingVertexPaths) != len(r.pendingNodeSites) || len(r.pendingWTORoutes) != len(r.index.refs) {
		return nil, false
	}
	paths := append([]keyspace.ContentID(nil), r.pendingVertexPaths...)
	seen := make(map[keyspace.ContentID]struct{}, len(paths))
...
func (r *Result) semanticWTOSites(vertex uint32) ([]uint32, keyspace.ContentID, bool) {
	if r == nil || int(vertex) >= len(r.pendingNodeSites) {
		return nil, keyspace.ContentID{}, false
	}
	sites := append([]uint32(nil), r.pendingNodeSites[vertex]...)
	if len(sites) == 0 {
		return nil, keyspace.ContentID{}, true
	}
	paths := make(map[uint32]keyspace.ContentID, len(sites))
...
func digestWTOSites(paths map[uint32]keyspace.ContentID, sites []uint32) keyspace.ContentID {
	if len(sites) == 0 {
		return keyspace.ContentID{}
	}
	...
	return keyspace.ContentID(sha256.Sum256(encoded.Bytes()))
}
...
func (r *Result) classifyWTORoutes(store *wtoStore, nodeRegion []uint32) error {
	...
	lcas := offlineWTOLCAs(parents, roots, left, right)
	if len(lcas) != len(r.index.refs) {
		return failWTO(wtoFailurePhaseClassify, wtoFailureReasonClassifyLCA, -1, -1, -1)
	}
	membership := make(map[keyspace.ContentID]keyspace.ContentID, len(r.index.refs))
	for index, ref := range r.index.refs {
...
// radixWTOSites and radixWTORoutes impose ContentID byte order in fixed
// 32-pass radix time. The digest width is constant, so this is linear in the
// number of issued Site/route members and never derives order from row IDs.
func radixWTOSites(sites []uint32, paths map[uint32]keyspace.ContentID) {
	if len(sites) < 2 {
		return
	}
	work := make([]uint32, len(sites))
	for byteIndex := len(keyspace.ContentID{}) - 1; byteIndex >= 0; byteIndex-- {
		var counts [256]int
		for _, site := range sites {
			counts[paths[site][byteIndex]]++
		}
		position := 0
		for index := range counts {
			position, counts[index] = position+counts[index], position
		}
		for _, site := range sites {
			byteValue := paths[site][byteIndex]
			work[counts[byteValue]] = site
			counts[byteValue]++
		}
		copy(sites, work)
	}
...
func radixWTORoutes(routes []successorRef) {
	if len(routes) < 2 {
		return
	}
	work := make([]successorRef, len(routes))
	for byteIndex := len(keyspace.ContentID{}) - 1; byteIndex >= 0; byteIndex-- {
		var counts [256]int
		for _, route := range routes {
			counts[route.semanticPath[byteIndex]]++
		}
		position := 0
		for index := range counts {
			position, counts[index] = position+counts[index], position
		...
		for _, route := range routes {
			byteValue := route.semanticPath[byteIndex]
			work[counts[byteValue]] = route
			counts[byteValue]++
		}
		copy(routes, work)
	}
...
// offlineWTOLCAs is Tarjan's offline LCA algorithm expressed iteratively so
// deep program nesting cannot consume the Go call stack. roots carry the
// explicit forest fence: queries across roots intentionally stay unassigned.
func offlineWTOLCAs(parents, roots, left, right []uint32) []uint32 {
	const none = ^uint32(0)
	...
	for root, parent := range parents {
		...
		for len(stack) != 0 {
			...
			for _, query := range queries[node] {
				...
				if black[other] && answers[query] == none {
...
func digestWTOParent(parent, path keyspace.ContentID) keyspace.ContentID {
	if !path.Available() {
		return keyspace.ContentID{}
	}
	if !parent.Available() {
