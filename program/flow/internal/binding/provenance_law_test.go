...
func bindingTestSourceID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 0x11
	return id
}

func bindingTestFlowID() keyspace.ContentID {
	var id keyspace.ContentID
	id[0] = 0x22
	return id
}
...
func TestSealAndMatchesRejectEqualDenominatorForeignOwners(t *testing.T) {
	...
	if Matches(&result, foreignSourceID, first.flowID) || Matches(&result, first.sourceID, foreignFlowID) ||
		Matches(&result, keyspace.ContentID{}, first.flowID) || Matches(&result, first.sourceID, keyspace.ContentID{}) {
