...
func validateDenominators(sourceView source.View, flow authored.View) (denominators, error) {
	...
	if identity.Name() == "" || identity.TermCount() == 0 || identity.ContentID() == (keyspace.ContentID{}) {
	...
	if counts.tableFields != identity.FamilyCount(keyspace.FamilyTableField) ||
		counts.exactLenses != identity.FamilyCount(keyspace.FamilyLensExact) ||
		counts.dynamicLenses != identity.FamilyCount(keyspace.FamilyLensKey) ||
		counts.reads != identity.FamilyCount(keyspace.FamilyRead) ||
		counts.writes != identity.FamilyCount(keyspace.FamilyWrite) ||
		counts.assigns != identity.FamilyCount(keyspace.FamilyAssign) {
...
func normalizedFieldKey(
	...
	case kind.FieldExact:
		...
		normalized, normalOK := source.NormalizeExactKey(literal)
		if !normalOK {
			return 0, true, nil
		}
