...
func TestModuleEntryRetainsExecutableReturnSlotsAndDirectFunction(t *testing.T) {
	...
	if input.Entry.ReturnIndex[fixture.returnedOrdinal] != 1 || input.Entry.RootRanges[fixture.returnedOrdinal] != (module.EntryRange{Start: 0, End: 1}) {
...
func openModuleEntryFunctionFixture(t *testing.T) *moduleEntryFunctionFixture {
	...
	executableResult, err := executable.Seal(sourceView, flowView, forest, controlGraph, staticID, moduleID)
	if err != nil {
...
func moduleEntryModule(t *testing.T, input module.Input) module.View {
	t.Helper()
	draft, err := module.Build(input)
	if err != nil {
		t.Fatalf("module.Build: %v", err)
	}
	finalizer, err := draft.Finalizer()
