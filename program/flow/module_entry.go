...
// projections, and receives one transient module.CommitInput to pass directly
...
// module.Finalizer.Commit; this helper never returns a Module Result, stores a
...
func sealModuleEntry(
	...
	entry keyspace.Term,
) (module.CommitInput, error) {
	var empty module.CommitInput
	...
	return module.CommitInput{Resolutions: resolutions, Entry: entryInput}, nil
}
...
func sealModuleImportResolutions(
	...
	resolutions := make([]module.Resolution, moduleView.Count())
	...
	for index := 0; index < moduleView.Count(); index++ {
		...
		resolutions[index] = module.Resolution{Request: row.Request, Key: key}
	}
	return resolutions, nil
}
...
func sealChunkEntry(
	...
	if returnCount < 0 || !keyspace.TermOrdinalFits(returnCount) || fields.Count() < 0 || !keyspace.TermOrdinalFits(fields.Count()) {
		return module.Entry{}, errors.New("program/flow: module entry denominator is unavailable")
	}
	keyScratch, err := newEntryKeyScratch(sourceView)
	if err != nil {
		return module.Entry{}, err
	}
	input := module.Entry{
		ReturnIndex:  make([]uint32, returnCount+1),
		RootRanges:   make([]module.EntryRange, returnCount+1),
		MemberRanges: make([]module.EntryRange, returnCount+1),
	...
	for index := 0; index < returnCount; index++ {
		...
		if !ok || term != keyspace.MakeTerm(keyspace.FamilyReturn, uint32(index+1)) {
			return module.Entry{}, errors.New("program/flow: malformed Return denominator")
		}
	}
	for index := 0; index < fields.Count(); index++ {
		...
		if !ok || term != keyspace.MakeTerm(keyspace.FamilyTableField, uint32(index+1)) {
			return module.Entry{}, errors.New("program/flow: malformed TableField denominator")
		}
	}
	...
	for index := 0; index < returnCount; index++ {
		...
		if !ok || keyspace.TermFamily(owner) != keyspace.FamilyBody || keyspace.TermOrdinal(owner) == 0 ||
			...
			return module.Entry{}, errors.New("program/flow: malformed Return row")
		}
		ownerActivation, ok := bodies.Activation(owner)
		if !ok {
			return module.Entry{}, errors.New("program/flow: Return Body activation is unavailable")
		}
		...
		if valuesOwner, _, valuesOK := values.Get(valuesTerm); !valuesOK || valuesOwner != owner {
			return module.Entry{}, errors.New("program/flow: Return Values owner disagrees")
		}

		fixedCount, ok := values.Len(valuesTerm)
		if !ok || fixedCount < 0 || !keyspace.TermOrdinalFits(fixedCount) {
			return module.Entry{}, errors.New("program/flow: Return fixed Pack is unavailable")
		}
		...
		for position := 0; position < fixedCount; position++ {
			...
			if !ok {
				return module.Entry{}, errors.New("program/flow: Return fixed Pack position is unavailable")
			}
		...
		input.RootRanges[index+1] = module.EntryRange{Start: start, End: uint32(len(input.Roots))}
		input.MemberRanges[index+1] = module.EntryRange{Start: memberStart, End: uint32(len(input.Members))}
