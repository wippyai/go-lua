...
type Bodies struct {
	...
	active map[symbol.ID]keyspace.Term
	undo   []activeUndo
	frames []bodyFrame
	source []sourceItem
...
func (b *Bodies) ScheduleVararg(expr *ast.Comma3Expr, owner keyspace.Term, span source.Span) error {
	if b == nil || b.collector == nil || b.binding == nil || b.phases == nil || expr == nil || owner == 0 {
		return fmt.Errorf("programlower: invalid lexical vararg authority")
	}
	if len(b.frames) == 0 || owner != b.Owner() {
		return fmt.Errorf("programlower: vararg request crossed Body boundary")
	}
	cell, err := b.Vararg(span)
	if err != nil {
		return err
	}
	term := b.collector.Flow().Storage().Vararg(span, owner, cell)
	if term == 0 {
		return fmt.Errorf("programlower: could not create Vararg")
	}
...
func (b *Bodies) Entry(span source.Span) (keyspace.Term, error) {
	if b == nil || b.collector == nil {
		return 0, fmt.Errorf("programlower: invalid lexical source authority")
	}
	body := b.collector.Source().Order().Body(span)
	if body == 0 {
		return 0, fmt.Errorf("programlower: could not create chunk body")
	}
	if !b.collector.Source().Order().SetEntry(body) {
...
func (b *Bodies) Finish() (keyspace.Term, error) {
	...
	for _, current := range b.localSteps {
		if current.owner == owner {
			return 0, fmt.Errorf("programlower: unfinished lexical local transaction")
		}
	}
	...
	for index, item := range items {
		if item.term == 0 || item.cell != 0 {
			return 0, fmt.Errorf("programlower: unresolved Body source evidence")
		}
		terms[index] = item.term
	}
	if !b.collector.Source().Order().SetBody(frame.body, terms...) {
...
func (b *Bodies) Append(term keyspace.Term) error {
	if term == 0 {
		return fmt.Errorf("programlower: could not append Body source Term")
	}
...
func (b *Bodies) ReserveCell(cell symbol.ID) (CellEvidence, error) {
	if cell == 0 || len(b.frames) == 0 {
		return CellEvidence{}, fmt.Errorf("programlower: invalid Body source Cell evidence")
	}
...
func (b *Bodies) ResolveCell(evidence CellEvidence) (keyspace.Term, error) {
	if !b.validEvidence(evidence) {
		return 0, fmt.Errorf("programlower: invalid Body source Cell evidence")
	}
	cell, ok := b.Resolve(evidence.cell)
	if !ok || cell == 0 {
		return 0, fmt.Errorf("programlower: Body source Cell is absent")
	}
	return cell, nil
}
...
func (b *Bodies) Fill(evidence CellEvidence, term keyspace.Term) error {
	if term == 0 || !b.validEvidence(evidence) {
		return fmt.Errorf("programlower: invalid Body source evidence fill")
	}
	item := &b.source[evidence.source]
	if item.term != 0 || item.cell != evidence.cell {
		return fmt.Errorf("programlower: Body source evidence already filled")
	}
	item.term = term
	item.cell = 0
	return nil
...
func (b *Bodies) Resolve(id symbol.ID) (keyspace.Term, bool) {
	term, ok := b.active[id]
...
func (b *Bodies) Vararg(span source.Span) (keyspace.Term, error) {
	if b == nil || b.binding == nil || len(b.frames) == 0 {
		return 0, fmt.Errorf("programlower: vararg expression outside Body")
	}
	function := b.frames[len(b.frames)-1].function
	if function == nil {
		if b.chunkVararg != 0 {
		...
		if b.collector == nil || b.collector.Source().Order().Entry() != entry {
			return 0, fmt.Errorf("programlower: chunk vararg requires the entry Body")
		}
		cell := b.collector.Flow().Storage().Cell(span, entry)
		if cell == 0 {
			return 0, fmt.Errorf("programlower: could not create chunk vararg Cell")
		}
		b.chunkVararg = cell
		return cell, nil
	}
...
func (b *Bodies) Declare(id symbol.ID, span source.Span) (keyspace.Term, error) {
...
func (b *Bodies) ScheduleLocal(stmt *ast.LocalAssignStmt, owner keyspace.Term, span source.Span) error {
	if b == nil || b.collector == nil || b.binding == nil || b.modules == nil || b.phases == nil || b.values == nil || b.statics == nil || stmt == nil {
		return fmt.Errorf("programlower: invalid lexical local authority")
	}
	if owner == 0 || owner != b.Owner() || span.File == "" || len(stmt.Names) == 0 || len(stmt.Types) > len(stmt.Names) {
		return fmt.Errorf("programlower: invalid local declaration")
	}
...
func (b *Bodies) Run() error {
	if b == nil || b.phases == nil || len(b.localSteps) == 0 {
		return fmt.Errorf("programlower: missing lexical continuation")
	}
	last := len(b.localSteps) - 1
	current := b.localSteps[last]
	b.localSteps = b.localSteps[:last]
	if current.owner == 0 || current.owner != b.Owner() {
		return fmt.Errorf("programlower: lexical local continuation crossed Body boundary")
	}
	...
	case localBind:
		...
		if values == 0 || open {
			return fmt.Errorf("programlower: invalid local initializer Values")
		}
		...
		if !ok || id == 0 {
			return fmt.Errorf("programlower: missing one-cell local binder identity")
		}
		cell, ok := b.Resolve(id)
		if !ok || cell == 0 {
			return fmt.Errorf("programlower: missing one-cell local Cell")
		}
		return b.modules.AttachAlias(current.stmt, cell)
	default:
		return fmt.Errorf("programlower: invalid lexical local continuation %d", current.kind)
	}
}
...
// local route.
func (b *Bodies) reserve(id symbol.ID, span source.Span) (keyspace.Term, error) {
	if id == 0 {
	...
	cell := b.collector.Flow().Storage().Cell(span, b.Owner())
	if cell == 0 {
...
func (b *Bodies) DeclareLoop(id symbol.ID, span source.Span) (keyspace.Term, error) {
...
func (b *Bodies) declare(
	...
	owner := b.Owner()
	cell := b.collector.Flow().Storage().Cell(span, owner)
	if cell == 0 {
...
func (b *Bodies) Bind(mark int, span source.Span, values keyspace.Term) error {
	...
	owner := b.Owner()
	term := b.collector.Flow().Storage().Bind(span, owner, b.cellSlice()[mark:], values)
...
func (b *Bodies) bindReserved(cellMark, reservedMark int, span source.Span, values keyspace.Term) error {
	...
	owner := b.Owner()
	term := b.collector.Flow().Storage().Bind(span, owner, cells, values)
	if term == 0 {
...
func (b *Bodies) Capture(
	...
	owner := b.Owner()
	inner := b.collector.Flow().Storage().Cell(span, owner)
	if inner == 0 {
...
func (b *Bodies) DeclareFunction(span source.Span) (keyspace.Term, error) {
	...
	term := b.collector.Flow().Functions().DeclareFunction(span, b.Owner())
	if term == 0 {
...
func (b *Bodies) FillFunction(
	...
	if !b.collector.Flow().Functions().FillFunction(function, body, formals, vararg, b.captures[captureMark:]) {
...
func (b *Bodies) install(id symbol.ID, term keyspace.Term) {
	if b.active == nil {
		b.active = make(map[symbol.ID]keyspace.Term)
	}
...
func (b *Bodies) span(holder ast.PositionHolder) source.Span {
	...
	span, ok := sourcecoord.Build(b.sourceName, holder.Line(), holder.Column(), holder.LastLine(), holder.LastColumn())
	if !ok {
		return sourcecoord.Invalid(b.sourceName)
	}
	return span
}
...
func (b *Bodies) nameSpan(stmt *ast.LocalAssignStmt, index int) source.Span {
	...
	span, ok := sourcecoord.Build(b.sourceName, position.Line, position.Column, position.EndLine, position.EndColumn)
	if !ok {
		return sourcecoord.Invalid(b.sourceName)
	}
	return span
