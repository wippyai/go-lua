...
type Writer struct {
	phase       *phase.Stack
	exprs       *inbox.Expressions
	values      *eval.Values
	store       *store.Writer
	static      *staticlower.Writer
	modules     *modulelower.Writer
	calls       collector.FlowCallsWriter
	operands    collector.FlowOperandsWriter
	constructed bool
	binding     *bind.Result
	file        string
...
func New(
	...
	var calls collector.FlowCallsWriter
	var operands collector.FlowOperandsWriter
	if construction != nil {
		calls = construction.Flow().Calls()
		operands = construction.Flow().Operands()
	}
	return &Writer{
		phase:       stack,
		exprs:       expressions,
...
func (w *Writer) Schedule(call *ast.FuncCallExpr, host keyspace.Term, span source.Span) error {
	if !w.ready() || call == nil || host == 0 || span.File == "" {
		return fmt.Errorf("programlower: invalid Call authority")
	}
	...
	if attribute, ok := call.Func.(*ast.AttrGetExpr); ok {
		if attribute == nil || attribute.Object == nil || attribute.Key == nil {
			return fmt.Errorf("programlower: invalid plain Call attribute")
		}
		attributeSpan := w.span(attribute)
		if attributeSpan.File == "" {
			return fmt.Errorf("programlower: unresolved plain Call attribute span")
		}
...
func (w *Writer) scheduleBase(expr ast.Expr, owner keyspace.Term, span source.Span) error {
	...
	if term == 0 {
		return fmt.Errorf("programlower: could not create runtime TypeValue")
	}
...
func (w *Writer) Run() error {
	if !w.ready() || len(w.steps) == 0 {
		return fmt.Errorf("programlower: missing Call continuation")
	}
	last := len(w.steps) - 1
	current := w.steps[last]
	w.steps = w.steps[:last]
	if current.owner == 0 || current.span.File == "" {
		return fmt.Errorf("programlower: Call continuation lost source identity")
	}
	...
	default:
		return fmt.Errorf("programlower: invalid Call continuation %d", current.kind)
	}
}
...
func (w *Writer) finishPlainBase(current step) error {
	...
	if base == 0 || current.call == nil || current.owner == 0 {
		return fmt.Errorf("programlower: missing plain Call callee")
	}
	...
	case ast.AttrKeyDot:
		...
		if !ok || name == nil {
			return fmt.Errorf("programlower: dot Call callee key is not a string literal")
		}
	...
	default:
		return fmt.Errorf("programlower: unsupported plain Call attribute syntax %d", attribute.KeySyntax)
	}
}
...
func (w *Writer) finishPlainAttributeKey(current step) error {
	...
	if !ok || attribute == nil || current.base == 0 || key == 0 {
		return fmt.Errorf("programlower: missing plain Call attribute key")
	}
	if current.attributeSpan.File == "" {
		return fmt.Errorf("programlower: plain Call attribute lost source span")
	}
...
func (w *Writer) finishMethodBase(current step) error {
	...
	if current.call == nil || receiver == 0 || current.owner == 0 || current.call.Method == "" || !current.call.MethodPosition.Valid() {
		return fmt.Errorf("programlower: invalid method Call completion")
	}
...
func (w *Writer) scheduleArguments(call *ast.FuncCallExpr, owner keyspace.Term, span source.Span, callee, receiver keyspace.Term) error {
	if call == nil || owner == 0 || span.File == "" || callee == 0 || w.values == nil {
		return fmt.Errorf("programlower: invalid Call arguments")
	}
...
func (w *Writer) finishArguments(current step) error {
	...
	if current.call == nil || current.owner == 0 || current.callee == 0 || actuals == 0 {
		return fmt.Errorf("programlower: incomplete Call arguments")
	}
	term := w.calls.DeclareCall(current.span, current.owner, current.callee, current.receiver, actuals)
	if term == 0 {
		return fmt.Errorf("programlower: could not declare Call")
	}
	mark := w.static.Mark()
	if mark < 0 {
		return fmt.Errorf("programlower: invalid Call static argument mark")
	}
...
func (w *Writer) finishTypeArguments(current step) error {
	if current.call == nil || current.callTerm == 0 || current.staticMark < 0 {
		return fmt.Errorf("programlower: invalid Call static argument continuation")
	}
	if current.waiting {
		argument, _ := w.phase.Result()
		if argument == 0 {
			return fmt.Errorf("programlower: missing Call static argument")
		}
	...
	if current.index == len(current.call.TypeArgs) {
		...
		if !w.calls.SetCallTypeArgs(current.callTerm, arguments) {
			return fmt.Errorf("programlower: could not finalize Call static arguments")
		}
	...
	if current.index < 0 || current.index >= len(current.call.TypeArgs) || current.call.TypeArgs[current.index] == nil {
		return fmt.Errorf("programlower: invalid Call static argument %d", current.index)
	}
...
func (w *Writer) expression(expr ast.Expr, owner keyspace.Term, span source.Span) error {
	if expr == nil || owner == 0 || span.File == "" {
		return fmt.Errorf("programlower: invalid ordinary Call expression")
	}
...
func (w *Writer) schedule(next step) error {
	if next.kind == 0 || next.owner == 0 || next.span.File == "" {
		return fmt.Errorf("programlower: invalid Call continuation identity")
	}
	w.steps = append(w.steps, next)
	w.phase.Push(phase.Call)
	return nil
}
...
func (w *Writer) span(holder ast.PositionHolder) source.Span {
	...
	span, ok := sourcecoord.Build(w.file, holder.Line(), holder.Column(), holder.LastLine(), holder.LastColumn())
	if !ok {
		return sourcecoord.Invalid(w.file)
	}
	return span
}
...
func (w *Writer) positionSpan(position ast.Position) source.Span {
	...
	span, ok := sourcecoord.Build(w.file, position.Line, position.Column, position.EndLine, position.EndColumn)
	if !ok {
		return sourcecoord.Invalid(w.file)
	}
	return span
