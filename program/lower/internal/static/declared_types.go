...
func (w *Writer) DeclareCellTypeAt(host keyspace.Term, span source.Span, target keyspace.Term) error {
	...
	if w.static.Declarations().DeclaredType(span, host, target) == 0 {
...
func (w *Writer) DeclareImplicitSelfType(host keyspace.Term, span source.Span, decl bind.TypeDecl) error {
	...
	ref := w.static.References().Declaration(span, []string{decl.Name}, 0, target)
	if ref == 0 {
		return fmt.Errorf("programlower: could not create implicit self type reference")
	}
	if w.static.Declarations().DeclaredType(span, host, ref) == 0 {
