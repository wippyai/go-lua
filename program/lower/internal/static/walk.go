...
func (w *Writer) ScheduleInterface(def *ast.InterfaceDefStmt, body keyspace.Term, span source.Span) error {
	...
	if def == nil || body == 0 || body != w.scopes.Owner() || span.File == "" || span != w.span(def) {
		return fmt.Errorf("programlower: nil interface declaration")
	}
...
func (w *Writer) ScheduleType(expr ast.TypeExpr, host, body keyspace.Term, span source.Span) error {
	...
	if !validTypeExpr(expr) || host == 0 || body == 0 || body != w.scopes.Owner() || span.File == "" || span != w.typeSpan(expr) {
		return fmt.Errorf("programlower: invalid static type schedule")
	}
...
func (w *Writer) scheduleType(expr ast.TypeExpr, host, body keyspace.Term, span source.Span) error {
	if !validTypeExpr(expr) || host == 0 || body == 0 || span.File == "" {
		return fmt.Errorf("programlower: invalid static type continuation")
	}
...
func (w *Writer) scheduleAnnotations(
	...
	if scope == 0 || target == 0 || body == 0 || span.File == "" {
		return fmt.Errorf("programlower: invalid annotation continuation")
	}
...
func (w *Writer) ScheduleDeclaredCellType(expr ast.TypeExpr, cell, body keyspace.Term, span source.Span) error {
	...
	if !validTypeExpr(expr) || cell == 0 || body == 0 || body != w.scopes.Owner() || span.File == "" || span != w.typeSpan(expr) {
		return fmt.Errorf("programlower: invalid declared Cell type schedule")
	}
...
func (w *Writer) Run() error {
	...
	if len(w.steps) == 0 {
		return fmt.Errorf("programlower: missing static continuation")
	}
	last := len(w.steps) - 1
	current := w.steps[last]
	w.steps = w.steps[:last]
	if current.body == 0 || current.body != w.scopes.Owner() {
		return fmt.Errorf("programlower: static continuation Body is not active")
	}
	...
	case finishAnnotatedWalk:
		...
		if !ok || expr == nil || expr.Inner == nil {
			return fmt.Errorf("programlower: invalid annotated type continuation")
		}
	...
	case finishKeyOfWalk:
		...
		if !ok || expr == nil {
			return fmt.Errorf("programlower: invalid keyof continuation")
		}
	...
	case finishIndexWalk:
		...
		if !ok || expr == nil {
			return fmt.Errorf("programlower: invalid indexed-access continuation")
		}
	...
	case finishConditionalWalk:
		...
		if !ok || expr == nil {
			return fmt.Errorf("programlower: invalid conditional continuation")
		}
	...
	case finishAnnotationWalk:
		...
		if values == 0 || open {
			return fmt.Errorf("programlower: annotation arguments are not closed Values")
		}
	...
	case finishArrayWalk:
		...
		if !ok || expr == nil {
			return fmt.Errorf("programlower: invalid array continuation")
		}
	...
	case finishMapKeyWalk:
		...
		if !ok || expr == nil {
			return fmt.Errorf("programlower: invalid map key continuation")
		}
	...
	case finishMapWalk:
		expr, ok := current.node.(*ast.MapTypeExpr)
		if !ok || expr == nil {
			return fmt.Errorf("programlower: invalid map continuation")
		}
	...
	default:
		return fmt.Errorf("programlower: unknown static continuation %d", current.kind)
	}
}
...
func (w *Writer) finishAlias(current walkStep) error {
	if current.alias == nil {
		return fmt.Errorf("programlower: invalid type alias completion")
	}
...
func (w *Writer) runInterfaceExtends(current walkStep) error {
	if current.iface == nil || current.index < 0 || current.index > len(current.iface.Extends) {
		return fmt.Errorf("programlower: invalid interface extends cursor")
	}
	...
	if extend == nil {
		return fmt.Errorf("programlower: absent interface extends reference at index %d", current.index)
	}
	current.index++
	w.push(current)
...
func (w *Writer) runInterfaceMembers(current walkStep) error {
	if current.iface == nil || current.index < 0 || current.index > len(current.iface.Members) {
		return fmt.Errorf("programlower: invalid interface member cursor")
	}
	...
	case ast.InterfaceFieldMember:
		if member.Name == "" || member.Type == nil {
			return fmt.Errorf("programlower: invalid interface field %d", current.index-1)
		}
		w.push(current)
	...
	case ast.InterfaceMethodMember:
		if member.Name == "" {
			return fmt.Errorf("programlower: invalid interface method %d", current.index-1)
		}
		if _, ok := member.Type.(*ast.FunctionTypeExpr); !ok {
			return fmt.Errorf("programlower: interface method %q has non-function signature", member.Name)
		}
		w.push(current)
		w.push(walkStep{kind: appendInterfaceMethodWalk, member: member, body: current.body, span: current.span})
		return w.scheduleType(member.Type, current.typeBase, current.body, w.typeSpan(member.Type))
	default:
		return fmt.Errorf("programlower: invalid interface member kind %d", member.Kind)
	}
}
...
func (w *Writer) runType(current walkStep) error {
	...
	case *ast.AnnotatedTypeExpr:
		if expr.Inner == nil {
			return fmt.Errorf("programlower: annotated type has no inner type")
		}
	...
	case *ast.TypeOfExpr:
		if expr.Expr == nil {
			return fmt.Errorf("programlower: typeof has no expression")
		}
		w.staticDepth++
	...
	default:
		...
		if !found {
			return fmt.Errorf("programlower: unsupported type expression %T", expr)
		}
		return w.publish(term, err)
	}
}
...
func (w *Writer) runIndexChildren(current walkStep) error {
	...
	if !ok || expr == nil || current.index < 0 || current.index > 2 || current.staticMark < 0 || w.Mark() != current.staticMark+current.index {
		return fmt.Errorf("programlower: invalid indexed-access child cursor")
	}
	if current.index == 2 {
		return nil
	}
	...
	if child == nil {
		return fmt.Errorf("programlower: absent indexed-access child")
	}
	current.index++
	w.push(current)
	w.push(walkStep{kind: appendTypeWalk, body: current.body, span: current.span})
...
func (w *Writer) runConditionalChildren(current walkStep) error {
	...
	if !ok || expr == nil || current.index < 0 || current.index > 4 || current.staticMark < 0 || w.Mark() != current.staticMark+current.index {
		return fmt.Errorf("programlower: invalid conditional type child cursor")
	}
	if current.index == 4 {
		return nil
	}
	...
	if child == nil {
		return fmt.Errorf("programlower: absent conditional type child")
	}
	current.index++
	w.push(current)
	w.push(walkStep{kind: appendTypeWalk, body: current.body, span: current.span})
...
func (w *Writer) runAnnotations(current walkStep) error {
	if current.typeHost == 0 || current.typeBase == 0 || current.index < 0 || current.index > len(current.annotations) {
		return fmt.Errorf("programlower: invalid annotation cursor")
	}
...
func (w *Writer) runRecordFields(current walkStep) error {
	...
	if !ok || expr == nil || current.index < 0 || current.index > len(expr.Fields) {
		return fmt.Errorf("programlower: invalid record-field cursor")
	}
	...
	if field.Type == nil {
		return fmt.Errorf("programlower: absent record field type %d", current.index)
	}
	current.index++
	w.push(current)
...
func (w *Writer) expression(expr ast.Expr, body keyspace.Term, span source.Span) error {
	if !validRuntimeExpr(expr) || body == 0 || span.File == "" {
		return fmt.Errorf("programlower: absent static expression operand")
	}
...
func (w *Writer) ready() error {
	if w == nil || w.binding == nil || w.scopes == nil || w.phases == nil ||
		...
		return fmt.Errorf("programlower: static writer is not scheduled")
	}
	return nil
}
