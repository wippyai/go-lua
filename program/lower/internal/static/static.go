...
type Writer struct {
	static           collector.StaticRoot
	flow             collector.FlowRoot
	binding          *bind.Result
	...
	interfaceMembers []collector.StaticInterfaceMember
	params           []collector.StaticParameter
...
func (w *Writer) AppendField(term keyspace.Term) error {
	if w == nil || term == 0 {
		return fmt.Errorf("programlower: invalid static field child")
	}
...
func (w *Writer) AppendInterfaceField(term keyspace.Term) error {
	if w == nil || term == 0 {
		return fmt.Errorf("programlower: invalid interface field child")
	}
	w.interfaceMembers = append(w.interfaceMembers, collector.StaticInterfaceMember{
...
func (w *Writer) AppendInterfaceMethod(name string, namePosition ast.Position, signature keyspace.Term) error {
	if w == nil || name == "" || signature == 0 {
		return fmt.Errorf("programlower: invalid interface method child")
	}
	w.interfaceMembers = append(w.interfaceMembers, collector.StaticInterfaceMember{
...
func New(
	phases *phase.Stack,
	staticRoot collector.StaticRoot,
	flowRoot collector.FlowRoot,
	binding *bind.Result,
	scopes *lexical.Bodies,
	expressions *inbox.Expressions,
	evaluations *eval.Values,
...
func (w *Writer) Predeclare(body keyspace.Term, stmts []ast.Stmt) error {
	...
	for _, stmt := range stmts {
		...
		case *ast.TypeDefStmt:
			...
			term := w.static.Declarations().Alias(
		...
		case *ast.InterfaceDefStmt:
			...
			if !ok || decl.Kind != bind.TypeDeclInterface || decl.ID == 0 || decl.Name != def.Name {
				return fmt.Errorf("programlower: missing interface binding for %q", def.Name)
			}
			if _, exists := w.terms[decl.ID]; exists {
				return fmt.Errorf("programlower: duplicate interface identity for %q", def.Name)
			}
			term := w.static.Declarations().Interface(
				w.span(def), w.nameSpan(def.NamePosition), body, def.Name,
			)
			if term == 0 {
				return fmt.Errorf("programlower: could not predeclare interface %q", def.Name)
			}
			w.terms[decl.ID] = term
		}
	}
...
func (w *Writer) BeginAlias(def *ast.TypeDefStmt) (keyspace.Term, error) {
	...
	if len(params) != len(def.TypeParams) {
		return 0, fmt.Errorf("programlower: type alias parameter binding/source count mismatch for %q", decl.Name)
	}
	terms := make([]keyspace.Term, 0, len(params))
	for index, param := range params {
		if param.ID == 0 || param.Kind != bind.TypeDeclParam || param.Name == "" {
			return 0, fmt.Errorf("programlower: invalid type parameter on %q", decl.Name)
		}
		if _, exists := w.terms[param.ID]; exists {
			return 0, fmt.Errorf("programlower: duplicate type parameter identity %q", param.Name)
		}
		if index >= len(def.TypeParams) || def.TypeParams[index].Name != param.Name {
			return 0, fmt.Errorf("programlower: invalid type parameter source position %q", param.Name)
		}
		term := w.static.Declarations().TypeParam(w.nameSpan(param.NamePosition), alias, param.Name)
		if term == 0 {
			return 0, fmt.Errorf("programlower: could not declare type parameter %q", param.Name)
		}
		w.terms[param.ID] = term
		terms = append(terms, term)
	}
	if !w.static.Declarations().AliasParams(alias, terms) {
		return 0, fmt.Errorf("programlower: could not set type parameters for %q", decl.Name)
	}
	return alias, nil
}
...
func (w *Writer) Alias(def *ast.TypeDefStmt) (keyspace.Term, bool) {
...
func (w *Writer) FinishParam(decl bind.TypeDecl, constraint keyspace.Term) error {
	term, ok := w.Host(decl)
	if !ok || decl.Kind != bind.TypeDeclParam || !w.static.Declarations().TypeParamConstraint(term, constraint) {
...
func (w *Writer) FinishAlias(def *ast.TypeDefStmt, target keyspace.Term) error {
	...
	if !w.static.Declarations().AliasTarget(alias, target) {
...
func (w *Writer) BeginSignature(expr *ast.FunctionTypeExpr, scope keyspace.Term) (keyspace.Term, []bind.TypeDecl, error) {
	...
	signature := w.static.Signatures().TypeFunction(w.span(expr), scope)
	if signature == 0 {
	...
	for index, param := range params {
		...
		term := w.static.Declarations().TypeParam(w.nameSpan(param.NamePosition), signature, param.Name)
		if term == 0 {
	...
	if !w.static.Signatures().TypeFunctionGenerics(signature, w.generics) {
...
func (w *Writer) BeginFunctionHeader(expr *ast.FunctionExpr, function keyspace.Term) ([]bind.TypeDecl, error) {
	...
	for index, param := range params {
		...
		term := w.static.Declarations().TypeParam(w.nameSpan(param.NamePosition), function, param.Name)
		if term == 0 {
	...
	if !w.flow.Functions().SetFunctionGenerics(function, w.generics) {
...
func (w *Writer) FinishFunctionReturns(expr *ast.FunctionExpr, function keyspace.Term, mark, count int) error {
	...
	if !w.flow.Functions().SetFunctionReturns(function, expr.ReturnsKnown, returns) {
...
func (w *Writer) FinishSignature(expr *ast.FunctionTypeExpr, signature keyspace.Term, paramMark, fixedCount, returnMark, returnCount int, variadic keyspace.Term) (keyspace.Term, error) {
	...
	for index, param := range expr.Params[:fixedCount] {
		...
		w.params = append(w.params, collector.StaticParameter{
	...
	signatures := w.static.Signatures()
	if !signatures.TypeFunctionParameters(signature, w.params) ||
		!signatures.TypeFunctionVariadic(signature, variadicSpan, variadic) ||
		!signatures.TypeFunctionReturns(signature, expr.Returns != nil, returns) {
		w.params = w.params[:0]
...
func FunctionTypeShape(expr *ast.FunctionTypeExpr) (fixedCount int, variadic ast.TypeExpr, err error) {
	...
	if expr.Variadic == nil && expr.VariadicPosition != (ast.Position{}) {
		return 0, nil, fmt.Errorf("programlower: function type variadic position without variadic type")
	}
	if expr.Variadic != nil && !expr.VariadicPosition.Valid() {
		return 0, nil, fmt.Errorf("programlower: function type variadic has no marker position")
	}
...
func (w *Writer) Assertion(expr *ast.AssertsTypeExpr, narrow keyspace.Term) (keyspace.Term, error) {
	...
	return w.term(w.static.Signatures().TypeAsserts(
...
func (w *Writer) PrimitiveOrRef(expr *ast.PrimitiveTypeExpr) (keyspace.Term, error) {
	...
	if kind, ok := programstatic.PrimitiveKindForName(expr.Name); ok {
		return w.term(w.static.Types().Primitive(span, kind), "primitive type")
	}
	return w.term(w.static.References().Unresolved(span, []string{expr.Name}, 0), "unresolved primitive reference")
}
...
func (w *Writer) Literal(expr *ast.LiteralTypeExpr) (keyspace.Term, error) {
	...
	case bool:
		return w.term(w.static.Types().LiteralBool(span, value), "boolean literal type")
	case int64:
		return w.term(w.static.Types().LiteralInteger(span, value), "integer literal type")
	case float64:
		return w.term(w.static.Types().LiteralFloat(span, value), "float literal type")
	case string:
		return w.term(w.static.Types().LiteralString(span, value), "string literal type")
	case nil:
...
func (w *Writer) Optional(expr *ast.OptionalTypeExpr, inner keyspace.Term) (keyspace.Term, error) {
	...
	return w.term(w.static.Types().Optional(w.span(expr), inner), "optional type")
}
...
func (w *Writer) Union(expr *ast.UnionTypeExpr, mark, count int) (keyspace.Term, error) {
	...
	return w.term(w.static.Types().Union(w.span(expr), members), "union type")
}
...
func (w *Writer) Intersection(expr *ast.IntersectionTypeExpr, mark, count int) (keyspace.Term, error) {
	...
	return w.term(w.static.Types().Intersection(w.span(expr), members), "intersection type")
}
...
func (w *Writer) Generic(expr *ast.GenericTypeExpr, base keyspace.Term, mark, count int) (keyspace.Term, error) {
	...
	return w.term(w.static.Types().Generic(w.span(expr), base, args), "generic type")
}
...
func (w *Writer) Array(expr *ast.ArrayTypeExpr, element keyspace.Term) (keyspace.Term, error) {
	...
	return w.term(w.static.Types().Array(w.span(expr), element, expr.Readonly), "array type")
}
...
func (w *Writer) Map(expr *ast.MapTypeExpr, key, value keyspace.Term) (keyspace.Term, error) {
	...
	return w.term(w.static.Types().Map(w.span(expr), key, value, expr.Readonly), "map type")
}
...
func (w *Writer) Field(field ast.RecordFieldExpr, typ keyspace.Term) (keyspace.Term, error) {
	if w == nil || field.Name == "" || typ == 0 {
		return 0, fmt.Errorf("programlower: invalid static field")
	}
	return w.term(w.static.Types().Field(
...
func (w *Writer) Record(expr *ast.RecordTypeExpr, mark, count int) (keyspace.Term, error) {
	...
	if mark < 0 || count < 0 || mark > len(w.fields) || len(w.fields)-mark != count {
		return 0, fmt.Errorf("programlower: incomplete record fields")
	}
	term := w.static.Types().Record(w.span(expr), w.fields[mark:], expr.Readonly)
...
func (w *Writer) DeclareAnnotation(expr ast.AnnotationExpr, scope, target keyspace.Term) (keyspace.Term, error) {
	if w == nil || scope == 0 || target == 0 || expr.Name == "" {
		return 0, fmt.Errorf("programlower: invalid annotation")
	}
	return w.term(w.static.Operands().Annotation(w.span(&expr), scope, target, expr.Name), "annotation")
}

func (w *Writer) FillAnnotation(annotation, values keyspace.Term) error {
	if w == nil || !w.static.Operands().AnnotationValues(annotation, values) {
		return fmt.Errorf("programlower: could not finalize annotation")
	}
	return nil
}
...
func (w *Writer) TypeOf(expr *ast.TypeOfExpr, host, operand keyspace.Term) (keyspace.Term, error) {
	...
	return w.term(w.static.Operators().TypeOf(w.span(expr), host, operand), "typeof")
}
...
func (w *Writer) KeyOf(expr *ast.KeyOfExpr, inner keyspace.Term) (keyspace.Term, error) {
	if w == nil || expr == nil || inner == 0 {
		return 0, fmt.Errorf("programlower: invalid keyof type")
	}
	return w.term(w.static.Operators().KeyOf(w.span(expr), inner), "keyof type")
}
...
func (w *Writer) IndexAccess(
	...
	if w == nil || expr == nil {
		return 0, fmt.Errorf("programlower: invalid indexed-access type")
	}
	children, err := w.rangeTerms(mark, 2)
	if err != nil {
		return 0, err
	}
	return w.term(w.static.Operators().IndexAccess(w.span(expr), children[0], children[1]), "indexed-access type")
}
...
func (w *Writer) Conditional(
	...
	if w == nil || expr == nil {
		return 0, fmt.Errorf("programlower: invalid conditional type")
	}
	children, err := w.rangeTerms(mark, 4)
	if err != nil {
		return 0, err
	}
	return w.term(w.static.Operators().Conditional(
...
func (w *Writer) TypeRef(ref *ast.TypeRefExpr) (keyspace.Term, error) {
	...
	if len(ref.Path) == 0 {
		return 0, fmt.Errorf("programlower: invalid type reference path")
	}
	for _, part := range ref.Path {
		if part == "" {
			return 0, fmt.Errorf("programlower: empty type reference component")
		}
	}
	span := w.span(ref)
	...
	if len(ref.Path) > 1 {
		...
		if !ok {
			return 0, fmt.Errorf("programlower: qualified type reference has no root symbol")
		}
		root, ok = w.scopes.Resolve(rootID)
		if !ok {
			kind, known := w.binding.Kind(rootID)
			name := w.binding.Name(rootID)
			if !known || kind != symbol.Global || name == "" || name != ref.Path[0] {
				return 0, fmt.Errorf("programlower: qualified type reference root is not visible")
			}
			if !ref.RootPosition.Valid() {
				return 0, fmt.Errorf("programlower: qualified global type root has no token span")
			}
			identity, global := w.binding.GlobalIdentityOf(rootID)
			if !global || !identity.Matches(ref.Path[0]) {
				return 0, fmt.Errorf("programlower: qualified type reference root is not visible")
			}
			root = w.flow.Storage().Global(identity)
		}
		if root == 0 {
			return 0, fmt.Errorf("programlower: could not materialize qualified type root")
		}
	}
	if decl, ok := w.binding.TypeRef(ref); ok {
		...
		return w.term(w.static.References().Declaration(span, ref.Path, root, target), "declaration type reference")
	}
	if resolved, ok := w.binding.QualifiedTypeRef(ref); ok {
		...
		if len(resolved.Path) != 0 {
			...
			return w.term(w.static.References().Canonical(span, ref.Path, resolved.Path, root), "canonical type reference")
		}
	}
	return w.term(w.static.References().Unresolved(span, ref.Path, root), "unresolved type reference")
}
...
func (w *Writer) RuntimeTypeTarget(span source.Span, value bind.RuntimeTypeValue) (keyspace.Term, error) {
	if w == nil || value.Base == nil || value.Name == "" || value.Base.Value != value.Name {
		return 0, fmt.Errorf("programlower: invalid runtime type value")
	}
	switch value.Kind {
	case bind.RuntimeTypeValuePrimitive:
		if value.Decl.ID != 0 {
			return 0, fmt.Errorf("programlower: primitive runtime type value %q has a declaration", value.Name)
		}
		kind, ok := programstatic.PrimitiveKindForName(value.Name)
		if !ok || !kind.RuntimeLoadable() {
			return 0, fmt.Errorf("programlower: unsupported runtime primitive type %q", value.Name)
		}
		return w.term(w.static.Types().Primitive(span, kind), "runtime primitive type")
	case bind.RuntimeTypeValueDeclaration:
		if value.Decl.ID == 0 || value.Decl.Name != value.Name {
			return 0, fmt.Errorf("programlower: invalid runtime declaration type %q", value.Name)
		}
		if value.Decl.Kind != bind.TypeDeclAlias && value.Decl.Kind != bind.TypeDeclInterface {
			return 0, fmt.Errorf("programlower: unsupported runtime declaration type %q", value.Name)
		}
		return w.declarationRef(span, []string{value.Name}, value.Decl)
	default:
		return 0, fmt.Errorf("programlower: unsupported runtime type value %q", value.Name)
	}
}
...
func (w *Writer) PublicationRef(
	...
	if w == nil {
		return 0, fmt.Errorf("programlower: invalid publication type reference")
	}
	if len(source) == 0 {
		return 0, fmt.Errorf("programlower: invalid publication type reference path")
	}
	for _, part := range source {
		if part == "" {
			return 0, fmt.Errorf("programlower: empty publication type reference component")
		}
	}
	if alias.Decl.ID != 0 {
		if len(alias.Path) != 0 {
			return 0, fmt.Errorf("programlower: ambiguous publication type reference")
		}
		target, targetErr := w.declarationTarget(alias.Decl)
		if targetErr != nil {
			return 0, targetErr
		}
		return w.term(w.static.References().Declaration(span, source, root, target), "declaration publication type reference")
	}
	if len(alias.Path) == 0 {
		return 0, fmt.Errorf("programlower: unresolved publication type reference")
	}
	for _, part := range alias.Path {
		if part == "" {
			return 0, fmt.Errorf("programlower: invalid canonical publication type path")
		}
	}
	return w.term(w.static.References().Canonical(span, source, alias.Path, root), "canonical publication type reference")
}
...
func (w *Writer) declarationRef(span source.Span, source []string, decl bind.TypeDecl) (keyspace.Term, error) {
	...
	if len(source) == 0 {
		return 0, fmt.Errorf("programlower: invalid declaration type reference path")
	}
	for _, part := range source {
		if part == "" {
			return 0, fmt.Errorf("programlower: empty declaration type reference component")
		}
	}
	return w.term(w.static.References().Declaration(span, source, 0, target), "declaration type reference")
}
...
func (w *Writer) nameSpan(position ast.Position) source.Span {
	...
	span, ok := sourcecoord.Build(file, position.Line, position.Column, position.EndLine, position.EndColumn)
	if !ok {
		return sourcecoord.Invalid(file)
	}
	return span
}
