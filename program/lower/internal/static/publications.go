...
func (w *Writer) PublishTypePublications(stmt *ast.AssignStmt, assign keyspace.Term) error {
	if w == nil || w.binding == nil || w.scopes == nil || stmt == nil || assign == 0 {
		return fmt.Errorf("programlower: invalid static type publication statement")
	}
	for _, publication := range w.binding.StaticTypePublications(stmt) {
		...
		if index < 0 || index >= len(stmt.Lhs) || index >= len(stmt.Rhs) {
			return fmt.Errorf("programlower: invalid static type publication index %d", publication.Index)
		}
		if len(publication.Source) == 0 {
			return fmt.Errorf("programlower: incomplete static type publication at index %d", index)
		}
		root, err := w.publicationRoot(stmt.Rhs[index], publication.Source)
		if err != nil {
			return fmt.Errorf("programlower: static type publication root %d: %w", index, err)
		}
		target, err := w.PublicationRef(w.span(stmt.Rhs[index]), publication.Source, root, publication.Alias)
		if err != nil {
			return fmt.Errorf("programlower: static type publication target %d: %w", index, err)
		}
		if w.static.Publications().Type(w.span(stmt), assign, uint32(index), target) == 0 {
			return fmt.Errorf("programlower: could not create static type publication at index %d", index)
		}
	}
	return nil
}
...
func (w *Writer) publicationCell(ident *ast.IdentExpr) (keyspace.Term, error) {
	...
	cell := w.flow.Storage().Global(identity)
	if cell == 0 {
