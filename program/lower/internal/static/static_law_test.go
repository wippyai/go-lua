...
func TestCheckedSourceSpanRejectsMalformedSignedCoordinates(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			if got, ok := sourcecoord.Build("types.lua", test.startLine, test.startCol, test.endLine, test.endCol); ok || got != (source.Span{}) {
				t.Fatalf("sourcecoord.Build = %#v/%v, want rejection", got, ok)
			}
		})
	}

	if got, ok := sourcecoord.Build("types.lua", 1, 2, 1, 3); !ok || got != (source.Span{
		File: "types.lua", StartLine: 1, StartCol: 2, EndLine: 1, EndCol: 3,
	}) {
		t.Fatalf("sourcecoord.Build(valid) = %#v/%v", got, ok)
	}
	if got, ok := sourcecoord.Build("types.lua", 0, 0, 0, 0); !ok || got != (source.Span{File: "types.lua"}) {
		t.Fatalf("sourcecoord.Build(absent) = %#v/%v", got, ok)
	}
}
...
func TestWriterSpanFailsClosedOnMalformedASTCoordinates(t *testing.T) {
	...
	if got := writer.span(badNode); got != sourcecoord.Invalid(sourceName) {
	...
	if got := writer.nameSpan(badPosition); got != sourcecoord.Invalid(sourceName) {
...
func TestMalformedWriterSpanTerminallyRejectsCollector(t *testing.T) {
	const sourceName = "types.lua"
	c := collector.New(sourceName, 0, bind.GlobalCensus{})
	body := c.Source().Order().Body(source.Span{File: sourceName})
	if body == 0 {
	...
	writer := &Writer{sourceName: sourceName}
	if got := c.Source().Literals().String(writer.span(node), body, "bad"); got != 0 {
...
func TestBeginAliasRequiresExactBinderTypeParameterCount(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			c := collector.New(sourceName, 0, binding.GlobalCensus())
			body := c.Source().Order().Body(source.Span{File: sourceName})
			writer := New(nil, c.Static(), c.Flow(), binding, nil, nil, nil, sourceName)
