...
// It owns no source-wide scheduler or alternate result channel: phase.Stack is
...
func (w *Writer) ScheduleExpr(fn *ast.FunctionExpr, host keyspace.Term, span source.Span) error {
	if host == 0 || span.File == "" {
		return fmt.Errorf("programlower: invalid Function expression host")
	}
...
func (w *Writer) ScheduleDef(
	...
	if host == 0 || host != w.scopes.Owner() || functionSpan.File == "" || completionSpan.File == "" {
		return fmt.Errorf("programlower: invalid function definition host")
	}
	targetMark := w.access.TargetMark()
	if stmt.Name.Method != "" || stmt.Name.Receiver != nil {
		if w.expressions == nil {
			return fmt.Errorf("programlower: missing expression inbox")
		}
	...
	if origin.Kind != bind.FunctionOriginDeclaration || !functionTarget(stmt.Name.Func) {
		return fmt.Errorf("programlower: unsupported function definition target")
	}
...
func (w *Writer) ScheduleRecursiveLocal(
	...
	if w == nil || w.stack == nil || w.binding == nil || w.scopes == nil || w.static == nil {
		return fmt.Errorf("programlower: missing recursive local function authority")
	}
	if stmt == nil || len(stmt.Exprs) != 1 {
		return fmt.Errorf("programlower: invalid recursive local function")
	}
	fn, ok := stmt.Exprs[0].(*ast.FunctionExpr)
	if !ok || fn == nil {
		return fmt.Errorf("programlower: recursive local function has no Function expression")
	}
	...
	if !exists || origin.Kind != bind.FunctionOriginLocalAssignment || origin.Func != fn || origin.Stmt != stmt || origin.LocalIndex != 0 || origin.Static != (w.static.StaticDepth() > 0) {
		return fmt.Errorf("programlower: unsupported recursive local function origin")
	}
	id, exists := w.binding.LocalSymbolAt(stmt, 0)
	if !exists || id == 0 || w.scopes.Has(id) {
		return fmt.Errorf("programlower: binder has no recursive local function symbol")
	}
	mark := w.scopes.CellMark()
	if host == 0 || host != w.scopes.Owner() || functionSpan.File == "" || completionSpan.File == "" {
		return fmt.Errorf("programlower: invalid recursive local function host")
	}
...
func (w *Writer) Run() error {
	if w == nil || w.stack == nil || len(w.steps) == 0 {
		return fmt.Errorf("programlower: invalid function continuation")
	}
	phases := w.stack
	current := w.pop()
	if current.owner == 0 {
		return fmt.Errorf("programlower: Function continuation has no active Body")
	}
	...
	case stepPlainTarget:
		...
		if target == 0 || open {
			return fmt.Errorf("programlower: invalid function definition target result")
		}
	...
	case stepMethodTarget:
		...
		if receiver == 0 || open {
			return fmt.Errorf("programlower: invalid method function receiver result")
		}
	...
	case stepFinishGeneric:
		...
		if term == 0 || open {
			return fmt.Errorf("programlower: invalid function generic constraint result")
		}
	...
	case stepRequestClose:
		if w.bodies == nil || current.body == 0 {
			return fmt.Errorf("programlower: missing Function Body closure inbox")
		}
	...
	case stepFinishFormalType:
		term, open := phases.Result()
		if term == 0 || open {
			return fmt.Errorf("programlower: invalid function parameter type result")
		}
	...
	case stepFinishReturnType:
		term, open := phases.Result()
		if term == 0 || open {
			return fmt.Errorf("programlower: invalid function return type result")
		}
	...
	case stepFinishRecursiveType:
		term, open := phases.Result()
		if term == 0 || open {
			return fmt.Errorf("programlower: invalid recursive local function type result")
		}
		if err := w.static.DeclareCellType(current.host, current.typeExpr, term); err != nil {
			return err
		}
	...
	case stepCloseBody:
		...
		if body == 0 || open || body != current.body {
			return fmt.Errorf("programlower: invalid function Body completion")
		}
		return w.complete(current.function, current.done, phases)
	default:
		return fmt.Errorf("programlower: invalid function continuation %d", current.kind)
	}
}
...
func (w *Writer) begin(fn *ast.FunctionExpr, owner keyspace.Term, span source.Span, done completion) error {
	if w == nil || w.stack == nil || w.collector == nil || w.binding == nil || w.scopes == nil || w.static == nil || fn == nil || owner == 0 || span.File == "" {
		return fmt.Errorf("programlower: missing Function authority")
	}
	phases := w.stack
...
func (w *Writer) runBegin(current step, phases *phase.Stack) error {
	...
	if current.index == len(current.typeParams) {
		...
		if err != nil {
			return fmt.Errorf("programlower: could not create Function Body: %w", err)
		}
		...
		if w.bodies == nil {
			return fmt.Errorf("programlower: missing Function Body preparation inbox")
		}
		w.push(current)
		phases.Push(phase.Function)
...
func (w *Writer) runFormal(current step, phases *phase.Stack) error {
	if current.fn == nil || current.index < 0 || current.index > len(current.slots) {
		return fmt.Errorf("programlower: invalid function formal cursor")
	}
	...
	if slot.Symbol == 0 || w.scopes.Has(slot.Symbol) {
		return fmt.Errorf("programlower: invalid binder symbol for function formal %q", slot.Name)
	}
	...
	if err != nil {
		return fmt.Errorf("programlower: could not create function formal Cell: %w", err)
	}
	if slot.ImplicitSelf {
...
func (w *Writer) runCapture(current step, phases *phase.Stack) error {
	...
	if current.index != len(current.captures) {
		...
		if !ok || outer == 0 {
			return fmt.Errorf("programlower: missing outer Cell for capture %q", capture.CapturedName)
		}
		if _, err := w.scopes.Capture(capture.Captured, current.span, outer); err != nil {
			return fmt.Errorf("programlower: could not create function capture Cell: %w", err)
		}
		w.push(current.next())
		phases.Push(phase.Function)
		return nil
...
func (w *Writer) runHeaderFormal(current step, phases *phase.Stack) error {
	if current.fn == nil || current.index < 0 || current.index > len(current.slots) {
...
func (w *Writer) runHeaderReturns(current step, phases *phase.Stack) error {
	...
	if current.index == len(current.fn.ReturnTypes) {
		...
		if w.bodies == nil {
			return fmt.Errorf("programlower: missing Function statements inbox")
		}
...
func (w *Writer) runRecursiveDeclaredType(current step, phases *phase.Stack) error {
	if current.local == nil || current.fn == nil || current.slot != 0 || len(current.local.Names) != 1 || len(current.local.Types) > 1 {
		return fmt.Errorf("programlower: invalid recursive local function continuation")
	}
	...
	if !ok || id == 0 {
		return fmt.Errorf("programlower: missing recursive local function symbol")
	}
	bound, ok := w.binding.SymbolTypeAnnotation(id)
	if !ok || bound != declared {
		return fmt.Errorf("programlower: mismatched recursive local function type binding")
	}
	host, ok := w.scopes.RetainedCell(current.mark, 0)
	if !ok || host == 0 {
		return fmt.Errorf("programlower: missing recursive local function Cell")
	}
...
func (w *Writer) complete(function keyspace.Term, done completion, phases *phase.Stack) error {
	if function == 0 || done.host == 0 || done.span.File == "" {
		return fmt.Errorf("programlower: missing completed Function")
	}
	...
	case completeDefinition:
		if done.def == nil || done.host == 0 || done.span.File == "" {
			return fmt.Errorf("programlower: invalid function definition completion")
		}
	...
	case completeRecursiveLocal:
		if done.local == nil || done.cellMark < 0 || done.host == 0 || done.span.File == "" {
			return fmt.Errorf("programlower: invalid recursive local function completion")
		}
		values, err := w.singletonValues(done.span, done.host, function)
		if err != nil {
			return err
		...
		return nil
	default:
		return fmt.Errorf("programlower: invalid Function completion")
	}
}
...
func (w *Writer) requestStaticType(typ ast.TypeExpr, host, body keyspace.Term, span source.Span) error {
	if w == nil || w.stack == nil || w.statics == nil || typ == nil || host == 0 || body == 0 || span.File == "" {
		return fmt.Errorf("programlower: invalid Function static type request")
	}
	w.stack.Push(phase.Function)
...
func (w *Writer) assertActive(body keyspace.Term) error {
	if w == nil || w.scopes == nil || body == 0 || w.scopes.Owner() != body {
		return fmt.Errorf("programlower: Function continuation crossed Body boundary")
	}
	return nil
}
...
func (w *Writer) singletonValues(span source.Span, owner, value keyspace.Term) (keyspace.Term, error) {
	if w == nil || w.packs == nil || owner == 0 || value == 0 {
		return 0, fmt.Errorf("programlower: invalid Function result Values")
	}
...
func (w *Writer) validExprOrigin(fn *ast.FunctionExpr) error {
	if w == nil || w.binding == nil || w.static == nil || fn == nil {
		return fmt.Errorf("programlower: invalid Function expression")
	}
	...
	case bind.FunctionOriginLocalAssignment:
		...
		if !ok || stmt == nil || origin.LocalIndex < 0 || origin.LocalIndex >= len(stmt.Exprs) || stmt.Exprs[origin.LocalIndex] != fn {
			return fmt.Errorf("programlower: invalid local Function origin")
		}
		return nil
	default:
		return fmt.Errorf("programlower: unsupported Function expression origin")
	}
}
...
func (w *Writer) validMethodDef(stmt *ast.FuncDefStmt, origin bind.FunctionOrigin) error {
	if stmt.Name.Method == "" || stmt.Name.Receiver == nil || stmt.Name.Func != nil || !functionTarget(stmt.Name.Receiver) || !stmt.Name.MethodPosition.Valid() ||
		...
		return fmt.Errorf("programlower: invalid method function definition")
	}
	return nil
}
...
func (w *Writer) positionSpan(position ast.Position) source.Span {
	...
	span, ok := sourcecoord.Build(w.sourceName, position.Line, position.Column, position.EndLine, position.EndColumn)
	if !ok {
		return sourcecoord.Invalid(w.sourceName)
	}
	return span
...
func (w *Writer) methodSelectorSpan(receiver ast.Expr, position ast.Position) source.Span {
	...
	receiverSpan, ok := sourcecoord.Build(span.File, receiver.Line(), receiver.Column(), receiver.LastLine(), receiver.LastColumn())
	if !ok {
		return sourcecoord.Invalid(span.File)
	}
	...
	selector, ok := sourcecoord.Build(span.File, int(span.StartLine), startCol, int(span.EndLine), int(span.EndCol))
	if !ok {
		return sourcecoord.Invalid(span.File)
	}
	return selector
