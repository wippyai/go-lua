// Package collector contains the one-shot lowering rows used by the Program
// assembler.  This file is the Static vertical.  It deliberately has no
...
func coordinateFromParts(parts [4]uint32) (source.Coordinate, error) {
	coordinate, ok := source.CoordinateFromParts(parts[0], parts[1], parts[2], parts[3])
	if !ok {
		return source.Coordinate{}, errors.New("program/lower/collector: invalid Static coordinate")
	}
...
func requireTerm(term keyspace.Term) error {
	if term == 0 || keyspace.TermFamily(term) == keyspace.FamilyInvalid {
		return errors.New("program/lower/collector: invalid Static term")
	}
	return nil
}
