...
type Writer struct {
	flow        collector.FlowControlWriter
	faults      collector.SourceFaults
	constructed bool
...
// step is private control scheduling state. phase.Stack holds the sole
...
func New(
	construction *collector.Collector,
	binding *bind.Result,
	scopes *lexical.Bodies,
	values *eval.Values,
	phases *phase.Stack,
	...
	var flow collector.FlowControlWriter
	var faults collector.SourceFaults
	if construction != nil {
		flow = construction.Flow().Control()
		faults = construction.Source().Faults()
	}
	return Writer{
		flow:        flow,
		faults:      faults,
...
func (w *Writer) Label(stmt *ast.LabelStmt) (keyspace.Term, error) {
	...
	if !ok || state.term == 0 {
		return 0, fmt.Errorf("programlower: Label was not predeclared")
	}
	if state.placed {
		return 0, fmt.Errorf("programlower: Label occurred twice")
	}
	state.placed = true
	w.labels[stmt] = state
	w.labelCount++
...
func (w *Writer) Fault(
	...
	if label != nil {
		...
		if !ok || state.term == 0 {
			return 0, fmt.Errorf("programlower: ControlFault Label was not predeclared")
		}
		labelTerm = state.term
	}
	term := w.faults.ControlFault(span, owner, kind, labelTerm, blocker)
	if term == 0 {
		return 0, fmt.Errorf("programlower: could not create ControlFault")
	}
	return term, nil
}
...
func (w *Writer) DeferFault(
	...
	if owner == 0 || kind != source.ControlFaultGotoEntersLocal || label == nil {
		return fmt.Errorf("programlower: invalid pending ControlFault")
	}
...
func (w *Writer) ResolveFaults(body keyspace.Term, scopes *lexical.Bodies) error {
	if scopes == nil {
		return fmt.Errorf("programlower: nil lexical authority for ControlFault")
	}
	if body == 0 || scopes.Owner() != body {
		return fmt.Errorf("programlower: ControlFault Body is not active")
	}
	write := 0
...
func (w *Writer) Predeclare(stmts []ast.Stmt, body keyspace.Term) error {
	...
	if body == 0 || body != w.scopes.Owner() {
		return fmt.Errorf("programlower: Label predeclaration crossed Body boundary")
	}
...
func (w *Writer) ScheduleIf(stmt *ast.IfStmt, host keyspace.Term, span source.Span) error {
	...
	if stmt == nil || stmt.Condition == nil || !w.activeHost(host, span) {
		return fmt.Errorf("programlower: invalid If")
	}
...
func (w *Writer) ScheduleWhile(stmt *ast.WhileStmt, host keyspace.Term, span source.Span) error {
	...
	if stmt == nil || stmt.Condition == nil || !w.activeHost(host, span) {
		return fmt.Errorf("programlower: invalid While")
	}
...
func (w *Writer) ScheduleNumberFor(stmt *ast.NumberForStmt, host keyspace.Term, span source.Span) error {
	...
	if stmt == nil || !w.activeHost(host, span) {
		return fmt.Errorf("programlower: absent NumberFor")
	}
...
func (w *Writer) ScheduleGenericFor(stmt *ast.GenericForStmt, host keyspace.Term, span source.Span) error {
	...
	if stmt == nil || !w.activeHost(host, span) {
		return fmt.Errorf("programlower: absent GenericFor")
	}
...
// is requested through its typed inbox; its result returns through phase.Result.
func (w *Writer) Run() error {
	if err := w.ready(); err != nil {
		return err
	}
	if len(w.steps) == 0 {
		return fmt.Errorf("programlower: missing control continuation")
	}
	last := len(w.steps) - 1
	...
	default:
		return fmt.Errorf("programlower: unknown control continuation %d", current.kind)
	}
}
...
func (w *Writer) appendFault(span source.Span, owner keyspace.Term, issue bind.ControlIssue) error {
	...
	default:
		return fmt.Errorf("programlower: unknown control issue %d", issue.Kind)
	}
...
func (w *Writer) finishReturn(current step) error {
	...
	if current.ret == nil || w.scopes.Owner() != current.host {
		return fmt.Errorf("programlower: return continuation lost its host Body")
	}
...
func (w *Writer) finishIfCondition(current step) error {
	...
	if stmt == nil || w.scopes.Owner() != current.host {
		return fmt.Errorf("programlower: invalid if continuation")
	}
...
func (w *Writer) finishIfThen(current step) error {
	...
	stmt := current.ifStmt
	if stmt == nil || w.scopes.Owner() != current.host {
		return fmt.Errorf("programlower: invalid then continuation")
	}
...
func (w *Writer) finishIfElse(current step) error {
	...
	if current.ifStmt == nil || w.scopes.Owner() != current.host {
		return fmt.Errorf("programlower: branch continuation lost its host Body")
	}
...
func (w *Writer) beginRepeat(stmt *ast.RepeatStmt, host keyspace.Term, span source.Span) error {
	if stmt == nil || stmt.Condition == nil {
		return fmt.Errorf("programlower: invalid repeat statement")
	}
	if host == 0 || w.scopes.Owner() != host {
		return fmt.Errorf("programlower: repeat began outside its host Body")
	}
...
func (w *Writer) finishRepeatCondition(current step) error {
	...
	if stmt == nil || stmt.Condition == nil || current.body != current.host || w.scopes.Owner() != current.host {
		return fmt.Errorf("programlower: invalid repeat continuation")
	}
	w.push(step{
...
func (w *Writer) finishRepeatControl(current step) error {
	...
	if stmt == nil || current.body != current.host || current.parent == 0 || w.scopes.Owner() != current.host {
		return fmt.Errorf("programlower: invalid repeat close continuation")
	}
	control, _ := w.phases.Result()
	w.push(step{
...
func (w *Writer) beginNumberFor(stmt *ast.NumberForStmt, host keyspace.Term, span source.Span) error {
	if stmt == nil || stmt.Init == nil || stmt.Limit == nil {
		return fmt.Errorf("programlower: invalid numeric for statement")
	}
	...
	if host == 0 || w.scopes.Owner() != host {
		return fmt.Errorf("programlower: numeric for began outside its host Body")
	}
...
func (w *Writer) runNumberControls(current step) error {
	...
	if current.index < 0 || current.index > len(current.exprs) {
		return fmt.Errorf("programlower: invalid numeric for cursor")
	}
	next := current
	next.index++
...
func (w *Writer) appendNumberControl(current step) error {
	...
	if term == 0 {
		return fmt.Errorf("programlower: absent numeric for control")
	}
	current.terms = append(current.terms, term)
	if current.number == nil || w.scopes.Owner() != current.host {
		return fmt.Errorf("programlower: numeric control continuation lost its host Body")
	}
...
func (w *Writer) beginGenericFor(stmt *ast.GenericForStmt, host keyspace.Term, span source.Span) error {
	if stmt == nil || len(stmt.Names) == 0 || len(stmt.Exprs) == 0 {
		return fmt.Errorf("programlower: invalid generic for statement")
	}
	if host == 0 || w.scopes.Owner() != host {
		return fmt.Errorf("programlower: generic for began outside its host Body")
	}
...
func (w *Writer) openLoopBody(stmts []ast.Stmt, span source.Span, next step) error {
	if next.host == 0 || w.scopes.Owner() != next.host {
		return fmt.Errorf("programlower: loop body opened outside its host Body")
	}
	body, err := w.scopes.EnterBlock(span)
	if err != nil {
		return fmt.Errorf("programlower: could not create loop Body: %w", err)
	}
	next.body = body
...
func (w *Writer) declareLoopCells(next step) error {
	if next.body == 0 || w.scopes.Owner() != next.body {
		return fmt.Errorf("programlower: loop Cells declared outside their Body")
	}
	switch {
	...
	case next.number != nil:
		...
		if !ok || id == 0 {
			return fmt.Errorf("programlower: binder has no numeric for symbol")
		}
	...
	case next.generic != nil:
		...
		for index, id := range ids {
			if id == 0 {
				return fmt.Errorf("programlower: binder has zero generic for symbol %d", index)
			}
			span := w.span(loop)
	...
	default:
		return fmt.Errorf("programlower: loop continuation has no declaration form")
	}
	return nil
}
...
func (w *Writer) finishLoop(current step) error {
	...
	default:
		return fmt.Errorf("programlower: loop continuation has no form")
	}
...
func (w *Writer) openBody(stmts []ast.Stmt, span source.Span, next step) error {
	if next.host == 0 || w.scopes.Owner() != next.host {
		return fmt.Errorf("programlower: branch Body opened outside its host Body")
	}
	body, err := w.scopes.EnterBlock(span)
	if err != nil {
		return fmt.Errorf("programlower: could not create branch Body: %w", err)
	}
	next.body = body
	return w.scheduleBody(stmts, span, next)
}
...
func (w *Writer) expression(expr ast.Expr, host keyspace.Term, span source.Span) error {
	if expr == nil || host == 0 || span.File == "" {
		return fmt.Errorf("programlower: absent control expression")
	}
...
func (w *Writer) appendTo(host, term keyspace.Term) error {
	if host == 0 || term == 0 || w.scopes.Owner() != host {
		return fmt.Errorf("programlower: control result has no active host Body")
	}
...
func (w *Writer) valueList(exprs []ast.Expr, host keyspace.Term, span source.Span) error {
	if host == 0 || span.File == "" {
		return fmt.Errorf("programlower: invalid control Values host")
	}
...
func (w *Writer) ready() error {
	if w == nil || !w.constructed || w.phases == nil || w.binding == nil ||
		...
		return fmt.Errorf("programlower: incomplete control authority")
	}
	return nil
}
...
func (w *Writer) chunkSpan(stmts []ast.Stmt) source.Span {
	...
	span, ok := sourcecoord.Build(w.sourceName, first.Line(), first.Column(), last.LastLine(), last.LastColumn())
	if !ok {
		return sourcecoord.Invalid(w.sourceName)
	}
	return span
