...
import (
	...
	"github.com/wippyai/go-lua/program/internal/grammarproof/requirements/parserproducts"
)
...
func writeSlots(w *canonical.Writer, rows []UseSlot) error {
...
func writeDirect(w *canonical.Writer, rows []UsePath) error {
...
func writeHelpers(w *canonical.Writer, rows []HelperUsePath) error {
...
func writeMutations(w *canonical.Writer, rows []MutationUsePath) error {
...
func writeAxes(w *canonical.Writer, role UseRole, child ChildClass, target ProgramUseClass, open OpenAxis, table TableAxis, lvalue LValueAxis, values ValuesPosition) error {
...
func writeApplications(w *canonical.Writer, rows []uint16) error {
	if err := w.Count(uint64(len(rows))); err != nil {
		return err
	}
...
func writeInstance(w *canonical.Writer, instance parserproducts.TermInstance) error {
...
func writeEdit(w *canonical.Writer, edit parserproducts.Edit) error {
...
func writeTails(w *canonical.Writer, rows []ValuesTail) error {
...
func writeSequenceCoordinate(w *canonical.Writer, coordinate SequenceCoordinate) error {
...
func writeLValues(w *canonical.Writer, rows []LValuePath) error {
