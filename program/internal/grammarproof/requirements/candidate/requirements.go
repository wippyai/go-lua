...
// Discharged exposes the contract rows implemented by Program's public source
// Term queries. A cold source-anchored witness must verify each row; this list
// alone is not a completion claim for Link selection or later Rule semantics.
func Discharged() []Requirement { return Requirements() }

// MissingCount is zero only for the Program candidate schema. Link's direct
// candidate indexing remains a separate outstanding schema obligation.
func MissingCount() int { return 0 }
