...
func TestDeriveRejectsMissingDuplicateAndUnknownIngressRows(t *testing.T) {
	...
	if _, err := derive(missing, digest, semanticsource.CatalogSchema()); !errors.Is(err, ErrMissingRow) {
	...
	if _, err := derive(duplicate, digest, semanticsource.CatalogSchema()); !errors.Is(err, ErrDuplicateRow) {
	...
	if _, err := derive(unknown, digest, semanticsource.CatalogSchema()); !errors.Is(err, ErrUnknownRow) {
