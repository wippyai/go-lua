...
func TestDirectRequirePreservesOneGlobalIdentityIntoProgram(t *testing.T) {
	...
	sealed, err := programlower.Lower(programlower.Source{Name: "require_identity.lua", Text: []byte(source)})
	if err != nil {
		t.Fatal(err)
	}
	...
	shadowProgram, err := programlower.Lower(programlower.Source{Name: "require_shadow.lua", Text: []byte(shadowed)})
	if err != nil {
		t.Fatal(err)
	}
