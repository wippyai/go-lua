...
import (
	...
	staticowner "github.com/wippyai/go-lua/program/static"
)
...
func TestMalformedNumericLiteralIsParserReachableButPublicIngressRejected(t *testing.T) {
	...
	if _, err := programlower.Lower(programlower.Source{Name: "invalid-number.lua", Text: []byte(input)}); err == nil || !strings.Contains(err.Error(), "malformed numeric literal type") {
...
func lowerResidueSource(t *testing.T, source string) *program.Program {
	t.Helper()
	p, err := programlower.Lower(programlower.Source{Name: "residue.lua", Text: []byte(source)})
	if err != nil {
		t.Fatal(err)
	}
	return p
