...
// ActionTemplates derives one symbolic action law for every parser
// alternative, including forwarding and parser-only assembly alternatives.
// It is intentionally total: an empty Constructors slice means the action
// transfers or assembles through a helper, not that the alternative vanished
// from the product-legality denominator.
func ActionTemplates(root string) ([]ActionTemplate, error) {
	...
	result := make([]ActionTemplate, len(rows))
	for index, row := range rows {
		tag, ok := tags[row.nonterminal]
		if !ok || tag == "" {
			return nil, fmt.Errorf("parser grammar %s: nonterminal %s has no declared %%type result tag", path, row.nonterminal)
		}
		result[index] = ActionTemplate{
			Key:          grammarKey(row),
			Nonterminal:  row.nonterminal,
			ResultTag:    tag,
			Ordinal:      row.ordinal,
