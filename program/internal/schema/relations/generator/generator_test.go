...
func TestCatalogRevisionAndFreshness(t *testing.T) {
	...
	if !strings.Contains(string(semantic), "Revision(1)") {
		t.Fatal("semantic output omitted explicit revision")
	}
	relations, err := emitRelations(entries)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(relations), "func CatalogToken") || !strings.Contains(string(relations), "func CatalogName") {
		t.Fatal("relations output omitted generated name/token lookup")
	}
	directory := t.TempDir()
	semanticPath := filepath.Join(directory, "semantic.go")
	...
	if err := fresh(semanticPath, semantic); err != nil {
		t.Fatal(err)
	}
...
func TestCheckedInOutputsAreFresh(t *testing.T) {
	...
	for _, output := range []struct {
		...
		{path: filepath.Join("..", "..", "..", "..", "semanticsource", "generated.go"), want: semantic},
...
func TestRetirementIsAnIrreversibleExactHistoryCut(t *testing.T) {
	...
	semantic, err := emitSemanticSource(removed)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, generated := range [][]byte{semantic, relations} {
		if bytes.Contains(generated, []byte("ProgramSourceControlFault")) {
...
func TestCheckReturnsImmutableCanonicalEvidence(t *testing.T) {
	root := checkFixture(t)
	evidence, err := Check(root)
	if err != nil {
		t.Fatal(err)
	}
	if evidence.Digest() != evidenceDigest(evidence.RawSchema(), evidence.RawHistory(), evidence.RawRetired(), evidence.SemanticSource(), evidence.RelationSource()) {
		t.Fatal("evidence digest does not cover the exact fixed-order bytes")
	}
	if evidence.SchemaDigest() != sha256.Sum256(evidence.RawSchema()) ||
		evidence.HistoryDigest() != sha256.Sum256(evidence.RawHistory()) ||
		evidence.RetiredDigest() != sha256.Sum256(evidence.RawRetired()) ||
		evidence.SemanticSourceDigest() != sha256.Sum256(evidence.SemanticSource()) ||
		evidence.RelationSourceDigest() != sha256.Sum256(evidence.RelationSource()) {
		t.Fatal("evidence part digest does not match its detached bytes")
	}
	schema := evidence.RawSchema()
	schema[0] ^= 0xff
	if bytes.Equal(schema, evidence.RawSchema()) {
		t.Fatal("mutating returned schema bytes changed evidence")
	}
}

func TestCheckRejectsStaleFixedFiles(t *testing.T) {
	tests := []struct {
		name   string
	...
	}{
		{
			name: "schema",
			mutate: func(t *testing.T, root string) {
				path := filepath.Join(root, relationSchemaPath)
		...
		},
		{
			name: "history",
			mutate: func(t *testing.T, root string) {
				path := filepath.Join(root, relationHistoryPath)
				content, err := os.ReadFile(path)
				if err != nil {
					t.Fatal(err)
				}
		...
		},
		{
			name: "retired",
			mutate: func(t *testing.T, root string) {
				path := filepath.Join(root, relationRetiredPath)
				if err := os.WriteFile(path, []byte("ProgramSourceControlFault 0x00010005 - 0 1 "+strings.Repeat("0", sha256.Size*2)+"\n"), 0o644); err != nil {
					t.Fatal(err)
				}
			},
		},
		{
			...
			mutate: func(t *testing.T, root string) {
				if err := os.WriteFile(filepath.Join(root, semanticSourcePath), []byte("stale"), 0o644); err != nil {
					t.Fatal(err)
				}
			},
		},
		{
			...
			mutate: func(t *testing.T, root string) {
				if err := os.WriteFile(filepath.Join(root, relationSourcePath), []byte("stale"), 0o644); err != nil {
					t.Fatal(err)
				}
			},
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := Check(root); err == nil {
				t.Fatal("Check accepted stale fixed file")
			}
		})
	}
}

func TestRunCheckUsesTheFixedCheckMachinery(t *testing.T) {
	root := checkFixture(t)
	files := fixedPaths(root)
	if err := Run(files.schema, files.history, files.retired, files.semantic, files.relations, true, false); err != nil {
...
func checkFixture(t *testing.T) string {
	...
	for destination, source := range map[string]string{
		filepath.Join(root, relationSchemaPath):  filepath.Join("..", "catalog.schema"),
		filepath.Join(root, relationHistoryPath): filepath.Join("..", "catalog.history"),
		filepath.Join(root, relationRetiredPath): filepath.Join("..", "catalog.retired"),
		filepath.Join(root, semanticSourcePath):  filepath.Join("..", "..", "..", "..", "semanticsource", "generated.go"),
		filepath.Join(root, relationSourcePath):  filepath.Join("..", "generated.go"),
	} {
