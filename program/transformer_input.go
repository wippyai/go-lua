...
// retains only the already-published Program root and scalar proof captured
// from its receipt; it creates no Program identity, row, cache, or relation.
type TransformerInput struct {
	owner                       *Program
	programID, sourceID, flowID keyspace.ContentID
	staticID, moduleID          keyspace.ContentID
...
// Publish has performed the complete receipt/range validation before exposing
// this immutable Program. Issuance therefore fences the raw receipt owner
// scalars to the current live quartet and Flow provenance without rehashing or
// rescanning immutable child publication ranges.
func (program *Program) TransformerInput() TransformerInput {
	...
	receipt, views := program.semanticReceipt, program.semanticReceipt.views
	if receipt.owner != program.id || views.owner != program.id ||
		views.source.program != program.id || views.flow.program != program.id ||
		views.static.program != program.id || views.module.program != program.id ||
		!receipt.owner.Available() || !views.owner.Available() {
		return TransformerInput{}
	}
	...
	if !sourceID.Available() || !flowID.Available() || !staticID.Available() || !moduleID.Available() ||
		views.source.owner != sourceID || views.flow.owner != flowID ||
		views.static.owner != staticID || views.module.owner != moduleID {
		return TransformerInput{}
	}
	input := TransformerInput{
		...
		pointAttachments:  program.pointAttachments,
	}
	if !input.valid() {
		return TransformerInput{}
	}
...
func (input TransformerInput) valid() bool {
	...
	if !input.pointAttachments.valid(input.owner) {
		return false
	}
...
func (input TransformerInput) ContentID() keyspace.ContentID {
	if !input.Available() {
...
func StaticOccurrenceID(owner keyspace.ContentID, family uint8, term keyspace.Term) (id keyspace.ContentID, ok bool) {
...
func (input TransformerInput) StaticFrontier(term keyspace.Term) (keyspace.ContentID, uint32, bool) {
...
func (operand StaticOperand) ID() keyspace.ContentID          { return operand.id }
func (operand StaticOperand) Literal() keyspace.LiteralValue  { return operand.literal }
func (operand StaticOperand) ReferenceID() keyspace.ContentID { return operand.reference }
func (operand StaticOperand) SubjectID() keyspace.ContentID   { return operand.subject }
func (operand StaticOperand) BodyPathID() keyspace.ContentID  { return operand.body }
...
func StaticExpressionID(owner keyspace.ContentID, ref programstatic.StaticTypeRef) (keyspace.ContentID, bool) {
...
func StaticInputID(owner keyspace.ContentID, family uint8, source keyspace.Term, index uint32) (keyspace.ContentID, bool) {
...
func StaticScopeID(owner keyspace.ContentID, scope keyspace.Term) (keyspace.ContentID, bool) {
...
func staticInputDigest(domain string, owner keyspace.ContentID, term keyspace.Term, index uint64) keyspace.ContentID {
	hash := sha256.New()
...
// guardTermAt is retained only as the private bridge for the opaque Guard
// proof. No transformer-facing API returns a raw Guard coordinate.
func (input TransformerInput) guardTermAt(site flow.Site, index int) (keyspace.Term, bool) {
	term, ok := input.ownedSite(site)
	if !ok {
		return 0, false
	}
	return input.owner.Flow().Continuation().GuardAt(term, index)
}
...
func (body Body) ContextID() keyspace.ContentID {
	if !body.Available() {
		return keyspace.ContentID{}
	}
...
func (body Body) ProgramID() keyspace.ContentID {
	if !body.Available() {
		return keyspace.ContentID{}
	}
...
func (body Body) PathID() keyspace.ContentID {
	if !body.Available() {
		return keyspace.ContentID{}
	}
	term, ok := body.boundary.Body()
...
func (root ExecutableRoot) ID() keyspace.ContentID {
	if !root.Available() {
		return keyspace.ContentID{}
	}
	return root.id
...
func (outcome Outcome) ContextID() keyspace.ContentID {
	site, ok := outcome.Site()
	if !ok {
		return keyspace.ContentID{}
	}
