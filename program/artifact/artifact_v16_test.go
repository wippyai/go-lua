...
func TestV16RoundTripRebuildsDerivedOwners(t *testing.T) {
	contract := mustProfile(t)
	original := mustLower(t, "v16-roundtrip.lua", `
local function f(x)
  return x
end
return {f = f}, f(1)
`)
	...
	if !bytes.Equal(want, again) {
		t.Fatal("v16 roundtrip changed canonical bytes")
	}
}

func TestV17RejectsV16VersionAndTrailingBytes(t *testing.T) {
	contract := mustProfile(t)
	encoded, err := artifact.Encode(mustLower(t, "v16-version.lua", `return 1`), contract, artifact.Metadata{})
	if err != nil {
		t.Fatal(err)
	}
	version := cloneEventPayload(encoded, 1, func(payload []byte) { payload[0] = 16 })
	if _, _, err := artifact.Decode(version, contract, nil); !errors.Is(err, artifact.ErrNoncanonical) {
		t.Fatalf("v16 artifact version error = %v, want noncanonical", err)
	}
...
func TestV16RejectsTargetClaimedIDAndEntryMutation(t *testing.T) {
	contract := mustProfile(t)
	encoded, err := artifact.Encode(mustLower(t, "v16-mutation.lua", `return 1`), contract, artifact.Metadata{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestV16CanonicalizesAndValidatesDependencies(t *testing.T) {
	contract := mustProfile(t)
	p := mustLower(t, "v16-dependencies.lua", `return 1`)
...
func TestV16RequiresExactExpectedDependencyManifest(t *testing.T) {
	contract := mustProfile(t)
	p := mustLower(t, "v16-manifest.lua", `return 1`)
...
func TestV16CanonicalDependencyRowsPrecedeManifestCountMismatch(t *testing.T) {
	contract := mustProfile(t)
	p := mustLower(t, "v16-manifest-order.lua", `return 1`)
...
func TestV16RejectsOversizeProvenanceBeforeWriting(t *testing.T) {
	contract := mustProfile(t)
	p := mustLower(t, "v16-provenance-limit.lua", `return 1`)
...
func cloneEventPayload(data []byte, index int, mutate func([]byte)) []byte {
	...
	// The helper is called only with indexes from a valid v17 envelope. Keep
