...
func Encode(p *program.Program, contract *target.Contract, metadata Metadata) ([]byte, error) {
	...
	if err := module.WriteArtifactSection(&writer, p.Module()); err != nil {
		return nil, encodeError(err)
	}
...
// Decode accepts only the v17 stream bound to contract and reconstructs a
...
func Decode(data []byte, contract *target.Contract, expectedDependencies []Dependency) (*program.Program, Metadata, error) {
	...
	reader, err := canonical.NewReader(data, artifactMaxBytes)
	if err != nil {
		return nil, Metadata{}, decodeError(err)
	}
	...
	moduleInput, err := module.ReadArtifactSection(reader)
	if err != nil {
		return nil, Metadata{}, decodeError(err)
	}
...
func writeEnvelope(
	...
	targetID, programID keyspace.ContentID,
	entry keyspace.Term,
	provenance string,
	dependencies []Dependency,
) error {
...
func readEnvelope(
	...
	expectedTarget keyspace.ContentID,
	stringBytes uint64,
	expected []Dependency,
) (envelope, error) {
	if reader == nil {
		return envelope{}, canonical.ErrMalformed
	}
...
func readID(reader *canonical.Reader) (keyspace.ContentID, error) {
	bytesValue, err := reader.Bytes(len(keyspace.ContentID{}))
	if err != nil {
		return keyspace.ContentID{}, err
	}
	if len(bytesValue) != len(keyspace.ContentID{}) {
		return keyspace.ContentID{}, canonical.ErrMalformed
	}
...
func targetIdentity(contract *target.Contract) (keyspace.ContentID, bool) {
	if contract == nil {
...
func encodeError(err error) error {
	...
	if errors.Is(err, ErrLimit) || errors.Is(err, canonical.ErrLimit) {
		return ErrLimit
	}
...
func abortModuleFinalizer(finalizer module.Finalizer) error {
	if finalizer.Abort() {
		return nil
	}
