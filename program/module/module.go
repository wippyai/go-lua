// Package module owns Program's authored module-boundary relations.
//
...
type Cold struct{ content keyspace.ContentID }
...
func Build(input Input) (*Draft, error) {
	...
	if !validAuthored(imports) {
		return nil, errors.New("program/module: invalid authored imports")
	}
	content := authoredContent(imports)
	if !content.Available() {
		return nil, errors.New("program/module: unavailable authored identity")
	}
