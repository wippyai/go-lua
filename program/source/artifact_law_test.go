...
func TestArtifactSectionRoundTripRebuildsAuthoredContent(t *testing.T) {
	...
	reader, err := canonical.NewReader(data.Bytes(), len(data.Bytes()))
	if err != nil {
		t.Fatal(err)
	}
...
func TestArtifactSectionRejectsTruncation(t *testing.T) {
	...
	for length := len(raw) - 1; length >= 0; length-- {
		reader, err := canonical.NewReader(raw[:length], length)
		if err != nil {
			continue
		}
	...
	tests := []struct {
	...
	}{
		{
			...
			mutate: func(data []byte) {
				...
				// payload as float 1 violates NormalizeExactKey.
...
func TestArtifactSectionRejectsSpanCountLimitBeforeAllocation(t *testing.T) {
	...
	if _, err := ReadArtifactSection(reader); !errors.Is(err, canonical.ErrLimit) {
