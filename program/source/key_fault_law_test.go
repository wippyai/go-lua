...
func TestKeyFaultRequiresDirectFaultPositionAndFencesDraft(t *testing.T) {
	input, index := keyFaultFixture()
	draft, err := Build(input)
	if err != nil {
		t.Fatal(err)
	}
	if key, ok := draft.FindKey(keyspace.LiteralValue{Kind: keyspace.LiteralFloat, FloatBits: math.Float64bits(1)}); !ok || key != 1 {
		t.Fatalf("Draft.FindKey(float 1) = %v/%v, want canonical key 1", key, ok)
	}
	fault := keyspace.MakeTerm(keyspace.FamilyControlFault, 1)
	...
	if key, ok := draft.FindKey(keyspace.LiteralValue{Kind: keyspace.LiteralInteger, Integer: 1}); ok || key != 0 {
		t.Fatalf("consumed Draft.FindKey = %v/%v, want unavailable", key, ok)
	}
}
...
func TestCopiedDraftSharesOneShotFinalization(t *testing.T) {
	...
	if key, ok := draft.FindKey(keyspace.LiteralValue{Kind: keyspace.LiteralInteger, Integer: 1}); ok || key != 0 {
		t.Fatalf("consumed original Draft.FindKey = %v/%v, want unavailable", key, ok)
	}
}
...
func TestFailedFinalizeConsumesEveryDraftCopy(t *testing.T) {
	...
	if key, ok := copy.FindKey(keyspace.LiteralValue{Kind: keyspace.LiteralInteger, Integer: 1}); ok || key != 0 {
		t.Fatalf("failed copied Draft.FindKey = %v/%v, want unavailable", key, ok)
	}
}
...
func TestNormalizeExactKeyPreservesLuaQuotient(t *testing.T) {
	integer := keyspace.LiteralValue{Kind: keyspace.LiteralInteger, Integer: 1}
	for _, input := range []keyspace.LiteralValue{
		integer,
		{Kind: keyspace.LiteralFloat, FloatBits: math.Float64bits(1)},
		{Kind: keyspace.LiteralFloat, FloatBits: math.Float64bits(0)},
		{Kind: keyspace.LiteralFloat, FloatBits: math.Float64bits(math.Copysign(0, -1))},
	} {
		got, ok := NormalizeExactKey(input)
		if !ok {
			t.Fatalf("NormalizeExactKey(%#v) rejected a valid exact key", input)
		}
		if input.Kind == keyspace.LiteralFloat && math.Float64frombits(input.FloatBits) != 1 && got != (keyspace.LiteralValue{Kind: keyspace.LiteralInteger}) {
			t.Fatalf("NormalizeExactKey(%#v) = %#v, want integer zero", input, got)
		}
	}
	if got, ok := NormalizeExactKey(keyspace.LiteralValue{Kind: keyspace.LiteralFloat, FloatBits: math.Float64bits(math.NaN())}); ok || got != (keyspace.LiteralValue{}) {
		t.Fatalf("NaN normalization = %#v/%v, want no exact identity", got, ok)
	}
}
...
func draftContentID(t *testing.T, input Input) keyspace.ContentID {
	t.Helper()
	draft, err := Build(input)
	if err != nil {
