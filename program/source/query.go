...
// FindKey is the sole construction-time capability. Root uses it to resolve
// Key foreign keys while assembling later owners; all other Source state is
// unpublished until Finalize succeeds.
func (d *Draft) FindKey(raw keyspace.LiteralValue) (keyspace.Key, bool) {
	...
	return findExact(state.authority, raw)
}

// Coordinates returns the construction-only token-coordinate capability.
// It shares Draft's one-shot lifecycle, so copied Drafts cannot retain it
// after any copy attempts finalization.
func (d *Draft) Coordinates() Coordinates {
	if d == nil {
		return Coordinates{}
	}
	return Coordinates{state: d.state}
}

// Token compacts one exact-file token span without retaining its filename.
// A token must have a start; its end is either absent or a valid coordinate at
// or after that start. The all-zero Span is absence, not a token.
func (v Coordinates) Token(span Span) (Coordinate, bool) {
	if v.state == nil {
		return Coordinate{}, false
	}
	v.state.mu.Lock()
	defer v.state.mu.Unlock()
	if v.state.phase != draftOpen || v.state.authority == nil || span.File != v.state.authority.identity.name {
		return Coordinate{}, false
	}
	coordinate, ok := CoordinateFromParts(span.StartLine, span.StartCol, span.EndLine, span.EndCol)
	if !ok || coordinate == (Coordinate{}) {
		return Coordinate{}, false
	}
...
func (c Cold) ContentID() keyspace.ContentID { return c.contentID }
...
func (v Identity) ContentID() keyspace.ContentID {
	a := liveAuthority(v.authority, v.state)
	if a == nil {
...
func findExact(a *authority, raw keyspace.LiteralValue) (keyspace.Key, bool) {
	...
	value, ok := NormalizeExactKey(raw)
	if !ok {
		return 0, false
	}
	atoms := a.keys.exact.atoms
	at := sort.Search(len(atoms), func(index int) bool {
		return compareCanonicalExactKey(exactKeyRefFromLiteral(atoms[index]), exactKeyRefFromLiteral(value)) >= 0
	})
	if at == len(atoms) {
		return 0, false
	}
	candidate := atoms[at]
	return keyspace.Key(at + 1), compareCanonicalExactKey(exactKeyRefFromLiteral(candidate), exactKeyRefFromLiteral(value)) == 0
}
