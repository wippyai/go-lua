...
// lookupSteps exposes the fixed-width traversal count to package-local laws
// without making it part of the published Source API.
func (locations directLocations) lookupSteps(term keyspace.Term) (directLocation, bool, int) {
	family := keyspace.TermFamily(term)
	if family == keyspace.FamilyInvalid {
		return directLocation{}, false, 0
	}
	return locations[family].lookupOrdinal(keyspace.TermOrdinal(term))
}
...
// Coordinates is a construction-only token capability. It carries no Source
// owner after finalization and cannot outlive its shared Draft authority.
type Coordinates struct{ state *draftState }
