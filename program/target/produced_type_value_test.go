...
func TestProducedTypeValueCaptureRequiresExactFreshFunctionResult(t *testing.T) {
	for _, test := range []struct {
		...
		},
		{
			name: "wrong kind",
			mutate: func(spec *Spec) {
				spec.Operations[0].Outcomes[0].FreshResults[0].Kind = FreshTable
			},
...
func TestProducedTypeValueFreshFunctionCanonicalRoundTrip(t *testing.T) {
	spec := func(reverse bool) Spec {
		...
		fresh := []FreshResultSpec{{Result: 0, Kind: FreshFunction}, {Result: 1, Kind: FreshFunction}}
		if reverse {
	...
	for _, contract := range []*Contract{left, right} {
		...
		for result := uint32(0); result < 2; result++ {
			...
			if !producedOK || !captureOK || uint32(formal) != result || !freshOK || kind != FreshFunction || fresh != int(ordinal) {
