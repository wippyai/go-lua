...
func normalizeOptionalExactKey(value keyspace.LiteralValue) (keyspace.LiteralValue, error) {
	if zeroLiteral(value) {
		return keyspace.LiteralValue{}, nil
	}
	return normalizeRequiredExactKey(value)
}
...
func compareNormalizedKey(left, right keyspace.LiteralValue) int {
	...
	order, ok := source.CompareExactKey(left, right)
	if !ok {
