...
func TestTransformerAccessRolesExposeNoRawTermSurface(t *testing.T) {
	termType := reflect.TypeOf(keyspace.Term(0))
	types := []reflect.Type{
		reflect.TypeOf(IndexReads{}), reflect.TypeOf(IndexWrites{}), reflect.TypeOf(IndexRead{}),
		reflect.TypeOf(IndexWrite{}), reflect.TypeOf(IndexBase{}), reflect.TypeOf(IndexLens{}),
		reflect.TypeOf(IndexResult{}), reflect.TypeOf(IndexValues{}),
	}
	for _, typ := range types {
		for index := 0; index < typ.NumMethod(); index++ {
			method := typ.Method(index)
			if method.Type.Out(0) == termType {
