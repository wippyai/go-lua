...
func mathDeclaration() declaration {
	unary := func(param string) signature.Function {
		return authored(typ.Func().Param(param, typ.Number).Returns(typ.Number).Build())
	}
	binary := func(left, right string) signature.Function {
	...
	return declaration{
		signatures: map[string]signature.Function{
			...
			"max": mathExtremumSignature(), "min": mathExtremumSignature(),
			"mod": binary("x", "y"),
			...
			"tointeger": authored(typ.Func().Param("x", typ.Any).
				Returns(normalize.Optional(typ.Integer)).Build()),
			"type": authored(typ.Func().Param("x", typ.Any).
				Returns(normalize.Optional(mathNumberKind)).Build()),
...
func mathExtremumSignature() signature.Function {
	number := typ.NewTypeParam("T", typ.Number)
	return authored(typ.Func().TypeParamRef(number).
		Param("x", number).Variadic(number).Returns(number).Build())
}
