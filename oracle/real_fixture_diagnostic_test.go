...
func analyzeCanonicalRealFixture(t *testing.T, name string) {
	...
	t.Logf("canonical real fixture %s engine: epochs=%d revisions=%d passes=%d refreshes=%d evaluates=%d evalFails=%d folds=%d rhs=%d restarts=%d activations=%d maxQueue=%d maxEpisode=%d pubs=%d semanticPubs=%d rawPubs=%d rawOnly=%d bumps=%d semanticWakes=%d coverageWakes=%d ifaceRefresh=%d ifaceDone=%d ifaceFallback=%d faces=%d leq=%d geq=%d eq=%d incomp=%d unknown=%d",
		...
		engine.InterfaceRefreshChangedFaces, engine.InterfaceRefreshOldLessEqNew, engine.InterfaceRefreshNewLessEqOld, engine.InterfaceRefreshEqual,
