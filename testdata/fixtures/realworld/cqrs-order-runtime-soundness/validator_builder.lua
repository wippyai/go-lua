...
type ValidatorBuilder = {
    ...
    build: (self: ValidatorBuilder) -> protocol.CommandValidator,
}
...
function Builder:build(): protocol.CommandValidator
    local name = self.name
    local required_tag = self.required_tag
    local flag_name = self.flag_name

    return function(state: protocol.StoreState, command: protocol.Command): protocol.ValidationResult
