...
function M.normalize_tool(tool_def: TraitToolDef): TraitToolEntry
    if type(tool_def) == "string" then
        return {id = tool_def}
    end
    local entry: TraitToolEntry = {
        id = tool_def.id,
...
function M.normalize_tools(tools_data: {TraitToolDef}?): {TraitToolEntry}
    if not tools_data or #tools_data == 0 then
        return {}
    end
    local result: {TraitToolEntry} = {}
...
function M.build_trait(entry: TraitRegistryEntry): (TraitSpec?, string?)
    if not entry.data then
        return nil, "trait has no data: " .. entry.id
    end
    local data = entry.data
    local spec: TraitSpec = {
        id = entry.id,
