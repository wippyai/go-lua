-- Derived from wippy-golua-seam/tests/app/src/test/websocket/echo_server/test.lua.
...
type Client = {
    send: (self: Client, message: string) -> (),
    channel: (self: Client) -> Channel<WebSocketMessage>,
    close: (self: Client, code: number, reason: string) -> (),
}

local websocket = { CLOSE_CODES = { NORMAL = 1000 } }
function websocket.connect(_url: string, _options: any): (Client, any?)
    return {} as Client, nil
end

local time = {}
function time.after(_duration: string): Channel<Timeout>
    return {} as Channel<Timeout>
end

local json = {}
function json.decode(_src: string): any
    return {}
end
