...
local registry: {[string]: Entry} = {}   -- ActorLocal module cache

local function put(key: string, value: string, ttl: number)
    local entry: Entry = { value = value, expires = ttl }  -- stored into cache -> ActorLocal
    registry[key] = entry
end

local function get(key: string, now: number): string?
    local hit: Entry? = registry[key]     -- FrameLocal: borrowed lookup handle, dies at return
