...
// State publication receives the compiler's explicit (root, result-slot,
// materializer) layout. Two observations at one Program coordinate must not
// be recovered from query declaration order or collapse into one result.
func TestStatePublicationKeepsDistinctSameCoordinateQueries(t *testing.T) {
	solver, value, shard := localLawSolver(t, `return 1`)
	at := localLawEntry(t, value)
	first := localLawFactor(t, solver, 60)
	second := localLawFactor(t, solver, 61)
	localLawDeclareAt(t, solver, first, 62, shard, at, func(access engine.Access[uint64, localLawBits]) bool {
		return access.Join(0, localLawOne)
	})
	localLawDeclareAt(t, solver, second, 63, shard, at, func(access engine.Access[uint64, localLawBits]) bool {
		return access.Join(0, localLawTwo)
	})
	firstQuery := localLawQuery(t, solver, first, shard, at)
	secondQuery := localLawQuery(t, solver, second, shard, at)
	state := localLawSealAndSolve(t, solver)
	if got := localLawRead(t, firstQuery, state); got != localLawOne {
		t.Fatalf("first same-coordinate query = %v, want %v", got, localLawOne)
	}
	if got := localLawRead(t, secondQuery, state); got != localLawTwo {
		t.Fatalf("second same-coordinate query = %v, want %v", got, localLawTwo)
	}
}

// A dependency SCC proves only that the declared Rule graph feeds back.  It
// cannot manufacture a recurrence interface: source Program must carry the
// exact canonical Mu witness that authorizes iteration.
func TestSealRejectsRuleCyclesWithoutProgramMu(t *testing.T) {
	t.Run("self read", func(t *testing.T) {
		solver, value, shard := localLawSolver(t, `return 1`)
		at := localLawEntry(t, value)
		factor := localLawFactor(t, solver, 70)
		rule := localLawDeclareAt(t, solver, factor, 71, shard, at, func(engine.Access[uint64, localLawBits]) bool { return true })
		if _, ok := engine.Read(rule, 0, factor); !ok {
			t.Fatal("Read(self) rejected declaration setup")
		}
		localLawQuery(t, solver, factor, shard, at)
		if solver.Seal() {
			t.Fatal("Seal accepted a self dependency SCC with no Program Mu")
		}
	})

	t.Run("mutual read", func(t *testing.T) {
		solver, value, shard := localLawSolver(t, `return 1`)
		at := localLawEntry(t, value)
		left := localLawFactor(t, solver, 72)
		right := localLawFactor(t, solver, 73)
		leftRule := localLawDeclareAt(t, solver, left, 74, shard, at, func(engine.Access[uint64, localLawBits]) bool { return true })
		rightRule := localLawDeclareAt(t, solver, right, 75, shard, at, func(engine.Access[uint64, localLawBits]) bool { return true })
		if _, ok := engine.Read(leftRule, 0, right); !ok {
			t.Fatal("Read(left <- right) rejected declaration setup")
		}
		if _, ok := engine.Read(rightRule, 0, left); !ok {
			t.Fatal("Read(right <- left) rejected declaration setup")
		}
		localLawQuery(t, solver, left, shard, at)
		if solver.Seal() {
			t.Fatal("Seal accepted a mutual dependency SCC with no Program Mu")
		}
	})

	// A real Program recurrence elsewhere in the demanded slice is not a
	// scheduler-wide permission to iterate unrelated Rule cycles. Its exact
	// feedback target is the only component that may consume that witness.
	t.Run("distant Program Mu", func(t *testing.T) {
		solver, value, shard := localLawSolver(t, `while true do end`)
		entry := localLawEntry(t, value)
		factor := localLawFactor(t, solver, 74)
		rule := localLawDeclareAt(t, solver, factor, 75, shard, entry, func(engine.Access[uint64, localLawBits]) bool { return true })
		if _, ok := engine.Read(rule, 0, factor); !ok {
			t.Fatal("Read(distant self) rejected declaration setup")
		}
		localLawQuery(t, solver, factor, shard, entry)
		if solver.Seal() {
			t.Fatal("Seal let a distant Program Mu authorize an unrelated self dependency")
		}
	})
}

func TestSealAcceptsProgramMuRecurrence(t *testing.T) {
	solver, value, shard := localLawSolver(t, `while true do end`)
	loop, ok := value.LoopAt(0)
	if !ok {
		t.Fatal("lowered loop is absent")
	}
	if head, ok := value.Mu(loop); !ok || head != loop {
		t.Fatalf("Program Mu(%v) = %v/%v, want loop head", loop, head, ok)
	}
	factor := localLawFactor(t, solver, 76)
	localLawQuery(t, solver, factor, shard, loop)
	if !solver.Seal() {
		t.Fatal("Seal rejected a cyclic Program edge with its canonical Mu witness")
	}
}
