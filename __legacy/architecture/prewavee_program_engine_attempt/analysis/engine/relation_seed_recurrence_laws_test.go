...
// A dynamically discovered self-cycle does not manufacture recurrence.  The
// first Seal cannot know its runtime relation topology, so the normal
// Solve/rebuild path must reject the activated cycle when Link supplies no
// recurrence witness.
func TestRelationDynamicSeedSelfCycleWithoutLinkWitnessFailsClosed(t *testing.T) {
	contract := seedRecurrenceContract(t, "op")
	...
	if _, ok := engine.DeclareQuery(solver, result, shard, entry, 0); !ok {
	...
	if solved || state != nil {
		t.Fatalf("Solve accepted an unwitnessed dynamic Seed self-cycle: %p/%t", state, solved)
	}
}
...
// A dynamically activated relation cannot use the root of f's activation as
// output while consuming g's body exit as its input.  That cross-body feedback
// is not a recurrence witness: the normal Solve path must reject it rather
// than infer a recurrence from the assembled relation graph.
func TestRelationRootToForeignExitFeedbackFailsClosed(t *testing.T) {
	contract := seedRecurrenceContract(t, "op")
...
`, contract)
	...
	if _, ok := engine.DeclareQuery(solver, leak, shard, root, 0); !ok {
		t.Fatal("DeclareQuery(static discharge sentinel)")
	}

	if !solver.Seal() {
		t.Fatal("Solver.Seal rejected unresolved relation topology")
	}
	state, solved := solver.Solve(context.Background(), nil)
	if solved || state != nil {
		t.Fatalf("Solve inferred recurrence from root-to-foreign-exit feedback: %p/%t", state, solved)
	}
}
