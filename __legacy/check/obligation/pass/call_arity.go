...
// CallArity emits call argument-count obligations from solved state.
type CallArity struct{}

func (CallArity) Name() string {
	return "call.arity"
}

func (CallArity) Produce(ctx Context) []judgment.Judgment {
	...
	ctx.Reader.ForEachCall(func(call readmodel.CallSite) bool {
		...
		out = append(out, callArityJudgment(ctx, functionKey, call))
		return true
	})
	return out
}

func callArityJudgment(ctx Context, functionKey string, call readmodel.CallSite) judgment.Judgment {
	report := call.Arity
