...
`)
	body := wirlower.LowerFunction("optional-object-literal-expected-root", fn, bindings, built)
	facts := LowerDetailed(built.Graph, Config{Registry: reg, WIR: body}).Facts
...
`)
	...
	lowered := LowerDetailed(built.Graph, Config{Registry: reg, TypeValues: typeValues, WIR: body})
	facts := lowered.Facts
...
func TestLowerNestedObjectLiteralEntriesUnderAssignmentExprRef(t *testing.T) {
	...
	assertLoweredObjectEntry(t, entries[0], fieldSuffix("x"), factflow.ValueSourceLiteral)
