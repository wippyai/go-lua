...
func (relConstraintFactLane) Normalize(in []RelConstraintFact) []RelConstraintFact {
	return normalizeRelConstraintFacts(in, true)
}

func (relConstraintFactLane) NormalizeOwned(in []RelConstraintFact) []RelConstraintFact {
	return normalizeRelConstraintFacts(in, false)
}

func normalizeRelConstraintFacts(in []RelConstraintFact, clone bool) []RelConstraintFact {
	if len(in) == 0 {
		return nil
	}
	byKey := make(map[relConstraintFactKey]RelConstraintFact, len(in))
	for _, fact := range in {
	...
	out := make([]RelConstraintFact, 0, len(byKey))
...
func (lane relConstraintFactLane) Clone(in []RelConstraintFact) []RelConstraintFact {
	in = lane.Normalize(in)
	if len(in) == 0 {
		return nil
	}
	out := make([]RelConstraintFact, len(in))
	for i, fact := range in {
...
func (lane relConstraintFactLane) Equal(a, b []RelConstraintFact) bool {
	if len(a) == len(b) {
		same := true
		for i := range a {
...
func (lane relConstraintFactLane) LessOrEq(a, b []RelConstraintFact) bool {
	a = lane.Normalize(a)
	b = lane.Normalize(b)
	if len(b) == 0 {
		return true
...
func (lane relConstraintFactLane) Join(a, b []RelConstraintFact) []RelConstraintFact {
	a = lane.Normalize(a)
	b = lane.Normalize(b)
	if len(a) == 0 || len(b) == 0 {
		return nil
	}
	right := make(map[relConstraintFactKey]RelConstraintFact, len(b))
	for _, fact := range b {
		right[relConstraintKeyOf(fact)] = fact
	}
	out := make([]RelConstraintFact, 0, len(a))
	for _, left := range a {
...
func (lane relConstraintFactLane) Widen(prev, next []RelConstraintFact) []RelConstraintFact {
	return lane.Join(prev, next)
}

func normalizeRelConstraintFact(fact RelConstraintFact, clone bool) (RelConstraintFact, bool) {
	if fact.A.Path.IsEmpty() || fact.C.Path.IsEmpty() || !fact.A.Path.IsPlaceholder() || !fact.C.Path.IsPlaceholder() {
		return RelConstraintFact{}, false
	}
	if clone {
...
func cloneRelConstraintFact(fact RelConstraintFact) RelConstraintFact {
	fact.A.Path = fact.A.Path.Clone()
	fact.B.Path = fact.B.Path.Clone()
	fact.C.Path = fact.C.Path.Clone()
	return fact
...
func relConstraintKeyOf(fact RelConstraintFact) relConstraintFactKey {
...
func relOperandKeyLess(left RelOperand, leftCoeff int64, right RelOperand, rightCoeff int64) bool {
