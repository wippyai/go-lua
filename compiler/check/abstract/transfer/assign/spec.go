...
func CollectSpecNarrowedTypes(
	...
	deps := BuildReceiverDependencies(assignments)

	assigns := make([]api.AssignmentEvidence, 0, len(assignments))
	assignByPoint := make(map[cfg.Point]*cfg.AssignInfo, len(assignments))
	for _, assign := range assignments {
		if assign.Info == nil {
			continue
		}
		assigns = append(assigns, assign)
		assignByPoint[assign.Point] = assign.Info
	}
	slices.SortFunc(assigns, func(a, b api.AssignmentEvidence) int {
...
func BuildReceiverDependencies(assignments []api.AssignmentEvidence) map[cfg.SymbolID][]cfg.Point {
