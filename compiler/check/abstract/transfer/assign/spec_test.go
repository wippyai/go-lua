...
func TestCollectSpecNarrowedTypes_EmptyGraph(t *testing.T) {
	...
	result := CollectSpecNarrowedTypes(graph, evidence.Assignments, scopes, nil, nil, nil, nil)
	if result == nil {
...
func TestBuildReceiverDependencies_EmptyGraph(t *testing.T) {
	...
	graph := cfg.Build(fn)
	evidence := trace.GraphEvidence(graph, graph.Bindings())
	result := BuildReceiverDependencies(evidence.Assignments)
	if result == nil {
		t.Error("expected non-nil result")
	}
}
...
func TestCollectSpecNarrowedTypes_MultiReturnTrailingTarget(t *testing.T) {
	...
	evidence := trace.GraphEvidence(graph, graph.Bindings())
	result := CollectSpecNarrowedTypes(graph, evidence.Assignments, map[cfg.Point]*scope.State{}, synth, nil, synthAPI, nil)
