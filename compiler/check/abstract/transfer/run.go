// Package transfer implements flow constraint extraction from control flow graphs.
...
import (
	...
	"github.com/wippyai/go-lua/compiler/check/abstract/transfer/returns"
...
func Run(fc *fbcore.FlowContext) *flow.Inputs {
	...
	assign.ExtractAssignments(fc, inputs, keyscoll.BuildKeysCollectorDetector(fc.Graph, fc.Evidence, fc.ModuleBindings, fc.Graphs))
	...
	for _, p := range fc.Graph.RPO() {
		...
		if !cond.PointHasTerminatingCallEvidence(fc.Graph, fc.Evidence.Calls, p, fc.Derived.Synth, fc.Derived.SymResolver, fc.Derived.RefinementBySym, fc.ModuleBindings) {
			continue
		}
	...
	markDeadReturns(fc.Graph, fc.Evidence.Returns, inputs)
