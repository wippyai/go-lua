...
func TestBuildReceiverDependencies_EmptyGraph(t *testing.T) {
	...
	result := assign.BuildReceiverDependencies(evidence.Assignments)
	if result == nil {
		t.Error("expected non-nil map")
	}
}
...
func TestSpecNarrowedTypes_EmptyGraph(t *testing.T) {
	...
	evidence := trace.GraphEvidence(graph, graph.Bindings())
	result := assign.CollectSpecNarrowedTypes(graph, evidence.Assignments, scopes, synth, nil, nil, nil)
	if result == nil {
