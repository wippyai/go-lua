...
func TestExtractReturnKinds_EmptyGraph(t *testing.T) {
	...
	returns.ExtractReturnKinds(&flowctx.FlowContext{Graph: graph, Scopes: scopes}, inputs)
...
func TestExtractReturnExprConstraints_Nil(t *testing.T) {
	result := cond.ExtractReturnExprConstraints(nil, 0, nil, nil, api.FlowEvidence{}, nil, nil, nil, nil)
...
func TestExtractReturnKinds_WithSynth(t *testing.T) {
	...
	returns.ExtractReturnKinds(&flowctx.FlowContext{
		Graph:   graph,
		Scopes:  scopes,
		Derived: &flowctx.Derived{Synth: synth},
	}, inputs)
