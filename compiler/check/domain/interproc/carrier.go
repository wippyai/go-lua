...
// carrier.go connects interproc captured/constructor/container carriers, which
// store interned product.AbstractValue, to the per-element merge helpers that
// operate on typ.Type. A carrier slot projects in for the helpers (ProjectValue)
// and the helper result lifts back out (FromType), so the merge precision is
// unchanged and only the stored representation is value-domain.
