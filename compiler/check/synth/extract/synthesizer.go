...
//
// Pre-flow (api.SynthModeDeclared / api.SynthModeResolve):
// Uses declared types and structural inference without flow analysis.
//
// Flow (api.SynthModeFlow):
...
func NewSynthesizer(deps *Deps, mode api.SynthMode) *Synthesizer {
...
// IsNarrowing reports whether the synthesizer is in flow-refined mode.
func (s *Synthesizer) IsNarrowing() bool {
	return s.mode == api.SynthModeFlow
}

// SynthMode returns the current synthesis mode.
func (s *Synthesizer) SynthMode() api.SynthMode {
	return s.mode
}
