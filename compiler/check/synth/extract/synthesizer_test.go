...
func newTestSynthesizer() *Synthesizer {
	...
	return NewSynthesizer(deps, api.SynthModeResolve)
}
...
func newTestSynthesizerWithSymbol(name string, t typ.Type) (*Synthesizer, *ast.IdentExpr) {
	...
	return NewSynthesizer(deps, api.SynthModeResolve), ident
}
...
func TestSynthesizer_TypeOf_IdentUsesGraphSymbolAt(t *testing.T) {
	s, _ := newTestSynthesizerWithSymbol("x", typ.Integer)
	result := s.TypeOf(&ast.IdentExpr{Value: "x"}, 0)
	if result != typ.Integer {
		t.Fatalf("got %v, want integer via SymbolAt", result)
	}
}
...
func TestSynthesizer_Mode_Declared(t *testing.T) {
	deps := &Deps{
		Ctx:    db.NewQueryContext(db.New()),
		Types:  mockTypeQuerier{},
		Scopes: make(api.ScopeMap),
	...
	if s.IsNarrowing() {
		t.Fatal("expected IsNarrowing()=false for SynthModeResolve")
	}
	if s.SynthMode() != api.SynthModeResolve {
		t.Fatalf("got mode %d, want SynthModeResolve", s.SynthMode())
	}
}

func TestSynthesizer_Mode_Flow(t *testing.T) {
	deps := &Deps{
		Ctx:    db.NewQueryContext(db.New()),
		Types:  mockTypeQuerier{},
		Scopes: make(api.ScopeMap),
	}
	s := NewSynthesizer(deps, api.SynthModeFlow)

	if !s.IsNarrowing() {
		t.Fatal("expected IsNarrowing()=true for SynthModeFlow")
	}
	if s.SynthMode() != api.SynthModeFlow {
		t.Fatalf("got mode %d, want SynthModeFlow", s.SynthMode())
	}
}
