...
import (
	...
	"github.com/wippyai/go-lua/compiler/check/domain/cond"
	...
	"github.com/wippyai/go-lua/compiler/check/synth/core"
...
func (s *Synthesizer) inferReturnTypesFromBody(
	...
	// If facts already know this function's returns, declared mode can use them
	// directly. Flow mode still analyzes the body so predicates can refine the
	// pre-flow fact.
...
func (f *returnSynthFactory) Synth() *Synthesizer {
	...
	f.synth = NewSynthesizer(deps, f.owner.mode)
	return f.synth
}
...
func (s *Synthesizer) applyReturnMutationOverlay(
	...
	mapWrites := overlaymut.CollectMapWriteAssignments(graphEvidence.Assignments, enrichedSynth, mutationBindings, nil)
	tableMutations := calleffect.CollectTableInsertMutations(graphEvidence.Calls, fnGraph, enrichedSynth, mutationBindings)
	overlaymut.MergeMapWriteMutations(mapWrites, tableMutations)
	overlaymut.ApplyMapWriteMergeToOverlay(overlay, mapWrites)
...
func (s *Synthesizer) newReturnInferenceSynth(
	...
	return deps, NewSynthesizer(deps, s.mode)
}
...
func (s *Synthesizer) collectReturnTypes(
	...
	for _, ret := range returns {
		...
		types := returnextract.ReturnVector(fnGraph, info.Exprs, p, returnVectorSynth{
			synth:    tempSynth,
			narrower: flowOps,
		})
		if !seenReturn {
...
func (s *Synthesizer) projectRecursiveFunctionType(
	...
	return functionfact.RecursiveTypeProjection(sig, expected, s.functionFactsInput(), s.symbolForFunction(fn), s.mode)
}
...
func (s *Synthesizer) inferCallbackOverlaySpec(
	...
	synthForGraph := func(graph *cfg.Graph) *Synthesizer {
		...
		tempSynths[graph] = NewSynthesizer(tempDeps, api.SynthModeResolve)
