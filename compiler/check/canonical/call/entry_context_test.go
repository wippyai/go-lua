...
func TestEntryContextKeyMatchesSummaryEntryContextKey(t *testing.T) {
	...
	ctx := NewEntryContext(ref, cells, refs, closures, values)
...
func TestEntryContextEntryValuesNoAliasAndDeterministic(t *testing.T) {
	...
	ctx := NewEntryContext(ref, flow.CaptureCellsDomain.Bottom(), nil, nil, values)
	keyBefore := ctx.Key()
	...
	sameCtx := NewEntryContext(ref, flow.CaptureCellsDomain.Bottom(), nil, nil, sameValues)
