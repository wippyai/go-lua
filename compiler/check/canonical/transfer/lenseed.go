...
// seedNumericForLength seeds the numeric component for a `for i = 1, #arr` loop:
// the induction variable is bounded above by the array length (i <= #arr) and
// below by its constant init, so a body read `arr[i]` is provably in range. The
// limit must be `#arr` over a tracked identifier; any other limit leaves the
// length reference unseeded (the sound carry-forward).
func (t *Transfer) seedNumericForLength(out *flow.PointState, idxSym cfg.SymbolID, info *cfg.NumericForInfo) {
	...
	ident, ok := lenExprBase(info.Limit)
	if !ok {
		return
	}
	arrSym := t.symbolOf(ident)
	if arrSym == 0 {
		return
	}
	...
	if c, ok := t.constInt(info.Init); ok {
...
func (t *Transfer) refineIndexRead(
	...
	baseIdent, isIdent := e.Object.(*ast.IdentExpr)
	var arrKey constraint.PathKey
	if isIdent {
		if sym := t.symbolOf(baseIdent); sym != 0 {
			arrKey = constraint.PathKey(symKey(sym))
		}
	}
	...
	if lenIdent, offset, ok := t.lengthIndexOffset(e.Key); ok {
		if lenSym := t.symbolOf(lenIdent); lenSym != 0 && constraint.PathKey(symKey(lenSym)) == arrKey {
