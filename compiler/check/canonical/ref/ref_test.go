...
func TestFromFlowPath(t *testing.T) {
	path := constraint.NewPath(1, "fn").Key()
	refs := flow.WithFunctionRef(nil, path, flow.FunctionRefSetOf(
		flow.FunctionRef{GraphID: 2, ParentHash: 3},
		flow.FunctionRef{GraphID: 4, ParentHash: 5},
	))

	got, ok := FromFlowPath(refs, path)
	if !ok {
		t.Fatal("FromFlowPath finite set ok = false")
	}
	want := []FuncRef{{GraphID: 2, ParentHash: 3}, {GraphID: 4, ParentHash: 5}}
	if len(got) != len(want) {
		t.Fatalf("FromFlowPath len = %d, want %d", len(got), len(want))
	}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("FromFlowPath[%d] = %+v, want %+v", i, got[i], want[i])
		}
	}

	topRefs := flow.WithFunctionRef(nil, path, flow.FunctionRefSetTop())
	if got, ok := FromFlowPath(topRefs, path); !ok || got != nil {
		t.Fatalf("FromFlowPath(top) = %+v/%v, want nil/true", got, ok)
	}
	if got, ok := FromFlowPath(refs, constraint.NewPath(9, "missing").Key()); ok || got != nil {
		t.Fatalf("FromFlowPath(missing) = %+v/%v, want nil/false", got, ok)
	}
}
