...
func TestMergeEntryValuesWithFixed_PreservesExplicitSlots(t *testing.T) {
	fixed := summary.EntryValues{
		0: product.FromType(typ.String),
	}
	fallback := summary.EntryValues{
		0: product.FromType(typ.Number),
		1: product.FromType(typ.Boolean),
	}

	got := summary.MergeEntryValuesWithFixed(fixed, fallback)
