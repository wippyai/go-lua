...
// TestZZRTNProbe reproduces realworld/result-type-narrowing's cross-module sibling
// correlation: get_email returns (string?, string?) in the (value, err) inverse
// pattern, so `if err == nil` must narrow email to non-nil. Debug probe.
func TestZZRTNProbe(t *testing.T) {
	resultSrc := `
