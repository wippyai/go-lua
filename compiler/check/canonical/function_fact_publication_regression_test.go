...
`, testutil.WithStdlib())
	...
	fact, ok := res.Session.Store.CanonicalFunctionFactProjection(root, parent, sym)
	if !ok {
		facts := res.Session.Store.CanonicalFunctionFactsProjectionForExport(root, parent)
