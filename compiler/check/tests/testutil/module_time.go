...
var (
	...
	tickerType      typ.Type
	timerType       typ.Type
	timeChannelType typ.Type
	timeChannelGen  *typ.Generic
)
...
func init() {
	...
	if timeChannelType == nil && timeChannelGen == nil {
		timeChannelType = typ.Any
	}
	...
	if timeChannelGen != nil {
		timeChannelType = typ.Instantiate(timeChannelGen, timeType)
	}

	// Ticker type
	tickerType = typ.NewInterface("time.Ticker", []typ.Method{
		{Name: "stop", Type: typ.Func().Param("self", typ.Self).Returns(typ.Boolean).Build()},
		{Name: "response", Type: typ.Func().Param("self", typ.Self).Returns(timeChannelType).Build()},
		{Name: "channel", Type: typ.Func().Param("self", typ.Self).Returns(timeChannelType).Build()},
	})
...
// TimeManifest mirrors the time module manifest of the wippy runtime
// (runtime/lua/modules/time/types.go).
func TimeManifest() *io.Manifest {
	...
	moduleType := typ.NewRecord().
		...
		Field("after", typ.Func().Param("d", typ.Any).Returns(timeChannelType).Build()).
