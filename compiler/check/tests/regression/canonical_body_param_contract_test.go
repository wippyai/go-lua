...
// canonicalFlow runs the checker under the canonical type-flow engine.
func canonicalFlow() testutil.Option { return testutil.WithCheckOption(check.WithCanonicalFlow()) }

// TestCanonical_ArithmeticOperandProvesNumericParam verifies the canonical flow
...
// site. This is the body-usage param-evidence the legacy flow proves from
// arithmetic; the canonical flow now proves it through the converged parameter
// Contracts component.
func TestCanonical_ArithmeticOperandProvesNumericParam(t *testing.T) {
	provider := testutil.CheckAndExport(`
		...
		return provider
	`, "provider", testutil.WithStdlib(), canonicalFlow())
	...
	source := `
		...
		return run
	`
	result := testutil.Check(source, testutil.WithStdlib(), testutil.WithModule("provider", provider), canonicalFlow())
	if !result.HasError() {
		t.Fatalf("expected canonical flow to reject an unknown forwarded into an arithmetic-constrained parameter")
	}
...
// TestCanonical_HelperForwardsArgToTypedMethodRejectsAny verifies the canonical flow
...
func TestCanonical_HelperForwardsArgToTypedMethodRejectsAny(t *testing.T) {
	clientModule := testutil.CheckAndExport(`
		...
		return client
	`, "bedrock_client", testutil.WithStdlib(), canonicalFlow())
	...
	`
	result := testutil.Check(source, testutil.WithStdlib(), testutil.WithModule("bedrock_client", clientModule), canonicalFlow())
	if !result.HasError() {
		t.Fatalf("expected canonical flow to reject an any forwarded into a string-typed method through an untyped helper")
	}
...
// TestCanonical_UntypedHelperWithUntypedCalleeStaysGradual verifies the companion
...
func TestCanonical_UntypedHelperWithUntypedCalleeStaysGradual(t *testing.T) {
	clientModule := testutil.CheckAndExport(`
		local client = {}
