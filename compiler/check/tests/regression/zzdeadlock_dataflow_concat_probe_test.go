...
// TestZZDeadlockDataflowConcatProbe verifies the checker no longer
...
func TestZZDeadlockDataflowConcatProbe(t *testing.T) {
	...
	t.Logf("deadlock-dataflow-node diagnostics: total=%d errors=%d", len(res.Diagnostics), len(res.Errors))
	for _, d := range res.Diagnostics {
