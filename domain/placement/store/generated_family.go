...
func (lane *familyWorker) Execute(frame execution.Frame, ticket execution.Ticket) (execution.Result, bool) {
	...
	switch lane.read0Cell(row.read0, ticket, &input0) {
	...
	for index := 0; index < count; index++ {
		...
		member, memberOK := lane.family.plane.RouteMember(uint32(dense), uint64(storageRouteTag))
		if !memberOK {
			return lane.settle(ticket, structure.Refuse)
		}
	...
	outcome := execution.FoldSelectedRoute(ticket, row.write, &lane.write, cells, members, familyReducer{candidate: row.candidate, input0: input0})
	if !ticket.Submit(outcome) {
		return execution.Result{}, false
	}
	written := 0
...
// value is used, and the read's declared explicit sparsity settles an unwritten
// coordinate as an absent candidate.
func (lane *familyWorker) read0Cell(read execution.ExactRead[value.DenseCoordinate, value.Value], ticket execution.Ticket, destination *value.Value) structure.ReductionOutcome {
...
func (install familyInstaller) InstallRuleFamily(plane execution.FormPlane[placement.DenseCoordinate, placement.Fact], ruleOrdinal uint32, rows []execution.FormRow) (execution.Family, []execution.FormAddress, bool) {
	...
	for _, planRow := range rows {
		...
		sealed.rows = append(sealed.rows, familyRow{candidate: candidate, read0: read0Sealed, read1: read1Sealed, write: writeSealed})
	}
