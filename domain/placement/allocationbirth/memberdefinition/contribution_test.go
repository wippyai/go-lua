...
func TestAllocationBirthContributionsComposeIntoTheirOwners(t *testing.T) {
	...
	if !ok || roster.Count() != 2 {
		t.Fatal("allocation-birth rows did not compose into their two owners")
	}
	for index := 0; index < roster.Count(); index++ {
		...
		if !sourceOK || !composedOK {
			t.Fatalf("owner source %d refused allocation birth", index)
		}
