...
// formalGeometry is the whole geometry the declaration carries, in the one
...
const formalGeometry = `operand              semantic/operand/placement/formal
...
join[2].relation     axis/placement:placement/formal/routes
join[2].key          axis/placement:placement/formal/route-key
join[2].predicate    axis/placement:placement/formal/route-tag
join[2].selection    axis/placement:placement/formal/route-selection
join[2].parent       -
...
carry                none
fold.reducer         axis/placement:placement/formal/reducer
fold.inputs          join 2
output[0]            mode=route value-slot=0 route=join 2 column=axis/placement:placement/facts destination=axis/placement:placement/formal/route-destination
transport            none
`

// TestFormalGeometryIsTheOneItsLawSuiteWasEmittedFrom holds the declaration to the
...
func TestFormalGeometryIsTheOneItsLawSuiteWasEmittedFrom(t *testing.T) {
	if got := emitlaw.Canonical(Formal()); got != formalGeometry {
		t.Fatalf("declared geometry has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, formalGeometry)
	}
}

// TestFormalAgreesWithTheCallShapeOfTheReducerItNames is the one structural law a
...
func TestFormalAgreesWithTheCallShapeOfTheReducerItNames(t *testing.T) {
	declaration := Formal()
	if problem, valid := declaration.Check(); !valid {
		t.Fatalf("declaration is malformed: %+v", problem)
	}
	reducer, reducerOK := axiscatalog.AxisMemberCatalog().Reducer(schemaapi.Key("placement/formal/reducer"))
	if !reducerOK {
		t.Fatal("the reducer axis publishes no reducer named placement/formal/reducer")
	}
...
// TestFormalIsHandedOutFresh states that the accessor yields a declaration a caller
...
func TestFormalIsHandedOutFresh(t *testing.T) {
	first := Formal()
...
// formalIdentity is the rule identity this declaration is sealed under.
const formalIdentity = `rule                 placement-formal
lane                 mounted
writes               placement
owner                placement
semantic             semantic/rule/placement/formal
role[0]              semantic/operand/placement/formal
issue[0]             occurrence=occurrence/call form=program-form/call-effect requirement=program-requirement/unrestricted
`

// TestFormalRuleEntryIsTheSealedDeclaration holds the rule entry to its identity, to
...
func TestFormalRuleEntryIsTheSealedDeclaration(t *testing.T) {
	entry := RuleEntry()
	if got := emitlaw.CanonicalEntry(entry); got != formalIdentity {
		t.Fatalf("rule identity has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, formalIdentity)
	}
	if got := emitlaw.Canonical(entry.Program); got != formalGeometry {
	...
	if got := emitlaw.CanonicalEntry(RuleEntry()); got != formalIdentity {
		t.Fatalf("a caller's edits reached the next rule entry:\n%s", got)
	}
}

// TestFormalRefusesEveryStructuralMutation is the negative half of the declaration's
...
// regeneration diff.
func TestFormalRefusesEveryStructuralMutation(t *testing.T) {
...
// TestFormalAdmitsTheseEditsAtDeclarationLevel states where Check's authority ends.
...
func TestFormalAdmitsTheseEditsAtDeclarationLevel(t *testing.T) {
	for _, law := range []struct {
		mutation string
		apply    func(declaration *ruleprogram.Program)
	}{
