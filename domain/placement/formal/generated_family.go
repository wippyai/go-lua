// Code generated by the rule family emitter from the "placement-formal" rule declaration; DO NOT EDIT.
//
...
func (fold familyReducer) Reduce(routeCoordinate heap.Key, cell operand.SelectedCell[placement.Fact]) (placement.Fact, structure.ReductionOutcome) {
	return FormalFold(cell.Tag, cell.Value)
}
...
func (lane *familyWorker) Execute(frame execution.Frame, ticket execution.Ticket) (execution.Result, bool) {
	...
	derived, derivedOK := DeriveFormalRoutes(lane.family.placementSchema, lane.family.valueSchema, lane.family.callSchema, lane.family.packSchema, row.candidate, input0, read1Vector)
	if !derivedOK {
		return lane.settle(ticket, structure.Refuse)
	}
	count := FormalRouteCount(derived)
	...
	for index := 0; index < count; index++ {
		selected, selectedOK := FormalRouteAt(derived, index)
		if !selectedOK {
			return lane.settle(ticket, structure.Refuse)
		}
		formalRouteKey, formalRouteKeyPaired, formalRouteKeyOK := selected.Coordinates()
		_ = formalRouteKeyPaired
		if !formalRouteKeyOK {
			return lane.settle(ticket, structure.Refuse)
		}
		formalRouteTag, formalRouteTagOK := selected.Predicate()
		if !formalRouteTagOK {
			return lane.settle(ticket, structure.Refuse)
		}
		formalRouteDestinationPaired, formalRouteDestination, formalRouteDestinationOK := selected.Coordinates()
		_ = formalRouteDestinationPaired
		if !formalRouteDestinationOK {
			return lane.settle(ticket, structure.Refuse)
		}
		dense, denseOK := lane.family.placementSchema.KeyIndex(formalRouteKey)
		if !denseOK {
			return lane.settle(ticket, structure.Refuse)
		}
		destinationDense, destinationDenseOK := lane.family.placementSchema.KeyIndex(formalRouteDestination)
		if !destinationDenseOK {
			return lane.settle(ticket, structure.Refuse)
		}
		member, memberOK := lane.family.plane.RouteMember(uint32(dense), uint32(destinationDense), uint64(formalRouteTag))
		if !memberOK {
			return lane.settle(ticket, structure.Refuse)
		}
		members[index] = member
		routes[index] = formalRouteDestination
	}
