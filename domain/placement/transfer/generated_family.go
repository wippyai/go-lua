// Code generated by the rule family emitter from the "placement-transfer" rule declaration; DO NOT EDIT.
//
...
func (fold familyReducer) Reduce(routeCoordinate heap.Key, cell execution.SelectedCell[placement.Fact]) (placement.Fact, structure.ReductionOutcome) {
	return TransferFold(cell.Tag, cell.Value)
}
...
func (lane *familyWorker) Execute(frame execution.Frame, ticket execution.Ticket) (execution.Result, bool) {
	...
	derived, derivedOK := DeriveTransferRoutes(lane.family.placementSchema, lane.family.valueSchema, lane.family.callSchema, lane.family.packSchema, row.candidate, input0, read1Vector)
	if !derivedOK {
		return lane.settle(ticket, structure.Refuse)
	}
	count := TransferRouteCount(derived)
	...
	for index := 0; index < count; index++ {
		selected, selectedOK := TransferRouteAt(derived, index)
		if !selectedOK {
			return lane.settle(ticket, structure.Refuse)
		}
		transferRouteKey, transferRouteKeyPaired, transferRouteKeyOK := selected.Coordinates()
		_ = transferRouteKeyPaired
		if !transferRouteKeyOK {
			return lane.settle(ticket, structure.Refuse)
		}
		transferRouteTag, transferRouteTagOK := selected.Predicate()
		if !transferRouteTagOK {
			return lane.settle(ticket, structure.Refuse)
		}
		transferRouteDestinationPaired, transferRouteDestination, transferRouteDestinationOK := selected.Coordinates()
		_ = transferRouteDestinationPaired
		if !transferRouteDestinationOK {
			return lane.settle(ticket, structure.Refuse)
		}
		dense, denseOK := lane.family.placementSchema.KeyIndex(transferRouteKey)
		if !denseOK {
			return lane.settle(ticket, structure.Refuse)
		}
		destinationDense, destinationDenseOK := lane.family.placementSchema.KeyIndex(transferRouteDestination)
		if !destinationDenseOK {
			return lane.settle(ticket, structure.Refuse)
		}
		member, memberOK := lane.family.plane.RouteMember(uint32(dense), uint32(destinationDense), uint64(transferRouteTag))
		if !memberOK {
			return lane.settle(ticket, structure.Refuse)
		}
		members[index] = member
		routes[index] = transferRouteDestination
	}
...
func NewFamilyInstaller(placementSchema placement.Schema, callSchema *call.Algebra, valueSchema *value.Schema, packSchema *pack.Schema) (execution.RuleFamilyInstaller[placement.DenseCoordinate, placement.Fact], bool) {
	install := familyInstaller{placementSchema: placementSchema, callSchema: callSchema, valueSchema: valueSchema, packSchema: packSchema}
...
func (install familyInstaller) InstallRuleFamily(plane execution.FormPlane[placement.DenseCoordinate, placement.Fact], _ uint32, rows []execution.FormRow) (execution.Family, []execution.FormAddress, bool) {
	...
	sealed := &sealedFamily{rows: make([]familyRow, 0, len(rows)), placementSchema: install.placementSchema, callSchema: install.callSchema, valueSchema: install.valueSchema, packSchema: install.packSchema}
