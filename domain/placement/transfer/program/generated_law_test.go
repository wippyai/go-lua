...
// transferGeometry is the whole geometry the declaration carries, in the one
...
const transferGeometry = `operand              semantic/operand/placement/transfer
...
fold.inputs          join 2
output[0]            mode=route value-slot=0 route=join 2 column=axis/placement:placement/facts destination=axis/placement:placement/transfer/route-destination
transport            none
`

// TestTransferGeometryIsTheOneItsLawSuiteWasEmittedFrom holds the declaration to the
...
func TestTransferGeometryIsTheOneItsLawSuiteWasEmittedFrom(t *testing.T) {
	if got := emitlaw.Canonical(Transfer()); got != transferGeometry {
		t.Fatalf("declared geometry has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, transferGeometry)
	}
}

// TestTransferAgreesWithTheCallShapeOfTheReducerItNames is the one structural law a
...
func TestTransferAgreesWithTheCallShapeOfTheReducerItNames(t *testing.T) {
	declaration := Transfer()
	if problem, valid := declaration.Check(); !valid {
		t.Fatalf("declaration is malformed: %+v", problem)
	}
	reducer, reducerOK := axiscatalog.AxisMemberCatalog().Reducer(schemaapi.Key("placement/transfer/reducer"))
	if !reducerOK {
		t.Fatal("the reducer axis publishes no reducer named placement/transfer/reducer")
	}
...
// TestTransferIsHandedOutFresh states that the accessor yields a declaration a caller
...
func TestTransferIsHandedOutFresh(t *testing.T) {
	first := Transfer()
...
// transferIdentity is the rule identity this declaration is sealed under.
const transferIdentity = `rule                 placement-transfer
lane                 mounted
writes               placement
owner                placement
semantic             semantic/rule/placement/transfer
role[0]              semantic/operand/placement/transfer
issue[0]             occurrence=occurrence/call form=program-form/call-effect requirement=program-requirement/unrestricted
`

// TestTransferRuleEntryIsTheSealedDeclaration holds the rule entry to its identity, to
...
func TestTransferRuleEntryIsTheSealedDeclaration(t *testing.T) {
	entry := RuleEntry()
	if got := emitlaw.CanonicalEntry(entry); got != transferIdentity {
		t.Fatalf("rule identity has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, transferIdentity)
	}
	if got := emitlaw.Canonical(entry.Program); got != transferGeometry {
	...
	if got := emitlaw.CanonicalEntry(RuleEntry()); got != transferIdentity {
		t.Fatalf("a caller's edits reached the next rule entry:\n%s", got)
	}
}

// TestTransferRefusesEveryStructuralMutation is the negative half of the declaration's
...
// regeneration diff.
func TestTransferRefusesEveryStructuralMutation(t *testing.T) {
...
// TestTransferAdmitsTheseEditsAtDeclarationLevel states where Check's authority ends.
...
func TestTransferAdmitsTheseEditsAtDeclarationLevel(t *testing.T) {
	for _, law := range []struct {
		mutation string
		apply    func(declaration *ruleprogram.Program)
	}{
