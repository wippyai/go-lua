...
func SuspensionFold(candidate lifecycle.MountedSubjectLiveness, sources execution.SummaryVector[valuedomain.Value], route heap.Key, routeTag uint64, selected placementdomain.Fact) (placementdomain.Fact, structure.ReductionOutcome) {
...
func SuspensionEvidenceFold(candidate lifecycle.MountedSubjectLiveness, sources execution.SummaryVector[valuedomain.Value], route heap.Key, routeTag uint64, selected Evidence) (Evidence, structure.ReductionOutcome) {
	if !candidate.Available() || !sources.Valid() || !route.Valid() || route.Kind() != heap.RootAllocation || routeTag == 0 || !candidate.Span().State().Valid() {
...
func sourceVectorUnknown(sources execution.SummaryVector[valuedomain.Value]) (bool, bool) {
