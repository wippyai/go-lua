...
import (
	...
	placement "github.com/wippyai/go-lua/domain/placement"
)

// DenseCoordinate is placement-suspension-evidence's dense Factor coordinate: the position a key of this
...
// ForeignRead seals one exact read of a bound placement-suspension-evidence Factor at this axis's own
...
func ForeignRead(foreign execution.ForeignFactor, coordinate execution.SelectedCoordinate, input uint16) (execution.ExactRead[DenseCoordinate, Evidence], bool) {
	return execution.ForeignExactRead[DenseCoordinate, Evidence](foreign, coordinate.Unit, input)
}

// ForeignSelectedMember resolves one dense coordinate of a bound placement-suspension-evidence Factor
...
func ForeignSelectedMember(foreign execution.ForeignFactor, dense uint32, tag uint64) (execution.RouteMember, bool) {
	return execution.ForeignSelectedMember[DenseCoordinate, Evidence](foreign, dense, tag)
}

// RelationOwner is the generated bind-time owner for placement-suspension-evidence's member relations.
...
func NewRelationOwner(schema placement.Schema) *RelationOwner {
