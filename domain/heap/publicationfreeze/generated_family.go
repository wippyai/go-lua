// Code generated by the rule family emitter from the "heap-publication-freeze" rule declaration; DO NOT EDIT.
//
...
func (fold familyReducer) Reduce(routeCoordinate heap.Key, cell execution.SelectedCell[heap.Value]) (heap.Value, structure.ReductionOutcome) {
	return heap.PublicationFreezeFact(cell.Tag, cell.Value)
}
...
func (sealed *sealedFamily) NewExecutor(run *execution.Run) execution.Executor {
	...
	return &familyWorker{
		...
		cells:      make([]execution.SelectedCell[heap.Value], sealed.width),
...
type familyWorker struct {
	...
	cells      []execution.SelectedCell[heap.Value]
	routes     []heap.Key
}
...
func (lane *familyWorker) Execute(frame execution.Frame, ticket execution.Ticket) (execution.Result, bool) {
	...
	derived, derivedOK := DerivePublicationFreezeRoutes(lane.family.heapSchema, lane.family.valueSchema, lane.family.callSchema, lane.family.effectSchema, lane.family.packSchema, row.candidate, input0, read1Vector)
	if !derivedOK {
		return lane.settle(ticket, structure.Refuse)
	}
	count := PublicationFreezeRouteCount(derived)
	...
	for index := 0; index < count; index++ {
		selected, selectedOK := PublicationFreezeRouteAt(derived, index)
		if !selectedOK {
			return lane.settle(ticket, structure.Refuse)
		}
		publicationFreezeRouteKey, publicationFreezeRouteKeyPaired, publicationFreezeRouteKeyOK := selected.Coordinates()
		_ = publicationFreezeRouteKeyPaired
		if !publicationFreezeRouteKeyOK {
			return lane.settle(ticket, structure.Refuse)
		}
		publicationFreezeRouteTag, publicationFreezeRouteTagOK := selected.Predicate()
		if !publicationFreezeRouteTagOK {
			return lane.settle(ticket, structure.Refuse)
		}
		publicationFreezeRouteDestinationPaired, publicationFreezeRouteDestination, publicationFreezeRouteDestinationOK := selected.Coordinates()
		_ = publicationFreezeRouteDestinationPaired
		if !publicationFreezeRouteDestinationOK {
			return lane.settle(ticket, structure.Refuse)
		}
		dense, denseOK := lane.family.heapSchema.DenseKeyIndex(publicationFreezeRouteKey)
		if !denseOK {
			return lane.settle(ticket, structure.Refuse)
		}
		destinationDense, destinationDenseOK := lane.family.heapSchema.DenseKeyIndex(publicationFreezeRouteDestination)
		if !destinationDenseOK {
			return lane.settle(ticket, structure.Refuse)
		}
		member, memberOK := lane.family.plane.RouteMember(uint32(dense), uint32(destinationDense), uint64(publicationFreezeRouteTag))
		if !memberOK {
			return lane.settle(ticket, structure.Refuse)
		}
		members[index] = member
		routes[index] = publicationFreezeRouteDestination
	}
...
func NewFamilyInstaller(heapSchema heap.Schema, callSchema *call.Algebra, valueSchema *value.Schema, effectSchema *factor.Algebra, packSchema *pack.Schema) (execution.RuleFamilyInstaller[heap.DenseCoordinate, heap.Value], bool) {
	install := familyInstaller{heapSchema: heapSchema, callSchema: callSchema, valueSchema: valueSchema, effectSchema: effectSchema, packSchema: packSchema}
...
func (install familyInstaller) available() bool {
	return install.callSchema != nil && install.valueSchema != nil && install.effectSchema != nil && install.packSchema != nil
}
...
func (install familyInstaller) InstallRuleFamily(plane execution.FormPlane[heap.DenseCoordinate, heap.Value], _ uint32, rows []execution.FormRow) (execution.Family, []execution.FormAddress, bool) {
	...
	sealed := &sealedFamily{rows: make([]familyRow, 0, len(rows)), heapSchema: install.heapSchema, callSchema: install.callSchema, valueSchema: install.valueSchema, effectSchema: install.effectSchema, packSchema: install.packSchema}
