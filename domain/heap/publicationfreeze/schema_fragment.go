...
// Heap routed read/write. Effect remains the owner of the sealed batch operand;
// it is intentionally not re-declared as a second Factor read.
type SchemaFragment struct {
	slot      *engine.RuleSlot[heapdomain.Value, effectfactor.MountedPublicationBatch]
...
func (fragment *SchemaFragment) RuleSlot() *engine.RuleSlot[heapdomain.Value, effectfactor.MountedPublicationBatch] {
	if fragment == nil {
		return nil
	}
...
// selected at solve time, so the cold shape remains one batch operand per call.
func DeclareSchema(
	...
	slot, ok := engine.NewRuleSlot[heapdomain.Value, effectfactor.MountedPublicationBatch](builder, engine.SchemaRuleSpec[heapdomain.Value]{
