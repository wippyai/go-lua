...
func TestBinaryEqualityProgramDeclaresTheTwoExactReadsAndIdentityCarry(t *testing.T) {
	...
	if declaration.Candidate.AxisRelation.Member != BinaryEqualityCandidates || declaration.JoinCount() != 2 {
	...
	for index := 0; index < declaration.JoinCount(); index++ {
		...
		if !joinOK || len(join.Sources) != 1 || !join.Sources[0].Candidate || join.Relation.Member != BinaryEqualitySources || join.Read.Input != 0 || join.Read.Axis.EntryReference() != declaration.Candidate.AxisRelation.Axis || join.Read.Form != ruleprogram.Exact || join.Read.Contract.Order != ruleprogram.OrderCanonical || join.Read.Contract.Sparse != ruleprogram.SparseExplicit || join.Read.Contract.OnOpaque != ruleprogram.OnOpaqueRefuse || join.Read.Contract.Multiplicity != ruleprogram.MultiplicityOne {
