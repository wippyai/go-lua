...
func TestRuleEntryDeclaresCanonicalSourceProgram(t *testing.T) {
	...
	if !ok {
		t.Fatalf("value-source Program rejected: %+v", problem)
	}
	if spec.Program.JoinCount() != 0 || len(spec.Program.Fold.Inputs) != 0 || len(spec.Program.Fold.Outputs) != 1 {
		t.Fatalf("value-source Program shape = joins:%d inputs:%d outputs:%d", spec.Program.JoinCount(), len(spec.Program.Fold.Inputs), len(spec.Program.Fold.Outputs))
	}
	if spec.Program.Carry != nil {
		t.Fatalf("value-source carry = %#v", spec.Program.Carry)
	}
	if spec.Program.Fold.Outputs[0].Mode != program.ModeExact {
		t.Fatalf("value-source output mode = %v", spec.Program.Fold.Outputs[0].Mode)
	}
}
...
func TestRoutedSourceOutputIsRefused(t *testing.T) {
	...
	if ok || problem.Kind != program.ProblemOutput {
		t.Fatalf("routed value-source output admitted: problem=%+v ok=%t", problem, ok)
	}
}
