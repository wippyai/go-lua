// Code generated by the rule family emitter from the "value-callresult-moduleload" rule declaration; DO NOT EDIT.
//
...
type familyReducer struct {
	state     Judgment
	candidate value.ModuleLoadCall
}
...
func (fold familyReducer) Reduce(cell0 value.Value, cell1 call.Value) (value.Value, structure.ReductionOutcome) {
	return fold.state.Result(fold.candidate, cell0, cell1)
}
...
type familyRow struct {
	...
	read1       execution.ExactRead[call.DenseCoordinate, call.Value]
	read1Policy execution.ReadCellPolicy[call.Value]
...
type (
	...
	productTuple1 = product.Cons[product.Cell[call.Value], productTuple0]
)
...
type familyWorker struct {
	...
	read1    execution.Scratch[call.DenseCoordinate, call.Value]
	product0 product.Extender[value.DenseCoordinate, value.Value, struct{}]
	product1 product.Extender[call.DenseCoordinate, call.Value, productTuple0]
...
func (lane *familyWorker) Execute(frame execution.Frame, ticket execution.Ticket) (execution.Result, bool) {
	...
	staged := 0
	for index := 0; index < rows1.Count(); index++ {
		...
		value, outcome := (familyReducer{state: lane.family.state, candidate: row.candidate}).Reduce(cell0, cell1)
		switch outcome {
...
func (install familyInstaller) InstallRuleFamily(plane execution.FormPlane[value.DenseCoordinate, value.Value], _ uint32, rows []execution.FormRow) (execution.Family, []execution.FormAddress, bool) {
	...
	for _, planRow := range rows {
		...
		candidate, candidateOK := install.valueSchema.ModuleLoadCallAt(int(planRow.Candidate))
		if !candidateOK {
			return nil, nil, false
		}
		...
		read1Sealed, read1SealedOK := execution.ForeignRowExactRead[call.DenseCoordinate, call.Value](foreign1, planRow, 1)
		read1Policy, read1PolicyOK := execution.ForeignReadCellPolicy[call.DenseCoordinate, call.Value](foreign1, plan1.Contract)
		if !read1PolicyOK {
			return nil, nil, false
		}
