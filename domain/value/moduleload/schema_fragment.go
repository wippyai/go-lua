...
// SchemaFragment is the callback-free shape of the mounted cross-axis rule.
// It reads one exact Call factor and one exact argument Value, carries the
// Value factor, and writes only the existing call-result Value coordinate.
type SchemaFragment struct {
	slot      *engine.RuleSlot[valuedomain.Value, valuedomain.ModuleLoadCall]
...
func (fragment *SchemaFragment) RuleSlot() *engine.RuleSlot[valuedomain.Value, valuedomain.ModuleLoadCall] {
	if fragment == nil {
		return nil
	}
...
func DeclareSchema(builder *engine.SchemaBuilder, semantic, operandFamily identity.SemanticKey, values *valueowner.SchemaFragment, calls *callowner.SchemaFragment) (*SchemaFragment, bool) {
	...
	slot, ok := engine.NewRuleSlot[valuedomain.Value, valuedomain.ModuleLoadCall](builder, engine.SchemaRuleSpec[valuedomain.Value]{
	...
	return &SchemaFragment{slot: slot, input: input, callRead: callRead, valueRead: valueRead, carry: carry, write: write, semantic: semantic}, true
}
