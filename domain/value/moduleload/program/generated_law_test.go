...
// moduleLoadCallResultGeometry is the whole geometry the declaration carries, in the one
...
const moduleLoadCallResultGeometry = `operand              semantic/operand/value/callresult-moduleload
candidate            relation=axis/value:value/module-load/candidates issued-row=-
join[0].read         form=exact input=0 axis=axis/value point-bound=bound
join[0].relation     axis/value:value/module-load/arguments
join[0].key          axis/value:value/module-load/argument-key
join[0].predicate    -
join[0].selection    -
join[0].parent       -
join[0].sources      candidate
join[0].contract     order=canonical sparse=explicit on-opaque=refuse multiplicity=one denominator=-
join[1].read         form=exact input=0 axis=axis/call point-bound=bound
join[1].relation     axis/call:call/module-load/sites
join[1].key          axis/call:call/module-load/site-key
join[1].predicate    -
join[1].selection    -
join[1].parent       -
...
fold.reducer         axis/value:value/module-load/reducer
fold.inputs          join 0, join 1
output[0]            mode=exact value-slot=0 route=- column=axis/value:value/facts destination=axis/value:value/module-load/coordinate
transport            none
`

// TestModuleLoadCallResultGeometryIsTheOneItsLawSuiteWasEmittedFrom holds the declaration to the
...
func TestModuleLoadCallResultGeometryIsTheOneItsLawSuiteWasEmittedFrom(t *testing.T) {
	if got := emitlaw.Canonical(ModuleLoadCallResult()); got != moduleLoadCallResultGeometry {
		t.Fatalf("declared geometry has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, moduleLoadCallResultGeometry)
	}
}

// TestModuleLoadCallResultAgreesWithTheCallShapeOfTheReducerItNames is the one structural law a
...
func TestModuleLoadCallResultAgreesWithTheCallShapeOfTheReducerItNames(t *testing.T) {
	declaration := ModuleLoadCallResult()
	if problem, valid := declaration.Check(); !valid {
		t.Fatalf("declaration is malformed: %+v", problem)
	}
	reducer, reducerOK := axiscatalog.AxisMemberCatalog().Reducer(schemaapi.Key("value/module-load/reducer"))
	if !reducerOK {
		t.Fatal("the reducer axis publishes no reducer named value/module-load/reducer")
	}
...
// TestModuleLoadCallResultIsHandedOutFresh states that the accessor yields a declaration a caller
...
func TestModuleLoadCallResultIsHandedOutFresh(t *testing.T) {
	first := ModuleLoadCallResult()
...
// moduleLoadCallResultIdentity is the rule identity this declaration is sealed under.
const moduleLoadCallResultIdentity = `rule                 value-callresult-moduleload
lane                 mounted
writes               value
owner                value
semantic             semantic/rule/value/callresult-moduleload
role[0]              semantic/operand/value/callresult-moduleload
issue[0]             occurrence=occurrence/call form=program-form/call-summary requirement=program-requirement/module-load-call-result
`

// TestModuleLoadCallResultRuleEntryIsTheSealedDeclaration holds the rule entry to its identity, to
...
func TestModuleLoadCallResultRuleEntryIsTheSealedDeclaration(t *testing.T) {
	entry := RuleEntry()
	if got := emitlaw.CanonicalEntry(entry); got != moduleLoadCallResultIdentity {
		t.Fatalf("rule identity has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, moduleLoadCallResultIdentity)
	}
	if got := emitlaw.Canonical(entry.Program); got != moduleLoadCallResultGeometry {
	...
	if got := emitlaw.CanonicalEntry(RuleEntry()); got != moduleLoadCallResultIdentity {
		t.Fatalf("a caller's edits reached the next rule entry:\n%s", got)
	}
}

// TestModuleLoadCallResultRefusesEveryStructuralMutation is the negative half of the declaration's
...
// regeneration diff.
func TestModuleLoadCallResultRefusesEveryStructuralMutation(t *testing.T) {
	for _, law := range []struct {
		...
		declaration := ModuleLoadCallResult().Clone()
...
// TestModuleLoadCallResultAdmitsTheseEditsAtDeclarationLevel states where Check's authority ends.
...
func TestModuleLoadCallResultAdmitsTheseEditsAtDeclarationLevel(t *testing.T) {
	for _, law := range []struct {
		mutation string
		apply    func(declaration *ruleprogram.Program)
	}{
