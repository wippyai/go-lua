// Code generated by the rule family emitter from the "value-runtime-kind-call" rule declaration; DO NOT EDIT.
//
...
type familyReducer struct {
	state     Judgment
	candidate value.RuntimeKindCall
}
...
func (fold familyReducer) Reduce(cell0 call.Value, cell1 value.Value, cell2 value.Value) (value.Value, structure.ReductionOutcome) {
	return fold.state.Result(fold.candidate, cell0, cell1, cell2)
}
...
type familyRow struct {
	...
	read2       execution.ExactRead[value.DenseCoordinate, value.Value]
	read2Policy execution.ReadCellPolicy[value.Value]
...
type (
	...
	productTuple2 = product.Cons[product.Cell[value.Value], productTuple1]
)
...
type familyWorker struct {
	...
	read2    execution.Scratch[value.DenseCoordinate, value.Value]
	...
	product2 product.Extender[value.DenseCoordinate, value.Value, productTuple1]
...
func (lane *familyWorker) Execute(frame execution.Frame, ticket execution.Ticket) (execution.Result, bool) {
	...
	if !status1OK || status1 != product.RefineAvailable {
		return execution.Result{}, false
	}
	rows2, status2, status2OK := lane.product2.Extend(ticket, rows1, row.read2, &lane.read2)
	if !status2OK || status2 != product.RefineAvailable {
		return execution.Result{}, false
	}
	staged := 0
	for index := 0; index < rows2.Count(); index++ {
		...
		region, tuple, tupleOK := rows2.At(index)
		if !tupleOK || !region.Valid() {
			_ = lane.write.Discard(ticket)
			return execution.Result{}, false
		}
		cell0, present0 := row.read0Policy.Cell(tuple.Tail().Tail().Head().Value(), tuple.Tail().Tail().Head().Present())
		cell1, present1 := row.read1Policy.Cell(tuple.Tail().Head().Value(), tuple.Tail().Head().Present())
		cell2, present2 := row.read2Policy.Cell(tuple.Head().Value(), tuple.Head().Present())
		...
		value, outcome := (familyReducer{state: lane.family.state, candidate: row.candidate}).Reduce(cell0, cell1, cell2)
		switch outcome {
...
func (install familyInstaller) InstallRuleFamily(plane execution.FormPlane[value.DenseCoordinate, value.Value], _ uint32, rows []execution.FormRow) (execution.Family, []execution.FormAddress, bool) {
	...
	for _, planRow := range rows {
		if planRow.Form != execution.FormExact || !planRow.Rule.Available() || planRow.Rule.ReadCount() != 3 || planRow.Rule.OutputCount() != 1 {
			return nil, nil, false
		}
		...
		if !plan1OK || plan1.Form != program.Exact || plan1.Input != 0 || plan1.PointBound != program.PointBound {
			return nil, nil, false
		}
		plan2, plan2OK := planRow.Rule.ReadAt(2)
		if !plan2OK || plan2.Form != program.Exact || plan2.Input != 0 || plan2.PointBound != program.PointBound {
			return nil, nil, false
		}
		candidate, candidateOK := install.valueSchema.RuntimeKindCallAt(int(planRow.Candidate))
		if !candidateOK {
			return nil, nil, false
		}
		...
		read1Sealed, read1SealedOK := execution.ForeignRowExactRead[value.DenseCoordinate, value.Value](foreign1, planRow, 1)
		read1Policy, read1PolicyOK := execution.ForeignReadCellPolicy[value.DenseCoordinate, value.Value](foreign1, plan1.Contract)
		if !read1PolicyOK {
			return nil, nil, false
		}
		foreign2, foreign2OK := plane.Foreign(plan2.Factor)
		if !foreign2OK {
			return nil, nil, false
		}
		read2Sealed, read2SealedOK := execution.ForeignRowExactRead[value.DenseCoordinate, value.Value](foreign2, planRow, 2)
		read2Policy, read2PolicyOK := execution.ForeignReadCellPolicy[value.DenseCoordinate, value.Value](foreign2, plan2.Contract)
		if !read2PolicyOK {
			return nil, nil, false
		}
		writeSealed, writeSealedOK := plane.ExactWrite(planRow.Target, uint16(output.Slot))
		if !writeSealedOK || !read0SealedOK || !read1SealedOK || !read2SealedOK {
			return nil, nil, false
		}
		addresses = append(addresses, execution.FormAddress{Member: planRow.Member, Local: uint32(len(sealed.rows))})
		sealed.rows = append(sealed.rows, familyRow{candidate: candidate, read0: read0Sealed, read0Policy: read0Policy, read1: read1Sealed, read1Policy: read1Policy, read2: read2Sealed, read2Policy: read2Policy, write: writeSealed})
	}
