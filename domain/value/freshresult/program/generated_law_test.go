...
// freshResultGeometry is the whole geometry the declaration carries, in the one
...
const freshResultGeometry = `operand              semantic/operand/value/callresult-freshresult
...
// TestFreshResultGeometryIsTheOneItsLawSuiteWasEmittedFrom holds the declaration to the
...
func TestFreshResultGeometryIsTheOneItsLawSuiteWasEmittedFrom(t *testing.T) {
	if got := emitlaw.Canonical(FreshResult()); got != freshResultGeometry {
		t.Fatalf("declared geometry has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, freshResultGeometry)
	}
}

// TestFreshResultAgreesWithTheCallShapeOfTheReducerItNames is the one structural law a
...
func TestFreshResultAgreesWithTheCallShapeOfTheReducerItNames(t *testing.T) {
	declaration := FreshResult()
	if problem, valid := declaration.Check(); !valid {
		t.Fatalf("declaration is malformed: %+v", problem)
	}
	reducer, reducerOK := axiscatalog.AxisMemberCatalog().Reducer(schemaapi.Key("value/reducer/fresh-result"))
	if !reducerOK {
		t.Fatal("the reducer axis publishes no reducer named value/reducer/fresh-result")
	}
...
// TestFreshResultIsHandedOutFresh states that the accessor yields a declaration a caller
...
func TestFreshResultIsHandedOutFresh(t *testing.T) {
	first := FreshResult()
...
// freshResultIdentity is the rule identity this declaration is sealed under.
const freshResultIdentity = `rule                 value-callresult-freshresult
lane                 mounted
writes               value
owner                value
semantic             semantic/rule/value/callresult-freshresult
role[0]              semantic/operand/value/callresult-freshresult
issue[0]             occurrence=occurrence/call form=program-form/call-effect requirement=program-requirement/unrestricted
`

// TestFreshResultRuleEntryIsTheSealedDeclaration holds the rule entry to its identity, to
...
func TestFreshResultRuleEntryIsTheSealedDeclaration(t *testing.T) {
	entry := RuleEntry()
	if got := emitlaw.CanonicalEntry(entry); got != freshResultIdentity {
		t.Fatalf("rule identity has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, freshResultIdentity)
	}
	if got := emitlaw.Canonical(entry.Program); got != freshResultGeometry {
	...
	if got := emitlaw.CanonicalEntry(RuleEntry()); got != freshResultIdentity {
		t.Fatalf("a caller's edits reached the next rule entry:\n%s", got)
	}
}

// TestFreshResultRefusesEveryStructuralMutation is the negative half of the declaration's
...
// regeneration diff.
func TestFreshResultRefusesEveryStructuralMutation(t *testing.T) {
...
// TestFreshResultAdmitsTheseEditsAtDeclarationLevel states where Check's authority ends.
...
func TestFreshResultAdmitsTheseEditsAtDeclarationLevel(t *testing.T) {
	for _, law := range []struct {
		mutation string
		apply    func(declaration *ruleprogram.Program)
	}{
