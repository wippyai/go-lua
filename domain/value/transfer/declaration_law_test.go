...
func TestRuleEntryDeclaresCanonicalStorageTransferProgram(t *testing.T) {
	spec := RuleEntry()
	problem, ok := spec.Program.Check()
	if !ok {
		t.Fatalf("storage-transfer Program rejected: %+v", problem)
	}
	if spec.Program.JoinCount() != 1 || len(spec.Program.Fold.Inputs) != 1 || len(spec.Program.Fold.Outputs) != 1 {
		t.Fatalf("storage-transfer Program shape = joins:%d inputs:%d outputs:%d", spec.Program.JoinCount(), len(spec.Program.Fold.Inputs), len(spec.Program.Fold.Outputs))
	}
	...
	if join.Read.Input != 0 || join.Read.Form != program.Exact || join.Read.Contract.Order != program.OrderCanonical ||
		...
		t.Fatalf("storage-transfer exact read contract = %+v", join.Read)
	}
	if spec.Program.Carry == nil || spec.Program.Carry.Input != 0 || spec.Program.Carry.Mode != program.CarryIdentity || spec.Program.Carry.Transform.Declared() {
		t.Fatalf("storage-transfer carry = %#v", spec.Program.Carry)
	}
}
