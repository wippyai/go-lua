...
func BindHot(fragment *SchemaFragment, owner *valueowner.HotOwner) (*HotRule, bool) {
	...
	var implementation *valueowner.RuleImplementation[value.BinaryArithmetic]
	bound := valueowner.BindSelectedRule(owner, fragment.slot, fragment.carry, fragment.write, owner.FactorRef(), engine.HotRuleSpec[value.Value, value.BinaryArithmetic]{
		...
		Transfer: func(access engine.Access[value.Value, value.BinaryArithmetic]) bool {
	...
	}, engine.HotCarrySpec[value.Value, value.BinaryArithmetic]{}, func(tx *valueowner.SelectedRuleBinding[value.BinaryArithmetic]) bool {
...
func (rule *HotRule) ReceiptForOccurrence(mount, id identity.ContentID) (value.BinaryArithmetic, bool) {
	...
	row, ok := rule.owner.Schema().BinaryArithmetic(mount, id)
...
func (rule *HotRule) AttachReceiptMember(compilation *engine.ReceiptCompilation, member engine.ReceiptRuleMember, operand value.BinaryArithmetic) (*engine.ReceiptMember, bool) {
	if rule == nil || rule.owner == nil || rule.owner.Schema() == nil || !rule.owner.Schema().OwnsBinaryArithmetic(operand) {
		return nil, false
	}
...
func hotChecker(owner *valueowner.HotOwner, semantic identity.SemanticKey, leftRead, rightRead *engine.Read[engine.OrderedCells[value.Value]]) engine.RuleDerivationChecker[value.Value, value.BinaryArithmetic] {
	return func(derivation engine.RuleDerivation[value.Value, value.BinaryArithmetic]) (engine.RuleEvidence, bool) {
		...
		for index := 0; index < derivation.DispositionCount(); index++ {
			...
			expected, expectedOK := owner.Schema().ApplyArithmetic(leftValue, rightValue, op)
			if !expectedOK {
				return engine.RuleEvidence{}, false
			}
