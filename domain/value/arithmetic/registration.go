...
// RuleEntry is this package's value-binary-arithmetic rule declaration. P and
// A are the composition's own principal and authority records, admitted by the
...
func RuleEntry[P rulePrincipals, A ruleAuthorities]() rule.Spec {
	return rule.Spec{
		...
		Roles:    []schema.Key{"semantic/operand/value/binary-arithmetic"},
	}
}

func DeclareRule[P rulePrincipals](builder *engine.SchemaBuilder, context rule.Declaration[P]) (*SchemaFragment, bool) {
	ruleSemantic, ruleOK := context.Roles.Key(vocabulary.RoleKey("rule/value/binary-arithmetic"))
	operandFamily, operandOK := context.Roles.Key(vocabulary.RoleKey("operand/value/binary-arithmetic"))
	if !ruleOK || !operandOK {
		return nil, false
	}
...
func StructureSpecs() []structure.Spec {
	return vocabulary.RoleSpecs("rule/value/binary-arithmetic", "operand/value/binary-arithmetic")
}
