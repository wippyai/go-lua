...
func RuleEntry[P rulePrincipals, A ruleAuthorities]() rule.Spec {
	return rule.Spec{
		...
		Issues: []rule.Issuance{
			{Occurrence: "occurrence/binary-order", Requirement: "program-requirement/unrestricted", Form: "program-form/computation"},
		},
		Lane:     rule.LaneMounted,
		Semantic: "semantic/rule/value/binary-order",
		Roles:    []schema.Key{"semantic/operand/value/binary-order"},
	}
}

func DeclareRule[P rulePrincipals](builder *engine.SchemaBuilder, context rule.Declaration[P]) (*SchemaFragment, bool) {
	ruleSemantic, ruleOK := context.Roles.Key(vocabulary.RoleKey("rule/value/binary-order"))
	operandFamily, operandOK := context.Roles.Key(vocabulary.RoleKey("operand/value/binary-order"))
	if !ruleOK || !operandOK {
		return nil, false
	}
...
func StructureSpecs() []structure.Spec {
	return vocabulary.RoleSpecs("rule/value/binary-order", "operand/value/binary-order")
}
