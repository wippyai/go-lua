...
// source. It is issued in the coordinate row during Schema sealing; hot rules
...
func (schema *valueBuilder) sealSourceSeedOccurrences() bool {
	...
	for mountIndex := 0; mountIndex < mounts.Count(); mountIndex++ {
		...
		for index := 0; index < count; index++ {
			...
			if !valueOK || !valueIDOK || !seedOK || !id.Available() || seed.result == nil || seed.result.occurrence.Available() || seed.result.module.Available() {
				return false
			}
			seed.result.module, seed.result.occurrence = module, id
	...
	for index := range schema.sourceSeedMounts {
		...
		for _, row := range rows.occurrences {
			if !row.seed.valid() || !row.occurrence.Available() || row.seed.result.module != rows.module || row.seed.result.occurrence != row.occurrence ||
				!row.seed.result.validFor(schema.Schema, row.seed.valueID) {
				return false
			}
		}
	}
