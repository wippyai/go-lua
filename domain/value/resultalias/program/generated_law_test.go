...
// resultAliasCallResultGeometry is the whole geometry the declaration carries, in the one
...
const resultAliasCallResultGeometry = `operand              semantic/operand/value/callresult-resultalias
candidate            relation=axis/value:value/mounted-call-result/candidates issued-row=-
join[0].read         form=exact input=0 axis=axis/call point-bound=bound
join[0].relation     axis/call:call/call-result/sites
join[0].key          axis/call:call/call-result/site-key
join[0].predicate    -
join[0].selection    -
join[0].parent       -
...
join[1].relation     axis/value:value/result-alias/routes
join[1].key          axis/value:value/result-alias/route-key
join[1].predicate    axis/value:value/result-alias/route-tag
join[1].selection    axis/value:value/result-alias/route-selection
join[1].parent       -
join[1].sources      candidate, join 0
join[1].contract     order=by-tag sparse=explicit on-opaque=refuse multiplicity=one denominator=denominator/coordinates/value
...
fold.reducer         axis/value:value/result-alias/reducer
fold.inputs          join 0, join 1
output[0]            mode=exact value-slot=0 route=- column=axis/value:value/facts destination=axis/value:value/mounted-call-result/coordinate
transport            none
`

// TestResultAliasCallResultGeometryIsTheOneItsLawSuiteWasEmittedFrom holds the declaration to the
...
func TestResultAliasCallResultGeometryIsTheOneItsLawSuiteWasEmittedFrom(t *testing.T) {
	if got := emitlaw.Canonical(ResultAliasCallResult()); got != resultAliasCallResultGeometry {
		t.Fatalf("declared geometry has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, resultAliasCallResultGeometry)
	}
}

// TestResultAliasCallResultAgreesWithTheCallShapeOfTheReducerItNames is the one structural law a
...
func TestResultAliasCallResultAgreesWithTheCallShapeOfTheReducerItNames(t *testing.T) {
	declaration := ResultAliasCallResult()
	if problem, valid := declaration.Check(); !valid {
		t.Fatalf("declaration is malformed: %+v", problem)
	}
	reducer, reducerOK := axiscatalog.AxisMemberCatalog().Reducer(schemaapi.Key("value/result-alias/reducer"))
	if !reducerOK {
		t.Fatal("the reducer axis publishes no reducer named value/result-alias/reducer")
	}
...
// TestResultAliasCallResultIsHandedOutFresh states that the accessor yields a declaration a caller
...
func TestResultAliasCallResultIsHandedOutFresh(t *testing.T) {
	first := ResultAliasCallResult()
...
// resultAliasCallResultIdentity is the rule identity this declaration is sealed under.
const resultAliasCallResultIdentity = `rule                 value-callresult-resultalias
lane                 mounted
writes               value
owner                value
semantic             semantic/rule/value/callresult-resultalias
role[0]              semantic/operand/value/callresult-resultalias
issue[0]             occurrence=occurrence/call form=program-form/call-summary requirement=program-requirement/call-result-slot
`

// TestResultAliasCallResultRuleEntryIsTheSealedDeclaration holds the rule entry to its identity, to
...
func TestResultAliasCallResultRuleEntryIsTheSealedDeclaration(t *testing.T) {
	entry := RuleEntry()
	if got := emitlaw.CanonicalEntry(entry); got != resultAliasCallResultIdentity {
		t.Fatalf("rule identity has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, resultAliasCallResultIdentity)
	}
	if got := emitlaw.Canonical(entry.Program); got != resultAliasCallResultGeometry {
	...
	if got := emitlaw.CanonicalEntry(RuleEntry()); got != resultAliasCallResultIdentity {
		t.Fatalf("a caller's edits reached the next rule entry:\n%s", got)
	}
}

// TestResultAliasCallResultRefusesEveryStructuralMutation is the negative half of the declaration's
...
// regeneration diff.
func TestResultAliasCallResultRefusesEveryStructuralMutation(t *testing.T) {
	for _, law := range []struct {
		...
		declaration := ResultAliasCallResult().Clone()
...
// TestResultAliasCallResultAdmitsTheseEditsAtDeclarationLevel states where Check's authority ends.
...
func TestResultAliasCallResultAdmitsTheseEditsAtDeclarationLevel(t *testing.T) {
	for _, law := range []struct {
		mutation string
		apply    func(declaration *ruleprogram.Program)
	}{
