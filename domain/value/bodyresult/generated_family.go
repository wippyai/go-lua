// Code generated by the rule family emitter from the "value-callresult-body" rule declaration; DO NOT EDIT.
//
...
func (lane *familyWorker) Execute(frame execution.Frame, ticket execution.Ticket) (execution.Result, bool) {
	...
	derived, derivedOK := returnroute.Derive(lane.family.valueSchema, lane.family.callSchema, row.candidate, input0)
	if !derivedOK {
		return lane.settle(ticket, structure.Refuse)
	}
	count := returnroute.Count(derived)
	...
	for index := 0; index < count; index++ {
		selected, selectedOK := returnroute.At(derived, index)
		if !selectedOK {
			return lane.settle(ticket, structure.Refuse)
		}
		bodyReturnRouteKey, bodyReturnRouteKeyOK := selected.Coordinate()
		if !bodyReturnRouteKeyOK {
			return lane.settle(ticket, structure.Refuse)
		}
		bodyReturnRouteTag, bodyReturnRouteTagOK := selected.Predicate()
		if !bodyReturnRouteTagOK {
			return lane.settle(ticket, structure.Refuse)
		}
		dense, denseOK := lane.family.valueSchema.CoordinateIndex(bodyReturnRouteKey)
		if !denseOK {
			return lane.settle(ticket, structure.Refuse)
		}
		...
		member, memberOK := lane.family.plane.SelectedMember(uint32(dense), uint64(bodyReturnRouteTag))
		if !memberOK {
			return lane.settle(ticket, structure.Refuse)
		}
...
func (install familyInstaller) InstallRuleFamily(plane execution.FormPlane[value.DenseCoordinate, value.Value], _ uint32, rows []execution.FormRow) (execution.Family, []execution.FormAddress, bool) {
	...
	state, stateOK := Derive(install.valueSchema, install.callSchema)
	if !stateOK {
		return nil, nil, false
	}
