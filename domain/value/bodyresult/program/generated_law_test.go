...
// bodyCallResultGeometry is the whole geometry the declaration carries, in the one
...
const bodyCallResultGeometry = `operand              semantic/operand/value/callresult-body
...
join[1].relation     axis/value:value/body-result/routes
join[1].key          axis/value:value/body-result/route-key
join[1].predicate    axis/value:value/body-result/route-tag
join[1].selection    axis/value:value/body-result/route-selection
join[1].parent       -
...
fold.reducer         axis/value:value/body-result/reducer
...
// TestBodyCallResultGeometryIsTheOneItsLawSuiteWasEmittedFrom holds the declaration to the
...
func TestBodyCallResultGeometryIsTheOneItsLawSuiteWasEmittedFrom(t *testing.T) {
	if got := emitlaw.Canonical(BodyCallResult()); got != bodyCallResultGeometry {
		t.Fatalf("declared geometry has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, bodyCallResultGeometry)
	}
}

// TestBodyCallResultAgreesWithTheCallShapeOfTheReducerItNames is the one structural law a
...
func TestBodyCallResultAgreesWithTheCallShapeOfTheReducerItNames(t *testing.T) {
	declaration := BodyCallResult()
	if problem, valid := declaration.Check(); !valid {
		t.Fatalf("declaration is malformed: %+v", problem)
	}
	reducer, reducerOK := axiscatalog.AxisMemberCatalog().Reducer(schemaapi.Key("value/body-result/reducer"))
	if !reducerOK {
		t.Fatal("the reducer axis publishes no reducer named value/body-result/reducer")
	}
...
// TestBodyCallResultIsHandedOutFresh states that the accessor yields a declaration a caller
...
func TestBodyCallResultIsHandedOutFresh(t *testing.T) {
	first := BodyCallResult()
...
// bodyCallResultIdentity is the rule identity this declaration is sealed under.
const bodyCallResultIdentity = `rule                 value-callresult-body
lane                 mounted
writes               value
owner                value
semantic             semantic/rule/value/callresult-body
role[0]              semantic/operand/value/callresult-body
issue[0]             occurrence=occurrence/call form=program-form/call-summary requirement=program-requirement/call-result-slot
`

// TestBodyCallResultRuleEntryIsTheSealedDeclaration holds the rule entry to its identity, to
...
func TestBodyCallResultRuleEntryIsTheSealedDeclaration(t *testing.T) {
	entry := RuleEntry()
	if got := emitlaw.CanonicalEntry(entry); got != bodyCallResultIdentity {
		t.Fatalf("rule identity has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, bodyCallResultIdentity)
	}
	if got := emitlaw.Canonical(entry.Program); got != bodyCallResultGeometry {
	...
	if got := emitlaw.CanonicalEntry(RuleEntry()); got != bodyCallResultIdentity {
		t.Fatalf("a caller's edits reached the next rule entry:\n%s", got)
	}
}

// TestBodyCallResultRefusesEveryStructuralMutation is the negative half of the declaration's
...
// regeneration diff.
func TestBodyCallResultRefusesEveryStructuralMutation(t *testing.T) {
	for _, law := range []struct {
		...
		declaration := BodyCallResult().Clone()
...
// TestBodyCallResultAdmitsTheseEditsAtDeclarationLevel states where Check's authority ends.
...
func TestBodyCallResultAdmitsTheseEditsAtDeclarationLevel(t *testing.T) {
	for _, law := range []struct {
		mutation string
		apply    func(declaration *ruleprogram.Program)
	}{
