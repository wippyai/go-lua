...
// bodyCallEffectGeometry is the whole geometry the declaration carries, in the one
...
const bodyCallEffectGeometry = `operand              semantic/operand/effect/callsite-body
...
join[1].read         form=selected input=0 axis=axis/effect point-bound=bound
...
// TestBodyCallEffectGeometryIsTheOneItsLawSuiteWasEmittedFrom holds the declaration to the
...
func TestBodyCallEffectGeometryIsTheOneItsLawSuiteWasEmittedFrom(t *testing.T) {
	if got := emitlaw.Canonical(BodyCallEffect()); got != bodyCallEffectGeometry {
		t.Fatalf("declared geometry has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, bodyCallEffectGeometry)
	}
}

// TestBodyCallEffectAgreesWithTheCallShapeOfTheReducerItNames is the one structural law a
...
func TestBodyCallEffectAgreesWithTheCallShapeOfTheReducerItNames(t *testing.T) {
	declaration := BodyCallEffect()
	if problem, valid := declaration.Check(); !valid {
		t.Fatalf("declaration is malformed: %+v", problem)
	}
	reducer, reducerOK := axiscatalog.AxisMemberCatalog().Reducer(schemaapi.Key("effect/callsite-body/reducer"))
	if !reducerOK {
		t.Fatal("the reducer axis publishes no reducer named effect/callsite-body/reducer")
	}
...
// TestBodyCallEffectIsHandedOutFresh states that the accessor yields a declaration a caller
...
func TestBodyCallEffectIsHandedOutFresh(t *testing.T) {
	first := BodyCallEffect()
...
// bodyCallEffectIdentity is the rule identity this declaration is sealed under.
const bodyCallEffectIdentity = `rule                 effect-body
lane                 mounted
writes               effect
owner                effect
semantic             semantic/rule/effect/callsite-body
role[0]              semantic/operand/effect/callsite-body
issue[0]             occurrence=occurrence/call form=program-form/call-effect requirement=program-requirement/unrestricted
`

// TestBodyCallEffectRuleEntryIsTheSealedDeclaration holds the rule entry to its identity, to
...
func TestBodyCallEffectRuleEntryIsTheSealedDeclaration(t *testing.T) {
	entry := RuleEntry()
	if got := emitlaw.CanonicalEntry(entry); got != bodyCallEffectIdentity {
		t.Fatalf("rule identity has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, bodyCallEffectIdentity)
	}
	if got := emitlaw.Canonical(entry.Program); got != bodyCallEffectGeometry {
	...
	if got := emitlaw.CanonicalEntry(RuleEntry()); got != bodyCallEffectIdentity {
		t.Fatalf("a caller's edits reached the next rule entry:\n%s", got)
	}
}

// TestBodyCallEffectRefusesEveryStructuralMutation is the negative half of the declaration's
...
// regeneration diff.
func TestBodyCallEffectRefusesEveryStructuralMutation(t *testing.T) {
	for _, law := range []struct {
		...
		declaration := BodyCallEffect().Clone()
...
// TestBodyCallEffectAdmitsTheseEditsAtDeclarationLevel states where Check's authority ends.
...
func TestBodyCallEffectAdmitsTheseEditsAtDeclarationLevel(t *testing.T) {
	for _, law := range []struct {
		mutation string
		apply    func(declaration *ruleprogram.Program)
	}{
