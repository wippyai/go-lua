// Package program owns the selected call site's callback-free rule declaration.
//
...
// The the selected call site family identities. These are declaration keys, not runtime handles:
...
const (
	...
	RuleKey     schema.Key = "effect-selected"
	RuleRole               = "rule/effect/callsite-selected"
	OperandRole            = "operand/effect/callsite-selected"
	...
	Reducer                     schema.Key = effectdomain.SelectedCallEffectReducer
)
...
// RuleEntry is the canonical callback-free the selected call site rule declaration. The family is
...
// SelectedCallEffect returns the immutable the selected call site rule declaration.
//
...
// predecessor world.
func SelectedCallEffect() ruleprogram.Program {
