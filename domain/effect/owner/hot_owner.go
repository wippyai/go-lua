...
func BindHot(binding *engine.SchemaBinding, fragment *SchemaFragment, algebra *factor.Algebra) (*HotOwner, bool) {
	...
	if !specOK || !engine.BindFactor[DenseCoordinate](binding, fragment.slot, spec) {
		return nil, false
	}
	return owner, true
}
...
// cannot drift.
func (owner *HotOwner) FactorSpec() (engine.HotFactorSpec[DenseCoordinate, factor.Value], bool) {
	if owner == nil || owner.algebra == nil || !owner.algebra.Valid() || !validCoordinateCount(owner.algebra.RootCount()) {
		return engine.HotFactorSpec[DenseCoordinate, factor.Value]{}, false
	}
	lattice, ok := owner.algebra.Lattice()
	if !ok {
		return engine.HotFactorSpec[DenseCoordinate, factor.Value]{}, false
	}
	return engine.HotFactorSpec[DenseCoordinate, factor.Value]{
		...
		WidenRank: engine.Measure[DenseCoordinate, factor.Value]{
			Width: 1,
			At:    owner.widenRank,
		},
	}, true
...
func BindExactReadRule[O, RV any](owner *HotOwner, slot *engine.RuleSlot[factor.Value, O], readSlot engine.SchemaReadSlot[RV], readFactor engine.FactorRef[RV], write engine.SchemaWriteSlot[factor.Value], spec engine.HotRuleSpec[factor.Value, O], projectRead func(O) (uint64, bool), projectWrite func(O) (uint64, bool)) (*RuleImplementation[O], engine.Read[engine.OrderedCells[RV]], bool) {
	...
	read, ok := engine.BindRuleWithOpaqueExactRead[DenseCoordinate](owner.binding, slot, readSlot, readFactor, write, owner.fragment.Ref(), spec, projectRead, projectWrite)
	if !ok {
		return nil, engine.Read[engine.OrderedCells[RV]]{}, false
	}
...
func BindSelectedRuleDirect[O any](owner *HotOwner, slot *engine.RuleSlot[factor.Value, O], write engine.SchemaWriteSlot[factor.Value], spec engine.HotRuleSpec[factor.Value, O], projectWrite func(O) (uint64, bool)) (*RuleImplementation[O], bool) {
	...
	if !engine.BindSelectedExactRuleDirect[DenseCoordinate](owner.binding, slot, write, owner.fragment.Ref(), spec, projectWrite) {
		return nil, false
	}
...
func AddSelectedRuleDirectExactRead[O, RV any](issuer *RuleImplementation[O], slot engine.SchemaReadSlot[RV], factor engine.FactorRef[RV], project func(O) (uint64, bool)) (engine.Read[engine.OrderedCells[RV]], bool) {
	...
	return engine.BindSelectedRuleDirectExactRead[DenseCoordinate](issuer.owner.binding, issuer.slot, slot, factor, project)
}
...
func AddSelectedRuleDirectOperandRead[O, RV any, Tag interface {
	...
	return engine.BindSelectedRuleDirectOperandRead[DenseCoordinate, factor.Value, O, RV, Tag](issuer.owner.binding, issuer.slot, slot, source, locate)
}

// ResolveRuleImplementation issues the engine receipt only after the exact
// shared SchemaBinding seals.
func ResolveRuleImplementation[O any](issuer *RuleImplementation[O]) (*engine.RuleImplementation[DenseCoordinate, factor.Value, O], bool) {
	if issuer == nil || issuer.owner == nil || issuer.slot == nil {
		return nil, false
	}
	implementation, ok := engine.RuleImplementationAt[DenseCoordinate, factor.Value, O](issuer.owner.binding, issuer.slot)
	if !ok {
		return nil, false
	}
...
func ResolveRuleImplementationFor[O any](owner *HotOwner, issuer *RuleImplementation[O]) (*engine.RuleImplementation[DenseCoordinate, factor.Value, O], bool) {
...
func (owner *HotOwner) Ref(root factor.Root) (engine.Ref[DenseCoordinate], bool) {
	if owner == nil || owner.binding == nil || owner.fragment == nil || owner.algebra == nil {
		return engine.Ref[DenseCoordinate]{}, false
	}
	index, ok := owner.algebra.RootIndex(root)
	if !ok || index < 0 || uint64(index) > uint64(^uint32(0)) {
		return engine.Ref[DenseCoordinate]{}, false
	}
	implementation, ok := engine.FactorImplementationAt[DenseCoordinate, factor.Value](owner.binding, owner.fragment.slot)
	if !ok {
		return engine.Ref[DenseCoordinate]{}, false
	}
	return implementation.Ref(DenseCoordinate(index))
}
...
func (owner *HotOwner) admits(index DenseCoordinate, value factor.Value) bool {
	root, ok := owner.rootAt(index)
	return ok && owner.algebra.Admit(root, value)
}

func (owner *HotOwner) widenRank(index DenseCoordinate, value factor.Value, component int) uint64 {
	root, ok := owner.rootAt(index)
	if !ok {
		return 0
	}
...
func (owner *HotOwner) rootAt(index DenseCoordinate) (factor.Root, bool) {
