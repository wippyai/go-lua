...
import (
	...
	factor "github.com/wippyai/go-lua/domain/effect/factor"
)

// DenseCoordinate is effect's dense Factor coordinate: the position a key of this
...
// ForeignRead seals one exact read of a bound effect Factor at this axis's own
...
func ForeignRead(foreign execution.ForeignFactor, coordinate execution.SelectedCoordinate, input uint16) (execution.ExactRead[DenseCoordinate, factor.Value], bool) {
	return execution.ForeignExactRead[DenseCoordinate, factor.Value](foreign, coordinate.Unit, input)
}

// ForeignSelectedMember resolves one dense coordinate of a bound effect Factor
...
func ForeignSelectedMember(foreign execution.ForeignFactor, dense uint32, tag uint64) (execution.RouteMember, bool) {
	return execution.ForeignSelectedMember[DenseCoordinate, factor.Value](foreign, dense, tag)
}

// RelationOwner is the generated bind-time owner for effect's member relations.
type RelationOwner struct {
	schema *factor.Algebra
}
...
func NewRelationOwner(schema *factor.Algebra) *RelationOwner {
	if schema == nil {
		return nil
	}
...
func (owner *RelationOwner) candidate(relationOrdinal uint32, mount, occurrence identity.ContentID) (uint32, bool) {
	...
	case 0:
		...
		candidate, candidateOK := owner.schema.MountedCallForOccurrence(mount, occurrence)
		if !candidateOK {
			return 0, false
		}
		return owner.schema.MountedCallOrdinal(candidate)
	default:
		return 0, false
	}
}
...
func (owner *RelationOwner) Project(relationOrdinal, projectionOrdinal, candidateOrdinal uint32) (uint32, bool) {
	...
	case 0:
		...
		case 0:
			candidate, candidateOK := owner.schema.MountedCallAt(int(candidateOrdinal))
			if !candidateOK {
				return 0, false
			}
