...
func AxisMemberCatalog() member.Catalog {
	...
	catalog, ok := member.NewCatalog(
		[]member.Relation{
			{Key: StateCells, Subject: StateCarrier, Inputs: []member.Carrier{MountedCallArgumentCarrier, ValueFactCarrier}, CandidateProvider: provider},
		},
		[]member.Projection{
			{Key: StateCellKey, Relation: StateCells, Role: member.Key, Result: CellCarrier, CandidateProvider: provider},
			{Key: StateCellProtocol, Relation: StateCells, Role: member.Predicate, Result: ProtocolCarrier, CandidateProvider: provider},
			{Key: StateCellDestination, Relation: StateCells, Role: member.Destination, Result: CellCarrier, CandidateProvider: provider},
		},
		[]member.Reducer{
...
func Obligation() ruleprogram.Program {
	...
	return ruleprogram.Program{
		...
		Candidate:   valueCandidateProvider(),
