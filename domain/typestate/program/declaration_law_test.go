...
func TestObligationProgramIsOneExactReadAndOneSelectedCellRead(t *testing.T) {
	...
	if declaration.Candidate.Axis.Key != valueAxisKey || declaration.Candidate.Member != MountedCallArgumentCandidates {
...
func TestObligationProgramRefusesUnownedAndUnkeyedRows(t *testing.T) {
	...
	foreignCandidate.Candidate.Axis = schema.EntryReference{Surface: schema.SurfaceKindAxis, Key: AxisKey}
...
func focusedValueCatalog(t *testing.T) member.Catalog {
	...
	catalog, ok := member.NewCatalog(
		[]member.Relation{
			{Key: MountedCallArgumentCandidates, Subject: MountedCallArgumentCarrier, CandidateProvider: provider},
			{Key: MountedCallArguments, Subject: ValueFactCarrier, Inputs: []member.Carrier{MountedCallArgumentCarrier}, CandidateProvider: provider},
		},
		[]member.Projection{
			{Key: MountedCallArgumentKey, Relation: MountedCallArguments, Role: member.Key, Result: ValueCoordinateCarrier, CandidateProvider: provider},
		},
		[]member.Reducer{
...
func compileObligationProgram(t *testing.T, declaration ruleprogram.Program) (ruleplan.Catalog, schema.SealFailure) {
	...
	if !ok {
		t.Fatal("focused Typestate rule rejected")
	}
