...
func TestBranchValueObservationRowAddressIsFenced(t *testing.T) {
	...
	id, ok := branchValueObservationAttachmentID(mount, point, branchValueObservationFenceProducer)
	if !ok || !id.Available() {
	...
	expected, expectedOK := identity.DeriveContentID(branchValueObservationFormulaFence, mount[:], point[:], []byte(branchValueObservationFenceProducer))
...
func TestBranchValueObservationRowAddressIsPositional(t *testing.T) {
	mount, point := rowAddressFenceID(0x21), rowAddressFenceID(0x22)
	forward, forwardOK := branchValueObservationAttachmentID(mount, point, branchValueObservationFenceProducer)
	reverse, reverseOK := branchValueObservationAttachmentID(point, mount, branchValueObservationFenceProducer)
	...
	foreign, foreignOK := identity.DeriveContentID(branchValueObservationFormulaFence+"x", mount[:], point[:], []byte(branchValueObservationFenceProducer))
	...
	if _, ok := branchValueObservationAttachmentID(identity.ContentID{}, point, branchValueObservationFenceProducer); ok {
		t.Fatal("the row address was derived without a mount")
	}
	if _, ok := branchValueObservationAttachmentID(mount, identity.ContentID{}, branchValueObservationFenceProducer); ok {
		t.Fatal("the row address was derived without a point")
	}
	if _, ok := branchValueObservationAttachmentID(mount, point, ""); ok {
