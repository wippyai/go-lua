...
func TestObservationConstructionUsesSealedProducers(t *testing.T) {
	...
	analysisRoot := filepath.Join(here, "..", "..", "analysis")
	for _, name := range []string{
		filepath.Join(analysisRoot, "result", "project.go"),
		filepath.Join(analysisRoot, "diagnostic", "attach.go"),
		filepath.Join(here, "publication", "branch_value_observation.go"),
	} {
		...
		if strings.Contains(string(src), `"value-summary"`) {
			t.Fatalf("%s restates producer %q; construction walks ObservationIssuance", name, "value-summary")
		}
	}
}
