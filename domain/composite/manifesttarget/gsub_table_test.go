...
func standardDeclarations(t *testing.T) *manifest.Catalogue {
	t.Helper()
...
func standardCatalogueSpec(t *testing.T) target.Spec {
	t.Helper()
	spec, err := compileCatalogue(standardDeclarations(t))
	if err != nil {
		t.Fatal(err)
	}
	return spec
...
func TestGsubTableReplacementClosedDenominatorAndAliases(t *testing.T) {
	contract, err := SealCatalogue(standardDeclarations(t))
	if err != nil {
		t.Fatal(err)
	}
