...
func TestRuleTableDrivesEveryDerivedView(t *testing.T) {
	...
	// The identity space is measured against the artifact format's own closed
	// role catalog rather than against the walk that produced it. The rule
	// surface pins the declaration order to the role ordinal, so the roles the
	// table declares are exactly the ordinal space the artifact carries, from
	// RuleRoleValueSource through RuleRoleValuePresenceRefinement.
	roles := int(programartifact.RuleRoleValuePresenceRefinement)
	if len(identities) != roles {
		t.Fatalf("table identities = %d, artifact role catalog = %d", len(identities), roles)
	}
