...
// dispatchGeometry is the whole geometry the declaration carries, in the one
...
const dispatchGeometry = `operand              semantic/operand/call/dispatch
...
join[0].relation     axis/value:value/mounted-call/parents
join[0].key          axis/value:value/mounted-call/callee-key
join[0].predicate    -
join[0].selection    -
join[0].parent       -
...
// TestDispatchGeometryIsTheOneItsLawSuiteWasEmittedFrom holds the declaration to the
...
func TestDispatchGeometryIsTheOneItsLawSuiteWasEmittedFrom(t *testing.T) {
	if got := emitlaw.Canonical(Dispatch()); got != dispatchGeometry {
		t.Fatalf("declared geometry has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, dispatchGeometry)
	}
}

// TestDispatchAgreesWithTheCallShapeOfTheReducerItNames is the one structural law a
...
func TestDispatchAgreesWithTheCallShapeOfTheReducerItNames(t *testing.T) {
	declaration := Dispatch()
	if problem, valid := declaration.Check(); !valid {
		t.Fatalf("declaration is malformed: %+v", problem)
	}
	reducer, reducerOK := axiscatalog.AxisMemberCatalog().Reducer(schemaapi.Key("call/dispatch/reducer"))
	if !reducerOK {
		t.Fatal("the reducer axis publishes no reducer named call/dispatch/reducer")
	}
...
// TestDispatchIsHandedOutFresh states that the accessor yields a declaration a caller
...
func TestDispatchIsHandedOutFresh(t *testing.T) {
	first := Dispatch()
...
// dispatchIdentity is the rule identity this declaration is sealed under.
const dispatchIdentity = `rule                 call-dispatch
lane                 mounted
writes               call
owner                call
semantic             semantic/rule/call/dispatch
role[0]              semantic/operand/call/dispatch
issue[0]             occurrence=occurrence/call form=program-form/call-dispatch requirement=program-requirement/unrestricted
`

// TestDispatchRuleEntryIsTheSealedDeclaration holds the rule entry to its identity, to
...
func TestDispatchRuleEntryIsTheSealedDeclaration(t *testing.T) {
	entry := RuleEntry()
	if got := emitlaw.CanonicalEntry(entry); got != dispatchIdentity {
		t.Fatalf("rule identity has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, dispatchIdentity)
	}
	if got := emitlaw.Canonical(entry.Program); got != dispatchGeometry {
	...
	if got := emitlaw.CanonicalEntry(RuleEntry()); got != dispatchIdentity {
		t.Fatalf("a caller's edits reached the next rule entry:\n%s", got)
	}
}

// TestDispatchRefusesEveryStructuralMutation is the negative half of the declaration's
...
// regeneration diff.
func TestDispatchRefusesEveryStructuralMutation(t *testing.T) {
...
// TestDispatchAdmitsTheseEditsAtDeclarationLevel states where Check's authority ends.
...
func TestDispatchAdmitsTheseEditsAtDeclarationLevel(t *testing.T) {
	for _, law := range []struct {
		mutation string
		apply    func(declaration *ruleprogram.Program)
	}{
