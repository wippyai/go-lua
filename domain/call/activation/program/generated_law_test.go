...
import (
	...
	axiscatalog "github.com/wippyai/go-lua/domain/call"
)

// activationGeometry is the whole geometry the declaration carries, in the one
...
const activationGeometry = `operand              semantic/operand/call/activation
...
carry                none
fold.reducer         axis/call:call/activation/reducer
fold.inputs          join 0
output[0]            mode=structural value-slot=0 route=- column=axis/call:call/facts destination=axis/call:call/mounted-call/coordinate
transport[0]         axis=axis/value exported=true
transport[1]         axis=axis/call exported=false
transport[2]         axis=axis/heap exported=true
transport[3]         axis=axis/pack exported=true
transport[4]         axis=axis/effect exported=true
transport[5]         axis=axis/placement exported=true
`

// TestActivationGeometryIsTheOneItsLawSuiteWasEmittedFrom holds the declaration to the
...
func TestActivationGeometryIsTheOneItsLawSuiteWasEmittedFrom(t *testing.T) {
	if got := emitlaw.Canonical(Activation()); got != activationGeometry {
		t.Fatalf("declared geometry has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, activationGeometry)
	}
}

// TestActivationAgreesWithTheCallShapeOfTheReducerItNames is the one structural law a
...
func TestActivationAgreesWithTheCallShapeOfTheReducerItNames(t *testing.T) {
	declaration := Activation()
	if problem, valid := declaration.Check(); !valid {
		t.Fatalf("declaration is malformed: %+v", problem)
	}
	reducer, reducerOK := axiscatalog.AxisMemberCatalog().Reducer(schemaapi.Key("call/activation/reducer"))
	if !reducerOK {
		t.Fatal("the reducer axis publishes no reducer named call/activation/reducer")
	}
...
// TestActivationIsHandedOutFresh states that the accessor yields a declaration a caller
...
func TestActivationIsHandedOutFresh(t *testing.T) {
	first := Activation()
...
// activationIdentity is the rule identity this declaration is sealed under.
const activationIdentity = `rule                 call-activation
lane                 mounted
writes               call
owner                call
semantic             semantic/rule/call/activation
role[0]              semantic/operand/call/activation
role[1]              semantic/activation-family/call/body
issue[0]             occurrence=occurrence/call-activation form=program-form/call-summary requirement=program-requirement/unrestricted
`

// TestActivationRuleEntryIsTheSealedDeclaration holds the rule entry to its identity, to
...
func TestActivationRuleEntryIsTheSealedDeclaration(t *testing.T) {
	entry := RuleEntry()
	if got := emitlaw.CanonicalEntry(entry); got != activationIdentity {
		t.Fatalf("rule identity has moved away from its law suite:\ngot:\n%s\nwant:\n%s", got, activationIdentity)
	}
	if got := emitlaw.Canonical(entry.Program); got != activationGeometry {
	...
	if got := emitlaw.CanonicalEntry(RuleEntry()); got != activationIdentity {
		t.Fatalf("a caller's edits reached the next rule entry:\n%s", got)
	}
}

// TestActivationRefusesEveryStructuralMutation is the negative half of the declaration's
...
// regeneration diff.
func TestActivationRefusesEveryStructuralMutation(t *testing.T) {
...
// TestActivationAdmitsTheseEditsAtDeclarationLevel states where Check's authority ends.
...
func TestActivationAdmitsTheseEditsAtDeclarationLevel(t *testing.T) {
	for _, law := range []struct {
		mutation string
		apply    func(declaration *ruleprogram.Program)
	}{
