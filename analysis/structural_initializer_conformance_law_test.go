...
func TestEveryPublishedPlacementNamesTheAxisItWrites(t *testing.T) {
	...
	if _, sealed := programschema.NewRuleOccurrenceWithInputs("value-source", "", 0, point, []identity.ContentID{input}, programissuance.StageLocal, programissuance.InputPreviousStage, identity.ContentID{}, false); sealed {
		t.Fatal("a placement sealed without the axis it writes")
	}
	if _, sealed := programschema.NewRuleOccurrenceWithInputs("value-source", "value", 0, point, []identity.ContentID{input}, programissuance.StageLocal, programissuance.InputPreviousStage, identity.ContentID{}, false); !sealed {
