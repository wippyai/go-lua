...
const (
	contractPackage   = "github.com/wippyai/go-lua/analysis/program/target/contract"
	compilerPackage   = "github.com/wippyai/go-lua/analysis/program/target/compiler"
	vocabularyPackage = "github.com/wippyai/go-lua/analysis/program/target/vocabulary"
)
...
func TestVocabularyImportsNothingFromTheContract(t *testing.T) {
	...
	for _, path := range files {
		...
		for _, spec := range parsed.Imports {
			...
			if imported == contractPackage || strings.HasPrefix(imported, contractPackage+"/") ||
				imported == compilerPackage || strings.HasPrefix(imported, compilerPackage+"/") {
...
func contractSources(t *testing.T) []string {
	t.Helper()
	return sourcesIn(t, filepath.Join(filepath.Dir(packageDirectory(t)), "contract"))
}
