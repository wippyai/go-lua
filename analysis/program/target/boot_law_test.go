...
func bootExactKey(t *testing.T, contract *Contract, text string) vocabulary.ExactKey {
	t.Helper()
...
func bootExactKeyText(contract *Contract, key vocabulary.ExactKey) string {
...
func TestBootLedgerRejectsMalformedRows(t *testing.T) {
	cases := []struct {
		...
		{"missing shape", func(spec *Spec) { spec.InitialRoots[0].Shape = vocabulary.BootShapeSpec{} }},
		...
		{"frozen global binding", func(spec *Spec) { spec.InitialEntries[1].Mutability = vocabulary.InitialFrozen }},
		{"global root is not table", func(spec *Spec) { spec.InitialRoots[1].Shape.Aggregate = vocabulary.BootAggregateMetatable }},
		...
		{"invalid denied operation", func(spec *Spec) {
...
func TestBootLedgerRequiresGlobalSelfAlias(t *testing.T) {
	for _, test := range []struct {
		...
		{"frozen _G", func(spec *Spec) { spec.InitialEntries[0].Mutability = vocabulary.InitialFrozen }},
...
func TestInitialMetatableAttachmentIsTypedCanonicalBootstrapData(t *testing.T) {
	...
	for _, edit := range []func(*Spec){
		func(spec *Spec) { spec.InitialMetatables[0].Base = vocabulary.InitialValueInteger },
...
func TestBootLedgerOptionalAsAWholeButNeverPartial(t *testing.T) {
	withoutBoot := mustSeal(t, Spec{Operations: []vocabulary.OperationSpec{bootOperation("plain")}})
	...
	partial := Spec{Operations: []vocabulary.OperationSpec{bootOperation("plain")}, InitialEntries: []vocabulary.InitialEntrySpec{{Root: "missing", Key: bootLiteralKey("key"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueNil}, Mutability: vocabulary.InitialFrozen}}}
...
func completeBootSpec(version string, versionMutability vocabulary.InitialMutability) Spec {
	return Spec{
