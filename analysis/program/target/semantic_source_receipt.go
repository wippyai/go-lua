...
func newTargetReceiptWriter(owner identity.ContentID, token semanticsource.Token) *targetReceiptWriter {
...
func (w *targetReceiptWriter) id(value identity.ContentID) {
...
func (w *targetReceiptWriter) finish() (identity.ContentID, bool) {
	if w == nil || !w.valid {
...
func (c *Contract) operationRowsNested(token semanticsource.Token, emit func(*targetRowEmitter, Operation, identity.ContentID)) (SemanticSourceView, bool) {
	return targetTypedRows(c, token, func(emitter *targetRowEmitter) {
		for index := 0; index < c.OperationCount(); index++ {
			op, ok := c.OperationAt(index)
			if !ok {
...
func (c *Contract) spawnRows(token semanticsource.Token, emit func(*targetRowEmitter, Operation, identity.ContentID)) (SemanticSourceView, bool) {
