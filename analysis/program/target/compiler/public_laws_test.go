...
func assertPublicContractEqual(t *testing.T, left, right *contract.Contract) {
	t.Helper()
	leftSnapshot := publicContractSnapshot(t, left)
	rightSnapshot := publicContractSnapshot(t, right)
	if leftSnapshot != rightSnapshot {
		t.Fatalf("permutation/alpha changed public contract.Contract observations\nleft:  %s\nright: %s", leftSnapshot, rightSnapshot)
	}
}

func publicContractSnapshot(t *testing.T, contract *contract.Contract) string {
	t.Helper()
	...
	for index := 0; index < protocolTable(contract).ProtocolCount(); index++ {
		protocol, ok := protocolTable(contract).ProtocolAt(index)
...
func writeProtocolSnapshot(out *strings.Builder, contract *contract.Contract, protocol vocabulary.Protocol) {
	fmt.Fprintf(out, "acquisitions=%d,states=%d,transitions=%d,escapes=%d,callback-holders=%d;",
		protocolTable(contract).ProtocolAcquisitionCount(protocol), protocolTable(contract).StateCount(protocol),
		protocolTable(contract).TransitionCount(protocol), protocolTable(contract).EscapeCount(protocol), protocolTable(contract).ProtocolCallbackHolderCount(protocol))
	for index := 0; index < protocolTable(contract).ProtocolAcquisitionCount(protocol); index++ {
		op, outcome, result, state, ok := protocolTable(contract).ProtocolAcquisitionAt(protocol, index)
		fmt.Fprintf(out, "acquisition[%d]=%d/%d/%d/%d/%v;", index, op, outcome, result, state, ok)
	}
	for index := 0; index < protocolTable(contract).StateCount(protocol); index++ {
		state, ok := protocolTable(contract).StateAt(protocol, index)
		name, nameOK := protocolTable(contract).StateName(protocol, state)
		final, finalOK := protocolTable(contract).StateFinal(protocol, state)
		fmt.Fprintf(out, "state[%d]=%d/%q/%v/%v/%v;", index, state, name, final, ok, nameOK && finalOK)
	}
	for index := 0; index < protocolTable(contract).TransitionCount(protocol); index++ {
		op, kind, source, from, ok := protocolTable(contract).TransitionAt(protocol, index)
		fmt.Fprintf(out, "transition[%d]=%d/%d/%d/%d/%v;", index, op, kind, source, from, ok)
		for outcomeIndex := 0; outcomeIndex < protocolTable(contract).TransitionOutcomeCount(protocol, index); outcomeIndex++ {
			outcome, to, outcomeOK := protocolTable(contract).TransitionOutcomeAt(protocol, index, outcomeIndex)
			fmt.Fprintf(out, "transition-outcome[%d]=%d/%d/%v;", outcomeIndex, outcome, to, outcomeOK)
		}
	}
	for index := 0; index < protocolTable(contract).EscapeCount(protocol); index++ {
		op, kind, source, ok := protocolTable(contract).EscapeAt(protocol, index)
		fmt.Fprintf(out, "escape[%d]=%d/%d/%d/%v;", index, op, kind, source, ok)
	}
	for index := 0; index < protocolTable(contract).ProtocolCallbackHolderCount(protocol); index++ {
		op, input, callback, ok := protocolTable(contract).ProtocolCallbackHolderAt(protocol, index)
		fmt.Fprintf(out, "callback-holder[%d]=%d/%d/%d/%d/%v;", index, op, input.Kind, input.Ordinal, callback, ok)
	}
}

func writeOperationSnapshot(t *testing.T, out *strings.Builder, contract *contract.Contract, op vocabulary.Operation) {
	t.Helper()
...
func writeSubedgeSnapshot(out *strings.Builder, contract *contract.Contract, op vocabulary.Operation) {
...
func writeEffectArguments(out *strings.Builder, contract *contract.Contract, op vocabulary.Operation, effect int) {
...
func writeCallbackEffectArguments(out *strings.Builder, contract *contract.Contract, callback vocabulary.CallbackID, effect int) {
...
func writeBindingSnapshot(out *strings.Builder, contract *contract.Contract, op vocabulary.Operation) {
...
func publicValuesSnapshot(t *testing.T, contract *contract.Contract, values vocabulary.Values, valuesOK bool) string {
	t.Helper()
	if !valuesOK {
		return "invalid"
	}
...
func publicTypeDigest(t *testing.T, contract *contract.Contract, value vocabulary.Type, ok bool) string {
	t.Helper()
	if !ok {
		return "invalid"
	}
