...
func TestProtocolMultipleAcquisitionsAndEntryRows(t *testing.T) {
	...
	if protocolTable(contract).ProtocolCount() != 1 {
		t.Fatalf("ProtocolCount = %d", protocolTable(contract).ProtocolCount())
	}
	p, _ := protocolTable(contract).ProtocolAt(0)
	if protocolTable(contract).ProtocolAcquisitionCount(p) != 2 {
		t.Fatalf("acquisition count = %d", protocolTable(contract).ProtocolAcquisitionCount(p))
	}
	for index, wantName := range []string{"open", "closed"} {
		state, ok := protocolTable(contract).StateAt(p, index)
		name, nameOK := protocolTable(contract).StateName(p, state)
		if !ok || !nameOK || name != wantName {
			t.Fatalf("state %d = %d/%q", index, state, name)
		}
		final, finalOK := protocolTable(contract).StateFinal(p, state)
		if !finalOK || final != (index == 1) {
			t.Fatalf("final %d = %v/%v", index, final, finalOK)
		}
	}
	if op, source, ordinal, from, ok := protocolTable(contract).TransitionAt(p, 0); !ok || source != vocabulary.InputSourceValueFormal || ordinal != 0 || from != 1 || op == 0 {
		t.Fatalf("transition = %d/%d/%d/%d/%v", op, source, ordinal, from, ok)
	}
	if outcome, to, ok := protocolTable(contract).TransitionOutcomeAt(p, 0, 0); !ok || outcome != 0 || to != 2 {
		t.Fatalf("transition outcome = %d/%d/%v", outcome, to, ok)
	}
	if protocolTable(contract).EscapeCount(p) != 2 {
		t.Fatalf("escape count = %d", protocolTable(contract).EscapeCount(p))
	}
	opaque, _ := contract.Operations.Opaque()
	op, source, ordinal, ok := protocolTable(contract).EscapeAt(p, 1)
	...
	if protocolTable(contract).TransitionCount(p) != 1 {
		t.Fatal("opaque fabricated a protocol transition")
	}
	if allocs := testing.AllocsPerRun(1000, func() { _, _, _, _ = protocolTable(contract).EscapeAt(p, 1) }); allocs != 0 {
...
func TestProtocolRejectsInvalidNominalAuthority(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{base}, Protocols: test.protocols}); err == nil {
...
func TestProtocolRejectsBadOutcomeAndTransitionCoordinates(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{base}, Protocols: []vocabulary.ProtocolSpec{test.value}}); err == nil {
...
func TestProtocolPublicObservablesIgnoreStateAndAcquisitionAuthorOrder(t *testing.T) {
	...
	left := mustSeal(t, declaration.Spec{Operations: operations, Protocols: []vocabulary.ProtocolSpec{{
		Acquisitions: []vocabulary.AcquisitionSpec{{Operation: 1, Outcome: 0, Result: 0, State: 1}, {Operation: 2, Outcome: 0, Result: 0, State: 2}},
		States:       []vocabulary.StateSpec{{Name: "open"}, {Name: "closed", Final: true}},
	}}})
	right := mustSeal(t, declaration.Spec{Operations: operations, Protocols: []vocabulary.ProtocolSpec{{
...
func TestProtocolStateCoordinatesAreAlphaInvariantAcrossCyclesAndRoots(t *testing.T) {
	...
	for _, contract := range []*contract.Contract{left, right} {
		protocol, _ := protocolTable(contract).ProtocolAt(0)
		if states := protocolTable(contract).StateCount(protocol); states != 3 {
			t.Fatalf("state count = %d, want 3", states)
		}
		state, _ := protocolTable(contract).StateAt(protocol, 1)
		if final, ok := protocolTable(contract).StateFinal(protocol, state); !ok || !final {
...
func TestProtocolRejectsUnreachableState(t *testing.T) {
	_, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{protocolOperation("reachable", nil)}, Protocols: []vocabulary.ProtocolSpec{{
		Acquisitions: []vocabulary.AcquisitionSpec{{Operation: 1, Outcome: 0, Result: 0, State: 1}},
...
func TestProtocolWideStatesSealIteratively(t *testing.T) {
	...
	p, _ := protocolTable(contract).ProtocolAt(0)
	if protocolTable(contract).StateCount(p) != width {
		t.Fatalf("state count = %d", protocolTable(contract).StateCount(p))
	}
	state, _ := protocolTable(contract).StateAt(p, 0)
	if name, ok := protocolTable(contract).StateName(p, state); !ok || name != "state-04096" {
...
func TestProtocolWideOutcomeRemapAvoidsQuadraticSearch(t *testing.T) {
	...
	p, _ := protocolTable(contract).ProtocolAt(0)
	if protocolTable(contract).ProtocolAcquisitionCount(p) != width {
		t.Fatalf("acquisition count = %d", protocolTable(contract).ProtocolAcquisitionCount(p))
	}
}
...
func TestProtocolCallbackHoldersSealExactRetainedRelation(t *testing.T) {
	...
	handle, ok := protocolTable(contract).ProtocolAt(0)
	if !ok || protocolTable(contract).ProtocolCallbackHolderCount(handle) != 1 {
		t.Fatalf("protocol callback-holder range = %d/%v", protocolTable(contract).ProtocolCallbackHolderCount(handle), ok)
	}
	op, input, callback, ok := protocolTable(contract).ProtocolCallbackHolderAt(handle, 0)
	...
	if _, _, _, found := protocolTable(contract).ProtocolCallbackHolderAt(handle, 1); found {
		t.Fatal("out-of-range callback holder resolved")
	}
	if protocolTable(contract).ProtocolCallbackHolderCount(vocabulary.Protocol(2)) != 0 {
		t.Fatal("opaque fabricated a callback-holder relation")
	}
	if allocs := testing.AllocsPerRun(1000, func() { _, _, _, _ = protocolTable(contract).ProtocolCallbackHolderAt(handle, 0) }); allocs != 0 {
...
func TestProtocolCallbackHoldersAcceptValuesVarAndIgnoreAuthorOrder(t *testing.T) {
	...
	handle, _ := protocolTable(leftContract).ProtocolAt(0)
	if protocolTable(leftContract).ProtocolCallbackHolderCount(handle) != 2 {
		t.Fatalf("callback-holder count = %d", protocolTable(leftContract).ProtocolCallbackHolderCount(handle))
	}
}
...
func TestProtocolCallbackHoldersRejectUnsealedAuthority(t *testing.T) {
	cases := []struct {
		...
		{"all-inputs", func(spec *declaration.Spec) {
			spec.Protocols[0].CallbackHolders[0].Input = vocabulary.InputSource{Kind: vocabulary.InputSourceAllInputs}
		}},
		{"formal outside scope", func(spec *declaration.Spec) {
			spec.Protocols[0].CallbackHolders[0].Input = vocabulary.InputSource{Kind: vocabulary.InputSourceValueFormal, Ordinal: 2}
		}},
		{"Values variable outside scope", func(spec *declaration.Spec) {
			spec.Protocols[0].CallbackHolders[0].Input = vocabulary.InputSource{Kind: vocabulary.InputSourceValuesVar, Ordinal: 1}
		}},
		{"callback outside operation", func(spec *declaration.Spec) { spec.Protocols[0].CallbackHolders[0].Callback = 2 }},
		{"synchronous callback", func(spec *declaration.Spec) {
			spec.Operations[0].Callbacks[0].Lifecycle = vocabulary.CallbackSyncOptionalOnce
		}},
		{"duplicate triple", func(spec *declaration.Spec) {
			spec.Protocols[0].CallbackHolders = append(spec.Protocols[0].CallbackHolders, spec.Protocols[0].CallbackHolders[0])
		}},
		{"unbound operation", func(spec *declaration.Spec) {
...
func specWithProtocolCallbackHolder(input vocabulary.InputSource) declaration.Spec {
