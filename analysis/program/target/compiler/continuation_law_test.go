...
func TestSuspensionCanonicalizesOutcomeOrdinalsAndKeepsCallbackValues(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{
		{
	...
	for index, want := range []struct {
		...
		if kind, _, ok := contract.Operations.OutcomeAt(yield, int(reentryOutcome)); !ok || kind != want.kind {
...
func TestResumeSealsCompleteCanonicalOutcomeCorrespondence(t *testing.T) {
	seal := func(outcomes []vocabulary.OutcomeSpec, mapping []vocabulary.ResumeOutcomeSpec) *contract.Contract {
		return mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{{
	...
	op, ok := left.Operations.Lookup(vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"resume-canonical"}})
	if !ok {
...
func TestResumeIDsCanonicalizePermutationAndRoundTrip(t *testing.T) {
	...
	left := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{operation([]vocabulary.ResumeSpec{
		completeResume(vocabulary.ResumeSourceValueFormal, 1, 0),
		completeResume(vocabulary.ResumeSourceValueFormal, 0, 0),
	})}})
	right := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{operation([]vocabulary.ResumeSpec{
		completeResume(vocabulary.ResumeSourceValueFormal, 0, 0),
		completeResume(vocabulary.ResumeSourceValueFormal, 1, 0),
	})}})
	if got, want := publicContractSnapshot(t, left), publicContractSnapshot(t, right); got != want {
...
func TestResumePayloadTransportUsesOnlyExistingValuesRelations(t *testing.T) {
	...
	_, reentryValues, ok := contract.Operations.OutcomeAt(yield, int(reentry))
	if !ok {
	...
	for _, item := range []struct {
		...
		_, values, ok := contract.Operations.OutcomeAt(resume, int(outcome))
		if !ok {
			t.Fatalf("mapped operation outcome %d missing", outcome)
		}
		if tail, variable, ok := contract.Operations.ValuesTail(values); !ok || tail != item.tail || variable != item.varID {
...
func TestSuspensionAuthorPermutationHasOnePublicContract(t *testing.T) {
	operation := func(outcomes []vocabulary.OutcomeSpec, suspensions []vocabulary.SuspensionSpec) *contract.Contract {
		return mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{{
...
func TestWideProducedResumesSealWithoutQuadraticValidation(t *testing.T) {
	...
	contract := mustSeal(t, declaration.Spec{Operations: operations})
	if contract.Operations.OperationCount() != width+2 {
...
func TestSpawnSealsOneTypedDetachedAuthority(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{spawnTestOperation("spawn")}})
	...
	if childOwner, found := contract.Operations.CallbackOwner(child); !found || childOwner != op {
...
func TestSpawnRejectsIncompleteAndDuplicateAuthority(t *testing.T) {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{bad}}); err == nil {
		t.Fatal("incomplete sibling alternatives sealed")
	}
	left, right := spawnTestOperation("left"), spawnTestOperation("right")
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{left, right}}); err == nil {
		t.Fatal("duplicate spawn authority sealed")
	}
}

func TestSpawnSiblingOrderCanonicalizesContentIdentity(t *testing.T) {
	left := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{spawnTestOperation("spawn")}})
	rightSpec := declaration.Spec{Operations: []vocabulary.OperationSpec{spawnTestOperation("spawn")}}
...
func TestTransferCanonicalizesEndpointPayloadAndOutcomes(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{transferOperation(
		[]vocabulary.OutcomeSpec{
			{Kind: flowkind.OutcomeYield, Values: vocabulary.ValuesSpec{Tail: vocabulary.ValuesVariable, Var: 0}},
			{Kind: flowkind.OutcomeCancel, Values: vocabulary.ValuesSpec{Tail: vocabulary.ValuesClosed}},
	...
	op, ok := contract.Operations.Lookup(vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"transfer"}})
	if !ok || contract.Operations.TransferCount(op) != 2 {
		t.Fatalf("transfer operation/count = %d/%v/%d", op, ok, contract.Operations.TransferCount(op))
	}
	want := []struct {
	...
	for index, expected := range want {
		id, idOK := contract.Operations.TransferIDAt(op, index)
		owner, ownerOK := contract.Operations.TransferOwner(id)
		declaredEndpoint, declaredPayload, declaredAlias, declaredIdentity, declaredCapabilities, declarationOK := contract.Operations.TransferDeclaration(id)
		...
		endpoint, endpointOK := contract.Operations.TransferEndpointAt(op, index)
		payload, payloadOK := contract.Operations.TransferPayloadAt(op, index)
		alias, aliasOK := contract.Operations.TransferAliasAt(op, index)
		identity, identityOK := contract.Operations.TransferIdentityAt(op, index)
		capabilities, capabilitiesOK := contract.Operations.TransferCapabilitiesAt(op, index)
		...
		for outcome, mask := range expected.masks {
			declaredOrdinal, declaredMask, declaredFound := contract.Operations.TransferDeclarationOutcomeAt(id, outcome)
			...
			ordinal, got, found := contract.Operations.TransferOutcomeAt(op, index, outcome)
...
func TestTransferAuthorPermutationHasOnePublicContract(t *testing.T) {
	...
	left := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{transferOperation(leftOutcomes, []vocabulary.TransferSpec{
	...
	right := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{transferOperation(rightOutcomes, []vocabulary.TransferSpec{
...
func TestTransferAliasIsCanonicalDeclarationAndContentAuthority(t *testing.T) {
	...
	left := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{transferOperation(outcomes, []vocabulary.TransferSpec{base})}})
	right := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{transferOperation(outcomes, []vocabulary.TransferSpec{other})}})
	if left.ContentID() == right.ContentID() {
		t.Fatal("transfer alias did not affect target ContentID")
	}
	op, ok := right.Operations.Lookup(vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"transfer"}})
	if !ok {
		t.Fatal("transfer operation")
	}
	alias, ok := right.Operations.TransferAliasAt(op, 0)
...
func TestTransferRejectsIncompleteOrInvalidAuthority(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			spec := baseTransfer()
			test.edit(&spec)
			if contract, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{transferOperation(base, []vocabulary.TransferSpec{spec})}}); err == nil || contract != nil {
	...
	if contract, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{transferOperation(base, []vocabulary.TransferSpec{first, second})}}); err == nil || contract != nil {
...
func TestOpaqueTransferIsMaximalAndAllocationFree(t *testing.T) {
	...
	if !ok || contract.Operations.TransferCount(opaque) != 1 {
		t.Fatalf("opaque/count = %d/%v/%d", opaque, ok, contract.Operations.TransferCount(opaque))
	}
	endpoint, endpointOK := contract.Operations.TransferEndpointAt(opaque, 0)
	payload, payloadOK := contract.Operations.TransferPayloadAt(opaque, 0)
	alias, aliasOK := contract.Operations.TransferAliasAt(opaque, 0)
	identity, identityOK := contract.Operations.TransferIdentityAt(opaque, 0)
	capabilities, capabilitiesOK := contract.Operations.TransferCapabilitiesAt(opaque, 0)
	...
	if allocs := testing.AllocsPerRun(1000, func() {
		if _, ok := contract.Operations.TransferEndpointAt(opaque, 0); !ok {
			panic("opaque transfer endpoint disappeared")
		}
		if _, ok := contract.Operations.TransferPayloadAt(opaque, 0); !ok {
...
func TestFreshOutcomeOrderingRemapsSuspensionAndResumeCoordinates(t *testing.T) {
	...
	if kind, _, found := contract.Operations.OutcomeAt(spawn, int(parentYield)); !found || kind != flowkind.OutcomeYield {
		t.Fatalf("spawn yield = %d/%d/%v", parentYield, kind, found)
	}
	if kind, values, found := contract.Operations.OutcomeAt(spawn, int(parentResume)); !found || kind != flowkind.OutcomeNormal || values != childEntry || values != resumeValues || contract.Operations.ValuesCount(values) != 0 {
