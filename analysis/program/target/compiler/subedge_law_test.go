...
func subedgeByRole(t *testing.T, c *contract.Contract, op vocabulary.Operation, role uint32) vocabulary.SubedgeID {
	t.Helper()
	for index := 0; index < c.Operations.SubedgeCount(op); index++ {
		edge, ok := c.Operations.SubedgeAt(op, index)
		got, roleOK := c.Operations.SubedgeRole(edge)
...
func TestSubedgeSealsExplicitTransportAndCanonicalRoles(t *testing.T) {
	left := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{protectedSubedgeOperation("protected-subedge", false, false, false)}})
	right := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{protectedSubedgeOperation("protected-subedge", false, true, true)}})
	...
	if !ok || left.Operations.SubedgeCount(op) != 2 {
		t.Fatalf("subedge count = %d/%v", left.Operations.SubedgeCount(op), ok)
	}
	protected := subedgeByRole(t, left, op, 100)
	handler := subedgeByRole(t, left, op, 200)
	route, adjustment, result, placement, offset, outcome, sibling, destination, routeOK := left.Operations.SubedgeRouteAt(protected, flowkind.OutcomeThrow)
	...
	arguments, argumentsOK := left.Operations.SubedgeArguments(handler)
	terminal, terminalOK := left.Operations.SubedgeTerminal(protected, flowkind.OutcomeThrow)
	...
	if _, callbackOK := left.Operations.SubedgeCallback(protected); !callbackOK {
	...
	for _, edge := range []vocabulary.SubedgeID{protected, handler} {
		route, adjustment, result, placement, offset, outcome, sibling, destination, found := left.Operations.SubedgeRouteAt(edge, flowkind.OutcomeYield)
	...
	if allocs := testing.AllocsPerRun(1000, func() {
		if _, _, _, _, _, _, _, _, ok := left.Operations.SubedgeRouteAt(handler, flowkind.OutcomeThrow); !ok {
			panic("subedge route disappeared")
		}
		if role, ok := left.Operations.SubedgeRole(handler); !ok || role != 200 {
...
func TestSubedgeScalarAdjustmentHasItsOwnClosedResult(t *testing.T) {
	pack := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{protectedSubedgeOperation("protected-pack", false, false, false)}})
	scalar := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{protectedSubedgeOperation("protected-scalar", true, false, false)}})
	...
	route, adjustment, result, placement, offset, _, _, destination, ok := scalar.Operations.SubedgeRouteAt(protected, flowkind.OutcomeThrow)
...
func TestSubedgeAdmissionFailureAndArgumentAuthorityAreExplicit(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{protectedSubedgeOperation("pcall-xpcall-admission", true, false, false)}})
	...
	failure, failureOK := contract.Operations.SubedgeAdmissionFailure(protected)
	route, adjustment, result, placement, offset, outcome, sibling, destination, routeOK := contract.Operations.SubedgeAdmissionRoute(protected)
	arguments, argumentsOK := contract.Operations.SubedgeArguments(handler)
	...
	if count := contract.Operations.SubedgeArgumentOriginCount(handler); count != 0 {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{orphaned}}); err == nil {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{mixed}}); err == nil {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{partial}}); err == nil {
...
func TestSubedgeNonCallFamiliesHaveClosedABIAndCanonicalOrigins(t *testing.T) {
	...
	if count := leftContract.Operations.SubedgeArgumentOriginCount(edge); count != 2 {
		t.Fatalf("non-Call origin count = %d", count)
	}
	for index, want := range []uint32{0, 1} {
		segment, ordinal, source, input, ok := leftContract.Operations.SubedgeArgumentOriginAt(edge, index)
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{invalid}}); err == nil {
...
func TestSubedgeRejectsInvalidAuthority(t *testing.T) {
	for _, check := range []struct {
		...
		t.Run(check.name, func(t *testing.T) {
			...
			check.edit(&op)
			if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{op}}); err == nil {
...
func TestSubedgeRejectYieldUsesTheExistingCanonicalThrow(t *testing.T) {
	...
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{op}})
	...
	route, adjustment, result, placement, offset, outcome, sibling, destination, ok := contract.Operations.SubedgeRouteAt(edge, flowkind.OutcomeYield)
...
func TestSubedgeNullaryCallbackMuRespectsLifecycleMultiplicity(t *testing.T) {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{once}}); err == nil {
	...
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{many}})
...
func TestSubedgeXPCALLHandlerSelfRecurrenceSealsIteratively(t *testing.T) {
	...
	if admission, ok := contract.Operations.SubedgeAdmission(handler); !ok || admission != schematype.CallableAdmissionDirectFunction {
		t.Fatalf("xpcall handler admission = %d/%v, want schematype.CallableAdmissionDirectFunction", admission, ok)
	}
	callback, callbackOK := contract.Operations.SubedgeCallback(handler)
	...
	for _, kind := range []flowkind.OutcomeKind{flowkind.OutcomeThrow, flowkind.OutcomeYield} {
		route, adjustment, result, placement, offset, outcome, sibling, destination, ok := contract.Operations.SubedgeRouteAt(handler, kind)
...
func TestSubedgeRecurrenceManyLifecycleIsCanonicalUnderAuthoringPermutation(t *testing.T) {
	left := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{xpcallHandlerSelfRecurrence("xpcall-many-permutation", vocabulary.CallbackSyncOptionalMany, false, false)}})
	right := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{xpcallHandlerSelfRecurrence("xpcall-many-permutation", vocabulary.CallbackSyncOptionalMany, true, true)}})
	assertPublicContractEqual(t, left, right)
	if left.ContentID() != right.ContentID() {
...
func TestSubedgeReachabilityRequiresAnExplicitRoot(t *testing.T) {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{closed}}); err == nil {
	...
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{direct}})
	...
	edge := subedgeByRole(t, contract, op, 10)
	if ruleEntry, ok := contract.Operations.SubedgeRuleEntry(edge); !ok || !ruleEntry {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{missing}}); err == nil {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{nonempty}}); err == nil {
...
func TestSubedgeFreezeResolvesAuthoredEdgesToDenseIDs(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{protectedSubedgeOperation("freeze-edge", false, false, false)}})
	op, ok := contract.Operations.Lookup(vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"freeze-edge"}})
	if !ok || contract.Operations.SubedgeCount(op) == 0 {
		t.Fatalf("subedge owner = %d/%v count=%d", op, ok, contract.Operations.SubedgeCount(op))
	}
	edge, ok := contract.Operations.SubedgeAt(op, 0)
	if !ok || edge == 0 {
		t.Fatalf("SubedgeAt = %d/%v", edge, ok)
	}
	if owner, ok := contract.Operations.SubedgeOwner(edge); !ok || owner != op {
		t.Fatalf("SubedgeOwner = %d/%v, want %d/true", owner, ok, op)
	}
}

func TestSubedgeRowsPreserveCanonicalRoleAndFamily(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{protectedSubedgeOperation("row-edge", false, false, false)}})
	...
	if role, ok := contract.Operations.SubedgeRole(edge); !ok || role == 0 {
		t.Fatalf("SubedgeRole = %d/%v", role, ok)
	}
	if family, ok := contract.Operations.SubedgeFamily(edge); !ok || family != vocabulary.SubedgeFamilyCall {
		t.Fatalf("SubedgeFamily = %d/%v", family, ok)
	}
}

func TestSubedgeValidationRejectsInvalidFamilyAuthority(t *testing.T) {
	spec := declaration.Spec{Operations: []vocabulary.OperationSpec{protectedSubedgeOperation("invalid-edge", false, false, false)}}
...
func TestOperationSubedgeRelationProjectsOnlySealedCoordinates(t *testing.T) {
	...
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{operation}})
	...
	operand, selector, subedge, outcome, result, ok := contract.Operations.OperationSubedgeRelation(op)
	if !ok || operand != 1 || selector != 37 || subedge == 0 || result != 0 {
		t.Fatalf("relation = %d/%d/%d/%d/%d/%v", operand, selector, subedge, outcome, result, ok)
	}
	if got, ok := contract.Operations.OperationSubedgeRelationOutcome(op, flowkind.OutcomeNormal); !ok || got != outcome {
		t.Fatalf("normal outcome = %d/%v", got, ok)
	}
	if count := contract.Operations.OperationSubedgeRelationEffectAliasCount(op); count != 0 {
...
func TestSealSubedgeRelationRejectsForeignCoordinates(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{operation}}); err == nil {
