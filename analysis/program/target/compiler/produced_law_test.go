...
func TestBoundOperationsStayCanonicalPrefixWithProducedChildren(t *testing.T) {
	makeSpec := func(alphaRef vocabulary.SpecRef, input []vocabulary.OperationSpec) *contract.Contract {
		input[0].Outcomes[0].Produced = []vocabulary.ProducedSpec{{Result: 0, Operation: alphaRef}}
		return mustSeal(t, declaration.Spec{Operations: input})
	}
	...
	second := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{beta, child, func() vocabulary.OperationSpec {
		copy := alpha
...
func TestProducedAnchorsRejectAmbiguityAndCycles(t *testing.T) {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{
...
func TestProducedTypeValueCaptureRequiresExactFreshFunctionResult(t *testing.T) {
	for _, test := range []struct {
		name   string
		mutate func(*declaration.Spec)
		want   string
	}{
		{
			name: "missing",
			mutate: func(spec *declaration.Spec) {
...
func TestProducedTypeValueFreshFunctionCanonicalRoundTrip(t *testing.T) {
	spec := func(reverse bool) declaration.Spec {
		produced := []vocabulary.ProducedSpec{
	...
	if left.ContentID() != right.ContentID() {
		t.Fatal("Produced/FreshFunction authoring permutation changed contract.Contract identity")
	}
...
func typeValueCaptureSpec(kind vocabulary.CaptureKind, ordinal uint32) declaration.Spec {
	return declaration.Spec{Operations: []vocabulary.OperationSpec{
		{
			Bindings: []vocabulary.BindingSpec{{Namespace: vocabulary.BindingBuiltin, Member: []string{"factory"}}},
...
func TestFreshResultDistinguishesSameShapeCasesAndProducedAnchors(t *testing.T) {
	...
	makeSpec := func(outcomes []vocabulary.OutcomeSpec) declaration.Spec {
		return declaration.Spec{Operations: []vocabulary.OperationSpec{{
...
func TestResultAliasPermutationAndWideLookup(t *testing.T) {
	...
	seal := func(aliasRows []vocabulary.ResultAliasSpec) *contract.Contract {
		contract, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{{
