...
func testSeal(spec *declaration.Spec) (*contract.Contract, error) {
	if spec != nil {
...
func mustSeal(t *testing.T, spec declaration.Spec) *contract.Contract {
	t.Helper()
...
func assertContractShapeEqual(t *testing.T, left, right *contract.Contract) {
	t.Helper()
	assertPublicContractEqual(t, left, right)
}

// protocolTable binds the value returned by Contract.Protocols before calling
// its pointer-receiver query methods. It is test-only and keeps tests from
// depending on Contract's private storage.
func protocolTable(value *contract.Contract) *protocolvalue.Table {
	table := value.Protocols()
	return &table
}
