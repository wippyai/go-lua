...
func bootExactKey(t *testing.T, contract *contract.Contract, text string) vocabulary.ExactKey {
	t.Helper()
...
func bootExactKeyText(contract *contract.Contract, key vocabulary.ExactKey) string {
...
func TestBootLedgerSealsTypedCanonicalRows(t *testing.T) {
	...
	if aggregate, ok := contract.BootShapeAggregate(shape); !ok || aggregate != vocabulary.BootAggregateTable {
	...
	if class, value, root, key, ok := contract.InitialBinding("_G"); !ok || class != vocabulary.InitialBindingOrdinary || value != globalValue || root != global || bootExactKeyText(contract, key) != "_G" {
	...
	if !ok || mutability != vocabulary.InitialMutable {
	...
	if namespace, ok := contract.InitialValueDeniedNamespace(denied); !ok || namespace != vocabulary.BindingBuiltin || contract.InitialValueDeniedMemberCount(denied) != 1 {
	...
	if class, value, _, _, ok := contract.InitialBinding("load"); !ok || class != vocabulary.InitialBindingDenied || value != denied {
	...
	if kind, ok := contract.InitialValueKind(absent); !ok || kind != vocabulary.InitialValueAbsent {
		t.Fatalf("absent kind = %d/%v", kind, ok)
	}
	if class, value, _, _, ok := contract.InitialBinding("not_present"); !ok || class != vocabulary.InitialBindingOrdinary || value != absent {
...
func TestBootLedgerRejectsMalformedRows(t *testing.T) {
	cases := []struct {
		name string
		edit func(*declaration.Spec)
	}{
		{"duplicate root", func(spec *declaration.Spec) { spec.InitialRoots = append(spec.InitialRoots, spec.InitialRoots[0]) }},
		{"missing shape", func(spec *declaration.Spec) { spec.InitialRoots[0].Shape = vocabulary.BootShapeSpec{} }},
		{"non-root shape value", func(spec *declaration.Spec) {
			spec.InitialRoots[0].Shape.Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueString, String: "not a root"}
		}},
		{"cross-root shape value", func(spec *declaration.Spec) {
			spec.InitialRoots[0].Shape.Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueRoot, Root: "GlobalEnvRoot"}
		}},
		{"foreign entry root", func(spec *declaration.Spec) { spec.InitialEntries[0].Root = "foreign" }},
		{"duplicate entry", func(spec *declaration.Spec) {
			spec.InitialEntries = append(spec.InitialEntries, spec.InitialEntries[0])
		}},
		{"missing binding entry", func(spec *declaration.Spec) { spec.InitialBindings[0].Key = bootLiteralKey("missing") }},
		{"binding name differs from key", func(spec *declaration.Spec) { spec.InitialBindings[0].Name = "different" }},
		{"frozen global binding", func(spec *declaration.Spec) { spec.InitialEntries[1].Mutability = vocabulary.InitialFrozen }},
		{"global root is not table", func(spec *declaration.Spec) { spec.InitialRoots[1].Shape.Aggregate = vocabulary.BootAggregateMetatable }},
		{"multiple global roots", func(spec *declaration.Spec) {
			spec.InitialRoots = append(spec.InitialRoots, vocabulary.InitialRootSpec{Identity: "OtherGlobalRoot", Shape: vocabulary.BootShapeSpec{Aggregate: vocabulary.BootAggregateTable, Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueRoot, Root: "OtherGlobalRoot"}}})
			spec.InitialEntries = append(spec.InitialEntries, vocabulary.InitialEntrySpec{Root: "OtherGlobalRoot", Key: bootLiteralKey("other"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueOperation, Operation: vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"assert"}}}, Mutability: vocabulary.InitialMutable})
			spec.InitialBindings = append(spec.InitialBindings, vocabulary.InitialBindingSpec{Name: "other", Root: "OtherGlobalRoot", Key: bootLiteralKey("other")})
		}},
		{"missing absent", func(spec *declaration.Spec) {
			spec.InitialEntries[5].Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueNil}
		}},
		{"foreign operation", func(spec *declaration.Spec) {
			spec.InitialEntries[1].Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueOperation, Operation: vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"foreign"}}}
		}},
		{"invalid denied operation", func(spec *declaration.Spec) {
			spec.InitialEntries[2].Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueDeniedOperation}
		}},
		{"admitted denied operation", func(spec *declaration.Spec) {
			spec.InitialEntries[2].Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueDeniedOperation, Operation: vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"assert"}}}
		}},
		{"invalid typed union", func(spec *declaration.Spec) {
			spec.InitialEntries[3].Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueString, String: "version", Integer: 1}
		}},
	}
...
func TestBootLedgerRequiresGlobalSelfAlias(t *testing.T) {
	for _, test := range []struct {
		...
		{"missing _G binding", func(spec *declaration.Spec) { spec.InitialBindings = spec.InitialBindings[1:] }},
		{"frozen _G", func(spec *declaration.Spec) { spec.InitialEntries[0].Mutability = vocabulary.InitialFrozen }},
		{"absent _G", func(spec *declaration.Spec) {
			spec.InitialEntries[0].Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueAbsent}
		}},
		{"denied _G", func(spec *declaration.Spec) {
			spec.InitialEntries[0].Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueDeniedOperation, Operation: vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"load"}}}
		}},
		{"operation _G", func(spec *declaration.Spec) {
			spec.InitialEntries[0].Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueOperation, Operation: vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"assert"}}}
		}},
		{"other-root _G", func(spec *declaration.Spec) {
			spec.InitialEntries[0].Value = vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueRoot, Root: "StringMetatableRoot"}
		}},
	} {
		t.Run(test.name, func(t *testing.T) {
			spec := completeBootSpec("Lua 5.3", vocabulary.InitialMutable)
...
func TestBootShapeImmutableIsCanonicalAndIndependentOfEntryPolicy(t *testing.T) {
	mutableSpec := completeBootSpec("Lua 5.3", vocabulary.InitialMutable)
	frozenSpec := completeBootSpec("Lua 5.3", vocabulary.InitialMutable)
	...
	if !mutableEntryOK || !frozenEntryOK || mutablePolicy != vocabulary.InitialMutable || frozenPolicy != vocabulary.InitialMutable || mutableValue == 0 || frozenValue == 0 {
...
func TestInitialMetatableAttachmentIsTypedCanonicalBootstrapData(t *testing.T) {
	spec := completeBootSpec("Lua 5.3", vocabulary.InitialMutable)
	spec.InitialMetatables = []vocabulary.InitialMetatableAttachmentSpec{{Base: vocabulary.InitialValueString, Metatable: "StringMetatableRoot"}}
	contract := mustSeal(t, spec)
	...
	if !shapeOK || !aggregateOK || aggregate != vocabulary.BootAggregateMetatable {
	...
	without := completeBootSpec("Lua 5.3", vocabulary.InitialMutable)
	if contract.ContentID() == mustSeal(t, without).ContentID() {
		t.Fatal("initial metatable attachment changed no target identity")
	}

	for _, edit := range []func(*declaration.Spec){
		func(spec *declaration.Spec) { spec.InitialMetatables[0].Base = vocabulary.InitialValueInteger },
		func(spec *declaration.Spec) { spec.InitialMetatables[0].Metatable = "GlobalEnvRoot" },
		func(spec *declaration.Spec) { spec.InitialMetatables[0].Metatable = "missing" },
		func(spec *declaration.Spec) {
			spec.InitialMetatables = append(spec.InitialMetatables, spec.InitialMetatables[0])
		},
	} {
		malformed := completeBootSpec("Lua 5.3", vocabulary.InitialMutable)
		malformed.InitialMetatables = []vocabulary.InitialMetatableAttachmentSpec{{Base: vocabulary.InitialValueString, Metatable: "StringMetatableRoot"}}
		edit(&malformed)
...
func TestBootLedgerCanonicalOrderAndContentID(t *testing.T) {
	left := mustSeal(t, completeBootSpec("Lua 5.3", vocabulary.InitialMutable))
	rightSpec := completeBootSpec("Lua 5.3", vocabulary.InitialMutable)
	...
	for _, test := range []struct {
		...
		{"static value", completeBootSpec("Luau", vocabulary.InitialMutable)},
...
func bootMutabilityDelta() declaration.Spec {
	spec := completeBootSpec("Lua 5.3", vocabulary.InitialMutable)
	spec.InitialEntries[4].Mutability = vocabulary.InitialMutable // pi is not a global binding.
	return spec
}
...
func TestBootLedgerPreservesClosedTypedLiterals(t *testing.T) {
	...
	spec.InitialEntries = append(spec.InitialEntries,
		vocabulary.InitialEntrySpec{Root: "GlobalEnvRoot", Key: bootLiteralKey("false"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueBoolean, Boolean: false}, Mutability: vocabulary.InitialFrozen},
		vocabulary.InitialEntrySpec{Root: "GlobalEnvRoot", Key: bootLiteralKey("mininteger"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueInteger, Integer: math.MinInt64}, Mutability: vocabulary.InitialFrozen},
		vocabulary.InitialEntrySpec{Root: "GlobalEnvRoot", Key: bootLiteralKey("maxinteger"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueInteger, Integer: math.MaxInt64}, Mutability: vocabulary.InitialFrozen},
		vocabulary.InitialEntrySpec{Root: "GlobalEnvRoot", Key: bootLiteralKey("signed_zero"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueFloat, FloatBits: math.Float64bits(math.Copysign(0, -1))}, Mutability: vocabulary.InitialFrozen},
		vocabulary.InitialEntrySpec{Root: "GlobalEnvRoot", Key: bootLiteralKey("bytes"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueString, String: "\x00\x7f\xc2\xf4"}, Mutability: vocabulary.InitialFrozen},
		vocabulary.InitialEntrySpec{Root: "GlobalEnvRoot", Key: bootLiteralKey("nil"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueNil}, Mutability: vocabulary.InitialFrozen},
	)
	contract := mustSeal(t, spec)
	...
	for _, test := range []struct {
		...
		{"false", vocabulary.InitialValueBoolean}, {"mininteger", vocabulary.InitialValueInteger}, {"maxinteger", vocabulary.InitialValueInteger},
		{"signed_zero", vocabulary.InitialValueFloat}, {"bytes", vocabulary.InitialValueString}, {"nil", vocabulary.InitialValueNil},
	} {
...
func TestBootLedgerOptionalAsAWholeButNeverPartial(t *testing.T) {
	withoutBoot := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{bootOperation("plain")}})
	...
	partial := declaration.Spec{Operations: []vocabulary.OperationSpec{bootOperation("plain")}, InitialEntries: []vocabulary.InitialEntrySpec{{Root: "missing", Key: bootLiteralKey("key"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueNil}, Mutability: vocabulary.InitialFrozen}}}
...
func TestBootLedgerContentIDUsesNewSchemaVersion(t *testing.T) {
	withoutBoot := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{bootOperation("plain")}})
	withBoot := mustSeal(t, completeBootSpec("Lua 5.3", vocabulary.InitialMutable))
...
func TestBootLedgerScalesWithoutSemanticFallback(t *testing.T) {
	...
	for index := 0; index < 20000; index++ {
		...
		spec.InitialEntries = append(spec.InitialEntries, vocabulary.InitialEntrySpec{
			...
			Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueInteger, Integer: int64(index)}, Mutability: vocabulary.InitialMutable,
		})
	}
	contract := mustSeal(t, spec)
	global, _ := contract.InitialRootAt(0)
...
func completeBootSpec(version string, versionMutability vocabulary.InitialMutability) declaration.Spec {
	return declaration.Spec{
		Operations: []vocabulary.OperationSpec{bootOperation("assert")},
		InitialRoots: []vocabulary.InitialRootSpec{
			{Identity: "StringMetatableRoot", Shape: vocabulary.BootShapeSpec{Aggregate: vocabulary.BootAggregateMetatable, Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueRoot, Root: "StringMetatableRoot"}}},
		...
		InitialEntries: []vocabulary.InitialEntrySpec{
			{Root: "GlobalEnvRoot", Key: bootLiteralKey("_G"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueRoot, Root: "GlobalEnvRoot"}, Mutability: vocabulary.InitialMutable},
			{Root: "GlobalEnvRoot", Key: bootLiteralKey("assert"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueOperation, Operation: vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"assert"}}}, Mutability: vocabulary.InitialMutable},
			{Root: "GlobalEnvRoot", Key: bootLiteralKey("load"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueDeniedOperation, Operation: vocabulary.BindingSpec{Namespace: vocabulary.BindingBuiltin, Member: []string{"load"}}}, Mutability: vocabulary.InitialMutable},
			{Root: "GlobalEnvRoot", Key: bootLiteralKey("_VERSION"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueString, String: version}, Mutability: versionMutability},
			{Root: "GlobalEnvRoot", Key: bootLiteralKey("pi"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueFloat, FloatBits: math.Float64bits(math.Pi)}, Mutability: vocabulary.InitialFrozen},
			{Root: "GlobalEnvRoot", Key: bootLiteralKey("not_present"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueAbsent}, Mutability: vocabulary.InitialMutable},
			{Root: "StringMetatableRoot", Key: bootLiteralKey("__index"), Value: vocabulary.InitialValueSpec{Kind: vocabulary.InitialValueRoot, Root: "StringMetatableRoot"}, Mutability: vocabulary.InitialMutable},
		},
...
func bootOperation(name string) vocabulary.OperationSpec {
...
func bootSnapshot(contract *contract.Contract) string {
	var out string
...
func reverseInitialRoots(items []vocabulary.InitialRootSpec) {
	for left, right := 0, len(items)-1; left < right; left, right = left+1, right-1 {
		items[left], items[right] = items[right], items[left]
	}
}

func reverseInitialEntries(items []vocabulary.InitialEntrySpec) {
	for left, right := 0, len(items)-1; left < right; left, right = left+1, right-1 {
		items[left], items[right] = items[right], items[left]
	}
}

func reverseInitialBindings(items []vocabulary.InitialBindingSpec) {
	for left, right := 0, len(items)-1; left < right; left, right = left+1, right-1 {
		items[left], items[right] = items[right], items[left]
	}
}
