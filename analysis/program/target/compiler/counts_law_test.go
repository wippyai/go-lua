...
// The sealed census is a set of measurements of the contract.Contract's own columns. What
...
func censusFromReadSurface(t *testing.T, contract *contract.Contract) map[schema.EntryID]uint64 {
	t.Helper()
	...
	for index := 0; index < protocolTable(contract).ProtocolCount(); index++ {
		...
		add(ids.TargetProtocolState, protocolTable(contract).StateCount(protocol))
...
func assertCensusMatchesReadSurface(t *testing.T, contract *contract.Contract) {
	t.Helper()
...
func TestCountRowsPublishesExactlyTargetOwnerRelations(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{builtin("counted", testString, vocabulary.RowSpec{Tail: vocabulary.RowClosed})}})
	rows := contract.CountRows()
	if !denominator.GeneratedCountRowsCompleteForOwners(rows, denominator.RelationOwnerTarget) {
