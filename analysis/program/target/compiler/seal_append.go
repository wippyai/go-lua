...
func appendOperation(builder *operationvalue.QueryBuilder, core operationvalue.Core, op vocabulary.Operation, draft *operationDraft, keys exactkey.Table, callbackIDs []vocabulary.CallbackID) error {
	if op == 0 {
		return errors.New("target: unavailable operation")
	}
	if _, ok := core.OperationAt(int(op) - 1); !ok {
	...
	subedges, subedgeRelation, subedgeErr := querySubedgeInputs(draft, valuesHandle, keys, len(operationEffects))
	if subedgeErr != nil {
		return subedgeErr
	}
...
func appendOpaque(builder *operationvalue.QueryBuilder, core operationvalue.Core, opaque vocabulary.Operation) error {
	if opaque == 0 {
		return errors.New("target: unavailable opaque operation")
	}
	knownOpaque, opaqueOK := core.Opaque()
	...
	issuedOpaque := callbackIDForOpaque(core, opaque)
