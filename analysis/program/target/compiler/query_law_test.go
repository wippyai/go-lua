...
func TestInvocationQueriesExposeCallbackOwnedRelations(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{callbackOwnerOperation("invoke-query")}})
...
func TestContinuationQueriesReturnCanonicalSuspensionCoordinates(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{deltaSuspension(vocabulary.ReentryMany)}})
...
func TestProtocolQueriesResolveCanonicalStatesAndTransitions(t *testing.T) {
	...
	protocol, ok := protocolTable(contract).ProtocolAt(0)
	if !ok || protocolTable(contract).ProtocolCount() != 1 {
		t.Fatalf("protocol handle = %d/%v count=%d", protocol, ok, protocolTable(contract).ProtocolCount())
	}
	if got := protocolTable(contract).StateCount(protocol); got != 3 {
		t.Fatalf("StateCount = %d, want 3", got)
	}
	state, ok := protocolTable(contract).StateAt(protocol, 1)
	if !ok {
	...
	if got := protocolTable(contract).TransitionCount(protocol); got != 1 {
		t.Fatalf("TransitionCount = %d, want 1", got)
	}
	if _, _, _, _, ok := protocolTable(contract).TransitionAt(protocol, 0); !ok {
