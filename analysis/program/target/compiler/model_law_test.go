...
func TestBehaviorDescriptorProjectsOpaqueResultAndPredicateRows(t *testing.T) {
	...
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{spec}})
	...
	withoutContract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{without}})
...
func TestBehaviorDescriptorRejectsUnidentifiedRelation(t *testing.T) {
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{spec}}); err == nil {
		t.Fatal("behavior row without a relation identity was accepted")
	}
}

func TestModelHandlesRemainDenseAndZeroInvalid(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{builtin("model", testString, vocabulary.RowSpec{Tail: vocabulary.RowClosed})}})
...
func TestModelActivationCallbackReferencesResolveToOwnedIDs(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{callbackOwnerOperation("activation-model")}})
...
func publicationEffectSpec(publication *vocabulary.PublicationEffectSpec, duplicate bool) declaration.Spec {
...
func publicationEffectContract(t *testing.T, publication *vocabulary.PublicationEffectSpec, duplicate bool) (*contract.Contract, vocabulary.Operation) {
	t.Helper()
	contract, owner, _ := firstEffectIdentityContract(t, publicationEffectSpec(publication, duplicate), "publication-owner")
	return contract, owner
}

func distinctPublicationEffectSpec(reverse bool) declaration.Spec {
...
func TestPublicationEffectDescriptorOwnerLaw(t *testing.T) {
	...
	if firstDistinct.ContentID() != secondDistinct.ContentID() {
		t.Fatal("permuted distinct publication effects changed contract.Contract identity")
	}
...
func sealsExactProjection(t *testing.T, name string, source, result vocabulary.ValuesSpec) bool {
	t.Helper()
	_, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{exactProjectionOperation(name, source, result)}})
	return err == nil
}
...
func TestValuesSuffixCanonicalizationAndQueries(t *testing.T) {
	contract, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{{
...
func TestValuesSuffixOutcomePermutationHasOnePublicContract(t *testing.T) {
	makeContract := func(outcomes []vocabulary.OutcomeSpec) *contract.Contract {
		contract, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{{
...
func TestAdmissionFailureTailContributesToTheOneValuesVarClassTable(t *testing.T) {
	strings := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{admissionTailClassOperation("admission-tail-string", testString)}})
	...
	numbers := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{admissionTailClassOperation("admission-tail-string", testNumber)}})
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{conflict}}); err == nil {
		t.Fatal("admission failure tail class conflicted with owner input but sealed")
	}
}

func sameFrozenType(t *testing.T, contract *contract.Contract, frozen vocabulary.Type, want schematype.Type) bool {
	t.Helper()
