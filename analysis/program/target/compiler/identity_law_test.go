...
func effectIdentitySpec(operation vocabulary.OperationSpec) declaration.Spec {
	target := effectIdentityOperation("effect-target", vocabulary.RowSpec{Tail: vocabulary.RowClosed})
	return declaration.Spec{Operations: []vocabulary.OperationSpec{operation, target}}
}

func firstEffectIdentityContract(t *testing.T, spec declaration.Spec, name string) (*contract.Contract, vocabulary.Operation, vocabulary.CallbackID) {
	t.Helper()
...
func TestEffectIdentityTracksFormalAndOpenABI(t *testing.T) {
	formalSpec := func(constraint schematype.Type) declaration.Spec {
	...
	formalID := func(spec declaration.Spec) (identity.ContentID, bool) {
		contract := mustSeal(t, spec)
	...
	valuesSpec := func(tailType schematype.Type) declaration.Spec {
		return declaration.Spec{Operations: []vocabulary.OperationSpec{{
	...
	valuesID := func(spec declaration.Spec) (identity.ContentID, bool) {
		contract := mustSeal(t, spec)
	...
	rowSpec := func(count uint32) declaration.Spec {
		return declaration.Spec{Operations: []vocabulary.OperationSpec{{
...
func TestInputFormalIDPermutationReplayAndForeignRangeFence(t *testing.T) {
	left := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{endpointIdentityOperation("alpha-formal"), endpointIdentityOperation("beta-formal")}})
	right := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{endpointIdentityOperation("beta-formal"), endpointIdentityOperation("alpha-formal")}})
	...
	foreign := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{endpointIdentityOperation("foreign-formal"), endpointIdentityOperation("foreign-other")}})
...
func TestSemanticIdentityQueriesTrackOperationAndOutcomeOwners(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{contentIDOperation("semantic-id", []vocabulary.OutcomeSpec{{
...
func TestRelationIdentityQueriesRoundTripFormalAndOutcomeRows(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{contentIDOperation("relation-id", []vocabulary.OutcomeSpec{{
		Kind:   flowkind.OutcomeNormal,
		Values: vocabulary.ValuesSpec{Fixed: []schematype.Type{testString}, Tail: vocabulary.ValuesClosed},
	}})}})
...
func TestCallbackAndResumeContentIDsTrackDescriptorMutation(t *testing.T) {
	base := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{callbackResumeContentOperation("mutable", 0, 0)}})
	...
	callbackContract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{changedCallback}})
	resumeContract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{changedResume}})
