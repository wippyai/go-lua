...
func TestSealDeterministicAcrossOperationOrder(t *testing.T) {
	first := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{
	...
	second := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{
		builtin("beta", testString, vocabulary.RowSpec{Tail: vocabulary.RowClosed}),
	...
	for _, contract := range []*contract.Contract{first, second} {
...
func TestTypeFormalsAreAlphaInvariant(t *testing.T) {
	...
	leftContract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{genericBuiltin("identity", left)}})
	rightContract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{genericBuiltin("identity", right)}})
...
func TestDeepAuthoringTypeUsesNoGoRecursion(t *testing.T) {
	...
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{builtin("deep", deep, vocabulary.RowSpec{Tail: vocabulary.RowClosed})}})
...
func TestSealOwnsInputsAndConsumesFailedSpec(t *testing.T) {
	...
	spec := declaration.Spec{Operations: operations}
	...
	bad := declaration.Spec{Operations: []vocabulary.OperationSpec{{Bindings: []vocabulary.BindingSpec{{Namespace: vocabulary.BindingBuiltin, Member: []string{"bad"}}}, Input: vocabulary.ValuesSpec{Tail: vocabulary.ValuesUnknown}}}}
	...
	if _, err := testSeal(&bad); err == nil || err.Error() != "target/declaration: consumed spec" {
...
func TestTargetTypeValidationRejectsErasureAndUnresolvedAuthority(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{builtin("reject", test.value, vocabulary.RowSpec{Tail: vocabulary.RowClosed})}}); err == nil {
...
func TestCorrelatedOutcomesRejectDuplicates(t *testing.T) {
	...
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{base}})
	...
	if _, err := testSeal(&declaration.Spec{Operations: []vocabulary.OperationSpec{dup}}); err == nil {
...
func TestEffectMultiplicityAndArgumentValidation(t *testing.T) {
	...
	sealed := mustSeal(t, declaration.Spec{Operations: withEffects})
	...
	if _, err := testSeal(&declaration.Spec{Operations: bad}); err == nil {
...
func TestSynthesizedOpaqueAndHotQueriesAllocateNothing(t *testing.T) {
	...
	bound := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{builtin("hot", testString, vocabulary.RowSpec{Tail: vocabulary.RowClosed})}})
...
func TestSealConsumesAuthoringSpecOnEveryFirstAttempt(t *testing.T) {
	spec := declaration.Spec{Operations: []vocabulary.OperationSpec{builtin("seal-once", testString, vocabulary.RowSpec{Tail: vocabulary.RowClosed})}}
	if _, err := testSeal(&spec); err != nil {
		t.Fatal(err)
	}
	if _, err := testSeal(&spec); err == nil {
...
func TestSealOperationRequiresCompleteOutcomeAuthority(t *testing.T) {
	valid := declaration.Spec{Operations: []vocabulary.OperationSpec{builtin("complete", testString, vocabulary.RowSpec{Tail: vocabulary.RowClosed})}}
	if _, err := testSeal(&valid); err != nil {
		t.Fatal(err)
	}
	invalid := declaration.Spec{Operations: []vocabulary.OperationSpec{{
...
func TestSealAppendPublishesDenseOperationRelations(t *testing.T) {
	contract := mustSeal(t, declaration.Spec{Operations: []vocabulary.OperationSpec{builtin("append-relations", testString, vocabulary.RowSpec{Tail: vocabulary.RowClosed})}})
...
func rowBoundarySpec(callbackRowTail vocabulary.RowTail, mode vocabulary.CallbackReleaseMode) declaration.Spec {
