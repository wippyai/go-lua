...
type Publisher struct {
	...
	expected    map[Token]struct{}
	accepted    map[Token]Publication
	sealed      bool
}

// NewPublisher starts a publication cut over one issued immutable
// ProgramSchema.
func NewPublisher(schema ProgramSchema) (*Publisher, error) {
	definitions, expected, err := schemaDefinitions(schema)
	if err != nil {
		return nil, err
	}
	return &Publisher{
		...
		expected:    expected,
		accepted:    make(map[Token]Publication, len(definitions)),
	}, nil
}
...
func (s *Publisher) Accept(value Publication) error {
	if s == nil || s.schema == nil || len(s.definitions) == 0 || s.accepted == nil {
	...
	if _, expected := s.expected[value.definition.token]; !expected {
...
func (s *Publisher) Seal() (Publications, error) {
	...
	if len(s.accepted) != len(s.definitions) {
		return Publications{}, ErrMissingPublication
	}
	measures := make([]Measure, 0, len(s.definitions))
	...
	s.sealed = true
	return Publications{schemaDigest: s.schema.SchemaDigest(), measures: measures}, nil
}
...
// SchemaDigest reports the identity of the exact immutable denominator used
// for publication. The schema object itself never crosses the publication
// boundary or becomes a second authority.
func (p Publications) SchemaDigest() identity.ContentID { return p.schemaDigest }
...
func (p Publications) At(index int) (Measure, bool) {
	if !p.schemaDigest.Available() || index < 0 || index >= len(p.measures) {
...
func (p Publications) Clone() Publications {
	...
	return Publications{schemaDigest: p.schemaDigest, measures: append([]Measure(nil), p.measures...)}
}
