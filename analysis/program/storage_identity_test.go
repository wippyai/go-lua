...
func TestStorageIdentityQueriesKeepAbsentRowsOutOfTheSnapshot(t *testing.T) {
	...
	if id, ok := published.Flow().StorageAssignmentID(keyspace.MakeTerm(keyspace.FamilyAssign, 1)); ok || id.Available() {
		t.Fatalf("Flow.StorageAssignmentID(assign 1) = %x/%v; want absent row", id, ok)
	}
