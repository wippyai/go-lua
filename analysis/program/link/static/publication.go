...
func (v Cold) Publications(schema semanticsource.ProgramSchema) ([]semanticsource.Publication, bool) {
	if !v.live() {
		return nil, false
	}
	if schema == nil {
	...
	definition, ok := schema.Definition(semanticsource.OriginLinkStatic, 0)
	if !ok {
		return nil, false
	}
