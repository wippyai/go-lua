...
func TestSourcePublicationsSealCompleteProject(t *testing.T) {
	...
	if got.Count() != schema.Count() || got.SchemaDigest() != schema.SchemaDigest() {
		t.Fatalf("publication count/digest = %d/%x, want exact %d/%x", got.Count(), got.SchemaDigest(), schema.Count(), schema.SchemaDigest())
	}
	...
	if len(seen) != len(want) || len(seen) != schema.Count() {
		t.Fatalf("sealed vocabulary = %d, want exact schema denominator %d", len(seen), schema.Count())
	}
}
...
func TestSourcePublicationsCountDuplicateProgramMounts(t *testing.T) {
	...
	fragment := programSemanticSourcePublications(t, p)
...
func TestSourcePublicationsRetainZeroRowsAndReplay(t *testing.T) {
	...
	for _, key := range []struct {
		...
		definition, ok := schema.Definition(key.origin, key.facet)
		if !ok {
...
func TestSemanticSourceAssemblyRejectsForeignIncompleteAndOverflowFragments(t *testing.T) {
	...
	p := source(t, `return 1`)
	programRows := programSemanticSourcePublications(t, p)
...
func semanticSourceSchema(t testing.TB) *relations.Schema {
	t.Helper()
	schema, err := relations.CanonicalSchema()
	if err != nil {
		t.Fatalf("relation schema: %v", err)
...
func programSemanticSourcePublications(t testing.TB, p *program.Program) []semanticsource.Publication {
	t.Helper()
	rows := p.SemanticSourcePublications(semanticSourceSchema(t))
	if len(rows) != 57 {
		t.Fatalf("Program semantic-source rows = %d, want 57", len(rows))
	}
	return rows
}
