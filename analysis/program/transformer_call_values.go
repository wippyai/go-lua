...
func (values CallValues) ContextID() identity.ContentID {
	if !values.Available() {
		return identity.ContentID{}
	}
	valuesID := values.receipt.ID()
	return transformerSemanticID("program/transformer/call-values", func(writer *framing.Writer) bool {
...
func (argument CallArgument) ContextID() identity.ContentID {
	if !argument.Available() {
		return identity.ContentID{}
	}
	valuesID := argument.values.ContextID()
	memberID := argument.member.ID()
	return transformerSemanticID("program/transformer/call-argument", func(writer *framing.Writer) bool {
