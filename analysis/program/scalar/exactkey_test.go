...
func TestNormalizeSemanticCases(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			got, ok := scalar.Normalize(test.input)
			if ok != test.ok || got != test.want {
				t.Fatalf("scalar.Normalize(%#v) = %#v/%v, want %#v/%v", test.input, got, ok, test.want, test.ok)
			}
		})
	}
}
...
func TestCompareSemanticOrderAndQuotient(t *testing.T) {
	...
	for left := range ordered {
		for right := range ordered {
			got, ok := scalar.Compare(ordered[left], ordered[right])
			if !ok {
				t.Fatalf("scalar.Compare rejected canonical pair %#v/%#v", ordered[left], ordered[right])
			}
			want := 0
			if left < right {
				want = -1
			...
			if got != want {
				t.Fatalf("scalar.Compare(%#v, %#v) = %d, want %d", ordered[left], ordered[right], got, want)
			}
			reverse, reverseOK := scalar.Compare(ordered[right], ordered[left])
			if !reverseOK || reverse != -got {
				t.Fatalf("scalar.Compare antisymmetry %#v/%#v = %d/%d", ordered[left], ordered[right], got, reverse)
			}
		}
	}
	...
	for _, test := range quotientCases {
		t.Run(test.name, func(t *testing.T) {
			got, ok := scalar.Compare(test.left, test.right)
			if !ok || got != 0 {
				t.Fatalf("scalar.Compare(%#v, %#v) = %d/%v, want 0/true", test.left, test.right, got, ok)
			}
		})
	}
	...
	for _, value := range invalid {
		if got, ok := scalar.Compare(value, ordered[0]); ok || got != 0 {
			t.Fatalf("scalar.Compare(%#v, valid) = %d/%v, want no order", value, got, ok)
		}
		if got, ok := scalar.Compare(ordered[0], value); ok || got != 0 {
			t.Fatalf("scalar.Compare(valid, %#v) = %d/%v, want no order", value, got, ok)
		}
	}
	...
	for round := range ordered {
		...
		sort.Slice(rotated, func(left, right int) bool {
			order, ok := scalar.Compare(rotated[left], rotated[right])
			return ok && order < 0
		})
...
func TestExactKeyPublicQueriesAllocateZero(t *testing.T) {
	...
	allocations := testing.AllocsPerRun(1000, func() {
		normalized, normalizeOK = scalar.Normalize(left)
		order, compareOK = scalar.Compare(left, right)
	})
	if allocations != 0 {
		t.Fatalf("scalar.Normalize/scalar.Compare allocated %v times", allocations)
	}
