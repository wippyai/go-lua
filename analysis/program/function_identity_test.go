...
func TestFunctionIdentityRequiresAnOwnedFlowBoundary(t *testing.T) {
	...
	if id, cell, ok := published.Flow().FunctionVarargIDs(boundary); ok || id.Available() || cell.Available() {
		t.Fatalf("Flow.FunctionVarargIDs(zero) = %x/%x/%v; want unavailable", id, cell, ok)
	}
