...
func TestRunCheckRejectsStaleCatalogFiles(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mutate: func(t *testing.T, files paths) {
				if err := os.WriteFile(files.semantic, []byte("stale"), 0o644); err != nil {
					t.Fatal(err)
				}
			},
		},
...
func checkFixture(t *testing.T) paths {
	...
	files := paths{
		...
		semantic:  filepath.Join(root, "semanticsource", "generated.go"),
	...
	for destination, source := range map[string]string{
		...
		files.semantic:  filepath.Join("..", "..", "..", "program", "semanticsource", "generated.go"),
