...
import (
	...
	"github.com/wippyai/go-lua/analysis/program/relations/internal/scc"
)
...
// emits both generated projections at explicit CLI paths. When check is true it
// only verifies freshness; when writeHistory is true it appends new compatible
// history records.
...
// checkedFiles proves the checked-in history and both generated projections
// are exactly the bytes the current catalog produces.
func checkedFiles(files paths, artifact artifact) error {
	...
	if err := fresh(files.semantic, artifact.semantic); err != nil {
		return err
	}
...
func emitSemanticSource(entries []entry) ([]byte, error) {
	...
	var out bytes.Buffer
	out.WriteString("// Code generated from program/relations/catalog.schema; DO NOT EDIT.\n\npackage semanticsource\n\nconst (\n")
	...
	seenOrigins := make(map[string]bool, len(origins))
	for _, item := range entries {
		if seenOrigins[item.origin] {
			continue
		}
		seenOrigins[item.origin] = true
		fmt.Fprintf(&out, "\tcase Origin%s:\n\t\treturn Revision(%d), true\n", item.origin, item.revision)
	}
