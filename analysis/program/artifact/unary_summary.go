...
func (row UnarySummaryRow) ID() identity.ContentID {
	if !row.Available() {
...
func (row UnarySummaryRow) OccurrenceID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
...
func (row UnarySummaryRow) BodyPathID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.body
...
func (row UnarySummaryRow) OutputPointID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.point
...
func unarySummaryID(occurrence, body, point identity.ContentID, op flowkind.UnaryOp, operand, result NumericRepresentation) identity.ContentID {
