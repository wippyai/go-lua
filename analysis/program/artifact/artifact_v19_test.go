...
func TestV19RoundTripRebuildsDerivedOwners(t *testing.T) {
	...
	original := mustLower(t, "v19-roundtrip.lua", `
local function f(x)
  return x
end
return {f = f}, f(1)
`)
	...
	if !bytes.Equal(want, again) {
		t.Fatal("v19 roundtrip changed canonical bytes")
	}
}

func TestV19RejectsV18VersionAndTrailingBytes(t *testing.T) {
	contract := mustProfile(t)
	encoded, err := artifact.Encode(mustLower(t, "v19-version.lua", `return 1`), contract, artifact.Metadata{})
	if err != nil {
		t.Fatal(err)
	}
	version := cloneEventPayload(encoded, 1, func(payload []byte) { payload[0] = 18 })
	if _, _, err := artifact.Decode(version, contract, nil); !errors.Is(err, artifact.ErrNoncanonical) {
		t.Fatalf("v18 artifact version error = %v, want noncanonical", err)
	}
...
func TestV19RejectsRelationsSchemaDigestMutation(t *testing.T) {
	contract := mustProfile(t)
	encoded, err := artifact.Encode(mustLower(t, "v19-schema-digest.lua", `return 1`), contract, artifact.Metadata{})
	if err != nil {
		t.Fatal(err)
	}
	// The envelope order is domain, version, envelope, target, Program,
	// relations schema, entry, provenance, and dependency count.
	mutated := cloneEventPayload(encoded, 5, func(payload []byte) { payload[0] ^= 1 })
	if _, _, err := artifact.Decode(mutated, contract, nil); !errors.Is(err, artifact.ErrSchemaMismatch) {
		t.Fatalf("mutated relations schema digest error = %v, want schema mismatch", err)
	}
}

func TestV19RejectsTargetClaimedIDAndEntryMutation(t *testing.T) {
	contract := mustProfile(t)
	encoded, err := artifact.Encode(mustLower(t, "v19-mutation.lua", `return 1`), contract, artifact.Metadata{})
	if err != nil {
		t.Fatal(err)
	}
	...
	entryMutation := cloneEventPayload(encoded, 6, func(payload []byte) { payload[0] ^= 1 })
...
func TestV19CanonicalizesAndValidatesDependencies(t *testing.T) {
	contract := mustProfile(t)
	p := mustLower(t, "v19-dependencies.lua", `return 1`)
	...
	noncanonical := cloneEventPayload(first, 10, func(payload []byte) { payload[0] = 'z' })
	noncanonical = cloneEventPayload(noncanonical, 13, func(payload []byte) { payload[0] = 'a' })
...
func TestV19RequiresExactExpectedDependencyManifest(t *testing.T) {
	contract := mustProfile(t)
	p := mustLower(t, "v19-manifest.lua", `return 1`)
...
func TestV19CanonicalDependencyRowsPrecedeManifestCountMismatch(t *testing.T) {
	contract := mustProfile(t)
	p := mustLower(t, "v19-manifest-order.lua", `return 1`)
	...
	noncanonical := cloneEventPayload(encoded, 10, func(payload []byte) { payload[0] = 'z' })
	noncanonical = cloneEventPayload(noncanonical, 13, func(payload []byte) { payload[0] = 'a' })
...
func TestV19RejectsOversizeProvenanceBeforeWriting(t *testing.T) {
	contract := mustProfile(t)
	p := mustLower(t, "v19-provenance-limit.lua", `return 1`)
...
func cloneEventPayload(data []byte, index int, mutate func([]byte)) []byte {
	...
	// The helper is called only with indexes from a valid v19 envelope. Keep
