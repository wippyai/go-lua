...
func (compiler *compiler) valueSourceAt(code uint64, index int) (valueSourceCompileRow, bool) {
	...
	canonicalID, canonicalSpanID, canonicalTerm, canonicalOK := artifactcompiler.ValueSourceIdentityAt(input, programID, keyspace.TermFamily(term), index)
	...
	if code < 6 {
		family, literal, literalOK := artifactcompiler.SourceLiteral(input, term)
		if !literalOK {
			return valueSourceCompileRow{}, false
		}
