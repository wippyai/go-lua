...
func (compiler *compiler) admitStructuralMemberObservations(
	...
	established := make(map[keyspace.Key]struct{}, row.FieldCount())
	openTail := false
	for index := 0; index < row.FieldCount(); index++ {
		field, fieldOK := row.FieldAt(index)
		finalOpen, finalOpenOK := field.FinalOpen()
		width, widthOK := field.Width()
		normalized, normalizedOK := field.Normalized()
		kind, kindOK := field.Kind()
		values, valuesOK := field.Values()
		memberTerm, memberTermOK := field.MemberTerm()
		if !fieldOK || !finalOpenOK || !widthOK || !kindOK || !valuesOK {
			return false
		}
		if finalOpen {
			openTail = true
		...
		if normalizedOK {
			established[normalized] = struct{}{}
		}
		memberDeclared, memberDeclaredOK := compiler.declaredStructuralMember(declared, kind, normalized, normalizedOK)
		member, memberRowOK := compiler.valueMemberAt(values, 0)
		if !memberDeclaredOK || !memberTermOK || !memberRowOK || !member.Available() {
			continue
		}
...
//
// A field is required when its absence is a value the declaration does not
// admit. In Lua an absent key reads nil, so a field whose declared type admits
// nil is satisfied by its own absence exactly as an optional field is, and both
// are held back here rather than reported as missing.
func (compiler *compiler) admitAbsentMemberObservations(
	...
	for position := 0; position < declaredFieldCount; position++ {
		...
		if !fieldTermOK || !shapeOK || optional || compiler.declaredTypeAdmitsNil(fieldType) {
			continue
		}
		...
		if !compiler.admitConformanceObservation(programdiagnostic.DiagnosticObservationSiteMemberAbsent, owner, measuredValue, fieldID, table, uint32(position)) {
			return false
		}
	}
	return true
...
// declaredTypeAdmitsNil reports whether one declared type admits nil. An
// optional admits it by construction, a union admits it when one of its
// members does, and the nil primitive is it; the wrappers that leave a
// declaration's inhabitants unchanged - an alias and a resolved reference - are
// followed through. A declaration graph that reaches itself is bounded by the
// visited set and answers no rather than looping.
func (compiler *compiler) declaredTypeAdmitsNil(term keyspace.Term) bool {
	visited := make(map[keyspace.Term]struct{}, 4)
	return compiler.declaredTermAdmitsNil(term, visited)
}
...
func (compiler *compiler) declaredTermAdmitsNil(term keyspace.Term, visited map[keyspace.Term]struct{}) bool {
	...
	for term != 0 {
		...
		visited[term] = struct{}{}
		if _, isOptional := view.Types().Optionals().Get(term); isOptional {
			return true
		}
		if kind, isPrimitive := view.Types().Primitives().Get(term); isPrimitive {
			return kind == statictypes.PrimitiveNil
		}
		if count, isUnion := view.Types().Unions().MemberCount(term); isUnion {
			for index := 0; index < count; index++ {
				member, memberOK := view.Types().Unions().MemberAt(term, index)
				if memberOK && compiler.declaredTermAdmitsNil(member, visited) {
					return true
				}
			}
			return false
