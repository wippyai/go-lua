...
func Build(input Input) (*Bundle, Fault) {
	...
	for _, row := range input.Values {
		if !row.Available() {
			return nil, failure(ReasonReturnValueReference, -1, -1)
		}
	...
	for bodyIndex := 0; bodyIndex < bodyCount; bodyIndex++ {
		...
		if !ok || !body.Available() {
			return nil, failure(ReasonUnavailable, bodyIndex, -1)
		}
		if !input.Program.OwnsBody(body) {
			return nil, failure(ReasonForeign, bodyIndex, -1)
		}
		bodyID := body.PathID()
		if !bodyID.Available() {
			return nil, failure(ReasonIdentity, bodyIndex, -1)
		}
		if _, duplicate := seenBodies[bodyID]; duplicate {
			return nil, failure(ReasonDuplicate, bodyIndex, -1)
		}
		...
		if !fitsUint32(len(bodyEntries)) || !fitsUint32(len(entryPoints)) {
			return nil, failure(ReasonRange, bodyIndex, -1)
		}
		...
		for pointIndex, point := range entryPoints {
			...
			if !rowOK {
				return nil, failure(ReasonUnavailable, bodyIndex, pointIndex)
			}
		...
		for rootIndex := 0; rootIndex < rootCount; rootIndex++ {
			...
			if !rootOK {
				return nil, failure(ReasonUnavailable, bodyIndex, rootIndex)
			}
			...
			if _, duplicate := seenRoots[row.ID()]; duplicate {
				return nil, failure(ReasonDuplicate, bodyIndex, rootIndex)
			}
		...
		for outcomeIndex := 0; outcomeIndex < outcomeCount; outcomeIndex++ {
			...
			if !outcomeOK || exit.Outcome == 0 {
				return nil, failure(ReasonOutcomeUnavailable, bodyIndex, outcomeIndex)
			}
			...
			if !ownerOK || !ownerBoundary.Available() || !boundaries.OwnsBody(ownerBoundary) || !ownerBodyOK || ownerBody != bodyTerm || !ownerExitOK || ownerOrdinal != outcomeIndex || ownerExit.Outcome != exit.Outcome || !metadataOK || metadataBody != bodyTerm || metadataKind != exit.Kind || metadataTarget != exit.Target {
				return nil, failure(ReasonOutcomeForeign, bodyIndex, outcomeIndex)
			}
			...
			if !outcomePathOK || !outcomeID.Available() {
				return nil, failure(ReasonOutcomeIdentity, bodyIndex, outcomeIndex)
			}
			if _, duplicate := seenOutcomes[outcomeID]; duplicate {
				return nil, failure(ReasonOutcomeDuplicate, bodyIndex, outcomeIndex)
			}
			kind, converted := outcomeKind(exit.Kind)
			if !converted {
				return nil, failure(ReasonOutcomeKind, bodyIndex, outcomeIndex)
			}
			...
			case programschema.OutcomeBreak:
				if keyspace.TermFamily(exit.Target) != keyspace.FamilyLoop {
					return nil, failure(ReasonOutcomeTarget, bodyIndex, outcomeIndex)
				}
				var targetOK bool
			...
			if nextTerm, propagated := outcomesView.Propagation(exit.Outcome); propagated {
				...
				if nextSiteOK && (!nextSite.Available() || !sites.Owns(nextSite)) {
					return nil, failure(ReasonOutcomePropagation, bodyIndex, outcomeIndex)
				}
			...
			if site, siteOK := sites.ForTerm(exit.Outcome); siteOK {
				if !site.Available() || !sites.Owns(site) {
					return nil, failure(ReasonOutcomeAttachment, bodyIndex, outcomeIndex)
				}
				points = pointIDs(input.PointIDsBySite, site)
			}
			if !fitsUint32(len(outcomePoints)) || !fitsUint32(len(points)) {
				return nil, failure(ReasonOutcomeRange, bodyIndex, outcomeIndex)
			}
			...
			for pointIndex, point := range points {
				...
				if !childOK {
					return nil, failure(ReasonOutcomeAttachment, bodyIndex, pointIndex)
				}
			...
			if hasReturn && outcomeID == returnedID {
				if matchedReturn || kind != programschema.OutcomeReturn {
					return nil, failure(ReasonOutcomeReturn, bodyIndex, outcomeIndex)
				}
				matchedReturn = true
				for returnIndex := 0; returnIndex < returnedValueCount; returnIndex++ {
					...
					if !projected || !valueOK || !valueSite.Available() || !sites.Owns(valueSite) || !termOK || valueTerm != projectedTerm || !rowOK || !valuesID.Available() {
						return nil, failure(ReasonReturnValueUnavailable, bodyIndex, returnIndex)
					}
					if _, exists := valueIDs[valuesID]; !exists {
						return nil, failure(ReasonReturnValueReference, bodyIndex, returnIndex)
					}
	...
	for index, row := range outcomes {
		...
		if !exists || nextIndex == index {
			return nil, failure(ReasonOutcomeReference, index, -1)
		}
		...
		if next.Kind() != row.Kind() || nextHasTarget != hasTarget || nextTarget != target {
			return nil, failure(ReasonOutcomePropagation, index, -1)
		}
	}
...
func buildFunctionBoundaries(input Input, bodies []programschema.Body, outcomes []programschema.Outcome) ([]programschema.FunctionBoundary, []programschema.FunctionFormal, []programschema.FunctionVararg, []programschema.FunctionCapture, map[keyspace.Term]identity.ContentID, map[identity.ContentID]programschema.FunctionBoundary, Fault) {
	if len(bodies) == 0 || len(outcomes) == 0 {
		return nil, nil, nil, nil, nil, nil, failure(ReasonUnavailable, -1, -1)
	}
	flowView := input.Program.Flow()
	boundaries := flowView.FunctionBoundaries()
	...
	for bodyIndex := 0; bodyIndex < input.Program.BodyCount(); bodyIndex++ {
		...
		if !bodyOK || !input.Program.OwnsBody(body) || bodyIndex >= len(bodies) {
			return nil, nil, nil, nil, nil, nil, failure(ReasonUnavailable, bodyIndex, -1)
		}
		...
		if _, duplicate := functionBoundaryByBody[copiedBody.ID()]; duplicate {
			return nil, nil, nil, nil, nil, nil, failure(ReasonDuplicate, bodyIndex, -1)
		}
		if !fitsUint32(len(functionFormals)) || !fitsUint32(len(functionCaptures)) || !fitsUint32(len(functionVarargs)) {
			return nil, nil, nil, nil, nil, nil, failure(ReasonRange, bodyIndex, -1)
		}
		...
		for position := 0; position < function.FormalCount(); position++ {
			...
			if !formalOK || !termOK || !storageOK || !storageID.Available() || uint64(position) > uint64(^uint32(0)) {
				return nil, nil, nil, nil, nil, nil, failure(ReasonUnavailable, bodyIndex, position)
			}
	...
	return functionBoundaries, functionFormals, functionVarargs, functionCaptures, functionIDs, functionBoundaryByBody, Fault{}
}
