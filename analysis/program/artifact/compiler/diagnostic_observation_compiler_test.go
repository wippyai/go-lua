...
func diagnosticProgram(t *testing.T, artifact *programartifact.Artifact) programdiagnostic.View {
	t.Helper()
	...
	if !catalogOK || !stateOK || !viewOK {
		t.Fatal("diagnostic view unavailable")
	}
	return view
}
...
`)
	...
	for index := 0; index < count; index++ {
		...
		if row.Site() != programdiagnostic.DiagnosticObservationSiteAssignment {
...
func diagnosticPath(program programdiagnostic.View, index int) ([]string, bool) {
