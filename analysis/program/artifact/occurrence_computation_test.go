...
func TestArtifactOccurrenceComputationCatalogRetainsBinaryArithmetic(t *testing.T) {
	published, err := lower.Lower(lower.Source{Name: "occurrence-computation.lua", Text: []byte(`
...
`)})
	...
	if artifact.OccurrenceKindCount(programartifact.OccurrenceBinaryArithmetic) == 0 {
