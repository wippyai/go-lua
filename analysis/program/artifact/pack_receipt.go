...
type PackBindRow struct {
	id, body, values identity.ContentID
...
func (row PackBindRow) ID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.id
}
func (row PackBindRow) BodyID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.body
}
func (row PackBindRow) ValuesID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.values
...
func (row PackBindRow) CellAt(index int) (identity.ContentID, bool) {
...
type PackFormalCell struct{ formal, cell, storage identity.ContentID }

func (row PackFormalCell) Available() bool {
	return row.formal.Available() && row.cell.Available() && row.storage.Available()
}
func (row PackFormalCell) FormalID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.formal
}
func (row PackFormalCell) CellID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.cell
...
func (row PackFormalCell) StorageCellID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.storage
...
func (row PackBodyReceiptRow) ID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.id
}
func (row PackBodyReceiptRow) ContextID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.context
...
type PackCallReceiptRow struct {
	id, body, formal, values, types, callee, actuals, receiver, tail identity.ContentID
	form                                                             flow.CallForm
	arguments                                                        []identity.ContentID
...
func (row PackCallReceiptRow) ID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.id
}
func (row PackCallReceiptRow) BodyID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.body
}
func (row PackCallReceiptRow) FormalID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.formal
}
func (row PackCallReceiptRow) ValuesID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.values
}
func (row PackCallReceiptRow) TypeArgumentsID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.types
}
func (row PackCallReceiptRow) CalleeID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.callee
}
func (row PackCallReceiptRow) ActualsID() identity.ContentID {
	if !row.Available() {
		return identity.ContentID{}
	}
	return row.actuals
...
func (row PackCallReceiptRow) ReceiverID() (identity.ContentID, bool) {
	return row.receiver, row.Available() && row.hasReceiver
}
func (row PackCallReceiptRow) TailID() (identity.ContentID, bool) {
...
func (row PackCallReceiptRow) ArgumentAt(index int) (identity.ContentID, bool) {
...
func (row PackCallReceiptRow) TypeArgumentAt(index int) (identity.ContentID, bool) {
...
func (receipt PackReceipt) validAgainst(artifact *Artifact) bool {
	...
	bodies := make(map[identity.ContentID]identity.ContentID, len(artifact.bodies))
	...
	functions := make(map[identity.ContentID]FunctionBoundaryRow, len(artifact.functionBoundaries))
	...
	values := make(map[identity.ContentID]struct{}, len(artifact.values))
	...
	occurrence := func(kind OccurrenceKind, id identity.ContentID) (OccurrenceRow, bool) {
	...
	seenBinds := make(map[identity.ContentID]struct{}, len(receipt.binds))
	for _, row := range receipt.binds {
	...
	seenBodies := make(map[identity.ContentID]struct{}, len(receipt.bodies))
	for _, row := range receipt.bodies {
	...
	seenCalls := make(map[identity.ContentID]struct{}, len(receipt.calls))
	for _, row := range receipt.calls {
		if !row.Available() || !bodies[row.body].Available() {
			return false
		}
