...
func newStructuralRoute(input TransformerInput, finalRoute flow.FinalRoute) (StructuralRoute, bool) {
	...
	result.context = transformerRoleID("program/transformer/structural-route", input.programID, func(writer *framing.Writer) bool {
...
func (route StructuralRoute) Available() bool {
	...
	expectedContext := transformerRoleID("program/transformer/structural-route", route.input.programID, func(writer *framing.Writer) bool {
...
func (route StructuralRoute) ContextID() identity.ContentID {
	if !route.Available() {
...
func (route StructuralRoute) FromPath() (identity.ContentID, bool) {
	if !route.Available() {
...
func (route StructuralRoute) ToPath() (identity.ContentID, bool) {
	if !route.Available() {
		return identity.ContentID{}, false
	}
...
// DecisionTerm returns the exact authored decision coordinate already sealed
// into this route identity. It is a construction-time join for artifact
// column builders; callers must consume it immediately through the canonical
// Flow view and must not retain the term as a semantic identity.
func (route StructuralRoute) DecisionTerm() (keyspace.Term, bool) {
	if !route.Available() || !route.identity.Available() || route.identity.Decision() == 0 {
		return 0, false
	}
	return route.identity.Decision(), true
}
...
func (guard RouteGuard) DecisionPathID() (identity.ContentID, bool) {
...
func (guard RouteGuard) ConditionValueSpanID() (identity.ContentID, bool) {
...
func (guard RouteGuard) ContextID() identity.ContentID {
	if !guard.Available() {
		return identity.ContentID{}
	}
...
func (route StructuralRoute) MuPathID() (identity.ContentID, bool) {
	if !route.Available() {
		return identity.ContentID{}, false
	}
...
func (route StructuralRoute) RouteDigest() (identity.ContentID, bool) {
	return route.final.ID()
}

func (route StructuralRoute) RouteID() (identity.ContentID, bool) {
	return route.final.ID()
}
...
func (route StructuralRoute) ResetDigest() (identity.ContentID, bool) {
	proof, proofOK := route.Recurrence()
	if !route.Available() || !proofOK {
		return identity.ContentID{}, false
	}
...
func (route StructuralRoute) ResetPathAt(index int) (identity.ContentID, bool) {
	if !route.Available() || index < 0 {
		return identity.ContentID{}, false
	}
