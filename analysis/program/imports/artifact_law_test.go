...
const (
	moduleArtifactTestDomain   = "program/imports-test-artifact"
...
func TestArtifactSectionMatchesCanonicalAuthoredRowsAndExpires(t *testing.T) {
	...
	var artifactWriter, authoredWriter framing.Writer
...
func TestArtifactSectionRejectsMalformedDenseRows(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			input, err := readModuleArtifactPayload(t, func(writer *framing.Writer) {
...
func TestArtifactSectionHostileCountDoesNotReserveRows(t *testing.T) {
	...
	allocations := testing.AllocsPerRun(100, func() {
		reader, err := framing.NewReader(payload, len(payload))
		if err != nil {
			panic(err)
		}
...
func newModuleArtifactTestReader(t *testing.T, data []byte) *framing.Reader {
	t.Helper()
	reader, err := framing.NewReader(data, len(data))
	if err != nil {
		t.Fatal(err)
...
func readModuleArtifactPayload(t *testing.T, write func(*framing.Writer)) (Input, error) {
	t.Helper()
	var data bytes.Buffer
	var writer framing.Writer
	if err := writer.Reset(&data, moduleArtifactTestDomain, moduleArtifactTestVersion); err != nil {
...
func writeModuleArtifactRows(t *testing.T, writer *framing.Writer, count uint64, rows ...uint64) {
	t.Helper()
