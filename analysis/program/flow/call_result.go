...
func deriveCallResultAdmissions(sourceView source.View, view authored.View) ([]callResultAdmission, bool) {
	...
	for index := 0; index < loops.Count(); index++ {
		...
		case flowkind.LoopNumericFor:
			...
			if !add(control, callResultAdmission{multiplicity: programschema.CallResultMultiplicityExact, capacity: uint32(fixed)}) {
				return nil, false
			}
		case flowkind.LoopGenericFor:
			...
			if !add(control, callResultAdmission{multiplicity: programschema.CallResultMultiplicityExact, count: uint32(remaining), capacity: 3}) {
				return nil, false
			}
	...
	for index := 0; index < fields.Count(); index++ {
		...
		if tail != 0 {
			fixed, fixedOK := view.Values().Len(values)
			if !finalOpen || !fixedOK || fixed != 0 || !add(values, callResultAdmission{multiplicity: programschema.CallResultMultiplicityOpen}) {
				return nil, false
			}
		} else {
			...
			fixed, fixedOK := view.Values().Len(values)
			if !fixedOK || fixed != 1 || !add(values, callResultAdmission{multiplicity: programschema.CallResultMultiplicityExact, count: 1, capacity: 1}) {
				return nil, false
			}
		}
	}
