...
// For returns the exact Function denoted by value.  A Function
...
func (r *Result) For(value keyspace.Term) (keyspace.Term, bool) {
	if !r.available() {
		return 0, false
	}
...
// Read returns the retained direct Function candidate for one Read.
func (r *Result) Read(read keyspace.Term) (keyspace.Term, bool) {
	return r.plane(r.readFunctions, read, keyspace.FamilyRead)
}

// Call returns the retained direct Function candidate for one Call.
func (r *Result) Call(call keyspace.Term) (keyspace.Term, bool) {
	return r.plane(r.callFunctions, call, keyspace.FamilyCall)
}

// GenericLoop returns the retained candidate for one GenericFor
