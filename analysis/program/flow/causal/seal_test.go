...
func TestSemanticMatrixDeadTailReturnIsSkippedBeforeTailTopology(t *testing.T) {
	...
	if !f.executable.Contains(terminalReturn) {
		t.Fatal("terminal Return is not executable")
	}
	if f.executable.Contains(deadReturn) || f.executable.Contains(call) {
		t.Fatalf("unreachable tail topology remained executable: Return=%v Call=%v", f.executable.Contains(deadReturn), f.executable.Contains(call))
	}
...
func TestSemanticMatrixNestedReturnPublishesTypedOutcomeChain(t *testing.T) {
	...
	if !f.executable.Contains(branch) || !f.executable.Contains(returned) {
		t.Fatalf("nested Branch/Return liveness = %v/%v", f.executable.Contains(branch), f.executable.Contains(returned))
	}
...
func TestSemanticMatrixBranchPublishesGuardedAlternatives(t *testing.T) {
	...
	if !f.executable.Contains(condition) {
		t.Fatalf("Branch condition Function is not executable")
	}
	if !f.executable.Contains(branch) {
		t.Fatalf("Branch term is not executable")
	}
	if !f.executable.Contains(whenTrue) || !f.executable.Contains(whenFalse) {
		t.Fatalf("Branch arm liveness = %v/%v", f.executable.Contains(whenTrue), f.executable.Contains(whenFalse))
	}
...
func TestSemanticMatrixForeignProvenanceFailsClosed(t *testing.T) {
	...
	// receives any plan capability, so no View-only foreign-plan entry remains
	// to probe here. The Result provenance fence remains the published
	// foreign-owner law.
}
...
func assertEveryArcDisposition(t *testing.T, f *causalFixture) {
	t.Helper()
	state, err := newSealState(f.sourceView, f.flow, f.bodies, f.forest, f.outcomes, f.control, f.recurrence, f.ports, f.executable, f.entries, f.staticView.ContentID(), f.moduleFinalize.View().ContentID())
	if err != nil {
...
func TestSemanticMatrixNestedBranchLoopBreakGotoArcLedger(t *testing.T) {
	...
	if !f.executable.Contains(branch) || !f.executable.Contains(loop) || !f.executable.Contains(breakTerm) || !f.executable.Contains(gotoTerm) {
...
func TestSemanticMatrixStaticCallTypeSubtreeIsExcludedFromCausalPlane(t *testing.T) {
	...
	if f.executable.Contains(optional) || f.executable.Contains(primitive) {
...
func openCausalFixture(t *testing.T, spec causalSpec) *causalFixture {
	...
	execResult, err := executable.Seal(sourceView, flowView, bodies, forest, graph, staticID, moduleID, certificate)
	if err != nil {
		flowtest.CloseFinalizers(source.Finalizer{}, flowFinalize, moduleFinalize)
