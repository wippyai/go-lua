...
// typed projections. Source contributes its committed immutable key/literal
// columns; Static and Module are read only through their typed views. No
// owner is retained in the returned Result.
func sealSelectors(
	sourceView sourceColumns,
	flow authored.View,
	bodies *body.Result,
	...
	if err := collectImportAliases(sourceView, flow, bindings, moduleView, aliases, cells.Count(), bodyCount, flow.Calls().Count()); err != nil {
		return nil, err
	}
	...
	builder := selectorBuilder{
		...
		aliases:           aliases,
		bodyCount:         bodyCount,
		rows:              make([]selectorRow, 0, readCount+publicationCount),
	...
	for index := 0; index < readCount; index++ {
		...
		if !ok {
			return nil, errors.New("program/flow/accessgeometry: Read ordinal is unavailable")
		}
	...
	return &Result{
		selectorRows:      builder.rows,
		selectorRowReads:  builder.rowReads,
		selectorReadSlots: builder.slots,
...
// SealSelectors is the construction-only selector column entry point used by
// StaticCheck fixtures that do not need the broader candidate geometry. The
// published Flow path uses Seal, which installs these same rows on the single
// AccessGeometry result.
func SealSelectors(
...
// sourceColumns is the narrow immutable Source contract required by selector
// sealing. Both the uncommitted Preimage used by the vertical laws and the
// committed View used by Flow assembly satisfy it. Keeping the selector
// column on this contract avoids coupling the canonical relation to one
// lifecycle wrapper.
type sourceColumns interface {
	Identity() source.Identity
	Binds() source.BindOrder
	Keys() source.Keys
	Literals() source.Literals
}

func (b *selectorBuilder) row(index uint32) (selectorRow, bool) {
...
func (b *selectorBuilder) appendRow(row selectorRow, read keyspace.Term) uint32 {
...
func (b *selectorBuilder) ensureRead(read keyspace.Term) (uint32, bool, error) {
	...
	for {
		...
		if b.state[currentOrdinal] == 1 {
			return 0, false, errors.New("program/flow/accessgeometry: cyclic exact Read chain")
		}
		...
		if trailLen >= len(b.trail) {
			return 0, false, errors.New("program/flow/accessgeometry: exact Read trail overflow")
		}
		...
		if !ok {
			return 0, false, errors.New("program/flow/accessgeometry: Read row is unavailable")
		}
		if !validFlowBody(owner, b.bodyCount) {
			return 0, false, errors.New("program/flow/accessgeometry: Read owner is outside Source Body universe")
		}
		...
		case keyspace.FamilyCell:
			...
			if !ok {
				return 0, false, errors.New("program/flow/accessgeometry: Read Cell is not an exact root")
			}
		...
		case keyspace.FamilyLensExact:
			...
			if !lensOK || lensOwner != owner {
				return 0, false, errors.New("program/flow/accessgeometry: exact Lens relation is malformed")
			}
			...
			// chain. It contributes no selector row; only a same-owner
			...
			if !baseOK || !validFlowBody(baseOwner, b.bodyCount) || baseOwner != owner {
				return 0, false, errors.New("program/flow/accessgeometry: exact Lens Read owner disagrees")
			}
...
func (b *selectorBuilder) expandRead(read keyspace.Term, parent uint32) (selectorRow, bool) {
...
func (b *selectorBuilder) rootRow(owner, sourceTerm keyspace.Term) (selectorRow, bool) {
	...
	case authored.CellGlobal:
		...
		return selectorRow{
			root: sourceTerm, external: true, plane: selectorPlaneRead, typePath: true,
		}, true
	case authored.CellLocal:
		...
		return selectorRow{
			root: sourceTerm, external: external, plane: selectorPlaneRead, typePath: true,
		}, true
	default:
		return selectorRow{}, false
	}
...
func (b *selectorBuilder) exactLiteral(term keyspace.Term) (keyspace.LiteralValue, keyspace.Term, bool) {
	...
	case keyspace.FamilyUnary:
		...
		case keyspace.FamilyInteger:
			_, operandOwner, integer, literalOK := b.sourceView.Literals().Integers().At(int(keyspace.TermOrdinal(operand) - 1))
			if literalOK && operandOwner == owner {
				value = keyspace.LiteralValue{Kind: keyspace.LiteralInteger, Integer: integer}
				valueOK = true
			}
		case keyspace.FamilyFloat:
			_, operandOwner, bits, literalOK := b.sourceView.Literals().Floats().At(int(keyspace.TermOrdinal(operand) - 1))
			if literalOK && operandOwner == owner {
...
func (b *selectorBuilder) buildPublications(view static.View, slots []uint32) error {
	...
	for index := 0; index < publications.Count(); index++ {
		...
		if !ok || assign == 0 || keyspace.TermFamily(assign) != keyspace.FamilyAssign || keyspace.TermOrdinal(assign) == 0 {
			return errors.New("program/flow/accessgeometry: malformed Static publication")
		}
		owner, _, assignOK := assigns.Get(assign)
		if !assignOK || !validFlowBody(owner, b.bodyCount) {
			return errors.New("program/flow/accessgeometry: publication Assign owner is unavailable")
		}
		...
		if !countOK || uint64(pair) >= uint64(writeCount) {
			return errors.New("program/flow/accessgeometry: publication Pair has no Write")
		}
		...
		if !writeOK || !targetOK || writeAssign != assign {
			return errors.New("program/flow/accessgeometry: publication Write parent disagrees")
		}
		if keyspace.TermFamily(target) != keyspace.FamilyLensExact || keyspace.TermOrdinal(target) == 0 {
			return errors.New("program/flow/accessgeometry: publication target is not exact")
		}
		...
		if !lensOK || lensOwner != owner || fieldKind != kind.FieldName || keyspace.TermFamily(base) != keyspace.FamilyRead {
			return errors.New("program/flow/accessgeometry: publication target is not a same-owner name lens")
		}
		...
		if !selected || parent == 0 {
			return errors.New("program/flow/accessgeometry: publication has no exact selection parent")
		}
		baseOwner, _, _, baseReadOK := b.flow.Storage().Reads().Get(base)
		...
		if !baseOK || !baseReadOK || !validFlowBody(baseOwner, b.bodyCount) || baseOwner != owner || !baseRow.typePath || baseRow.depth == ^uint32(0) {
			return errors.New("program/flow/accessgeometry: publication path is not a same-owner dotted path")
		}
		suffix, typeSegment := b.exactSuffix(target, owner, fieldKind)
		if suffix == 0 || !typeSegment {
			return errors.New("program/flow/accessgeometry: publication target has no normalized name key")
		}
		b.appendRow(selectorRow{
			root: baseRow.root, parent: parent, suffix: suffix,
			depth: baseRow.depth + 1, external: baseRow.external, plane: selectorPlanePublication, typePath: true,
		}, 0)
...
func (b *selectorBuilder) buildCalls(slots []directCallRow) error {
	...
	for index := 0; index < calls.Count(); index++ {
		...
		if !ok || !validFlowBody(owner, b.bodyCount) {
			return errors.New("program/flow/accessgeometry: malformed Call row")
		}
		...
		form := selectorCallPlain
		if receiver != 0 {
			form = selectorCallMethod
		}
