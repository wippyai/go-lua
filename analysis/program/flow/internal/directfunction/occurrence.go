...
func (s *solver) populate(result *Result) error {
	...
	for index := 0; index < reads.Count(); index++ {
		...
		if s.forest.Static(read) || !s.executable.Executable(read) {
			continue
		}
	...
	for index := 0; index < calls.Count(); index++ {
		...
		if !s.executable.Executable(call) {
			continue
		}
	...
	for index := 0; index < loops.Count(); index++ {
		...
		if !rowOK || loopKind != kind.LoopGenericFor || !s.executable.Executable(loop) {
			continue
		}
...
func (s *solver) candidate(value, owner keyspace.Term, occurrence uint32) keyspace.Term {
	...
	if keyspace.TermFamily(value) == keyspace.FamilyFunction {
		...
		// standalone Result.DirectFunction(Function) query intentionally has a
		...
		if !functionOK || functionOwner != owner || s.forest == nil || s.executable == nil ||
			s.forest.Static(value) || !s.executable.Executable(value) || !s.forest.Contains(owner, value) {
			return 0
		}
		return value
	}
	...
	if !ok || readOwner != owner || s.forest.Static(value) || !s.executable.Executable(value) {
		return 0
	}
