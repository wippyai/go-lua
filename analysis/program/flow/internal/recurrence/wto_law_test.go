...
func TestOutcomeMultiCarrierLCAUsesExactExitAndRejectsMalformedBracket(t *testing.T) {
	...
	answers, lcaErr := regions.lcas([]regionLCAQuery{{left: inner, right: outer}, {left: outer, right: sibling}})
	if lcaErr != nil {
		t.Fatalf("hierarchy region LCAs: %v", lcaErr)
	}
