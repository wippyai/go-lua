...
// FamilyCount reports how many rows view publishes for family, or -1 when the
// family names no authored relation. Authored owns this because View is the
// sole authority on which query answers each Source family; every seal that
// reconciles a family cardinality against the authored side asks here.
func FamilyCount(view View, family keyspace.Family) int {
	switch family {
