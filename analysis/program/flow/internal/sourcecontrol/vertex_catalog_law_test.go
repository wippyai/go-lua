...
func TestOutcomePhaseReceiptForeignConsumeBurnsExactRetry(t *testing.T) {
	...
	state := &outcomePhaseLifecycle{
		...
		phases: []OutcomePhase{{path: identity.ContentID{9}}},
		byTerm: []identity.ContentID{{}, {9}},
	}
...
func TestOutcomeSubdivisionForeignConsumeBurnsAndCannotSpliceCSRCarrier(t *testing.T) {
	...
	result.outcomePhases = &outcomePhaseLifecycle{
		owner:     result,
		state:     outcomePhaseIssued,
		byTerm:    []identity.ContentID{{}, outcome.path},
...
func TestCarrierlessLoopOutcomeRequiresTypedOperationRole(t *testing.T) {
	...
	outcome := PhaseRef{result: result, path: identity.ContentID{3}, class: phaseOutcome, node: noNode}
	base := RouteSegment{from: csr, to: outcome, carrier: NoSegmentCarrier(), role: identity.ContentID{4}, relation: segmentRelationRootOutcome, fromFamily: keyspace.FamilyLoop}
	if base.valid() {
