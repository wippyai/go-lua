...
func TestClassifyWTORoutesUsesLowestCommonRegionAndExplicitCrossDisposition(t *testing.T) {
	...
	store := wtoStore{
		...
		byID:    map[identity.ContentID]uint32{rootID: 0, childID: 1, otherID: 2},
	}
...
func TestProvenanceAndQueryPathsAreAllocationFree(t *testing.T) {
	r := syntheticResult()
	body := keyspace.MakeTerm(keyspace.FamilyBody, 1)
	if !Matches(r, identity.ContentID{1}, identity.ContentID{2}, identity.ContentID{3}, identity.ContentID{4}) {
		t.Fatal("matching Source/Flow/Static/Module identities rejected")
	}
	if Matches(r, identity.ContentID{5}, identity.ContentID{2}, identity.ContentID{3}, identity.ContentID{4}) ||
		Matches(r, identity.ContentID{1}, identity.ContentID{5}, identity.ContentID{3}, identity.ContentID{4}) ||
		Matches(r, identity.ContentID{1}, identity.ContentID{2}, identity.ContentID{5}, identity.ContentID{4}) ||
		Matches(r, identity.ContentID{1}, identity.ContentID{2}, identity.ContentID{3}, identity.ContentID{5}) {
...
func TestMalformedBoundaryArmAndResetWitnessFailClosed(t *testing.T) {
	...
	malformedIdentities := []RouteIdentity{
		func() RouteIdentity { copy := routeIdentity; copy.Arm = BoundaryCancel + 1; return copy }(),
		func() RouteIdentity { copy := routeIdentity; copy.Truth = true; return copy }(),
		func() RouteIdentity { copy := routeIdentity; copy.ResetCount = 1; return copy }(),
		func() RouteIdentity { copy := routeIdentity; copy.ResetDigest = identity.ContentID{7}; return copy }(),
	}
