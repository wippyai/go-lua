...
func (b *entryBuilder) resolve(term keyspace.Term) (keyspace.Term, error) {
	...
	for {
		...
		case 2:
			...
			if entry != 0 && !b.exec.Contains(entry) {
...
func (b *entryBuilder) step(term keyspace.Term) (entry, child keyspace.Term, follow bool) {
	if b.exec.Contains(term) {
		if projected, ok := b.ports.Entry(term); ok && b.exec.Contains(projected) {
...
func (b *entryBuilder) firstChild(term keyspace.Term) (keyspace.Term, bool) {
	...
	case keyspace.FamilyRead:
		...
		return child, ok && keyspace.TermFamily(child) != keyspace.FamilyCell && b.exec.Contains(child)
	case keyspace.FamilyLensExact:
		_, child, sourceTerm, fieldKind, ok := b.flow.Access().Exact().Get(term)
		if !ok {
			return 0, false
		}
		if b.exec.Contains(child) {
			return child, true
		}
		return sourceTerm, fieldKind == kind.FieldExact && sourceTerm != 0 && b.exec.Contains(sourceTerm)
	case keyspace.FamilyLensKey:
		_, base, key, ok := b.flow.Access().Dynamic().Get(term)
		if !ok {
			return 0, false
		}
		if b.exec.Contains(base) {
			return base, true
		}
		return key, key != 0 && b.exec.Contains(key)
	case keyspace.FamilyUnary:
		_, _, child, ok := b.flow.Operators().Unaries().Get(term)
		return child, ok && b.exec.Contains(child)
	case keyspace.FamilyBinary:
		_, _, left, right, ok := b.flow.Operators().Binaries().Get(term)
		if !ok {
			return 0, false
		}
		if b.exec.Contains(left) {
			return left, true
		}
		return right, b.exec.Contains(right)
	...
	case keyspace.FamilyValues:
		...
		for index := 0; index < length; index++ {
			...
			if memberOK && b.exec.Contains(member) {
				return member, true
			}
		}
		_, tail, rowOK := values.Get(term)
		return tail, rowOK && tail != 0 && b.exec.Contains(tail)
	...
	case keyspace.FamilyAssign:
		...
		for index := 0; index < count; index++ {
			...
			if targetOK && keyspace.TermFamily(target) != keyspace.FamilyCell && b.exec.Contains(target) {
				return target, true
			}
		}
		_, child, rowOK := assigns.Get(term)
		return child, rowOK && b.exec.Contains(child)
	case keyspace.FamilyCall:
		_, callee, _, actuals, ok := b.flow.Calls().Get(term)
		if !ok {
			return 0, false
		}
		if b.exec.Contains(callee) {
			return callee, true
		}
		return actuals, actuals != 0 && b.exec.Contains(actuals)
	case keyspace.FamilyTableField:
		_, key, values, fieldKind, ok := b.flow.Fields().Get(term)
		if !ok {
			return 0, false
		}
		if (fieldKind == kind.FieldKey || fieldKind == kind.FieldExact) && b.exec.Contains(key) {
			return key, true
		}
		return values, values != 0 && b.exec.Contains(values)
	...
	case keyspace.FamilyLoop:
		...
		if loopKind == kind.LoopRepeat {
			return bodyTerm, b.exec.Contains(bodyTerm)
		}
		return controlTerm, b.exec.Contains(controlTerm)
	default:
		return 0, false
	}
}
