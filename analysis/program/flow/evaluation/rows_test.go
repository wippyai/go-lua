...
func TestSealPendingProductionMatrixCoversSixSubjectPlanesAndTenBuckets(t *testing.T) {
	...
	if _, tail, ok := fixture.flowView.Values().Get(pendingTerm(keyspace.FamilyValues, 5)); !ok || tail != pendingTerm(keyspace.FamilyVararg, 1) || !fixture.executable.Contains(tail) {
