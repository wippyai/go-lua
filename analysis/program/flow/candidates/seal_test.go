...
func candidateSourceInput(spec candidateSpec) source.Input {
	...
	input.Nil = flowtest.LiteralRows(spec.counts[keyspace.FamilyNil], spec.nilOwners, candidateTerm(keyspace.FamilyBody, 1), func(owner keyspace.Term, _ uint32) source.NilLiteral {
		return source.NilLiteral{Owner: owner}
	})
	input.Integer = flowtest.LiteralRows(spec.counts[keyspace.FamilyInteger], spec.intOwners, candidateTerm(keyspace.FamilyBody, 1), func(owner keyspace.Term, ordinal uint32) source.IntegerLiteral {
...
func TestCandidateSealHonestFixture(t *testing.T) {
	...
	if fixture.proof.Contains(candidateTerm(keyspace.FamilyUnary, 1)) || fixture.proof.Contains(candidateTerm(keyspace.FamilyRead, 2)) {
		t.Fatal("honest executable proof retained static negative-key closure")
	}
	if fixture.proof.Contains(candidateTerm(keyspace.FamilyUnary, 5)) || result.UnaryNumeric().Contains(candidateTerm(keyspace.FamilyUnary, 5)) {
		t.Fatal("honest executable proof retained a post-Return dead Unary")
	}
	if fixture.proof.Contains(candidateTerm(keyspace.FamilySelect, 1)) == false || fixture.proof.Contains(candidateTerm(keyspace.FamilySelect, 2)) == false {
