...
func Seal(
	...
	for index := 0; index < unaries.Count(); index++ {
		...
		if proof.Contains(term) && class != unaryNoCandidate {
	...
	for index := 0; index < binaries.Count(); index++ {
		...
		if !proof.Contains(term) || class == binaryNoCandidate {
			continue
		}
	...
	for index := 0; index < reads.Count(); index++ {
		...
		if proof.Contains(term) && lensFamily(family) {
	...
	for index := 0; index < loops.Count(); index++ {
		...
		if proof.Contains(term) && loopKind == kind.LoopGenericFor && fixedHeader(values, control) {
