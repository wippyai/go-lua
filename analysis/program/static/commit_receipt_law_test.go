...
func receiptFixture(t *testing.T) Input {
	t.Helper()
...
func validReceiptForFixture() CommitInput {
	return CommitInput{
...
func TestStaticCommitAcceptsExactCanonicalReceiptAndRetainsNone(t *testing.T) {
	...
	receipt := validReceiptForFixture()
	component, err := finalizer.Commit(receipt)
	if err != nil {
		t.Fatalf("Commit(exact receipt) error = %v", err)
	}
	receipt.TypeOf[0] = 0
	receipt.Annotations[0] = 0
	receipt.Publications[0] = 0
	if component == nil {
		t.Fatal("Commit returned nil Component for exact receipt")
	}
...
func TestStaticCommitRejectsNonCanonicalReceiptsAndClosesTerminalState(t *testing.T) {
	cases := []struct {
		name string
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := finalizer.Commit(receipt); err == nil {
				t.Fatal("Commit accepted invalid canonical receipt")
			}
			if view.Available() {
			...
			if _, err := copied.Commit(validReceiptForFixture()); err == nil {
...
func TestStaticCommitRejectsForeignReceiptDenominatorWithoutChangingContent(t *testing.T) {
	draft, err := Build(receiptFixture(t))
	if err != nil {
		t.Fatalf("Build() error = %v", err)
	}
	...
	if err == nil || component != nil {
		t.Fatalf("Commit accepted extra receipt row: component=%v err=%v", component, err)
	}
	if got := draft.state.component; got != nil {
		t.Fatal("invalid Commit retained construction Component")
	}
	if got := wantID; !got.Available() {
		t.Fatal("invalid receipt test lost pre-commit identity")
	}
}
