...
func buildValuesCatalog(owner *Program) (*valuesCatalog, bool) {
	...
	for index := 0; index < authored.Count(); index++ {
		...
		identity.SortByContentID(rows[index].byID, valuesCatalogIndexID)
		byID[index] = valuesCatalogIndex{id: rows[index].id, index: uint32(index)}
	}
	identity.SortByContentID(byID, valuesCatalogIndexID)
...
func sealValuesTail(input TransformerInput, rootPath identity.ContentID, tail keyspace.Term) (identity.ContentID, TailProducerKind, TailProducer, bool) {
	if tail == 0 {
		return identity.ContentID{}, TailProducerInvalid, TailProducer{}, true
	}
	...
	if !pathOK || !spanOK || !producerOK {
		return identity.ContentID{}, TailProducerInvalid, TailProducer{}, false
	}
...
func validateValuesCatalog(catalog *valuesCatalog) bool {
	...
	seenMembers := make(map[identity.ContentID]struct{})
	for index := range catalog.rows {
...
func valuesCatalogIndexID(value valuesCatalogIndex) identity.ContentID { return value.id }
...
func (view ValuesCatalog) ForID(id identity.ContentID) (ValuesOccurrence, bool) {
	catalog := view.catalog()
...
func (values ValuesOccurrence) ID() identity.ContentID {
	row, ok := values.row()
	if !ok {
...
func (values ValuesOccurrence) ContextID() identity.ContentID { return values.ID() }
...
func (values ValuesOccurrence) BodyPathID() identity.ContentID {
	row, ok := values.row()
	if !ok {
		return identity.ContentID{}
	}
...
func (values ValuesOccurrence) ForID(id identity.ContentID) (ValuesMember, bool) {
	row, ok := values.row()
	if !ok || !id.Available() {
		return ValuesMember{}, false
	}
...
func (member ValuesMember) ContextID() identity.ContentID { return member.ID() }
...
func valuesOccurrenceID(rootPath identity.ContentID, members []valuesCatalogMember, tailPath identity.ContentID, tailKind TailProducerKind) identity.ContentID {
	return transformerSemanticID("program/transformer/values-occurrence", func(writer *framing.Writer) bool {
...
func valuesMemberID(rootPath identity.ContentID, index uint32, memberPath identity.ContentID) identity.ContentID {
	return transformerSemanticID("program/transformer/values-member", func(writer *framing.Writer) bool {
