...
func TestTransformerValueSourcesFollowExactProgramDenominators(t *testing.T) {
	...
	seen := make(map[identity.ContentID]string)
	sawLiteral := false
...
func assertUniqueTransformerRoleID(t *testing.T, seen map[identity.ContentID]string, id identity.ContentID, kind string) {
	t.Helper()
	if !id.Available() {
