...
func (rows *staticRows) freeze(preimage source.Preimage, counts [keyspace.FamilyCount]uint32) (programstatic.Input, error) {
	...
	input.Types.Primitive = append([]programstatic.Primitive(nil), rows.primitive...)
	input.Types.Optional = append([]programstatic.Optional(nil), rows.optional...)
	input.Types.Union = make([]programstatic.Union, len(rows.union))
	for index, row := range rows.union {
		input.Types.Union[index] = programstatic.Union{Members: append([]keyspace.Term(nil), row.Members...)}
	}
	input.Types.Intersection = make([]programstatic.Intersection, len(rows.intersection))
	for index, row := range rows.intersection {
		input.Types.Intersection[index] = programstatic.Intersection{Members: append([]keyspace.Term(nil), row.Members...)}
	}
	input.Types.Generic = make([]programstatic.Generic, len(rows.generic))
	for index, row := range rows.generic {
		input.Types.Generic[index] = programstatic.Generic{Base: row.base, Args: append([]keyspace.Term(nil), row.args...)}
	}
	input.Types.Array = append([]programstatic.Array(nil), rows.array...)
	input.Types.Map = append([]programstatic.Map(nil), rows.mapType...)
	input.Types.Record = make([]programstatic.Record, len(rows.record))
	for index, row := range rows.record {
		input.Types.Record[index] = programstatic.Record{Fields: append([]keyspace.Term(nil), row.Fields...), ReadOnly: row.ReadOnly}
	}
	input.Types.Field = make([]programstatic.Field, len(rows.field))
	for index, row := range rows.field {
		...
		input.Types.Field[index] = programstatic.Field{Key: key, Type: row.typ, Optional: row.optional}
	}
	input.Types.Literal = make([]programstatic.Literal, len(rows.literal))
	for index, row := range rows.literal {
		literal := programstatic.Literal{Kind: row.kind, FloatBits: row.floatBits}
	...
	input.References.TypeRef = make([]programstatic.TypeRef, len(rows.references))
	for index, row := range rows.references {
		...
		input.References.TypeRef[index] = programstatic.TypeRef{Resolution: row.resolution, Target: row.target, Root: row.root, Source: path, Canonical: canonical}
	}

	input.Declarations.Alias = make([]programstatic.TypeAlias, len(rows.aliases))
	for index, row := range rows.aliases {
		...
		input.Declarations.Alias[index] = programstatic.TypeAlias{Owner: row.owner, Target: row.target, Name: name, NameCoordinate: row.coordinate, Params: append([]keyspace.Term(nil), row.params...)}
	}
	input.Declarations.TypeParam = make([]programstatic.TypeParam, len(rows.params))
	for index, row := range rows.params {
		name, err := resolveStaticKey(keys, row.name, false)
		if err != nil {
			return programstatic.Input{}, fmt.Errorf("type parameter %d name: %w", index, err)
		}
		input.Declarations.TypeParam[index] = programstatic.TypeParam{Owner: row.owner, Name: name, Constraint: row.constraint}
	}
	input.Declarations.Interface = make([]programstatic.Interface, len(rows.interfaces))
	for index, row := range rows.interfaces {
		name, err := resolveStaticKey(keys, row.name, false)
		if err != nil {
			return programstatic.Input{}, fmt.Errorf("interface %d name: %w", index, err)
		}
		members := make([]programstatic.InterfaceMember, len(row.membersRaw))
		for memberIndex, member := range row.membersRaw {
			result := programstatic.InterfaceMember{Kind: member.kind, Field: member.field, NameCoordinate: member.coordinate, Signature: member.signature}
			if member.kind == programstatic.InterfaceMethod {
		...
		input.Declarations.Interface[index] = programstatic.Interface{Owner: row.owner, Name: name, NameCoordinate: row.coordinate, Extends: append([]keyspace.Term(nil), row.extends...), Members: members}
	}
	input.Declarations.DeclaredType = append([]programstatic.DeclaredType(nil), rows.declared...)

	input.Signatures.TypeFunction = make([]programstatic.TypeFunction, len(rows.typeFunctions))
	for index, row := range rows.typeFunctions {
		parameters := make([]programstatic.Parameter, len(row.parameters))
		for parameterIndex, parameter := range row.parameters {
			...
			parameters[parameterIndex] = programstatic.Parameter{Name: name, NameCoordinate: parameter.coordinate, Type: parameter.typ}
		}
		input.Signatures.TypeFunction[index] = programstatic.TypeFunction{Scope: row.scope, TypeParams: append([]keyspace.Term(nil), row.typeParams...), Parameters: parameters, Variadic: row.variadic, VariadicCoordinate: row.variadicCoordinate, ReturnsKnown: row.returnsKnown, Returns: append([]keyspace.Term(nil), row.returns...)}
	}
	input.Signatures.TypeAsserts = make([]programstatic.TypeAsserts, len(rows.assertions))
	for index, row := range rows.assertions {
		name, err := resolveStaticKey(keys, row.name, false)
		if err != nil {
			return programstatic.Input{}, fmt.Errorf("TypeAsserts %d name: %w", index, err)
		}
		input.Signatures.TypeAsserts[index] = programstatic.TypeAsserts{Name: name, ParamCoordinate: row.coordinate, Bound: row.bound, Param: row.param, Narrow: row.narrow}
	}

	input.Contracts.Function = make([]programstatic.FunctionContract, len(rows.functionContracts))
	for index, row := range rows.functionContracts {
		input.Contracts.Function[index] = programstatic.FunctionContract{TypeParams: append([]keyspace.Term(nil), row.typeParams...), ReturnsKnown: row.returnsKnown, Returns: append([]keyspace.Term(nil), row.returns...)}
	}
	input.Contracts.Call = make([]programstatic.CallContract, len(rows.callContracts))
	for index, row := range rows.callContracts {
		input.Contracts.Call[index] = programstatic.CallContract{TypeArguments: append([]keyspace.Term(nil), row.arguments...)}
	}

	input.Operators.TypeOf = append([]programstatic.TypeOf(nil), rows.typeOf...)
	input.Operators.KeyOf = append([]programstatic.KeyOf(nil), rows.keyOf...)
	input.Operators.IndexAccess = append([]programstatic.IndexAccess(nil), rows.indexAccess...)
	input.Operators.Conditional = append([]programstatic.Conditional(nil), rows.conditional...)
	input.Publications.Type = append([]programstatic.Publication(nil), rows.publications...)

	input.Operands.Claim = append([]programstatic.ClaimTarget(nil), rows.claims...)
	input.Operands.TypeValue = append([]programstatic.TypeValueTarget(nil), rows.typeValues...)
	input.Operands.Annotation = make([]programstatic.Annotation, len(rows.annotations))
...
func validateSparseClaims(rows []programstatic.ClaimTarget, count uint32) error {
	var previous uint32
