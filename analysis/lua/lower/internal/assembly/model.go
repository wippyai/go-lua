// Package assembly owns the lowerer's single construction cursor.
//
...
// remain construction-only and are consumed by the one terminal Publish
...
// New creates one fresh lower assembly. moduleImports and globals are
...
func New(name string, moduleImports int, globals bind.GlobalCensus) *Collector {
	...
	c.module = moduleowner.New(moduleImports)
	return c
}
...
// built by a successful Publish own their compacted copies and do not retain
// these slice headers, maps, or census arrays.
