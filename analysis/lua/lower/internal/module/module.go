...
func BuildCensus(binding *bind.Result) (Census, error) {
	if binding == nil {
		return Census{}, fmt.Errorf("lualower: nil binding for module census")
	}
	ordinal := make(map[*ast.FuncCallExpr]int)
	for _, occurrence := range binding.DirectGlobalCalls() {
		if occurrence.Call == nil {
			return Census{}, fmt.Errorf("lualower: malformed direct global call evidence")
		}
		...
		if _, duplicate := ordinal[occurrence.Call]; duplicate {
			return Census{}, fmt.Errorf("lualower: duplicate direct require evidence")
		}
...
func New(module *assembly.Collector, census Census) Writer {
...
func (w *Writer) ObserveCall(call *ast.FuncCallExpr, span source.Span, term keyspace.Term) error {
	if w == nil || call == nil || term == 0 {
		return fmt.Errorf("lualower: invalid module Call observation")
	}
	...
	if ordinal < 0 || ordinal >= len(w.filled) || w.filled[ordinal] {
		return fmt.Errorf("lualower: direct require has no unique module census slot")
	}
	importTerm := w.module.Import(ordinal, span, term)
	if importTerm == 0 {
		return fmt.Errorf("lualower: could not create Import")
	}
...
func (w *Writer) AttachAlias(stmt *ast.LocalAssignStmt, alias keyspace.Term) error {
	if w == nil {
		return fmt.Errorf("lualower: missing module authority")
	}
	...
	if alias == 0 {
		return fmt.Errorf("lualower: Import alias Cell is absent")
	}
	if !w.module.SetImportAlias(importTerm, alias) {
		return fmt.Errorf("lualower: could not set Import alias")
	}
	return nil
}
