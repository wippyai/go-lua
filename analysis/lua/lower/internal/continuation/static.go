...
type Statics struct {
	phases       *Stack
	types        []PendingStaticType
	declaredCell []DeclaredCellType
}

func NewStatics(phases *Stack) *Statics {
...
func (q *Statics) PushType(typ ast.TypeExpr, host, body keyspace.Term, span source.Span) error {
	...
	if q == nil || q.phases == nil || !ok || host == 0 || body == 0 || span.File == "" || span != exact {
		return fmt.Errorf("lualower: invalid pending static type")
	}
	q.types = append(q.types, PendingStaticType{Type: typ, Host: host, Body: body, Span: span})
	q.phases.Push(StaticType)
	return nil
}

func (q *Statics) PopType() (PendingStaticType, error) {
	if q == nil || len(q.types) == 0 {
		return PendingStaticType{}, fmt.Errorf("lualower: static type token has no payload")
	}
	last := len(q.types) - 1
...
func (q *Statics) PushDeclaredCell(typ ast.TypeExpr, cell, body keyspace.Term, span source.Span) error {
	...
	if q == nil || q.phases == nil || !ok || cell == 0 || body == 0 || span.File == "" || span != exact {
		return fmt.Errorf("lualower: invalid pending declared Cell type")
	}
	q.declaredCell = append(q.declaredCell, DeclaredCellType{Type: typ, Cell: cell, Body: body, Span: span})
	q.phases.Push(StaticDeclaredCellType)
	return nil
}
...
func (q *Statics) PopDeclaredCell() (DeclaredCellType, error) {
	if q == nil || len(q.declaredCell) == 0 {
		return DeclaredCellType{}, fmt.Errorf("lualower: declared Cell type token has no payload")
	}
