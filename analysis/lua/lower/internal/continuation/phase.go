// Package continuation owns the single continuation stack used while lowering one
...
func (s *Stack) Push(owner Owner) {
	if owner == 0 {
		panic("lualower: zero phase owner")
	}
...
// SetResult publishes one completed expression for the next
