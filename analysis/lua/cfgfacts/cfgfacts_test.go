...
func TestGenericForFactCopiesSlices(t *testing.T) {
	...
	sources := []valuesource.Source{{Kind: valuesource.Expression}, {Kind: valuesource.Nil}}
	...
	sources[0].Kind = valuesource.Call
	symbols[0] = 9
	...
	if fact.Names[0] != "k" || fact.Exprs[0] != iterExpr || fact.Sources[0].Kind != valuesource.Expression || fact.Symbols[0] != 1 {
	...
	fact.Sources[0].Kind = valuesource.Call
	fact.Symbols[0] = 42
	again, _ := meta.GenericFor(7)
	if again.Names[0] != "k" || again.Sources[0].Kind != valuesource.Expression || again.Symbols[0] != 1 {
