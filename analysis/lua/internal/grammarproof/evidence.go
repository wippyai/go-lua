...
func (e Evidence) Validate(live []liveProduction, sources []source, digest, traceDigest string) error {
	...
	if e.TraceDigest != traceDigest {
		return fmt.Errorf("grammar reduction trace inputs are stale: run go generate ./lua/internal/grammarproof")
	}
...
func (s Snapshot) ValidateGenerated() error {
	if !sameEvidence(Generated, s.Evidence) {
		return fmt.Errorf("grammar proof evidence is stale: run go generate ./lua/internal/grammarproof")
	}
	return nil
}
