...
func Generate(root, out string, check bool) error {
	...
	if check {
		...
		if readErr != nil || !bytes.Equal(current, codec) {
			return fmt.Errorf("grammarproof AST codec is stale: run go generate ./lua/internal/grammarproof")
		}
...
func validateGeneratedEvidence(root, out string) error {
	...
	if Generated.IngressDigest != ingressDigest(ingress) {
		return fmt.Errorf("grammar ingress evidence is stale: run go generate ./lua/internal/grammarproof")
	}
	...
	if err != nil || !bytes.Equal(current, rendered) {
		return fmt.Errorf("grammar reduction evidence is stale: run go generate ./lua/internal/grammarproof")
	}
	return nil
}
...
func collectIngress(sources []source) ([]Ingress, error) {
	...
	for _, source := range sources {
		name := source.id + ".lua"
		sealed, err := lualower.Lower(lualower.Source{Name: name, Text: []byte(source.text)})
		if err != nil {
