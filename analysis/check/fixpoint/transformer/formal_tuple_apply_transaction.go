...
// joinFormalApplyLeafPublications joins alternatives which are simultaneously
// live in one shared-DD region (for example two normal Outcome occurrences).
// Dependent product groups invoke their registered lattice once; independent
// symbolic fibers use their existing typed component authority.
func (a *formalTupleAlgebra) joinFormalApplyLeafPublications(
	variable relationVar,
	left, right []decisionLeaf,
) ([]decisionLeaf, error) {
	span, _, authority, ok := a.span(variable)
	if !ok || len(left) != span.count || len(right) != span.count {
		return nil, errFormalComponentForeignOwner
	}
	out := make([]decisionLeaf, span.count)
	grouped := make([]bool, span.count)
	layout := &authority.body.factors
	joinGroup := func(group formalFiberGroupDescriptor) error {
		leftGroup, err := formalEffectGroupLeaves(group, left)
		if err != nil {
			return err
		}
		rightGroup, err := formalEffectGroupLeaves(group, right)
		if err != nil {
			return err
		}
		joined, err := a.combineGroupLeaves(formalComponentJoin, authority, span, group, leftGroup, rightGroup)
		...
		for index, ordinal := range group.members {
			if int(ordinal) < 0 || int(ordinal) >= span.count || grouped[ordinal] {
				return errFormalComponentMalformed
			}
			grouped[ordinal] = true
			out[ordinal] = joined[index]
		}
		return nil
	}
	if err := joinGroup(layout.values); err != nil {
		return nil, err
	}
	for _, group := range layout.nonValues {
		if err := joinGroup(group); err != nil {
			return nil, err
		}
	}
	for ordinal, descriptor := range span.forest.descriptors[span.first : span.first+span.count] {
		if grouped[ordinal] {
			continue
		}
		switch descriptor.role {
		case formalFiberCare:
			if left[ordinal] > 1 || right[ordinal] > 1 {
		...
		default:
			leaf, err := authority.combine(a.ctx, formalComponentJoin, left[ordinal], right[ordinal])
			if err != nil {
				return nil, err
			}
			out[ordinal] = leaf
		}
		if err := a.validateDescriptorLeaf(authority, descriptor, out[ordinal]); err != nil {
			return nil, err
		}
	}
	return out, nil
