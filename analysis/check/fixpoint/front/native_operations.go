...
func nativeOperationContractsWithTostring(stmts []ast.Stmt) []NativeContract {
	...
	walkExpr = func(expr ast.Expr, scope nativeOperationScope, inLoop bool) {
		...
		case *ast.FunctionExpr:
			...
			if !inLoop && nativeCaptureEpochRoot(x) {
				value := "active_epoch_root=1 begin_coordinate=entry coverage_proof=complete uniqueness_proof=complete"
				if nativeCaptureUsesEdgeCoordinate(x) {
					value = "active_epoch_root=1 begin_coordinate=edge_form coordinate_fields=[from,to,edge_kind,edge_ordinal,begin_ordinal] coverage_proof=complete uniqueness_proof=complete"
					if nativeNestedFunctionLevels(x) >= 3 {
						value += " levels=3"
					}
				}
				add("capture_epoch_root", value)
			}
...
// The binder represents a locally declared function as a FunctionExpr.  Mark
// those bindings so pcall can distinguish a closed callback from a parameter.
func nativeCaptureEpochRoot(fn *ast.FunctionExpr) bool {
	if fn == nil || nativeClosureCreatedInLoop(fn.Stmts, false) {
		return false
	}
	return nativeContainsFunctionLiteral(fn.Stmts) || nativeDirectFreeReferenceCount(fn) >= 2
}

func nativeCaptureUsesEdgeCoordinate(fn *ast.FunctionExpr) bool {
	free := nativeDirectFreeReferenceCount(fn)
	return nativeNestedFunctionLevels(fn) >= 3 || nativeContainsFunctionLiteral(fn.Stmts) && free > 0 || free >= 2
}

func nativeNestedFunctionLevels(fn *ast.FunctionExpr) int {
	if fn == nil {
		return 0
	}
	var depth func([]ast.Stmt) int
	depth = func(body []ast.Stmt) int {
		best := 0
		for _, stmt := range body {
			var exprs []ast.Expr
			switch item := stmt.(type) {
			...
			for _, expr := range exprs {
				if child, ok := expr.(*ast.FunctionExpr); ok {
					if level := 1 + depth(child.Stmts); level > best {
						best = level
					}
				}
			}
...
func nativeContainsFunctionLiteral(body []ast.Stmt) bool {
	for _, stmt := range body {
		if nativeStmtContainsFunction(stmt, false) {
			return true
		}
	}
	return false
}
func nativeClosureCreatedInLoop(body []ast.Stmt, loop bool) bool {
	for _, stmt := range body {
		if nativeStmtContainsFunction(stmt, loop) {
			return loop
		}
		switch item := stmt.(type) {
		case *ast.WhileStmt:
			if nativeClosureCreatedInLoop(item.Stmts, true) {
				return true
			}
		case *ast.RepeatStmt:
			if nativeClosureCreatedInLoop(item.Stmts, true) {
		...
		case *ast.IfStmt:
			if nativeClosureCreatedInLoop(item.Then, loop) || nativeClosureCreatedInLoop(item.Else, loop) {
				return true
			}
		case *ast.DoBlockStmt:
			if nativeClosureCreatedInLoop(item.Stmts, loop) {
				return true
			}
		}
	}
...
func nativeStmtContainsFunction(stmt ast.Stmt, _ bool) bool {
	switch item := stmt.(type) {
	case *ast.LocalAssignStmt:
		for _, expr := range item.Exprs {
	...
	case *ast.ReturnStmt:
		for _, expr := range item.Exprs {
			if nativeExprContainsFunction(expr) {
				return true
			}
		}
	}
...
func nativeExprContainsFunction(expr ast.Expr) bool {
	switch item := expr.(type) {
	case *ast.FunctionExpr:
		return true
	case *ast.StringConcatOpExpr:
		return nativeExprContainsFunction(item.Lhs) || nativeExprContainsFunction(item.Rhs)
	case *ast.LogicalOpExpr:
		return nativeExprContainsFunction(item.Lhs) || nativeExprContainsFunction(item.Rhs)
	case *ast.ArithmeticOpExpr:
		return nativeExprContainsFunction(item.Lhs) || nativeExprContainsFunction(item.Rhs)
	case *ast.RelationalOpExpr:
		return nativeExprContainsFunction(item.Lhs) || nativeExprContainsFunction(item.Rhs)
	case *ast.FuncCallExpr:
		if nativeExprContainsFunction(item.Func) || nativeExprContainsFunction(item.Receiver) {
			return true
		}
		for _, arg := range item.Args {
			if nativeExprContainsFunction(arg) {
				return true
			}
		}
	}
...
func nativeDirectFreeReferenceCount(fn *ast.FunctionExpr) int {
	if fn == nil {
		return 0
	}
	declared, used := make(map[string]bool), make(map[string]bool)
	if fn.ParList != nil {
