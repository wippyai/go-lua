...
func TestFromResultUnresolvedReturnExpressionPreservesUnknownSlot(t *testing.T) {
	reg, _ := projectTestRegistry(t)
	...
	if len(got.Returns) != 1 || !product.Equal(reg, got.Returns[0], product.Top()) {
		t.Fatalf("FromResult returned %#v, want one unknown return coordinate", got)
	}
}
...
`)
	...
	resultProgram1205, err := program.RunBoundFunction(def.Func, bindings, program.Config{Check: body.Config{Registry: reg}})
	if err != nil {
		t.Fatalf("CheckBoundFunction: %v", err)
	}
	result := resultProgram1205.RootResult()
...
func projectTestRegistry(t *testing.T) (*axis.Registry, axis.Key[projectMark]) {
	...
	reg, err := standard.RegistryWithAxes(axis.Spec[projectMark]{
		...
		Retention: axis.ImmutableRetention[projectMark](),
		Canonical: axis.PendingCanonical[projectMark]("test-only axis"),
	}.Erase())
	if err != nil {
...
func projectCheckChunk(t *testing.T, stmts []ast.Stmt, config body.Config) *body.Result {
	t.Helper()
	resultProgram1335, err := program.RunChunk(stmts, program.Config{Check: config})
	if err != nil {
		t.Fatalf("CheckChunk: %v", err)
	}
	result := resultProgram1335.RootResult()
	return result
}

func projectCheckFunction(t *testing.T, fn *ast.FunctionExpr, config body.Config) *body.Result {
	t.Helper()
	resultProgram1344, err := program.RunFunction(fn, program.Config{Check: config})
	if err != nil {
		t.Fatalf("CheckFunction: %v", err)
	}
	result := resultProgram1344.RootResult()
	return result
}
