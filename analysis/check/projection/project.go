...
func FromResultContext(ctx context.Context, result ResultReader) (summary.Summary, error) {
	...
	normalReturnFacts, err := projectNormalReturnFacts(reg, result, exit)
	if err != nil {
		return summary.Summary{}, err
	}
	out := summary.Summary{
		...
		ParamSinkExposures:              paramSinkExposures,
		CapturedPathObligations:         capturedPathObligations,
	...
	out.FreshHeapAllocations = projectFreshHeapAllocations(reg, result, exit, out.HeapTableObjects, out.Returns, declared)
	if err := projectionContextErr(ctx); err != nil {
		return summary.Summary{}, err
	}
