...
func TestCallArityReportsImportedSignatureTooFewAndTooMany(t *testing.T) {
	result := checkFunction(t, `function f()
    need_string()
    need_string("ok", "extra")
end`, stringSignatureManifest())

	got := obligationpass.New(obligationpass.CallArity{}).Run(obligationpass.Context{
...
func TestCallArityReportsLocalFunctionThroughSummaryContract(t *testing.T) {
