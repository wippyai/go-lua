...
func TestCorpusSmoke(t *testing.T) {
	if os.Getenv("CORPUS_SMOKE") != "1" {
		t.Skip("set CORPUS_SMOKE=1 to sweep the fixture source corpus")
	}
	...
	if completed != len(files) {
		t.Fatalf("CORPUS_SMOKE completed=%d, want all %d inputs to finish within %s", completed, len(files), corpusFileTimeout)
	}
