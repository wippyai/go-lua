// Package lattice defines the abstract-domain contract for the type system's
...
// algebraic foundation for fixed-point algorithms: a domain that satisfies the
// laws (idempotency, commutativity, associativity, monotonicity, ACC under
// widening) makes those algorithms terminate with a sound over-approximation of
// the collecting semantics. A domain that does not satisfy the laws does not —
// and domain law tests catch the violation as a unit-test failure rather than
// as a fixture hang.
//
...
//     does not consume a greatest lower bound (e.g. AbstractValue); law tests
//     skip the meet-side laws (including absorption) when nil.
