...
func TestCanonicalEncodingMatchesEqualAcrossEveryRawRepresentation(t *testing.T) {
	var representatives []Value
	var encodedClasses [][]byte
	byBytes := make(map[string]int)
	for rawState := 0; rawState <= int(^uint8(0)); rawState++ {
		for rawFlags := 0; rawFlags <= int(^uint8(0)); rawFlags++ {
			value := Value{state: state(rawState), flags: Flag(rawFlags)}
...
func TestCanonicalEncodingSeparatesKnownClaimsWithoutDelimiterAssumptions(t *testing.T) {
	values := []Value{
		Bottom(), Top(), Type(), Any(), NonNil(), Runtime(),
		Of(TypeClaim, RuntimeClaim), Of(AnyClaim, NonNilClaim),
	}
	for left, a := range values {
		for right, b := range values {
			...
			if equal != same {
				t.Fatalf("values %d/%d Equal=%v bytes=%v", left, right, equal, same)
			}
		}
	}
}
...
func TestCanonicalEncodingPropagatesCancellationWithoutAuthority(t *testing.T) {
	...
	cancel()
	if err := encodeCanonical(&writer, Runtime()); !errors.Is(err, context.Canceled) {
...
func BenchmarkCanonicalEncoding(b *testing.B) {
	var writer canonical.Writer
	value := Of(TypeClaim, NonNilClaim, RuntimeClaim)
	b.ReportAllocs()
	for range b.N {
