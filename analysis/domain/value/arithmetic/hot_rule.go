...
func BindHot(fragment *SchemaFragment, owner *valueowner.HotOwner) (*HotRule, bool) {
	...
	tx, ok := valueowner.BeginSelectedRuleBinding(owner, fragment.slot, fragment.carry, fragment.write, owner.FactorRef(), engine.HotRuleSpec[value.Value, value.BinaryArithmetic]{
	...
	}, engine.HotCarrySpec[value.Value, value.BinaryArithmetic]{})
	if !ok || tx == nil {
		return nil, false
	}
