...
type EGraph struct {
	...
	parent map[pathdom.PathKey]pathdom.PathKey
	rank   map[pathdom.PathKey]int
	...
	children map[pathdom.PathKey]map[string]pathdom.PathKey

	// Reverse mapping: child path -> (parent path, segment)
	// Used for propagating child path updates up to parent paths.
	parentOf map[pathdom.PathKey]parentInfo
}

type parentInfo struct {
	parent  pathdom.PathKey
	segment string
}
...
func NewEGraph() *EGraph {
	return &EGraph{
		parent:   make(map[pathdom.PathKey]pathdom.PathKey),
		rank:     make(map[pathdom.PathKey]int),
		children: make(map[pathdom.PathKey]map[string]pathdom.PathKey),
		parentOf: make(map[pathdom.PathKey]parentInfo),
	}
}
...
func (e *EGraph) Clone() *EGraph {
	c := &EGraph{
		parent:   make(map[pathdom.PathKey]pathdom.PathKey, len(e.parent)),
		rank:     make(map[pathdom.PathKey]int, len(e.rank)),
		children: make(map[pathdom.PathKey]map[string]pathdom.PathKey, len(e.children)),
		parentOf: make(map[pathdom.PathKey]parentInfo, len(e.parentOf)),
	}
	...
	for k, segs := range e.children {
		c.children[k] = make(map[string]pathdom.PathKey, len(segs))
...
func (e *EGraph) Register(path pathdom.Path) {
	...
	if len(path.Segments) > 0 {
		...
		if e.children[parentKey] == nil {
			e.children[parentKey] = make(map[string]pathdom.PathKey)
		}
...
func (e *EGraph) RegisterKey(key pathdom.PathKey) {
	e.makeSet(key)
}
...
func (e *EGraph) makeSet(key pathdom.PathKey) {
...
func (e *EGraph) Find(key pathdom.PathKey) pathdom.PathKey {
	e.makeSet(key)
	if e.parent[key] != key {
...
func (e *EGraph) Union(x, y pathdom.PathKey) bool {
	rootX := e.Find(x)
	rootY := e.Find(y)
...
func (e *EGraph) propagateCongruence(rootX, rootY pathdom.PathKey) {
	...
	if childrenX == nil || childrenY == nil {
		...
		if childrenY != nil {
			if e.children[rootX] == nil {
				e.children[rootX] = make(map[string]pathdom.PathKey)
			}
...
func (e *EGraph) AreEqual(x, y pathdom.PathKey) bool {
	return e.Find(x) == e.Find(y)
}

// GetEquivalenceClass returns all paths equivalent to the given path.
func (e *EGraph) GetEquivalenceClass(key pathdom.PathKey) []pathdom.PathKey {
	root := e.Find(key)
	var result []pathdom.PathKey
...
func (e *EGraph) AllPaths() []pathdom.PathKey {
	result := make([]pathdom.PathKey, 0, len(e.parent))
	for k := range e.parent {
		result = append(result, k)
	}
	sort.Slice(result, func(i, j int) bool {
...
func (e *EGraph) ClassRepresentatives() []pathdom.PathKey {
	seen := make(map[pathdom.PathKey]bool)
	var result []pathdom.PathKey
	for k := range e.parent {
		root := e.Find(k)
		if !seen[root] {
