...
func TestContractsExactInventoryAndCanonicalDeterminism(t *testing.T) {
	...
	if !firstOK || !secondOK || len(first) != len(inventory) || !reflect.DeepEqual(first, second) {
		t.Fatal("non-canonical Ownership contract inventory")
	}
	...
	for _, item := range inventory {
		...
		if !ok || !derived {
			t.Fatal("invalid Ownership source inventory")
		}
	...
	for _, contract := range first {
		if _, found := expected[contract]; !found {
			t.Fatal("Ownership contract escaped exact source inventory")
		}
	}
	if !canonical(first) {
		t.Fatal("Ownership contracts are not in canonical source order")
	}
...
func TestContractsAllowMultipleConclusionsButNoDuplicateRequirement(t *testing.T) {
	...
	for _, contract := range contracts {
		if _, duplicate := unique[contract]; duplicate {
			t.Fatal("duplicate Ownership coverage requirement")
		}
	...
	if !multiple {
		t.Fatal("Ownership collapsed distinct duty conclusions")
	}
}
...
func TestContractsSeparateExactTransferAndContinuationSources(t *testing.T) {
	...
	for _, expected := range []struct {
		...
		{semanticsource.FacetTargetOutcome, returnConclusion},
		...
		if !hasTargetConclusion(contracts, expected.facet, expected.conclusion, factor) {
			t.Fatalf("missing exact Ownership target source facet %d", expected.facet)
		}
	}
	if hasTargetFacet(contracts, semanticsource.FacetTargetCallbackRelease) {
		t.Fatal("Ownership claimed callback release without observing release")
	}
}

func TestContractsRejectInvalidOwner(t *testing.T) {
	if contracts, ok := Contracts(engine.SemanticKey{}); ok || contracts != nil {
		t.Fatal("invalid Ownership Factor owner admitted")
	}
}
...
func lawFactor(value byte) engine.SemanticKey {
	digest := sha256.Sum256([]byte{0x4f, value})
