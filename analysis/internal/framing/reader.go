...
// StreamMeasure is an exact allocation-free census of one framed stream.
...
var (
	ErrMalformed = errors.New("program framing: malformed stream")
	ErrLimit     = errors.New("program framing: size limit")
)

// NewReader bounds one in-memory framed stream. A non-positive limit
...
// Scan validates frame shape without allocating decoded payloads or
