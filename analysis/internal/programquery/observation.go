// Package programquery owns the detached result contracts for the two global
// query families. It owns only detached domain result types, so both the cold
// schema owner and top-level hot binding can use the exact same contracts.
...
type EffectObservation struct {
	...
	certificates []keyspace.ContentID
}
...
func CloneEffect(input EffectObservation) EffectObservation {
	input.Atoms = append([]keyspace.ContentID(nil), input.Atoms...)
	input.certificates = append([]keyspace.ContentID(nil), input.certificates...)
