...
func (row ExactScalarSummaryRow) ID() keyspace.ContentID {
	if !row.Available() {
...
func (row ExactScalarSummaryRow) OccurrenceID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
...
func (row ExactScalarSummaryRow) BodyPathID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.body
...
func (row ExactScalarSummaryRow) SubjectID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.subject
...
func exactScalarSummaryID(occurrence, subject, body keyspace.ContentID, role ExactScalarSummaryRole, literal keyspace.LiteralValue) keyspace.ContentID {
...
func (compiler *compiler) deriveExactScalarSummariesFailure() CompileFailure {
	...
	states := make(map[keyspace.ContentID]exactScalarState)
	var equations []exactScalarEquation
	var arithmetic []OccurrenceRow

	join := func(id keyspace.ContentID, incoming exactScalarState) bool {
	...
	for changed {
		...
		for _, equation := range equations {
			...
			case exactScalarEquationUnaryNeg:
				...
				result, resultOK := program.ExactUnaryNegLiteral(literal)
			...
			case exactScalarEquationArithmetic:
				...
				result, resultOK := program.ExactArithmeticLiteral(leftLiteral, rightLiteral, equation.op)
				if !resultOK {
					if join(equation.output, unknown) {
						changed = true
					}
	...
	seen := make(map[keyspace.ContentID]struct{}, len(arithmetic)*3)
	...
	radixRows(summaries, func(row ExactScalarSummaryRow) keyspace.ContentID { return row.id })
