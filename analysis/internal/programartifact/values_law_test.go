...
func valuesLawID(value byte) keyspace.ContentID {
...
func TestProgramArtifactValuesRowsPreserveOrderAndTail(t *testing.T) {
	...
	for index, want := range []keyspace.ContentID{valuesLawID(3), valuesLawID(4)} {
...
func TestProgramArtifactValuesTailShapeFailsClosed(t *testing.T) {
	invalid := []ValuesTail{
		...
		{id: keyspace.ContentID{}, kind: ValuesTailCall, present: true},
