...
func TestProgramArtifactOwnsCanonicalFunctionBoundaryPorts(t *testing.T) {
	...
	packBodies := make(map[keyspace.ContentID]programartifact.PackBodyReceiptRow, pack.BodyCount())
	...
	bodyRows := make(map[keyspace.ContentID]programartifact.BodyRow, left.BodyCount())
...
`)})
	...
	staticNodes := make(map[keyspace.ContentID]struct{}, artifact.StaticTypeNodeCount())
