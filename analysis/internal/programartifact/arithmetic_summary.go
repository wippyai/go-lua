...
type ArithmeticSummaryRow struct {
	id, occurrence, body keyspace.ContentID
...
func (row ArithmeticSummaryRow) ID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.id
}
func (row ArithmeticSummaryRow) OccurrenceID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.occurrence
}
func (row ArithmeticSummaryRow) BodyPathID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.body
...
func arithmeticSummaryID(occurrence, body keyspace.ContentID, op flowkind.BinaryOp, left, right, result NumericRepresentation, divisor ArithmeticDivisorProperty) keyspace.ContentID {
...
func mergeArithmeticGuardFacts(left, right map[keyspace.ContentID]arithmeticGuardMask) map[keyspace.ContentID]arithmeticGuardMask {
	merged := make(map[keyspace.ContentID]arithmeticGuardMask, len(left)+len(right))
...
func intersectArithmeticGuardFacts(left, right map[keyspace.ContentID]arithmeticGuardMask) map[keyspace.ContentID]arithmeticGuardMask {
	intersected := make(map[keyspace.ContentID]arithmeticGuardMask)
...
// proof.
func (compiler *compiler) arithmeticDivisorProperties() (map[keyspace.ContentID]ArithmeticDivisorProperty, CompileFailure) {
	...
	equalities := make(map[keyspace.ContentID]OccurrenceRow)
	selects := make(map[keyspace.ContentID]OccurrenceRow)
	...
	var conditionFacts func(keyspace.ContentID, bool, map[guardVisit]bool) map[keyspace.ContentID]arithmeticGuardMask
	conditionFacts = func(condition keyspace.ContentID, truth bool, visiting map[guardVisit]bool) map[keyspace.ContentID]arithmeticGuardMask {
		...
		return map[keyspace.ContentID]arithmeticGuardMask{cell: mask}
	}

	bodyByEntry := make(map[keyspace.ContentID]keyspace.ContentID, len(compiler.bodies))
	ambiguousEntry := make(map[keyspace.ContentID]struct{})
	...
	bodyFacts := make(map[keyspace.ContentID]map[keyspace.ContentID]arithmeticGuardMask)
	bodyIncoming := make(map[keyspace.ContentID]bool)
	for edgeIndex, edge := range compiler.environment {
		...
		facts := map[keyspace.ContentID]arithmeticGuardMask(nil)
	...
	properties := make(map[keyspace.ContentID]ArithmeticDivisorProperty)
	for index, row := range compiler.occurrences {
...
func (compiler *compiler) deriveArithmeticSummariesFailure() CompileFailure {
	...
	states := make(map[keyspace.ContentID]numericSummaryState)
	join := func(id keyspace.ContentID, incoming numericSummaryState) bool {
	...
	types := make(map[keyspace.ContentID]StaticTypeNodeRow, len(compiler.staticTypeNodes))
	...
	var typeMask func(keyspace.ContentID, map[keyspace.ContentID]bool) (uint8, bool)
	typeMask = func(id keyspace.ContentID, visiting map[keyspace.ContentID]bool) (uint8, bool) {
		row, ok := types[id]
		if !ok || visiting[id] {
			return 0, false
		}
	...
	for _, input := range compiler.staticInputs {
		...
		mask, numericOK := typeMask(input.TargetID(), make(map[keyspace.ContentID]bool))
		if numericOK {
	...
	for _, boundary := range compiler.functionBoundaries {
		for _, formal := range boundary.formals {
			if formal.declared.Available() {
				mask, numericOK := typeMask(formal.declared, make(map[keyspace.ContentID]bool))
				if numericOK {
	...
	radixRows(summaries, func(row ArithmeticSummaryRow) keyspace.ContentID { return row.id })
	...
	radixRows(unarySummaries, func(row UnarySummaryRow) keyspace.ContentID { return row.id })
