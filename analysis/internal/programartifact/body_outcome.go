...
type BodyRow struct {
	...
	function     keyspace.ContentID
	formal       keyspace.ContentID
	callable     bool
	entryPoints  []keyspace.ContentID
	roots        []RootRow
	outcomeStart uint32
	outcomeEnd   uint32
	sealed       bool
...
func (row RootRow) ID() keyspace.ContentID {
	if !row.Available() {
...
func (row BodyRow) EntryID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.entry
...
func (row BodyRow) ID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.id
...
func (row BodyRow) ContextID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.context
...
func (row BodyRow) FunctionContextID() (keyspace.ContentID, bool) {
	return row.function, row.Callable() && row.function.Available()
}
func (row BodyRow) CallFormalID() (keyspace.ContentID, bool) {
...
func (row CallTargetRow) AllocationID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.allocation
}
func (row CallTargetRow) BodyID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.body
}
func (row CallTargetRow) ContextID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.context
}
func (row CallTargetRow) FunctionContextID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.function
}
func (row CallTargetRow) CallFormalID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.formal
...
func (row BodyRow) EntryPointAt(index int) (keyspace.ContentID, bool) {
...
type OutcomeRow struct {
	id             keyspace.ContentID
	body           keyspace.ContentID
	target         keyspace.ContentID
	propagation    keyspace.ContentID
	kind           OutcomeKind
	hasTarget      bool
	hasPropagation bool
	returnStart    uint32
...
func (row OutcomeRow) ID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.id
...
func (row OutcomeRow) BodyID() keyspace.ContentID {
	if !row.Available() {
		return keyspace.ContentID{}
	}
	return row.body
...
func (row OutcomeRow) TargetID() (keyspace.ContentID, bool) {
	return row.target, row.Available() && row.hasTarget
}

func (row OutcomeRow) PropagationID() (keyspace.ContentID, bool) {
...
func (row OutcomeRow) PointAt(index int) (keyspace.ContentID, bool) {
...
type ReturnValue struct{ id keyspace.ContentID }

func (value ReturnValue) Available() bool { return value.id.Available() }
func (value ReturnValue) ID() keyspace.ContentID {
	if !value.Available() {
		return keyspace.ContentID{}
	}
	return value.id
...
func (compiler *compiler) copyBodiesAndOutcomesFailure() CompileFailure {
	...
	valueIDs := make(map[keyspace.ContentID]struct{}, len(compiler.values))
	...
	seenBodies := make(map[keyspace.ContentID]struct{}, bodyCount)
	seenBodyContexts := make(map[keyspace.ContentID]struct{}, bodyCount)
	seenOutcomes := make(map[keyspace.ContentID]int)
	...
	for bodyIndex := 0; bodyIndex < bodyCount; bodyIndex++ {
		...
		functionID, formalID := keyspace.ContentID{}, keyspace.ContentID{}
		callable := false
		if function, functionOK := body.TransformerFunction(); functionOK {
			formal, formalOK := body.CallTarget()
			functionID, formalID = function.ContextID(), keyspace.ContentID{}
			if formalOK {
		...
		seenRoots := make(map[keyspace.ContentID]struct{}, rootCount)
		...
		for outcomeIndex := 0; outcomeIndex < outcomeCount; outcomeIndex++ {
			...
			propagationID := keyspace.ContentID{}
...
func (compiler *compiler) copyCallTargetsFailure() CompileFailure {
	...
	bodyByContext := make(map[keyspace.ContentID]BodyRow, len(compiler.bodies))
	...
	seenAllocations := make(map[keyspace.ContentID]struct{})
	seenBodies := make(map[keyspace.ContentID]struct{})
