...
func TestComputationOccurrenceRowsAreClosedAndPayloadScoped(t *testing.T) {
	...
	binary := OccurrenceRow{kind: OccurrenceBinaryEquality, id: id, inputs: []keyspace.ContentID{operand, {3}}, code: binaryCode}
	left, right, op, binaryOK := binary.BinaryEquality()
	if !codeOK || !binaryOK || left != operand || right != (keyspace.ContentID{3}) || op != flowkind.BinaryNotEqual {
		t.Fatal("binary equality row did not preserve ordered operands and polarity")
	}
	order := OccurrenceRow{kind: OccurrenceBinaryOrder, id: id, inputs: []keyspace.ContentID{operand, {4}}, code: uint64(flowkind.BinaryGreater)}
	left, right, op, orderOK := order.BinaryOrder()
	if !orderOK || left != operand || right != (keyspace.ContentID{4}) || op != flowkind.BinaryGreater {
		t.Fatal("binary order row did not preserve ordered operands and operator")
	}
	for _, hostile := range []OccurrenceRow{
		{kind: OccurrenceBinaryOrder, id: id, inputs: []keyspace.ContentID{operand, {4}}, code: uint64(flowkind.BinaryEqual)},
		{kind: OccurrenceBinaryOrder, id: id, inputs: []keyspace.ContentID{operand}, code: uint64(flowkind.BinaryLess)},
		{kind: OccurrenceBinaryOrder, id: id, inputs: []keyspace.ContentID{operand, {4}, {5}}, code: uint64(flowkind.BinaryLess)},
	} {
		if hostile.Available() {
			t.Fatal("binary order row accepted malformed operator/arity")
		}
	}
	literal := OccurrenceRow{kind: OccurrenceValueSource, id: id, inputs: []keyspace.ContentID{operand}, literalFamily: keyspace.FamilyString, literal: keyspace.LiteralValue{Kind: keyspace.LiteralString, String: "x"}, literalOK: true}
