...
// state. Framing is the same package-owned schema emitted by Writer.
...
func (w *DigestWriter) event(tag byte, payloadLength uint64) error {
	w.header[0] = tag
	length := binary.PutUvarint(w.header[1:], payloadLength)
	return w.writeBytes(w.header[:length+1])
}
