...
type ArtifactAuthority struct {
	rows map[keyspace.ContentID]programartifact.StaticTypeNodeRow
}
...
type artifactResolver struct {
	rows    map[keyspace.ContentID]programartifact.StaticTypeNodeRow
	entries map[keyspace.ContentID]*artifactResolution
}
...
func SealArtifacts(artifacts []*programartifact.Artifact) (*ArtifactAuthority, error) {
	...
	a := &ArtifactAuthority{rows: make(map[keyspace.ContentID]programartifact.StaticTypeNodeRow)}
...
func (a *ArtifactAuthority) Row(id keyspace.ContentID) (programartifact.StaticTypeNodeRow, bool) {
	if a == nil {
...
func (a *ArtifactAuthority) Resolve(id keyspace.ContentID) (typ.Type, bool) {
	if a == nil || !id.Available() {
		return nil, false
	}
	resolver := &artifactResolver{
		rows:    a.rows,
		entries: make(map[keyspace.ContentID]*artifactResolution),
	}
...
func (a *artifactResolver) resolve(id keyspace.ContentID) (typ.Type, bool) {
	row, ok := a.rows[id]
	if !ok {
		return nil, false
	}
	...
	if entry != nil {
		...
		if entry.active {
			placeholder, placeholderOK := a.installBackedgePlaceholder(id, row, entry)
			if !placeholderOK {
				return nil, false
			}
...
func (a *artifactResolver) installBackedgePlaceholder(id keyspace.ContentID, row programartifact.StaticTypeNodeRow, entry *artifactResolution) (typ.Type, bool) {
	if row.Kind() != programartifact.StaticNodeInterface &&
		(row.Kind() != programartifact.StaticNodeAlias || row.AliasParamCount() != 0) {
		return nil, false
	}
