...
// A native publication row's value is read by consumers as bytes, so what this
// producer renders is part of its contract. These laws state the rendering two
// ways: the catalogs are total over the sealed vocabularies they spell, and the
// published rows of a solved corpus are exactly the bytes pinned here.
...
// TestNativeLiteralRenderingIsOneTableLaw states that a scalar has one
// rendering here. Both published paths reach it: the exact-scalar path through
// its Value projection and the reusable-summary path through its own literal.
...
// TestCorpusNativePublicationRenderingIsPinnedLaw is the byte-level receipt:
// every value a solved fixture publishes, spelled out. It covers both scalar
// paths, all three overflow disciplines, and the divisor clause.
