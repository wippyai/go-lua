...
func hasFixedOutcome(contract *target.Contract, op target.Operation, want []typ.Type) bool {
	for outcome := 0; outcome < contract.OutcomeCount(op); outcome++ {
		...
		for index, expected := range want {
			...
			declaration, declarationOK := contract.TypeDeclaration(frozen)
			decoded, err := domaincontract.Decode(context.Background(), declaration, nil)
			match = match && declarationOK && err == nil && typ.TypeEquals(decoded, expected)
		}
		if match {
			return true
		}
...
func frozenTailClassMatches(t *testing.T, contract *target.Contract, values target.Values, want typ.Type) bool {
	...
	declaration, declarationOK := contract.TypeDeclaration(got)
	decoded, err := domaincontract.Decode(context.Background(), declaration, nil)
	return declarationOK && err == nil && typ.TypeEquals(decoded, want)
}
...
func hasFixedOutcomeForValues(contract *target.Contract, values target.Values, want []typ.Type) bool {
	...
	for index, expected := range want {
		...
		declaration, declarationOK := contract.TypeDeclaration(frozen)
		decoded, err := domaincontract.Decode(context.Background(), declaration, nil)
		if !declarationOK || err != nil || !typ.TypeEquals(decoded, expected) {
			return false
		}
	}
	return true
...
func TestAuthoredCatalogueRejectsMissingIdentityBeforeSpecRef(t *testing.T) {
	...
	spec := target.Spec{Semantics: domaincontract.NewSemantics(), Operations: catalogue.operations}
