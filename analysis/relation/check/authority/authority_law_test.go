...
func newAuthorityFixture(t *testing.T) authorityFixture {
	...
	scope := model.DefineScopeSchema(scopeID, []model.ColumnID{column})
	foreignScope := model.DefineScopeSchema(foreignScopeID, []model.ColumnID{foreignColumn})
...
func (value authorityFixture) signature(t *testing.T, schema model.SchemaID, inputRelation model.RelationID, inputColumn model.ColumnID, inputType model.TypeID, inputKey model.KeyID, outputDenominator model.DenominatorRef, outputs []signature.Output) signature.Signature {
	t.Helper()
	...
	sealed, ok := signature.Seal(signature.Spec{
		...
		Outputs:     outputs,
		Authority:   signature.OutputAuthority{Denominator: outputDenominator},
...
func (value authorityFixture) output(t *testing.T, relation model.RelationID, column model.ColumnID, typeID model.TypeID) signature.Output {
	t.Helper()
	return signature.Output{Relation: relation, Column: column, Type: typeID, Presence: signature.ProducePresent}
}
...
func validSchema(t *testing.T) (authorityFixture, plan.ExecutionSchema) {
	...
	apply := algebra.NewApply([]algebra.Expression{input}, algebra.NewApplyContract(sig.Identity()))
...
func TestForeignSchemaFenceIsRejected(t *testing.T) {
	...
	sig := value.signature(t, value.foreignSchema, value.foreignRelation, value.foreignColumn, value.foreignType, value.foreignKey, denominator, []signature.Output{value.output(t, value.relation, value.column, value.typeID)})
	input := algebra.NewInput(value.foreignRelation)
	publish := algebra.NewPublish(algebra.NewApply([]algebra.Expression{input}, algebra.NewApplyContract(sig.Identity())), algebra.NewPublishContract(value.relation, value.key))
...
func TestWrongOutputDenominatorIsRejected(t *testing.T) {
	...
	sig := value.signature(t, value.schemaID, value.foreignRelation, value.foreignColumn, value.foreignType, value.foreignKey, wrong, []signature.Output{value.output(t, value.relation, value.column, value.typeID)})
	input := algebra.NewInput(value.foreignRelation)
	publish := algebra.NewPublish(algebra.NewApply([]algebra.Expression{input}, algebra.NewApplyContract(sig.Identity())), algebra.NewPublishContract(value.relation, value.key))
	if report := Check(value.schema(t, sig, value.foreignRelation, value.foreignColumn, value.foreignKey, publish)); !hasCode(report, CodeInvalidOutputAuthority) {
...
func TestDuplicateOutputIsRejected(t *testing.T) {
	value := newAuthorityFixture(t)
	denominator, _ := model.NewDenominatorRef(value.relation, value.key)
	output := value.output(t, value.relation, value.column, value.typeID)
	sig := value.signature(t, value.schemaID, value.foreignRelation, value.foreignColumn, value.foreignType, value.foreignKey, denominator, []signature.Output{output, output})
	input := algebra.NewInput(value.foreignRelation)
	publish := algebra.NewPublish(algebra.NewApply([]algebra.Expression{input}, algebra.NewApplyContract(sig.Identity())), algebra.NewPublishContract(value.relation, value.key))
...
func TestSameOwnerForeignRelationStillRequiresSchemaMembership(t *testing.T) {
	value := newAuthorityFixture(t)
	denominator, _ := model.NewDenominatorRef(value.relation, value.key)
	sig := value.signature(t, value.schemaID, value.missingRelation, value.missingColumn, value.typeID, value.missingKey, denominator, []signature.Output{value.output(t, value.relation, value.column, value.typeID)})
