...
// recurrence head. It carries the stable logical endpoints, certificate
// evidence, and the exact mounted runtime fence. The fence is deliberately
// part of the opaque value: a permit copied from a different mount must not
// be redeemable merely because both mounts used the same schema certificate.
...
func (permit WideningPermit) Available() bool {
	return permit.dependency.Available() && permit.relation.Available() && permit.evidence.Available() && permit.fence.Available()
}
...
func newWideningPermit(head plan.WideningHead, fence binding.Fence) (WideningPermit, bool) {
	if !head.Available() || !head.Dependency().Available() || !head.Relation().Available() || !fence.Available() {
	...
	return WideningPermit{dependency: dependency, relation: relation, evidence: head.Digest(), fence: fence}, true
}
