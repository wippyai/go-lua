...
func (issuer *MountedActivationCandidateIssuer) validFor(builder *BindingTopologyBuilder, occurrence ruleOccurrence) bool {
	if issuer == nil || issuer.state == nil || builder == nil || builder.inner == nil || occurrence.builder != builder || !occurrence.role.mounted() || !occurrence.role.activation || !issuer.rule.Available() || !issuer.family.Available() {
		return false
	}
...
// edge slices.
func (issuer *MountedActivationCandidateIssuer) AddMountedActivationCandidate(builder *BindingTopologyBuilder, occurrence ruleOccurrence, application, target, endpoint identity.SemanticKey, mount, body identity.ContentID) bool {
	if !issuer.validFor(builder, occurrence) || !application.Available() || !target.Available() || !endpoint.Available() || target == endpoint || !mount.Available() || !body.Available() {
		return false
	}
	...
	receipt, issued := builder.issueDirectActivationCandidate(candidate)
	return issued && builder.addDirectActivationCandidate(receipt)
}
...
func (issuer *MountedActivationCandidateIssuer) CompleteMountedActivationCandidates(builder *BindingTopologyBuilder, occurrence ruleOccurrence, application identity.SemanticKey, expected uint64) bool {
	if !issuer.validFor(builder, occurrence) || !application.Available() {
		return false
	}
	inner, locked := builder.lockTopologyOpen()
	if !locked {
