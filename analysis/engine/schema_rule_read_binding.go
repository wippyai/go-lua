...
// immutable Schema through the exact sealed Rule identity; it never stores a
// cold dependency slice or a copied Rule row.
...
func (selector *schemaSelectedReadSelector) selectorDeclaresRead(index int) bool {
	...
	for dependency := uint64(0); ; dependency++ {
		if selector.proof == nil || selector.proof.schema == nil {
			return false
		}
		value, ok := selector.proof.schema.ruleReadDependencyAt(selector.proof.ordinal, selector.read, dependency)
		if !ok {
			return false
		}
...
func (selector *schemaSelectedReadSelector) selectorDependencyCount() int {
	if selector == nil || selector.proof == nil || !selector.proof.selectedReadAt(selector.read) {
		return 0
	}
	shape, ok := selector.proof.schema.ruleReadShapeAt(selector.proof.ordinal, selector.read)
...
func (binding *schemaSelectedRuleReadBinding[K, V, Tag]) complete(state *schemaBindingState, cell schemaRuleBindingCell, ordinal uint64) bool {
	if binding == nil || binding.origin == nil || binding.factor == nil || binding.locate == nil || state == nil || cell == nil || binding.origin.state != state || binding.origin.cell != cell || binding.origin.ruleOrdinal != ordinal || binding.origin.readKind() != composition.ReadSelect || binding.read.origin != binding.origin || binding.read.index != int(binding.origin.readOrdinal) || binding.read.resolve == nil || binding.factor.impl == nil || binding.factor.impl.algebra == nil || binding.factor.impl.state != state {
		return false
	}
	shape, ok := state.schema.ruleReadShapeAt(ordinal, binding.origin.readOrdinal)
	return ok && shape.Kind == composition.ReadSelect && shape.Input == binding.origin.inputOrdinal() && shape.Factor == state.schema.factorSemanticAt(binding.origin.factorIndex()) && shape.Semantic == shape.Factor && shape.Normalizer == (composition.Key{}) && shape.DependencyCount == binding.origin.dependencyCount() && binding.origin.dependencyCount() != 0
}
...
func (binding *schemaSelectedRuleReadBinding[K, V, Tag]) bind(bound readBinding, member equation.RuleMember, factors map[composition.Key]runtimeFactor) bool {
	...
	if !proof.selectedReadAt(binding.origin.readOrdinal) {
		return false
	}
	...
	if !present || !typed || !staged || factor == nil || factor.implementation == nil || !factor.implementation.binding.valid() || factor.implementation.binding.state != binding.origin.state || factor.implementation.binding.authority != binding.origin.state.authority || factor.implementation.binding.ordinal != binding.origin.factorIndex() || factor.implementation.binding.algebra != binding.factor.impl.algebra {
		return false
	}
	...
	if !surfaceOK || !rowOK || row.Kind != composition.ReadSelect || row.Input != binding.origin.inputOrdinal() || row.Factor != factorKey || surface.Factor != factorKey || surface.Form != equation.SurfaceReadSelect || surface.Semantic != factorKey || surface.Normalizer.Available() || surface.Mode != equation.TargetModeNone || !surface.LocalAvailable() {
		return false
	}
	selector := &schemaSelectedReadSelector{proof: proof, read: binding.origin.readOrdinal}
	if selector.selectorDependencyCount() != int(binding.origin.dependencyCount()) || selector.selectorDependencyCount() == 0 {
		return false
	}
	normalize := func(value OrderedCells[V]) OrderedCells[V] { return value }
	locate := binding.locate
	return bound.appendReadRuntime(&stagedReadRuntime[V, OrderedCells[V], Tag]{input: int(binding.origin.inputOrdinal()), selector: selector, target: target, locate: locate, normalize: normalize})
}
...
func (binding *schemaOpaqueExactRuleReadBinding[V]) complete(state *schemaBindingState, cell schemaRuleBindingCell, ordinal uint64) bool {
	...
	shape, ok := state.schema.ruleReadShapeAt(ordinal, binding.origin.readOrdinal)
	return ok && shape.Kind == composition.ReadExact && shape.Input == binding.origin.inputOrdinal() && shape.Factor == state.schema.factorSemanticAt(binding.origin.factorIndex()) && shape.DependencyCount == 0
}
...
func (binding *schemaOpaqueSummaryRuleReadBinding[V, S]) complete(state *schemaBindingState, cell schemaRuleBindingCell, ordinal uint64) bool {
	...
	shape, ok := state.schema.ruleReadShapeAt(ordinal, binding.origin.readOrdinal)
	return ok && shape.Kind == composition.ReadSummary && shape.Input == binding.origin.inputOrdinal() &&
		shape.Factor == state.schema.factorSemanticAt(binding.origin.factorIndex()) && shape.Semantic == binding.origin.semanticKey() &&
		shape.Normalizer == binding.origin.semanticKey() && shape.DependencyCount == 0
}
...
func (binding *schemaOpaqueSelectedRuleReadBinding[V, Tag]) complete(state *schemaBindingState, cell schemaRuleBindingCell, ordinal uint64) bool {
	...
	shape, ok := state.schema.ruleReadShapeAt(ordinal, binding.origin.readOrdinal)
	return ok && shape.Kind == composition.ReadSelect && shape.Input == binding.origin.inputOrdinal() && shape.Factor == state.schema.factorSemanticAt(binding.origin.factorIndex()) && shape.Semantic == shape.Factor && shape.DependencyCount == binding.origin.dependencyCount() && binding.origin.dependencyCount() != 0
}
...
func (binding *schemaOpaqueSelectedRuleReadBinding[V, Tag]) bind(bound readBinding, member equation.RuleMember, factors map[composition.Key]runtimeFactor) bool {
	...
	return bound.appendReadRuntime(&stagedReadRuntime[V, OrderedCells[V], Tag]{input: int(binding.origin.inputOrdinal()), selector: selector, target: targetProvider.stagedFactorTarget(), locate: binding.locate, normalize: func(value OrderedCells[V]) OrderedCells[V] { return value }})
}
...
func (binding *schemaOpaqueOperandSelectedRuleReadBinding[RV, O, Tag]) bind(bound readBinding, member equation.RuleMember, factors map[composition.Key]runtimeFactor) bool {
	...
	return bound.appendReadRuntime(&stagedReadRuntime[RV, OrderedCells[RV], Tag]{input: int(binding.origin.inputOrdinal()), selector: selector, target: targetProvider.stagedFactorTarget(), locate: locate, normalize: func(value OrderedCells[RV]) OrderedCells[RV] { return value }})
}
...
func (binding *schemaSummaryRuleReadBinding[K, V, S]) complete(state *schemaBindingState, cell schemaRuleBindingCell, ordinal uint64) bool {
	if binding == nil || binding.origin == nil || binding.factor == nil || binding.form == nil || state == nil || cell == nil || binding.origin.state != state || binding.origin.cell != cell || binding.origin.ruleOrdinal != ordinal || binding.origin.readKind() != composition.ReadSummary || binding.read.origin != binding.origin || binding.read.index != int(binding.origin.readOrdinal) || binding.read.resolve == nil || binding.factor.impl == nil || binding.factor.impl.algebra == nil || binding.factor.impl.state != state || binding.factor.ordinal != binding.origin.factorIndex() || binding.form.schema != state.schema || binding.form.factor != binding.factor || binding.form.form.cell == nil || binding.form.form.cell.schema != state.schema || !summaryReadFormKind(binding.form.form.cell.kind) || !binding.form.schemaFactorFormComplete() {
		return false
	}
	shape, ok := state.schema.ruleReadShapeAt(ordinal, binding.origin.readOrdinal)
	return ok && shape.Kind == composition.ReadSummary && shape.Input == binding.origin.inputOrdinal() && shape.Factor == state.schema.factorSemanticAt(binding.origin.factorIndex()) && shape.Semantic == binding.origin.semanticKey() && shape.Normalizer == binding.origin.semanticKey() && shape.DependencyCount == 0
}
...
func (binding *schemaSummaryRuleReadBinding[K, V, S]) bind(bound readBinding, member equation.RuleMember, factors map[composition.Key]runtimeFactor) bool {
	...
	runtime, present := factors[factorKey]
	factor, typed := runtime.(*boundFactor[K, V])
	if !present || !typed || factor == nil || factor.implementation == nil || !factor.implementation.binding.valid() || factor.implementation.binding.state != binding.origin.state || factor.implementation.binding.authority != binding.origin.state.authority || factor.implementation.binding.ordinal != binding.origin.factorIndex() || factor.implementation.binding.algebra != binding.factor.impl.algebra {
		return false
	}
	surface, surfaceOK := member.ReadAt(binding.read.index)
	row, rowOK := binding.origin.state.schema.ruleReadShapeAt(binding.origin.ruleOrdinal, binding.origin.readOrdinal)
	if !surfaceOK || !rowOK || row.Kind != composition.ReadSummary || row.Input != binding.origin.inputOrdinal() || row.Factor != factorKey || row.Semantic != binding.origin.semanticKey() || row.Normalizer != binding.origin.semanticKey() || row.DependencyCount != 0 || surface.Factor != factorKey || surface.Form != equation.SurfaceReadSummary || surface.Semantic != binding.origin.semanticKey() || surface.Normalizer != binding.origin.semanticKey() || surface.Mode != equation.TargetModeNone {
		return false
	}
	unit, unitOK := factor.readUnit(surface)
	unitRow, rowPresent := factor.reads[surface]
	summaryFactor, summaryForm, summaryKeys, summaryDigest, summaryOK := factor.summaryReadAddress(surface, uint64(uint32(binding.form.ordinal)), binding.origin.semanticKey())
	if !unitOK || !rowPresent || unitRow.kind != carrier.SummaryUnit || !summaryOK {
		return false
	}
	return bound.appendReadRuntime(&typedReadRuntime[K, V, S]{input: int(binding.origin.inputOrdinal()), binding: factor.binding, unit: unit, summaryFactor: summaryFactor, summaryForm: summaryForm, summaryKeys: summaryKeys, summaryDigest: summaryDigest, summary: true, normalize: binding.form.normalize, equal: binding.form.equal, fingerprint: binding.form.fingerprint})
}
...
func (binding *schemaExactRuleReadBinding[K, V]) bind(bound readBinding, member equation.RuleMember, factors map[composition.Key]runtimeFactor) bool {
	...
	surface, surfaceOK := member.ReadAt(binding.read.index)
	row, rowOK := binding.origin.state.schema.ruleReadShapeAt(binding.origin.ruleOrdinal, binding.origin.readOrdinal)
	if !surfaceOK || !rowOK || row.Kind != composition.ReadExact || row.Input != binding.origin.inputOrdinal() || row.Factor != factorKey || row.DependencyCount != 0 || surface.Factor != factorKey || surface.Form != equation.SurfaceReadExact || surface.Mode != equation.TargetModeNone || surface.Local == 0 {
		return false
	}
	unit, unitOK := factor.readUnit(surface)
	...
	return bound.appendReadRuntime(&typedReadRuntime[K, V, OrderedCells[V]]{input: int(binding.origin.inputOrdinal()), binding: factor.binding, unit: unit, exactFactor: factor.implementation.binding, exactRaw: readLocal, exact: true, normalize: normalize, equal: equal, fingerprint: fingerprint})
}
