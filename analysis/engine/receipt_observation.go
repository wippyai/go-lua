...
// issued only while a ProgramConstruction is open and is bound to an exact
...
// AttachMountedSummaryObservationWithFailure looks up the authored mounted
// member on this construction and binds the summary observation to it.
func AttachMountedSummaryObservationWithFailure[V, R any](compilation *ProgramConstruction, implementation *SummaryQueryImplementation[V, R], id identity.ContentID, role RuleSlotCapability, mount, point, occurrence identity.ContentID) (ReceiptObservation[R], ReceiptObservationAttachFailure) {
	if compilation == nil || compilation.graph == nil {
		return ReceiptObservation[R]{}, ReceiptObservationAttachFailureArguments
	}
	member, memberOK := compilation.graph.MountedRuleMember(role, mount, point, occurrence)
	if !memberOK {
...
func AttachRuleSummaryObservationWithFailure[V, R any](compilation *ProgramConstruction, implementation *SummaryQueryImplementation[V, R], id identity.ContentID, member ReceiptRuleMember) (ReceiptObservation[R], ReceiptObservationAttachFailure) {
...
// AttachMountedExactObservation looks up the authored mounted member on this
// construction and binds the exact observation to it.
...
func AttachRuleExactObservation[V, R any](compilation *ProgramConstruction, implementation *ExactQueryImplementation[V, R], id identity.ContentID, member ReceiptRuleMember) (ReceiptObservation[R], bool) {
...
func AttachRuleExactObservationWithFailure[V, R any](compilation *ProgramConstruction, implementation *ExactQueryImplementation[V, R], id identity.ContentID, member ReceiptRuleMember) (ReceiptObservation[R], ReceiptObservationAttachFailure) {
	if compilation == nil || compilation.inner == nil || compilation.graph == nil || !compilation.graph.valid() || implementation == nil || !id.Available() || member.graph != compilation.graph || !compilation.graph.graph.OwnsMember(member.member) {
		return ReceiptObservation[R]{}, ReceiptObservationAttachFailureArguments
	}
	locator := member.locator
