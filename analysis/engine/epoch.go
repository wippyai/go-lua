...
func (solver *Solver) rebuildEpoch(relations []activeRelation) bool {
	...
	candidate := *solver
	candidate.bodyReuse = newBodyReuse()
	if !candidate.refreshCarrier(relations) || !candidate.compileQueries(relations) {
		return false
	}
	// A retained body projection belongs to one Guard/Fiber epoch. Swap the
	// complete derived view only after both carrier and schedule construction
	// succeed; no partial compiler state can escape a failed reformation.
	solver.guards, solver.decisionAtoms = candidate.guards, candidate.decisionAtoms
	solver.fibers, solver.zero, solver.initial = candidate.fibers, candidate.zero, candidate.initial
	solver.actions, solver.schedule, solver.regions = candidate.actions, candidate.schedule, candidate.regions
	solver.followers, solver.presenceFollowers = candidate.followers, candidate.presenceFollowers
	solver.roots, solver.queryRoots, solver.entrySeeds = candidate.roots, candidate.queryRoots, candidate.entrySeeds
	solver.supportCatalog, solver.supportTargets, solver.bodyReuse = candidate.supportCatalog, candidate.supportTargets, candidate.bodyReuse
	return true
}
