...
// A ranked domain self-read is one finite compiled equation. The resulting
// open gate restores the guarded downstream Program edge in the same State.
func TestStage4LawSelfReadRestoresReachability(t *testing.T) {
	value, project, shard, _, _, rejoin := stage4BranchProgram(t)
	entry, edge := stage4EntryEdge(t, value)
	solver, err := engine.New(project)
	if err != nil {
		t.Fatal(err)
	}
	gate := stage4GateFactor(t, solver, "recursive-reachability-gate")
	evidence := stage4PairsFactor(t, solver, "restored-reachability")

	var self engine.ReadRef[uint64, stage4Gate]
	gateRule := stage4DeclareGateAt(t, solver, gate, "recursive-gate", shard, entry, func(access engine.Access[uint64, stage4Gate]) bool {
		prior, present, valid := engine.ReadAt(access, self, stage4PackKey)
		if !valid {
			return false
		}
		switch {
		case !present || prior == stage4GateBottom:
			return access.Set(stage4PackKey, stage4GateClosed)
		case prior == stage4GateClosed:
			return access.Set(stage4PackKey, stage4GateOpen)
		case prior == stage4GateOpen:
			return access.Set(stage4PackKey, stage4GateOpen)
		default:
			return false
		}
	...
	var ok bool
	self, ok = engine.Read(gateRule, 0, gate)
	if !ok {
		t.Fatal("Read recursive Gate")
	}

	var gateInput engine.ReadRef[uint64, stage4Gate]
	edgeRule := stage4DeclarePairFrom(t, solver, evidence, "gate-controlled-edge", shard, edge, func(access engine.Access[uint64, stage4Pairs]) bool {
		value, present, valid := engine.ReadAt(access, gateInput, stage4PackKey)
		if !valid {
			return false
		}
		if !present || value != stage4GateOpen {
			return access.Prune()
		}
		return access.Set(stage4PackKey, stage4LeftOne)
	})
	gateInput, ok = engine.Read(edgeRule, 0, gate)
	if !ok {
		t.Fatal("Read gate-controlled edge input")
	}

	gateQuery := stage4GateQuery(t, solver, gate, shard, entry)
	evidenceQuery := stage4Query(t, solver, evidence, shard, rejoin)
	state := stage4Solve(t, solver)
	if got, present := gateQuery.Read(state); !present || got != stage4GateOpen {
		t.Fatalf("self-recursive Gate = %d, want open %d", got, stage4GateOpen)
	}
	if got, present := evidenceQuery.Read(state); !present || got != stage4LeftOne {
		t.Fatalf("restored reachability = %#x, want %#x", got, stage4LeftOne)
	}
}
