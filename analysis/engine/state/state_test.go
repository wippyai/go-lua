...
func TestDomainWithLanesSelectsIndependentAxes(t *testing.T) {
	reg := standard.Registry()
	...
	valueAndFrozen := DomainWithLanes(reg, []LaneID{LaneValues, LaneFrozenTables})
...
func TestDomainWithLanesValidatesLaneSelection(t *testing.T) {
	reg := standard.Registry()

	lanes := DefaultDomainLanes()
	...
	requirePanic(t, func() {
		_ = DomainWithLanes(reg, []LaneID{LaneID("not-a-lane")})
	})
	requirePanic(t, func() {
