...
// reads as no floor.
func (s State) ReadLenFloor(ks *keyspace.KeySpace, pathKey pathdom.PathKey) (int64, bool) {
...
// floor is a no-op.
func (s State) WriteLenFloor(ks *keyspace.KeySpace, pathKey pathdom.PathKey, lo int64) State {
	key, ok := ks.FromStateKey(pathKey)
	if !ok {
		return s
	}
