...
type ExpressionOperation struct {
	...
	left      ValueSource
	right     ValueSource
	intrinsic intrinsic.Kind
}

func NewUnaryExpressionOperation(op string, operand ValueSource) (ExpressionOperation, bool) {
	if op == "" || op == "lua_type" || !operand.Valid() {
		return ExpressionOperation{}, false
	}
	return ExpressionOperation{kind: ExpressionOperationUnary, op: op, left: operand}, true
}

func NewIntrinsicExpressionOperation(kind intrinsic.Kind, operand ValueSource) (ExpressionOperation, bool) {
	if !kind.Valid() || !operand.Valid() {
		return ExpressionOperation{}, false
	}
	return ExpressionOperation{kind: ExpressionOperationUnary, left: operand, intrinsic: kind}, true
}
...
func (o ExpressionOperation) Intrinsic() (intrinsic.Kind, bool) {
