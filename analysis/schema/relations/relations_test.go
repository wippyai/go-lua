...
func TestCanonicalSchemaDigestKnownAnswer(t *testing.T) {
	schema, err := CanonicalSchema()
	if err != nil {
		t.Fatal(err)
	}
	const want = "9b15a963276c27be60e7c1c09335c360d837230f4b9d7300fc38c1f1b527d1aa"
	if got := schema.Digest().String(); got != want {
		t.Fatalf("canonical relation digest = %s, want %s", got, want)
	}
}
...
func TestSealRejectsInvalidRows(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := Seal(test.rows)
			if !errors.Is(err, test.want) {
				t.Fatalf("Seal() error = %v, want %v", err, test.want)
			}
		})
	}
	if _, err := Seal(nil); !errors.Is(err, ErrInvalidDefinitions) {
		t.Fatalf("empty relation table error = %v, want %v", err, ErrInvalidDefinitions)
	}
}
