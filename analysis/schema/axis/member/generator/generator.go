...
func renderCold(packageName string, source definition.Definition) ([]byte, error) {
	...
	fmt.Fprintf(&out, "//go:generate go run ../../analysis/schema/axis/member/generator/cmd -source %s -cold rule_members.go -relations %s%s\n\n", source.Axis, relationsPath, relationsPackageFlag)
