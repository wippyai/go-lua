...
func TestAxisTableDrivesEveryDerivedView(t *testing.T) {
	...
	semantics := make(map[engine.SemanticKey]bool, view.Count())
	...
	if len(semantics) != view.Count() {
		t.Fatalf("%d axes publish %d canonical identities", view.Count(), len(semantics))
	}
	// The canonical identity of an axis is the vocabulary's factor identity;
	// the table is not free to invent one.
	for principal, expected := range map[programartifact.RuleOutputKind]engine.SemanticKey{
...
// binding transaction depends on: the axis surface is the first member of the
// catalog, so every factor principal is declared and bound before a rule that
// writes it.
