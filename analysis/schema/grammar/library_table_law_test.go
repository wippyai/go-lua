...
func TestLibraryTableSeals(t *testing.T) {
	...
	if view.Count() != len(specs) {
		t.Fatalf("sealed library surface holds %d of %d authored kinds", view.Count(), len(specs))
	}
...
// and the five forms only it may own, and neither declares a form outside its
// class.
