...
// A parent names its ordered child ranges by half-open dense spans; child rows
// carry no slice, map, compiler, or owner pointer.
const (
	slotDiagnosticObservation = slotRuleOccurrence + 1
	slotDiagnosticEvidence    = slotDiagnosticObservation + 1
	slotDiagnosticPath        = slotDiagnosticEvidence + 1
)

var (
	diagnosticObservationFamily = Family[DiagnosticObservation]{slot: slotDiagnosticObservation, name: "diagnostic-observation"}
	diagnosticEvidenceFamily    = Family[DiagnosticEvidence]{slot: slotDiagnosticEvidence, name: "diagnostic-evidence"}
	diagnosticPathFamily        = Family[DiagnosticPath]{slot: slotDiagnosticPath, name: "diagnostic-path"}
)

func DiagnosticObservationFamily() Family[DiagnosticObservation] { return diagnosticObservationFamily }

func DiagnosticEvidenceFamily() Family[DiagnosticEvidence] { return diagnosticEvidenceFamily }

func DiagnosticPathFamily() Family[DiagnosticPath] { return diagnosticPathFamily }
