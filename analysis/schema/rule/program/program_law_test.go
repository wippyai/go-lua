...
func lawProgram(t *testing.T) Program {
	...
	return Program{
		...
		Candidate:   lawRelation("candidate"),
...
func TestProgramAllowsZeroJoinSeedWithUnitFold(t *testing.T) {
	seed := Program{
		OperandRole: "semantic/operand/law",
		Candidate:   lawRelation("seed/candidate"),
		Fold:        lawFold(nil),
	}
...
func TestProgramRejectsMissingCandidateAndMalformedSeed(t *testing.T) {
	...
	seed = Program{OperandRole: "semantic/operand/law", Candidate: lawRelation("seed/candidate"), Fold: lawFold([]JoinRef{0})}
	problem, valid = seed.Check()
...
func TestProgramAdmitsOnlyTheFiveNormalFormCombinations(t *testing.T) {
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			program := Program{OperandRole: "semantic/operand/law", Candidate: lawRelation("candidate/" + test.name), Joins: []JoinDecl{test.join}, Fold: lawFold([]JoinRef{0})}
			if problem, valid := program.Check(); !valid {
...
func TestProgramAdmitsCarryOnlyInputPortWithoutARead(t *testing.T) {
	program := Program{OperandRole: "semantic/operand/law", Candidate: lawRelation("carry-only/candidate"), Fold: lawFold(nil), Carry: &CarryDecl{Input: 0, Mode: CarryIdentity}}
	if problem, valid := program.Check(); !valid {
