...
func (activation ActivationDecl) references() schema.EntryReferences {
	references := make(schema.EntryReferences, 0, 5+len(activation.Transport))
...
func (program Program) checkActivation() bool {
	if (program.Activation != nil) != (program.transportCount() != 0) {
		return false
	}
	if program.Activation == nil {
		return true
...
func (program Program) checkTransport() bool {
	...
	if (program.transportCount() != 0) != program.ActivationRole.Available() {
		return false
	}
	if program.transportCount() == 0 {
		return true
	}
	seen := make(map[schema.EntryReference]struct{}, program.transportCount())
	for _, transport := range program.Activation.Transport {
...
func (program Program) WriteContent(content *framing.Writer) error {
	...
	if program.transportCount() != 0 {
		...
		if err := content.Count(uint64(program.transportCount())); err != nil {
			return err
		}
		for _, transport := range program.Activation.Transport {
