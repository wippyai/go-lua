...
func rewriteFunction(v *typ.Function, orig typ.Type, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) typ.Type {
	typeParams, typeParamReplacements, changed := rewriteTypeParamList(v.TypeParams, fn, memo)
	...
	for i, p := range v.Params {
		newType := rewriteDepth(p.Type, rewriteChild, childMemo)
		if newType != p.Type {
			if params == nil {
	...
	for i, r := range v.Returns {
		newRet := rewriteDepth(r, rewriteChild, childMemo)
		if newRet != r {
			if returns == nil {
	...
	if v.Variadic != nil {
		variadic = rewriteDepth(v.Variadic, rewriteChild, childMemo)
...
func rewriteMeta(v *typ.Meta, orig typ.Type, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) typ.Type {
	of := rewriteDepth(v.Of, fn, memo)
	if of == v.Of {
		return orig
	}
...
func rewriteTypeParam(v *typ.TypeParam, orig typ.Type, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) typ.Type {
	constraint := rewriteDepth(v.Constraint, fn, memo)
...
func rewriteGeneric(v *typ.Generic, orig typ.Type, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) typ.Type {
	typeParams, typeParamReplacements, changed := rewriteTypeParamList(v.TypeParams, fn, memo)
...
func rewriteTypeParamList(params []*typ.TypeParam, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) ([]*typ.TypeParam, map[*typ.TypeParam]*typ.TypeParam, bool) {
	var out []*typ.TypeParam
	var replacements map[*typ.TypeParam]*typ.TypeParam
	changed := false
	for i, p := range params {
		newParam := rewriteTypeParamBinder(p, fn, memo)
		if newParam == p {
			if out != nil {
				out[i] = p
			}
...
func rewriteTypeParamBinder(p *typ.TypeParam, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) *typ.TypeParam {
	if p == nil {
		return p
	}
	...
	constraint := rewriteDepth(p.Constraint, fn, memo)
...
func rewriteMemoForTypeParamScope(memo map[rewriteKey]typ.Type, binders []*typ.TypeParam, replacements map[*typ.TypeParam]*typ.TypeParam) map[rewriteKey]typ.Type {
	...
	// A binder changes the rewrite function's meaning. Give that lexical scope
	// an independent exact memo instead of disabling cycle detection.
	return make(map[rewriteKey]typ.Type)
}

func rewriteRecursive(v *typ.Recursive, orig typ.Type, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) typ.Type {
	if v.Body == nil {
		return orig
	}
	replacement := typ.NewRecursivePlaceholder(v.Name)
	if memo != nil {
		memo[rewriteKey{t: v}] = replacement
	}
	...
	body := rewriteDepth(v.Body, selfAware, memo)
	if body == v.Body {
		return orig
	}
...
func rewriteRecord(v *typ.Record, orig typ.Type, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) typ.Type {
	changed := false

	fields, fieldsChanged := rewriteRecordFields(v.Fields, fn, memo)
	if fieldsChanged {
		changed = true
	}
	staticMembers, staticMembersChanged := rewriteRecordStaticMembers(v.StaticMembers, fn, memo)
	...
	if v.Metatable != nil {
		metatable = rewriteDepth(v.Metatable, fn, memo)
	...
	if v.HasMapComponent() {
		mapKey = rewriteDepth(v.MapKey, fn, memo)
		if mapKey != v.MapKey {
			changed = true
		}
		mapValue = rewriteDepth(v.MapValue, fn, memo)
...
func rewriteRecordFields(fields []typ.Field, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) ([]typ.Field, bool) {
	var out []typ.Field
	for i, f := range fields {
		newType := rewriteDepth(f.Type, fn, memo)
		if newType == f.Type {
			if out != nil {
				out[i] = f
			}
...
func rewriteRecordStaticMembers(members []typ.StaticMember, fn func(typ.Type) (typ.Type, bool), memo map[rewriteKey]typ.Type) ([]typ.StaticMember, bool) {
