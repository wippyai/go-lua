...
func typeEqualsGuard(a, b Type, guard recursion.Guard, seen map[typePair]bool) bool {
	...
	case *Recursive:
		...
		// Keyed families compare by owner key, independent of the body slot, so
		// two observations of one family are equal even before their bodies have
		// widened to a common fixed point.
		if va.keyed || vb.keyed {
			return va.keyed && vb.keyed && va.familyNS == vb.familyNS && va.familyOwnerStr == vb.familyOwnerStr
		}
		if va.Name != vb.Name {
			return false
		}
