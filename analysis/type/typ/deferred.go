...
func NewFieldAccess(base Type, field string) *FieldAccess {
	h := hash.HashCombine(uint64(kind.FieldAccess), base.Hash())
	h = hash.HashCombine(h, hash.FnvString(field))
...
func NewIndexAccess(base, index Type) *IndexAccess {
	h := hash.HashCombine(uint64(kind.IndexAccess), base.Hash())
	h = hash.HashCombine(h, index.Hash())
